...
class Template_
{
    ...
    var $compile_ext   = 'php';
    var $skin          = '';
    var $browser	   = 'web'; //{web, mobile} compile,cache 폴더  웹, 모바일 구분처리 2014.07.29 bgh
    var $notice        = false;
    ...
    {
        //web, mobile 구분처리
        //나중에 구조 변경시 해당 내용 삭제후 탬플릿 선언 후 지정해줘야함!
    ...
    function _get_compile_path($rel_path)
    {
        ...
        $compile_base = $this->compile_dir.'/'.($this->skin?$this->skin.'/':'').'/'.$this->browser.'/'.$rel_path;
        ...
        if (@!is_file($template_path))
        {
            ...
            header("Location:/etc/error_404.php");
            exit;
        }
    ...
    function isCached($fid)
    {
        ...
        $cache['path'] = $this->cache_dir.'/%cache'.'/'.$this->browser.'/'.$_SERVER['PHP_SELF'].'/'.md5($cache['header']);
