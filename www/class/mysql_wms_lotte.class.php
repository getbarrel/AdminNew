...
//기초정보 관련 업체 연결고리 정의 테이블 E
if (!class_exists(WmsMySQL))
{ // 클래스선언이 없으면...
    ...
    class WmsMySQL {
        ...
        var $slave_db_host;	// 디비 호스트
        var $slave_db_user;	// 디비 사용자
        var $slave_db_pass;	// 디비 비밀번호
        var $slave_db_name;	// 디비 이름
        ...
        function WmsMySQL($_db_host="", $_db_user="", $_db_pass="", $_db_name=""){

            if ($_SERVER ["HTTP_HOST"] == "admin.dewytree.com") {
                $this->db_host = 'db-1825u.cdb.ntruss.com';
                $this->db_user = 'dewywms';
                $this->db_pass = 'La4OsONi6I5u';
                $this->db_name = 'wms_dewytree';
            } else {
                $this->db_host = '106.10.56.19';
                $this->db_user = 'wms';
                $this->db_pass = 'Wmsdewy##77';
                $this->db_name = 'wms_dewytree';
            }
        ...
        {
            global $cnt;
            $this->dbcon = mysql_connect($this->db_host,$this->db_user,$this->db_pass, true) or $this->error();
        ...
        function query($sql)
        {
            ...
            if($this->debug){
                echo "<b>".$this->conn_name."</b><br>";
                echo "<b>".$this->db_host."</b><br>";
                //echo "<b>".$this->dbcon."</b><br>";
            ...
            $this->result = mysql_query("$sql", $this->dbcon) or $this->error();
        ...
        function fetchall($type = 'array', $result_type = MYSQL_BOTH) {
            $i = 0;
            switch ($type) {
        ...
        function mysql_table_exists($table) {
            ...
            $exists = mysql_query("SELECT 1 FROM $table LIMIT 0", $this->dbcon);
        ...
        function error()
        {
            include_once $_SERVER['DOCUMENT_ROOT'].'/include/logSender/send_error_log.php';
			...
			<!--
				if(window.name =='act' || window.name =='iframe_act'){
					alert(\"wms lotte 데이터베이스 처리중에 문제가 생겼습니다. 계속해서 문제가 발생할시 관리자에게 문의 바랍니다. 에러 메세지 : [".addslashes(mysql_error())."]\");
				}
			//-->
			</script>";
            ...
            $mstring .= "<tr><td align=center valign=middle><div style='height:200px;width:400px;border:10px solid #efefef;font-size:12px;font-family:돋움;'>MYSQL 오류 <br><br> ".addslashes(mysql_errno())." ".addslashes(mysql_error())." <br><br>".$this->sql."</div></td></tr>";
            $mstring .= "<table>";
            $mstring .= "</html>";
            ...
            if($_SERVER["REMOTE_ADDR"]=="192.168.0.1" || $_SERVER["HTTP_HOST"]=="unimind.forbiz.co.kr" || $_SERVER["HTTP_HOST"]=="dev.forbiz.co.kr" || $_SERVER["HTTP_HOST"]=="dev2.forbiz.co.kr" || $_SERVER["HTTP_HOST"]=="daiso.forbiz.co.kr" || $_SERVER["HTTP_HOST"]=="daisodev.forbiz.co.kr" || $_SERVER["HTTP_HOST"]=="daisomall.mallstory.com" || $_SERVER["HTTP_HOST"]=="newdev.forbiz.co.kr" || $_SERVER["HTTP_HOST"]=="s1.mallstory.com" || $_SERVER["HTTP_HOST"]=="s2.mallstory.com" || $_SERVER["HTTP_HOST"]=="s3.mallstory.com" || $_SERVER["REMOTE_ADDR"]=="175.209.244.68" || $_SERVER["HTTP_HOST"]=="sohotest.forbiz.co.kr" || $_SERVER["HTTP_HOST"]=="skin1.forbiz.co.kr" || $_SERVER["HTTP_HOST"]=="m.forbiz.co.kr" ) {
                $mstring.='<div style="color:black;">MYSQL 오류 : '.$_SERVER["SCRIPT_FILENAME"].'<br><br>'.addslashes(mysql_errno()).' '.addslashes(mysql_error()).'<br><br>'.$this->sql.'</div>';
            }
        ...
        function showExplain($sql)
        {
            ...
            if(preg_match('/^select/i', $sql) || preg_match('/^SELECT/i', $sql))	{
                $result = mysql_query('EXPLAIN '.$sql, $this->dbcon) or die (mysql_error());
            ...
            foreach($table[0] as $_key=>$_val)	{
                ...
                $result = mysql_query('SELECT COUNT(*) FROM useQuery uq INNER JOIN useTable ut ON ut.uq_idx = uq.uq_idx WHERE uq.uq_location = "'.$_SERVER['PHP_SELF'].'" AND uq.uq_type = "'.$type.'" AND ut.ut_tableName = "'.$_val.'" AND ut.ut_selectType = "'.$ut_eSelectType[$eKey].'" AND ut.ut_type = "'.$ut_eType[$eKey].'"', $this->dbcon) or die (mysql_error());
            ...
            if(count($ut_querys) > 0)	{
                ...
                mysql_query('INSERT INTO useQuery (uq_type, uq_location, uq_param, uq_sql, uq_regdate) VALUES '.$uq_querys, $this->dbcon) or die (mysql_error());
                $uq_idx = mysql_insert_id();
                foreach($ut_querys as $_key2=>$_val2)	{
                    $_val2[] = 'uq_idx = '.$uq_idx;
                    mysql_query('INSERT INTO useTable SET '.implode(',', $_val2), $this->dbcon) or die (mysql_error());
                }
            }
        }
