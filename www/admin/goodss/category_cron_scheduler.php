...
if($goods_act == "cron_goodss_update"){
	...
	$shmop->SetFilePath();
	$cron_goodss = $shmop->getObjectForKey("cron_goodss_update");
}else{
	$shmop = new Shared("cron_goodss_reg");
	$shmop->filepath = $_SERVER["DOCUMENT_ROOT"].$admininfo["mall_data_root"]."/_shared/";
	$shmop->SetFilePath();
	$cron_goodss = $shmop->getObjectForKey("cron_goodss_reg");
}
$cron_goodss = unserialize(urldecode($cron_goodss));
...
$Contents01 = "
	<table width='100%' cellpadding=0 cellspacing=0 border='0' align='left'>
	  <tr >
			<td align='left' colspan=4> ".GetTitleNavigation("상품자동연동 스케줄링", "기본정보 설정 > 상품자동연동 스케줄링 ")."</td>
	  </tr>
	  <tr>
	    ...
	    <div class='tab'>
			...
			$Contents01 .=	"
						...
						<table id='tab_02' ".($goods_act == "cron_goodss_reg" ||  $goods_act == ""  ? "class='on'":"")." >
						<tr>
							<th class='box_01'></th>
							<td class='box_02' onclick=\"document.location.href='?goods_act=cron_goodss_reg'\">신규상품등록</td>
							<th class='box_03'></th>
						</tr>
						</table>
						<table id='tab_04' ".($goods_act == "cron_goodss_update" ? "class='on'":"")." >
						<tr>
							<th class='box_01'></th>
							<td class='box_02' onclick=\"document.location.href='?goods_act=cron_goodss_update'\">가격/재고 업데이트</td>
							<th class='box_03'></th>
						</tr>
						</table>";
...
$Contents01 .= "
	  ...
	  <form name='search_form' method='get' action='category_cron_scheduler.act.php' onsubmit='return CheckFormValue(this);' style='display:inline;' target='iframe_act'>
					...
					<table cellpadding=3>
						...
						<tr>
							...
							<select name='cron_minutes'  id='cron_minutes'  style=\"font-size:12px;width:90px;\" >
								...
								for($i=1; $i < 6;$i++){
								$Contents01 .= "<option value='".($i*10)."' ".( $cron_goodss[cron_minutes] == $i*10 ? 'selected' : '' ).">".($i*10)."</option>";
								}
								$Contents01 .= "
							</select>
							</td>
							...
							<select name='cron_hours' id='cron_hours' style=\"font-size:12px;width:90px;\" >
								<!--option value='00'>00</option-->";
								for($i=0; $i < 24;$i++){
								$Contents01 .= "<option value='".$i."' ".( $cron_goodss[cron_hours] == $i ? 'selected' : '' ).">".$i."</option>";
								}
								$Contents01 .= "
							</select>
							</td>
							...
							<select name='cron_days' id='cron_days' style=\"font-size:12px;width:90px;\" disabled >
								...
								for($i=0; $i < 31;$i++){
								$Contents01 .= "<option value='".$i."' ".( $cron_goodss[cron_days] == $i ? 'selected' : '' ).">".$i."</option>";
								}
								$Contents01 .= "
							</select>
							</td>
							...
							<select name='cron_months' id='cron_months' style=\"font-size:12px;width:90px;\" disabled >
								...
								for($i=0; $i < 31;$i++){
								$Contents01 .= "<option value='".$i."' ".( $cron_goodss[cron_months] == $i ? 'selected' : '' ).">".$i." 월</option>";
								}
								$Contents01 .= "
							</select>
							</td>
							...
							<select name='cron_weekdays' id='cron_weekdays' style=\"font-size:12px;width:90px;\" disabled >
								...
								for($i=0; $i < 31;$i++){
								$Contents01 .= "<option value='".$i."' ".( $cron_goodss[cron_weekdays] == $i ? 'selected' : '' ).">".$i." 주</option>";
								}
								$Contents01 .= "
							</select>
							</td>
...
if($db->total){
	for($i=0;$i  < $db->total; $i++){
		...
		$Contents02 .= "<tr align=center height=30>
				...
				if($price_setting == "9"){
					$Contents02 .= "권장판매가 사용";
				}else if($price_setting == "1"){
					$Contents02 .= "공급가 + 마진";
				}else if($price_setting == "2"){
					$Contents02 .= "공급가*배수";
				}
		...
		$Contents02 .= "			
				...
				if($usable_round == "Y"){
					$Contents02 .= "가격반올림 사용";
					if($round_precision == "2"){
						$Contents02 .= "100자리";
					}else if($round_precision == "3"){
						$Contents02 .= "1000자리";
					}else if($round_precision == "4"){
						$Contents02 .= "10000자리";
					}

					if($round_type == "floor"){
						$Contents02 .= "버림";
					}else if($round_type == "round"){
						$Contents02 .= "반올림";
					}
				}else{
					$Contents02 .= "가격반올림 사용안함";
				}
				$Contents02 .= "</td>
				<td class='list_box_td list_bg_gray' rowspan=2>".$db->dt[regdate]."</td>
			...
			<tr height=25>
				<td  >".getCategoryPathByAdmin($db->dt[cid], 4)."</td>
			</tr>";
	}
	$Contents02 .= "";
}else{
		$Contents02 .= "<tr height=60><td colspan=7 align=center>상품자동연동 설정  정보가 없습니다.</td></tr>";
...
$Contents02 .= "<ul class='paging_area' >
						<li class='front'>".page_bar($total, $page, $max,"&cid2=$cid2&depth=$depth&search_type=$search_type&search_text=$search_text&FromYY=$FromYY&FromMM=$FromMM&FromDD=$FromDD&ToYY=$ToYY&ToMM=$ToMM&ToDD=$ToDD&bs_site=$bs_site","")."</li>
						<li class='back' style='text-align:right;'><a href=\"javascript:PoPWindow3('../goodss/category_mapping.edit.php?mmode=pop&gcs_ix=".$db->dt[gcs_ix]."',800,600,'category_mapping_edit')\"'><img src='../images/".$admininfo["language"]."/btc_modify.gif' border=0></a><-- 상품자동연동 설정등록 버튼으로 교체</li>
					  </ul>";
*/
...
if($mmode == "pop"){
	...
	$P->strLeftMenu = goodss_menu();
	$P->title = "상품자동연동 스케줄링";
	$P->Navigation = "도매아이템 > 기본정보 설정 > 상품자동연동 스케줄링";
	$P->strContents = $Contents;
	echo $P->PrintLayOut();
}
