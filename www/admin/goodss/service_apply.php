...
$service_detail = (array)$soapclient->call("getServiceDetailInfo",$params = array("pid"=> $_GET["pid"],"parent_service_code"=> $_GET["parent_service_code"],"service_code"=> $_GET["service_code"],"mall_ix"=>$admininfo[mall_ix]),	$options);
...
<TABLE cellSpacing=0 cellPadding=0 width='100%' align=center border=0>
	<TR>
		...
		<table border='0' width='100%' cellpadding='0' cellspacing='0' align='center'>
			<tr >
				<td align='left' colspan=2> ".GetTitleNavigation("서비스 신청/연장", "마이서비스 > 서비스 신청/연장", false)."</td>
			</tr>
			<tr height=30><td class='p11 ls1' style='padding:0 0 0 15px;text-align:left;' ><img src='../image/title_head.gif' align=absmiddle>  <b class=blk>".$service_info[pname]." </b> ".($_GET["si_ix"] == "" ? "신규 서비스 신청합니다.":"서비스 연장합니다")." </td></tr>
			<tr>
				<td align=center style='padding: 0 10px 0 10px;vertical-align:top'>
					  ...
					  <input type='hidden' name='mall_domain' value='".$_SERVER[HTTP_HOST]."'>
					  <input type='hidden' name='si_ix' value='".$_GET[si_ix]."'>
					  <input type='hidden' name='oid' value='".$payment_oid."'>
					  <input type='hidden' name='parent_service_code' value='".$_GET[parent_service_code]."'>
					  <input type='hidden' name='service_code' value='".$_GET[service_code]."'>
					  <input type='hidden' name='pname' value='".$service_info[pname]."'>
						...
						<td >
							<table border='0' width='100%' cellspacing='0' cellpadding='0' class='member_table input_table_box' style='text-align:left;'>
								...
								<tr>
									...
									<td class='input_box_item' style='padding:0px 10px;'><b class=blk>".$service_info[pname]."</b><b style='color:white;'>(".$admininfo[mall_ix].")</b></td>
							    </tr>";
			if($service_info[si_status] != "" &&  $service_info[sm_sdate] != NULL){
			$Contents .= "
								<tr>
			...
			$Contents .= "
				...
				if($_GET["si_ix"] == ""){
						$Contents .= "".$service_info["unit_value"]." ".$service_info["unit_text"]." <div style='margin:0px 10px;display:inline;'>*</div> 
									...
									<select name='unit_cnt' id='unit_cnt' validation=true title='수량' style='width:50px;' onchange=\"ServiceSum(this,'".$service_info["unit_text"]."');\">
										<!--option>수량</option-->";
						...
						$Contents .=  "<div id='service_edit' style='display:none;margin-right:5px;'><div style='margin:0px 5px;display:inline;'>==></div>
									...
									<select name='change_unit_cnt' validation=true title='수량' style='width:100px;' onchange=\"ServiceEditSum(this,'".$service_info["unit_text"]."');\">";
						for($i=0;$i < count($amount) ;$i++){
...
$Contents .= "
								<tr>
									...
									<td class='input_box_item' ><input type=hidden name='unit_cnt' id='unit_cnt' value='1'> ";
							...
							$Contents .= "
										...
										<b id='display_service_price'>".number_format($service_info[sellprice])." 원</b>   (VAT 별도)";
							}else{
							$Contents .= "
										<input type='hidden' name='sellprice' value='".$service_info["sellprice"]."'  id='sellprice' style='border:0px;' >
										<!--s>".number_format($service_info[listprice])."</s-->
							...
							$Contents .="
								...
								<tr>
									<td class='input_box_title' nowrap> 도매사이트 아이디</td>
									<td class='input_box_item' style='padding:5px'>
										<table width=100%>
											...
											<tr>
												<td>
												<input type='text' name='b2b_user_id' id='b2b_user_id'  validation=true title='도매사이트 아이디'>
												</td>
												<td>
												<div style='padding:5px 0px;' class=small><b>해당 B2B 사이트</b> 가입 후 <b>ID</b>를 기재하여 주십시요.<br>
												아이디 미기재시는 서비스 사용 불가 합니다.</div>
												</td>
											</tr>
										</table>
									</td>
								...
								<tr>
									...
									<td class='input_box_item' style='padding:5px;'>
										<input type='radio' name='gopaymethod' value='bank' ".$gopaymethod." id='gopaymethod_1' style='border:0px;' onclick=\"click_method(this)\"  validation=true title='결제방법' checked><label for='gopaymethod_1' >무통장입금</label>
										<input type='radio' name='gopaymethod' value='Card' ".$gopaymethod." id='gopaymethod_2' style='display:none;border:0px;' onclick=\"click_method(this)\" validation=true title='결제방법' ><label for='gopaymethod_2' style='display:none;'>신용카드</label>
										...
										<b style='padding-left:20px;'>신한은행 140-009-517556 (주) 포비즈코리아 </b>
									</td>
								</tr>
							</table>
						</td>
				...
				<table width='100%' border='0'>
					<tr>
						<td align='left' style='line-height:120%;'>
							※ <span class='small'>  카드결제 준비중입니다.</span><br>
							※ <span class='small'>  서비스 신청을 원하시는 기간과 결제 방법을 선택하시고 결제 하시면 서비스 신청이 완료됩니다.</span><br>
							※ <span class='small'>  서비스 연장시 기본 조건이 변경시 변경하신 서비스로 환산해서 연산 적용됩니다.</span><br>
							※ <span class='small'>  서비스 신청시 서비스에 필요한 <b>업체 정보</b> 및 <b>개인정보</b>가 <u>www.goodss.co.kr</u> 사이트로 전송됩니다. </span>
						</td>
					...
					<tr>
						...
						<table>
							<tr>
								...
								$Contents .="<input type=image src='../images/".$admininfo["language"]."/bts_ok.gif' id='submit_btn' border=0 style='cursor:pointer;border:0px;'>";
								}else{
