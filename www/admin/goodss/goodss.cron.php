...
if($goods_act == "cron_goodss_reg"){
	...
	//syslog(LOG_INFO, $mall_ename.'도매아이탬 자동상품등록 START');
	...
	for($a=0;$a < count($category_setting_infos);$a++){
		...
		$category_setting_info[gcs_state] = $category_setting_infos[$a][gcs_state];
		$category_setting_info[gcs_disp] = $category_setting_infos[$a][gcs_disp];

		include $_SERVER["DOCUMENT_ROOT"]."/admin/goodss/server_goods_list.act.php";
	}

	//syslog(LOG_INFO, $mall_ename.'도매아이탬 자동상품등록 END');
	closelog();
}
