				...
				for($i=0;$i < count($pid);$i++){
					...
					if($db->total){
						...
						if($db->total){
							...
							$sql = "select id,pid,opn_ix,option_div,option_price,option_coprice,option_stock,option_safestock,option_etc1,insert_yn from shop_product_options_detail where pid = '".$pid[$i]."' ";
							//echo $sql;
							$db->query ($sql);
							if($db->total){
