...
if($mode == "infoupdate"){
	$sql = "select * from co_product_category_info where cid = '$cid'";
	$db->query($sql);
	//echo $sql;
	if($db->total){
		$db->fetch();
...
if ($mode == "modify"){
	...
	$sql = "update co_product_category_info set 
	...
	if($del_num>0) {
		$sql="UPDATE co_product_category_info SET ".$del_query." WHERE cid = '$cid'";
		$db->query($sql);
	}

	if($category_use != "1"){
		$sql = "update co_product_category_info set category_use ='$category_use' where cid LIKE '".substr($cid,0,($this_depth+1)*3)."%' ";
		$db->query($sql);
	...
	//exit;
	updateCategoryXML();
	echo "<script language='JavaScript' src='/admin/_language/language.php'></Script><Script Language='JavaScript'>alert(language_data['category.save.php']['A'][language]);parent.document.location.href='category.php?cid=$cid';</Script>";
...
function ParentCategoryUseUpdate($mdb, $cid, $this_depth){
	...
	//$mdb->debug = true;
	$sql = "select cid, cname,  depth, category_use from co_product_category_info   $where ";
	
	$mdb->query($sql);
	
	$parent_category = $mdb->fetchall();
	for($i=0;$i < count($parent_category);$i++){
		$sql = "update co_product_category_info set category_use ='1' where cid = '".$parent_category[$i][cid]."' ";
		//echo $sql;
		$mdb->query($sql);
	}
...
if ($mode == "del"){
	...
	if (CheckSubCategory($cid,$this_depth)){
		if($sub_cartegory_delete == "1"){
			...
			$sql = "delete from co_product_category_info where cid LIKE '".substr($cid,0,($this_depth+1)*3)."%'";
			$db->query($sql);
			...
			$sql = "delete from co_product_category_info where cid = '$cid'";
			$db->query($sql);
...
if ($mode == "insert"){
	...
	if($sub_cid!="" || $cid!="") {// 카테고리 정보가 제대로 안넘어 올 경우를 검사 kbk 12/03/22
		$sql = "select * from co_product_category_info where cid = '$cid'";
		$db->query($sql);
		$db->fetch(0);
...
function getMaxlevel($cid,$depth)
{
	...
	$sPos = $depth*3;
	$sql = "select max(vlevel$strdepth)+1 as maxlevel from co_product_category_info where cid LIKE '".substr($cid,0,$sPos)."%'";
...
function CheckSubCategory($cid,$depth){
	...
	$endpos = $depth*3+3;
	$this_depth = $depth;
	$sql = "select * from co_product_category_info where depth > $this_depth and cid LIKE '".substr($cid,0,$endpos)."%'";
	$db->query($sql);
	echo "$sql<br>";
...
function updateCategoryXML(){
	...
	$xml = new XmlWriter_();
	$mdb = new Database;
	$mdb->query("select * from co_product_category_info where category_use = 1 order by vlevel1, vlevel2, vlevel3, vlevel4, vlevel5");
