...
if($act == "inversely_taxbill"){
	...
	for($i=0;$i<count($ar_ix);$i++){
		...
		$tax_per="1"; // 과세 면세 구분이 없네.. 일단 무조건 과세
		
		$p_tax_coprice = $db->dt[p_tax_coprice];//상품대금 공급가
		$p_tax_price = $db->dt[p_tax_price];//상품대금 세액
		$p_tax_total_price = $db->dt[p_tax_total_price];//상품대금 합계
		
		$d_tax_coprice = $db->dt[d_tax_coprice];//배송대금 공급가
		$d_tax_price = $db->dt[d_tax_price];//배송대금 세액
		$d_tax_total_price = $db->dt[d_tax_total_price];//배송대금 합계
		
		$total_tax_coprice = $p_tax_coprice+$d_tax_coprice; //총 공급가 합계
		$total_tax_price = $p_tax_price+$d_tax_price; // 총 세액 합계
		$total_tax_total_price = $p_tax_total_price+$d_tax_total_price; // 총 세액 합계
		
		$tex_item_info = array();
		
		$tex_item_info[0][product] = '상품대금';
		$tex_item_info[0][supply_price] = $p_tax_coprice;
		$tex_item_info[0][tax_price] = $p_tax_price;
		
		$tex_item_info[1][product] = '배송대금';
		$tex_item_info[1][supply_price] = $d_tax_coprice;
		...
		try {
			...
			if($result->code=="1"){
				...
				$sql = "update shop_accounts_remittance set tax_ix = '$p_idx' where ar_ix='$ar_ix[$i]'	";	
				$db->query($sql);	
	...
	echo "<script>alert('총: ".$t_cnt." 건중 성공: ".$s_cnt." 실패: ".$f_cnt." 건이 처리되었습니다.');</script>";
	if($f_cnt > 0){
...
if($act == "taxbill_issue"){

	if($tax_ix){
		include_once($DOCUMENT_ROOT."/admin/tax/popbill/common.php");
		$result = $TaxinvoiceService->Issue($com_num,ENumMgtKeyType::SELL,$tax_ix,$_SESSION["admininfo"]["company_name"]."(".$_SESSION["admininfo"]["charger_id"].") 발행",null,true);
		if($result->code=="1"){
			$SQL = "
				update  
					tax_sales 
			...
			$sql = "update shop_accounts_remittance set tax_no = '$tax_ix', tax_status='1', tax_date=NOW() where ar_ix='$ar_ix'";
			$db->query($sql);
...
if($act == "taxbill_request"){
    if($tax_ix){
        include_once($DOCUMENT_ROOT."/admin/tax/popbill/common.php");
        $db->query("SELECT * FROM tax_sales WHERE idx = '$tax_ix'");
        $db->fetch();

        $r_company_number = str_replace('-','',$db->dt[r_company_number]);
        //public function Request($CorpNum,$MgtKeyType,$MgtKey,$Memo = '',$UserID = null) {
        $result = $TaxinvoiceService->Request($r_company_number, ENumMgtKeyType::BUY, $tax_ix, $db->dt[s_company_name].' : 역)발행요청','bienbien');

        if($result->code=="1"){
            $SQL = "update tax_sales SET status ='1' where idx = '$tax_ix' ";
            $db->query($SQL);

            $sql = "update shop_accounts_remittance set tax_no = '$tax_ix', tax_status='1', tax_date=NOW() where ar_ix='$ar_ix'";
            $db->query($sql);

            echo "<script>alert('정상적으로 역발행되었습니다.');</script>";
            exit;
        }else{
            echo "<script>alert('".$result->message."');</script>";
            exit;
        ...
        echo "<script>alert('발행에 역실패 하였습니다.');</script>";
        exit;
    }
}

if($act == "getPopbillJoinMember"){

	$result = getPopbillJoinMember($_SESSION["admininfo"]["charger_ix"]);

	if($result->code==1){
		echo "<script>alert('가입이 정상적으로 되었습니다. ID : ".$_SESSION["admininfo"]["charger_id"]." PW : ".$_SESSION["admininfo"]["charger_id"]."!@#$ 입니다.');top.location.reload();</script>";
		exit;
	}else{
		echo "<script>alert('".$result->message."');</script>";
		exit;
