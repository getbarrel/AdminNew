										...
										<td align='right' style='padding-bottom:5px;'>";
											...
											if($_SESSION["admininfo"]["admin_level"] < 9){
												try {
													...
													/*
													$checkismember = $TaxinvoiceService->CheckIsMember($com_number,$LinkID);
													print_r($checkismember);
													*/
													...
													//if($popbillurl){
														$Contents .= "
														<a href=\"javascript:PopSWindow('../tax/popbill_pop.php?act=popbillurl&com_number=$com_number&charger_id=".$_SESSION["admininfo"]["charger_id"]."',820,560,'popbill_cert');\" >
									...
									<TR>
										...
										$Contents .= "
											...
											<tr>
												...
												<td class='search_box_item' colspan='3'>
													<input type='radio' name='tax_status' value='' id='tax_status_' ".($tax_status == "" ? "checked":"")."><label for='tax_status_'>전체</label> 
													<input type='radio' name='tax_status' value='0' id='tax_status_0' ".($tax_status == "0" ? "checked":"")."><label for='tax_status_0'>미발행</label> 
													<input type='radio' name='tax_status' value='1' id='tax_status_1' ".($tax_status == "1" ? "checked":"")."><label for='tax_status_1'>발행</label>
												</td>
											</tr>";
										if($_SESSION["admininfo"]["admin_level"]==9){
											$Contents .= "
...
$where = "WHERE ar.status in ('".ORDER_STATUS_ACCOUNT_COMPLETE."','".ORDER_STATUS_ACCOUNT_PAYMENT."') and (p_tax_total_price != 0 OR d_tax_price != 0) ";

if($pre_type=="inversely_apply"){//세금계산서 신청
	$where .= " and ar.tax_ix='0' ";
}elseif($pre_type=="inversely_complete"){//세금계산서 발급
	$where .= " and ar.tax_ix!='0' ";
}else{
	echo "잘못된 접근입니다.";
	exit;
...
if($tax_status!=""){
	$where .= " and ar.tax_status='".$tax_status."' ";
}
...
$Contents .= "
	...
	$Contents .= "
	<form name=listform method=post action='taxbill.act.php' onsubmit='return CheckStatusUpdate(this)' target='act'>
	...
	if($db->total){
		for ($i = 0; $i < $db->total; $i++)
		{
			...
			if($db->dt[tax_status]=="0"){
				$tax_status="미발행";
			}elseif($db->dt[tax_status]=="1"){
				$tax_status="발행";
			}
			...
			<tr height=28 >
				...
				<td class='list_box_td' align='center' nowrap>과세</b></td>
				<td class='list_box_td' align='center' nowrap>".number_format($db->dt[p_tax_coprice])."</td>
				<td class='list_box_td' align='center' nowrap>".number_format($db->dt[p_tax_price])."</td>
				<td class='list_box_td' align='center' nowrap>".number_format($db->dt[p_tax_total_price])."</td>
				<td class='list_box_td' align='center' nowrap>".number_format($db->dt[d_tax_coprice])."</td>
				<td class='list_box_td' align='center' nowrap>".number_format($db->dt[d_tax_price])."</td>
				<td class='list_box_td' align='center' nowrap>".number_format($db->dt[d_tax_total_price])."</td>
				<td class='list_box_td' align='center' nowrap>".number_format($db->dt[p_tax_total_price]+$db->dt[d_tax_total_price])."</td>";
				if($pre_type!="inversely_apply"){
					$Contents .= "
					<td class='list_box_td' align='center' nowrap>".$tax_status."</td>
					<td class='list_box_td' align='center' nowrap>".$db->dt[tax_no]."</td>
					<td class='list_box_td' align='center' nowrap>".substr($db->dt[tax_date],0,10)."</td>
					";
				}
				...
				<td class='list_box_td' align='center' nowrap>
					...
					if($pre_type=="inversely_complete"){
						if($db->dt[tax_status]=='0'){
							//$Contents .= "<input type='button' value='발행' onclick=\"taxbill_issue('".str_replace("-","",$db->dt[com_number])."','".$db->dt[tax_ix]."','".$db->dt[ar_ix]."');\" />";
                            $Contents .= "<input type='button' value='역발행' onclick=\"taxbill_request('".$db->dt[tax_ix]."','".$db->dt[ar_ix]."');\" />";
						}else{
					...
					if($db->dt[tax_ix]!='0'){
						$Contents .="<br><input type='button' value='상세보기' onclick=\"PopSWindow('../tax/sales_view.php?idx=".$db->dt[tax_ix]."&mmode=pop',1300,660,'sales_view');\" />";
					}
				$Contents .="
				</td>
			</tr>";
	...
	<table cellpadding=3 cellspacing=0 width=100% class='input_table_box' >
		...
		<tr id='ht_level0_status'>
			...
			<td class='input_box_item'>
				<input type='radio' name='act' id='inversely_taxbill' value='inversely_taxbill' onclick=\"\" checked><label for='inversely_taxbill' >세금계산서 발급</label>
			</td>
		</tr>
	</table>
	</div>
...
<script type='text/javascript'>
<!--
	function taxbill_issue(com_num,tax_ix,ar_ix){
		if(confirm('해당 세금계산서를 발행 하시겠습니까?')){
			window.frames['act'].location.href='./taxbill.act.php?act=taxbill_issue&com_num='+com_num+'&tax_ix='+tax_ix+'&ar_ix='+ar_ix;
		}
	}
	
	function taxbill_request(tax_ix,ar_ix){
		if(confirm('해당 세금계산서를 역발행 하시겠습니까?')){
			window.frames['act'].location.href='./taxbill.act.php?act=taxbill_request&tax_ix='+tax_ix+'&ar_ix='+ar_ix;
		}
	}
