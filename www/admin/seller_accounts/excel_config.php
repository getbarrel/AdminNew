...
if($excel_type == "account_list"){
	$excel_name = "정산예정내역 ";
	$act = "accounts_plan_excel";

	$conf_name="account_list_".$info_type;
	$conf_name_check="check_account_list_".$info_type;
	...
	foreach($colums as $key => $value){
		$Contents .= "<li class='ui-state-default leftmenu' id='".$key."' style='float:left;height:25px;width:150px;padding:4px 0px 0px 3px;margin:1px;' >
						...
						<tr>
							...
							<td>
							<input type='checkbox' id='colums' style='cursor:pointer;' name='colums[".$key."]' value='".$key."'  validation='false' title='".$value[title]."' ".($check_colums[$key] ? "checked":"")." >
							</td>
