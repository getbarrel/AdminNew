...
function selectedCouponReg(form_id){
	...
	//}
	var confirm_text = '쿠폰을 발급 하시겠습니까?';
...
function searchCouponReg(form_id){
	...
	if(search_issue_total > 0){
	    ...
	    if(total_count=='' || total_count == '0' || total_count == 0){
	        alert('발급 회원이 없습니다. 다시 검색 해주세요.');
	    }else{
            var first_person = '';
            //alert(form_id);
...
function resultExcelDown(){
    $('#member_search').find('input[name=view_type]').val('excel');
    $('#member_search').submit();
}
...
if(!$_REQUEST[mode]){
	$mode = "search";
		...
		if($db->dbms_type == "oracle"){
			...
			if($multi_search == 1){
                if($search_type != "" && $search_texts != "") {
                    $search_array = explode("\r\n", trim($search_texts));
                    if ($search_type == "name" || $search_type == "mail" || $search_type == "tel") {
		...
		if($startDate != "" && $endDate != ""){
		    if($_REQUEST['mode'] == 'result'){
                $where .= " and  cr.regdate between  '" . $startDate . " 00:00:00' and '" . $endDate . " 23:59:59' ";
            }else if ($_REQUEST['mode'] == 'statistics'){
                $where .= " and  cr.usedate between  '" . $startDate . " 00:00:00' and '" . $endDate . " 23:59:59' ";
            }else {
                if ($publish_div == "2") {
...
if($_REQUEST[mode] == "" || $_REQUEST[mode] == "search"){

    $sql = "SELECT mall_ix FROM ".TBL_SHOP_CUPON_PUBLISH." WHERE publish_ix = $publish_ix";
    $db->query($sql);
    $cinfo = $db->fetch();

    $coupon_mall_ix = $cinfo['mall_ix'];

    if(!empty($coupon_mall_ix)){
        $where .= " and cu.mall_ix in ('$coupon_mall_ix')";
    }
...
*/
	...
	if($db->dbms_type == "oracle"){
		...
		$sql = "select cr.regist_ix, cp.use_date_type,cr.mem_ix,AES_DECRYPT(UNHEX(cmd.name),'".$db->ase_encrypt_key."') as name, cu.id, cr.regdate, AES_DECRYPT(UNHEX(cmd.mail),'".$db->ase_encrypt_key."') as mail, cr.use_yn ,cr.use_sdate, cr.use_date_limit,
			...
			limit $start,$max ";
        */
        $sql = "select cr.regist_ix, cp.use_date_type,cr.mem_ix,AES_DECRYPT(UNHEX(cmd.name),'".$db->ase_encrypt_key."') as name, cu.id, cr.regdate, AES_DECRYPT(UNHEX(cmd.mail),'".$db->ase_encrypt_key."') as mail, cr.use_yn 
                ,cp.use_sdate, cp.use_edate as use_date_limit,
                cr.use_date_limit AS use_date_limit_cr, cr.use_sdate AS use_sdate_cr,
			case when (cr.use_sdate <= ".date("Ymd")." and ".date("Ymd")." <= cr.use_date_limit) then 1 else 0 end use_priod_yn
			from ".TBL_SHOP_CUPON_PUBLISH." cp, shop_cupon_regist cr, ".TBL_COMMON_USER." cu, ".TBL_COMMON_MEMBER_DETAIL." cmd, ".TBL_SHOP_GROUPINFO." mg
			".$where." and cp.publish_ix = '".$publish_ix."' AND cp.publish_ix = cr.publish_ix and cr.mem_ix = cu.code  
			order by name
    ...
    //echo '<hr><pre>'.print_r($issued_members).'</pre>';

    if($view_type == 'excel' && $total > 0){
        ...
        $productListXL->getActiveSheet(0)->setCellValue('B' . 1, "아이디");
        $productListXL->getActiveSheet(0)->setCellValue('C' . 1, "사용기간");
        $productListXL->getActiveSheet(0)->setCellValue('D' . 1, "등록일자");
        $productListXL->getActiveSheet(0)->setCellValue('E' . 1, "잔여기간");
        $productListXL->getActiveSheet(0)->setCellValue('F' . 1, "사용여부");
        ...
        for($i=0;$i<count($issued_members);$i++){
            ...
            if($issued_members[$i][use_date_type] != 9){
                if($issued_members[$i][use_yn] == 0) {
                    ...
                    if($issued_members[$i][use_date_type] == 2) {
                        if (date('Ymd', strtotime($issued_members[$i][use_sdate])) <= date("Ymd") && date("Ymd") <= date('Ymd', strtotime($issued_members[$i][use_date_limit_cr]))) {
                            $use_str = "사용가능";
                            $useable_yn = 1;
                            $dday = date_diff2(date("Y-m-d"), ChangeDate(date('Ymd', strtotime($issued_members[$i][use_date_limit_cr])), "Y-m-d"));
                        } else if (date('Ymd', strtotime($issued_members[$i][use_sdate])) > date("Ymd")) {
                            $use_str = "사용대기";
                            $useable_yn = 1;
                            $dday = date_diff2(date("Y-m-d"), ChangeDate(date('Ymd', strtotime($issued_members[$i][use_date_limit_cr])), "Y-m-d"));
                        } else {
                        ...
                        if (date('Ymd', strtotime($issued_members[$i][use_sdate])) <= date("Ymd") && date("Ymd") <= date('Ymd', strtotime($issued_members[$i][use_date_limit]))) {
                            $use_str = "사용가능";
                            $useable_yn = 1;
                            $dday = date_diff2(date("Y-m-d"), ChangeDate(date('Ymd', strtotime($issued_members[$i][use_date_limit])), "Y-m-d"));
                        } else if (date('Ymd', strtotime($issued_members[$i][use_sdate])) > date("Ymd")) {
                            $use_str = "사용대기";
                            $useable_yn = 1;
                            $dday = date_diff2(date("Y-m-d"), ChangeDate(date('Ymd', strtotime($issued_members[$i][use_date_limit])), "Y-m-d"));
            ...
            if($issued_members[$i][use_date_type] == 2) {
                $cu_date = ChangeDate(date('Ymd', strtotime($issued_members[$i][use_sdate_cr])), "Y-m-d") . "~" . ChangeDate(date('Ymd', strtotime($issued_members[$i][use_date_limit_cr])), "Y-m-d");
            }elseif($issued_members[$i][use_date_type]!=9){
                $cu_date = ChangeDate(date('Ymd',strtotime($issued_members[$i][use_sdate])),"Y-m-d")."~".ChangeDate(date('Ymd',strtotime($issued_members[$i][use_date_limit])),"Y-m-d");
            }else{
                $cu_date = "기간제한없음";
            }

            $productListXL->getActiveSheet()->setCellValue('A' . ($i + 2), $issued_members[$i][name]);
            $productListXL->getActiveSheet()->setCellValue('B' . ($i + 2), $issued_members[$i][id]);
            $productListXL->getActiveSheet()->setCellValue('C' . ($i + 2), $cu_date);
            $productListXL->getActiveSheet()->setCellValue('D' . ($i + 2), $issued_members[$i][regdate]);
            $productListXL->getActiveSheet()->setCellValue('E' . ($i + 2), $dday);
            $productListXL->getActiveSheet()->setCellValue('F' . ($i + 2), $use_str);
        }
...
// 첫번째 시트 선택
        ...
        header('Content-Disposition: attachment;filename="coupon_result.xls"');
	...
	$sql = "select sum(od.dcprice) as total_price, sum(odd.dc_price) as use_coupon_price from shop_cupon_regist cr					
					left join shop_order_detail_discount odd on cr.use_oid = odd.oid and cr.regist_ix = odd.dc_ix
	...
	if($db->dbms_type == "oracle"){
		...
		//and mem_type in ('M','C','F','S')
		$sql = "select cp.cupon_div,cr.regist_ix, cp.use_date_type,cr.mem_ix,AES_DECRYPT(UNHEX(cmd.name),'".$db->ase_encrypt_key."') as name, cu.id, cr.regdate, AES_DECRYPT(UNHEX(cmd.mail),'".$db->ase_encrypt_key."') as mail, cr.use_yn ,cr.use_sdate, cr.use_date_limit, cr.use_oid, cr.use_pid,
			case when (cr.use_sdate <= ".date("Ymd")." and ".date("Ymd")." <= cr.use_date_limit) then 1 else 0 end use_priod_yn
			from ".TBL_SHOP_CUPON_PUBLISH." cp, shop_cupon_regist cr, ".TBL_COMMON_USER." cu, ".TBL_COMMON_MEMBER_DETAIL." cmd, ".TBL_SHOP_GROUPINFO." mg
			".$where." and cp.publish_ix = '".$publish_ix."' AND cp.publish_ix = cr.publish_ix and cr.mem_ix = cu.code  and cr.use_yn != '0' 
			order by name
					...
					<table width=100% border=0 cellpadding=0 cellspacing=0>
					    ...
					    <td align='left' colspan=4 style='padding-bottom:20px;'>
					    	<div class='tab'>
									...
									<tr>
										<td class='tab'>
											<table id='tab_01' ".($_GET[mode] == ""  ? "class='on'":"")." >
											<tr>
												<th class='box_01'></th>
												<td class='box_02' onclick=\"document.location.href='?publish_ix=".$publish_ix."&mmode=".$mmode."&mem_ix=".$mem_ix."&cp_state=".$cp_state."'\">쿠폰 발급하기</td>
												<th class='box_03'></th>
											</tr>
											</table>
											...
											<tr>
												<th class='box_01'></th>
												<td class='box_02' onclick=\"document.location.href='?mode=result&publish_ix=".$publish_ix."&mmode=".$mmode."&mem_ix=".$mem_ix."&cp_state=".$cp_state."'\">쿠폰 발급현황(".$pre_issue_total.")</td>
												<th class='box_03'></th>
											</tr>
											</table>
											...
											<tr>
												<th class='box_01'></th>
												<td class='box_02' onclick=\"document.location.href='?mode=statistics&publish_ix=".$publish_ix."&mmode=".$mmode."&mem_ix=".$mem_ix."&cp_state=".$cp_state."'\">쿠폰통계</td>
												<th class='box_03'></th>
											</tr>
											</table>
...
if($_REQUEST[mode] == "result"){
	...
	$Contents .= "			<tr><td style='padding-left:0px;text-align:left;'><img src='../images/dot_org.gif' align=absmiddle> <b class='blue' id='coupon_kind'>".$cupon_kind."</b> 발급한 통계 목록입니다</td></tr>";
}else{
								...
								<tr style='padding-bottom:30px;'>
									...
									<table border='0' cellspacing='0' cellpadding='3' width='100%' class='table_box'>
										...
										<tr height='30' valign='middle'>
											...
											$Contents .= "
											<td class='search_box_title'> <b>발급수</b></td>
											<td class='search_box_item'>".$pre_issue_total."</td>
										</tr>
										<tr height='30' valign='middle'>
											<td class='search_box_title'> <b>사용수/할인총액</b></td>
											<td class='search_box_item' colspan='3'>".$user_total." / ".number_format($use_coupon_price)."</td>
                            <!--
...
$Contents .= "			<tr height=30px>
							<td style='padding-top:10px' colspan=2>
								...
								<tr> 
								    <td colspan='3'> 
								    <span style='font-size:11px; color:blue;'># 검색 데이터 량 제한을 풀기 위해 검색 기능을 get 방식에서 post 방식으로 변경 함에 따라 paging 버튼 동작이 불가능 함을 안내드립니다. 2019-11-15 </span> 
								    </td>
								</tr>
								<tr>
									<th class='box_04'></th>
									<td class='box_05' align=center >
										<form name='member_search' id='member_search' method='post' >
										<input type='hidden' name='dupe_check' value=''>
										<input type='hidden' name='sub_mode' value='search'>
										<input type='hidden' name='cp_state' value=".$cp_state.">
										...
										<table border='0' cellspacing='0' cellpadding='3' width='100%' class='search_table_box'>
											...
											<tr height=10>
												  <td class='search_box_title'  ><label for='regdate'><b>일자</b></label><input type='checkbox' name='regdate' id='regdate' value='1' ".($regdate == '1' ? "checked" : "")." onclick='ChangeRegistDate(document.member_search);'></td>
												  <td class='search_box_item' colspan=3 >";
                                                    if($_GET[mode] == ""){
                                                        $Contents .= "
                                                    <div style='padding:5px;'>
                                                        ...
                                                        <input type='radio' name='publish_div' id='publish_div_2' onFocus='this.blur();' align='middle' value='2' style='border:0px;' ".CompareReturnValue($publish_div,'2',' checked')."><label for='publish_div_2'>회원가입일검색</label>&nbsp;
                                                        <input type='radio' name='publish_div' id='publish_div_3' onFocus='this.blur();' align='middle' value='3' style='border:0px;' ".CompareReturnValue($publish_div,'3',' checked')."><label for='publish_div_3'>최근구매일검색</label>
                                                        <input type='radio' name='publish_div' id='publish_div_4' onFocus='this.blur();' align='middle' value='4' style='border:0px;' ".CompareReturnValue($publish_div,'4',' checked')."><label for='publish_div_4'>최근방문일검색</label>
													</div>";
													 }
...
$Contents .= "
												  ".search_date('startDate','endDate',$startDate,$endDate,'N','D',''.$disabled_select.' autocomplete="off" ')."
...
$Contents .= "
						<tr align=right>
							<td style='text-align:left;'>회원수 : ".$total." 명<input type=hidden name='search_total' id='search_total'  value='".$total."'></td>
...
$Contents .= "
									<tr height=26>
										...
										<td class='list_box_td point' >".wel_masking_seLen($members[$i][name], 1, 1)."</td>
										<td class='list_box_td list_bg_gray' >".$members[$i][id]."</td>
										<td class='list_box_td' >".wel_masking("E", $members[$i][mail])."</td>
										<td class='list_box_td list_bg_gray' >".wel_masking("P",$members[$i][pcs])."</td>
...
if($cp_state == 1) {//발급중지, 발급종료시 미노출
    ...
    $Contents .= "
									...
									<tr>
										<td colspan=5 align=right style='padding:10px 0;letter-spacing:0'>" . page_bar($total, $page, $max, $query_string, "") . "</td></tr>
    ...
    $Contents .= "
                                ...
                                <tr>
                                    <td colspan=4 style='padding:10px 0px' align=left>
                                        <a href=\"javascript:alert('쿠폰 발급 중지 상태 입니다. 상태변경 후 사용가능합니다.');\"><img src='../images/" . $admininfo["language"] . "/btn_selected_cupon_reg.gif' align=absmiddle  border=0></a>
                                        &nbsp;&nbsp;&nbsp;
                                        <a href=\"javascript:alert('쿠폰 발급 중지 상태 입니다. 상태변경 후 사용가능합니다.');\"><img src='../images/" . $admininfo["language"] . "/btn_searched_cupon_reg.gif' align=absmiddle border=0></a> 
                                        &nbsp;&nbsp;&nbsp; <label for='dupe_check'>※ 쿠폰 발급중지 상태입니다.</label>
                                    </td>
                                </tr>
                                <tr>
                                    <td colspan=5 align=right style='padding:10px 0;letter-spacing:0'></td></tr>
                            </table>
                            </form>
                        </td>
                    </tr>
...
$Contents .= "
                <tr height=30px>
					...
					if(checkMenuAuth(md5($_SERVER["PHP_SELF"]),"E")){
						$Contents .= "<a href='javascript:void(0);' onclick=\"resultExcelDown()\"><input type='image' src='../images/".$admininfo["language"]."/btn_excel_save.gif' style='cursor:pointer;' ></a>";
					}else{
...
$Contents .= "
						<tr height=30 align=center>
							...
							<td class='list_box_td'>".$cu_date."</td>
...
$Contents .= "
 ...
 $Contents .= "	</table>
						...
						<tr>
							...
							<td colspan=4 align=right style='padding:10px 0px;letter-spacing:0'>".page_bar($total, $page, $max,$query_string,"")."</td>
						</tr>
					</table>
				</td>
			</tr>
...
$Contents .= "
				...
				<tr height=30px>
					...
					if($total){
							...
							for($i=0;$i<count($issued_members);$i++){
                                ...
                                $sub_pname = "";
                                $sub_pcnt = "";
                                if($issued_members[$i]['cupon_div'] == 'C'){
                                    $sql = "select 
                                              sum(od.dcprice) as dcprice,
                                              sum(odd.dc_price) as couponprice, 
                                              od.oid,
                                              od.pname, 
                                              od.option_text, 
                                              od.pcode, 
                                    ...
                                    where od.oid = '".$issued_members[$i][use_oid]."' and odd.dc_ix = '".$issued_members[$i][regist_ix]."' and odd.dc_type = 'cp' ";
                                    $db->query($sql);
                                    $db->fetch();

                                    if($db->dt['cnt'] > 1){
                                        $sub_pname = " (외 ".($db->dt['cnt']-1) ."건)";
                                        $sub_pcnt = "총 ";
                                    }
                                }else{
            ...
            $Contents .= "
                                    <tr height=30 align=center>
                                        ...
                                        <td class='list_box_td'>".$db->dt[pname]."".$sub_pname."<br>".$pcode."</td>
                                        <td class='list_box_td list_bg_gray'>".$db->dt[option_text]."</td>
                                        <td class='list_box_td'> ".$sub_pcnt."".$db->dt[pcnt]."</td>
...
$Contents .= "
						<tr height=100><td colspan=8 class='list_box_td'  align=center>사용된 쿠폰이 없습니다.</td></tr>";
