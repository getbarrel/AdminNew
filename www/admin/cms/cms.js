...
function loadDataGroup(sel,target) {
	//alert(target);
	...
	window.frames['act'].location.href = 'data_group.load.php?form=' + form + '&trigger=' + trigger + '&target=' + target+'&depth=2';
...
function DeleteContent(di_ix){
	if(confirm('해당 컨텐츠를 정말로 삭제하시겠습니까?')){
		$.ajax({ 
			type: 'GET', 
...
var treeImageGroupData = [];
	...
	$.ajax({ 
		...
		url: './group.tree.php?mode=data_gruop',  
		dataType: 'json', 
		async: false, 
		beforeSend: function(){ 
...
$(document).ready(function() {
	...
	$('#tree_image_group').mouseover(function(){
		...
		$('#tree_image_group').css('height','350px');
	...
	$('#result_area').mouseover(function(){
		...
		$('#tree_image_group').css('width','157px').delay(3000);
