...
if ($act == "insert" || $act == "json_insert"){
	$sql = "select
                *
            from
                sellertool_category_linked_relation
            where
                origin_cid = '".$origin_cid."'
				AND target_cid = '".$target_cid."'
	...
	$sql = "insert into sellertool_category_linked_relation 
				...
				values
				('$cla_ix','$site_code','$origin_cid','$origin_name','$origin_depth','$target_cid','$target_name','$target_depth',NOW())";
	//echo $sql;
	//exit;
	$db->query($sql);

	//$db->query("update shop_category_linked_all set relation_yn = 'Y' where cla_ix = $cla_ix");
	//echo  "<script language='javacript'>alert('정상적으로 수정되었습니다.');parent.document.location.reload();</script>";
	if($act == "json_insert"){
		$db->query("SELECT clr_ix, rel_date FROM sellertool_category_linked_relation WHERE clr_ix=LAST_INSERT_ID()");
		$db->fetch();
...
if ($act == 'delete'){
        ...
        $db->query("delete from sellertool_category_linked_relation where clr_ix = '$clr_ix'");

       // $db->query("update shop_category_linked_all set relation_yn = 'N' where cla_ix = '$cla_ix'");

        echo 'SUCCESS';
}

if ($act == 'get_category_list'){
