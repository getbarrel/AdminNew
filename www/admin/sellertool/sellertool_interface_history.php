...
if($mode == "search"){
	...
	if($admininfo[admin_level] == 9)
	{
		...
		$sql = "SELECT  p.id, p.pname,p.brand, p.sellprice, p.regdate,p.vieworder,c.com_name,  r.cid,  p.search_keyword,state, p.brand_name,
		...
		FROM ".TBL_SHOP_PRODUCT." p, ".TBL_SHOP_PRODUCT_RELATION." r, ".TBL_COMMON_COMPANY_DETAIL." c , sellertool_log srr
...
// 	echo "<!-- #sql={$sql} -->";
	...
	if($act=='excel_save')
	{
		...
		$phpexcel->getProperties()->setTitle("제휴사 연동 로그 _" . str_replace('-', '', $sdate) . "-" . str_replace('-', '', $edate));
		$phpexcel->getProperties()->setSubject("제휴사 연동 로그 _" . str_replace('-', '', $sdate) . "-" . str_replace('-', '', $edate));
		$phpexcel->getProperties()->setDescription("제휴사 연동 로그 _" . str_replace('-', '', $sdate) . "-" . str_replace('-', '', $edate));
...
// 		$phpexcel_style->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
	...
	if ($cid2 == ""){
		if($admininfo[admin_level] == 9){
			...
			$sql = "SELECT  p.id as id, p.pname, p.brand, p.sellprice, p.regdate, p.vieworder,c.com_name, r.cid, p.search_keyword,state, p.brand_name,
			p.company, p.pcode, p.coprice, p.listprice, p.disp, p.editdate, p.reserve, p.reserve_rate, p.reserve_yn,
			...
			$sql = "SELECT  p.id as id ,p.brand, p.pname, p.sellprice, p.regdate, p.vieworder,c.com_name, r.cid, p.search_keyword,state, p.brand_name,
			p.company, p.pcode, p.coprice, p.listprice, p.disp, p.editdate, p.reserve, p.reserve_rate, p.reserve_yn,
			case when vieworder = 0 then 100000 else vieworder end as vieworder2,p.disp, srr.*
			FROM ".TBL_SHOP_PRODUCT." p, ".TBL_SHOP_PRODUCT_RELATION." r, ".TBL_COMMON_COMPANY_DETAIL." c, sellertool_log srr
		...
		if($admininfo[admin_level] == 9)
		{
			$sql = "SELECT p.id as id, p.pname, p.sellprice, p.regdate,c.com_name,p.vieworder,c.com_name, r.cid, p.search_keyword,state, p.brand, p.brand_name,
...
//		$where .= " and  date_format(p.regdate,'%Y%m%d') between  $sdate and $edate ";
	...
	$sql = "SELECT count(*) as total FROM ".TBL_SHOP_PRODUCT." p, ".TBL_SHOP_PRODUCT_RELATION." r , sellertool_log srr $where  ";
	//echo $sql;
	$db2->query($sql);
	...
	if ($cid2 == ""){
		if($admininfo[admin_level] == 9){
			...
			$sql = "SELECT count(*) as total FROM ".TBL_SHOP_PRODUCT." p, ".TBL_SHOP_PRODUCT_RELATION." r , sellertool_log srr   $addWhere ";

			if(!empty($_SERVER["QUERY_STRING"])){
				$db2->query($sql);
			}
		}else{
			$sql = "SELECT count(*) as total FROM ".TBL_SHOP_PRODUCT." p, ".TBL_SHOP_PRODUCT_RELATION." r , sellertool_log srr   where  p.id = r.pid AND p.product_type NOT IN (".implode(',',$sns_product_type).") and admin ='".$admininfo[company_id]." '";
		...
		if($admininfo[admin_level] == 9){
			$sql = "SELECT count(*) as total FROM ".TBL_SHOP_PRODUCT." p, ".TBL_SHOP_PRODUCT_RELATION." r , sellertool_log srr   where p.id = r.pid AND p.product_type NOT IN (".implode(',',$sns_product_type).") and r.basic = 1 and r.cid LIKE '".substr($cid2,0,$cut_num)."%' ";
			...
			$sql = "SELECT count(*) as total FROM ".TBL_SHOP_PRODUCT." p, ".TBL_SHOP_PRODUCT_RELATION." r , sellertool_log srr  where p.id = r.pid AND p.product_type NOT IN (".implode(',',$sns_product_type).") and r.basic = 1 and r.cid LIKE '".substr($cid2,0,$cut_num)."%' and admin ='".$admininfo[company_id]."' ";
...
<script  id='dynamic'></script>
	<tr>
		<td align='left' colspan=4> ".GetTitleNavigation("제휴사 연동 로그", "상품관리 > 제휴사 연동 로그")."</td>
	</tr>
...
$innerview = "
			...
			<table cellpadding=2 cellspacing=0 bgcolor=gray border=0 width=100% class='list_table_box'>
				...
				<tr bgcolor='#cccccc' align=center>
					...
					<td class=m_td nowrap>메세지</td>
					<td class=m_td >상품정보</td>
                    <td class=m_td nowrap>처리상태</td>
...
if($db->total == 0){
	...
	$innerview .= "<tr bgcolor='#ffffff' height=31>
						<td class='list_box_td list_bg_gray' ><input type=checkbox class=nonborder id='cpid' name='select_pid[]' value='".$db->dt[srl_ix]."'></td>
						<td align=center class=''>".$db->dt[site_code]."</td>
						
						<td class='list_box_td point' style='line-height:140%;padding:10px;text-align:left;'  >".$db->dt[result_msg]."";
if(false){
$innerview .= "
		...
		if($db->dt[result_code] != "200"){
										$innerview .= "<br><br><b style='color:red;'></b>";
...
$innerview .= "
					...
					<td align=center class='' style='text-align:left;padding:10px;line-height:150%;'>".$db->dt[pname]."</td>
...
//$Contents .= HelpBox("제휴사 연동 로그", $help_text);
