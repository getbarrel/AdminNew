...
if($div_ix){
	$db->query("SELECT * FROM shop_main_div where div_ix = '$div_ix' and agent_type = '".$agent_type."' ");
	$db->fetch();
...
$Contents01 = "
	<table width='100%' cellpadding=0 cellspacing=0 border='0' align='left'>
	  <tr >
		<td align='left' colspan=6 style='padding-bottom:10px;'> ".GetTitleNavigation("메인상품 그룹관리", "메인페이지 전시관리 > 메인상품 그룹관리 ")."</td>
	  </tr>	
	  <tr>
				...
				<table class='s_org_tab'>
					<tr>
						<td class='tab'>
							...
							<tr>
								<th class='box_01'></th>
								<td class='box_02'  ><a href='main_goods_category.php'>".($agent_type == "M" ? "모바일":"")." 메인전시 분류관리</a></td>
								<th class='box_03'></th>
							</tr>
							</table>
							<table id='tab_02' >
							<tr>
								<th class='box_01'></th>
								<td class='box_02' ><a href='main_goods_category_position.php'>".($agent_type == "M" ? "모바일":"")." 메인전시 위치관리</a></td>
								<th class='box_03'></th>
							</tr>
							</table>
						</td>
	  ...
	  <tr bgcolor=#ffffff >
		...
		<td class='input_box_item'><input type=text class='textbox2' name='div_code' value='".$db->dt[div_code]."' style='width:230px;' validation=true title='분류코드'> <span class=small></span></td>
	  </tr>
	  <tr bgcolor=#ffffff >
...
$sql = 	"SELECT *
		FROM shop_main_div where agent_type = '".$agent_type."'
		";
...
if($db->total){
	...
	$Contents02 .= "
		  <tr bgcolor=#ffffff height=30 align=center>	    
		    ...
		    <td class='list_box_td'>
		    	<!--a href=\"javascript:updateMainGoodsCategory('".$db->dt[div_ix]."','".$db->dt[div_name]."','".$db->dt[div_bbs_cnt]."','".$db->dt[disp]."')\"><img src='../images/".$admininfo["language"]."/btc_modify.gif' border=0></a-->
		    	<a href=\"?div_ix=".$db->dt[div_ix]."\"><img src='../images/".$admininfo["language"]."/btc_modify.gif' border=0></a>";
		    	if($db->dt[is_basic] == "0"){
	$Contents02 .= " <a href=\"javascript:deleteMainGoodsCategory('delete','".$db->dt[div_ix]."')\"><img src='../images/".$admininfo["language"]."/btc_del.gif' border=0></a>";
				}
	$Contents02 .= "    		
		    </td>
		  </tr> ";
	...
	$Contents02 .= "
		  <tr bgcolor=#ffffff height=50>
		    <td align=center colspan=5>등록된 ".($agent_type == "M" ? "모바일":"")."  메인전시 분류가 없습니다. </td>
		  </tr>";
}
$Contents02 .= "	   
	  </table>";
...
$Contents = $Contents."<form name='div_form' action='../display/main_goods_category.act.php' method='post' onsubmit='return CheckFormValue(this)' target='iframe_act'>
...
$help_text = "
	<table cellpadding=1 cellspacing=0 class='small' >
		...
		<tr><td valign=top><img src='/admin/image/icon_list.gif' ></td><td class='small' >사용안함으로 설정한 분류는 메인상품 상품 관리에서도 노출 되지 않습니다.</td></tr>
	</table>
	";

	
	$help_text = HelpBox("메인상품 그룹관리", $help_text);				
$Contents = $Contents.$help_text;	

 $Script = "
 <script language='javascript'>
 function updateMainGoodsCategory(div_ix,div_name,disp){
 ...
 function deleteMainGoodsCategory(act, div_ix){
...
if($agent_type == "M"  || $agent_type == "mobile"){
	...
	if($mmode == "pop"){
		...
		$P->addScript = $Script;
		$P->strLeftMenu = display_menu();
		$P->Navigation = "메인페이지 전시관리 > 메인상품 그룹관리";
		$P->NaviTitle = "메인상품 그룹관리";
		$P->strContents = $Contents;
		echo $P->PrintLayOut();	
	}else{
		$P = new LayOut();
