...
function getBrandDisplayDiv($selected="", $return_type = "selectbox"){
	$mdb = new Database;

	$sql = 	"SELECT *
			FROM shop_display_brand_div
			where disp=1 ";

	$mdb->query($sql);

	if($return_type == "selectbox"){
		$mstring = "<select name='div_ix' id='div_ix' validation=true title='브랜드 분류'>";
		$mstring .= "<option value=''>브랜드 분류</option>";
		if($mdb->total){
