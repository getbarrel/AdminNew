<?php
	...
	function SellerDisplayDelete(db_ix){
		if(confirm('해당 셀러 전시를 정말로 삭제 하시겠습니까? '))
		{//'해당 프로모션  정말로 삭제하시겠습니까? 삭제하시면 관련된 모든 이미지가 삭제됩니다.'
			window.frames['act'].location.href= 'seller_display.act.php?act=delete&db_ix='+db_ix;//kbk
				...
				<td align='left' >								
					...
					<tr>							
						<td class='tab' >
							...
							<tr>
								<th class='box_01'></th>
								<td class='box_02' onclick=\"document.location.href='seller_display_list.php'\">분류별 전시관리</td>
								<th class='box_03'></th>							
							</tr>
							</table>
							...
							<tr>
								<th class='box_01'></th>
								<td class='box_02' onclick=\"alert('준비중입니다')\">분류별 전시 결과분석</td><!--document.location.href='seller_display_report.php-->
								<th class='box_03'></th>				
							</tr>
							</table>
						</td>							
						<td align='right'>
							...
							<a href='#;' onclick='location.href=\"seller_display_div.php?display_div=".$display_div."\"'><img src='../images/".$admininfo["language"]."/btn_disp_div.gif' align=absmiddle ></a-->
							<a href='seller_display_div.php'><img src='../images/".$admininfo["language"]."/btn_disp_div.gif' align=absmiddle></a>
						</td>
					</tr>
					</table>										
					</div>					
					...
					<table cellpadding=0 cellspacing=1 border=0 width=100% align='center' class='search_table_box'>
						...
						<tr>
							...
							<td class='search_box_item'>
								...
								<input type='radio' name='display_type'  id='display_type_B' value='B' ".ReturnStringAfterCompare($display_type, "0", " checked")."><label for='display_type_B'>셀러관</label>
							</td>
	...
	$sql = 	"SELECT * FROM shop_display_seller_div where disp=1   ";
	...
	$help_text = HelpBox("셀러전시 목록", $help_text);
	...
	$P = new LayOut();
	$P->addScript = $Script;
	$P->Navigation = "프로모션/전시 > 셀러 전시관리 > 셀러전시 목록";
	$P->title = "셀러전시 목록";
	...
	function PrintPromotionGoods(){
		...
		if($div_ix){
			$sql = "select count(*) as total 
						from shop_display_seller ds 
						left join shop_display_seller_div cmd on ds.div_ix = cmd.div_ix 
						left join common_member_detail cmd2 on ds.md_mem_ix = cmd2.code
						where  cmd.disp = 1  
						and ds.div_ix = '$div_ix'";
		}else{
			$sql = "select count(*) as total from shop_display_seller ds 
						left join shop_display_seller_div cmd on ds.div_ix = cmd.div_ix 
						left join common_member_detail cmd2 on ds.md_mem_ix = cmd2.code
						where cmd.disp = 1  
						$where";
	...
	//echo $total;
		...
		if ($total == 0){
			...
			$sql ="select * from shop_display_seller_group where bg_ix <>'0' ";
			$db->query($sql);
			$group_info = $db->fetch();

			if($div_ix){
				$sql = "select ds.ds_ix,ds.display_type, ds.ds_title, dsd.div_name, ds.disp, ds.ds_use_sdate, ds.ds_use_edate , ds.md_mem_ix,
							case when '".date("Ymd")."' between ds.ds_use_sdate and ds.ds_use_edate then 1 else 0 end as is_live
							from shop_display_seller db 
							left join shop_display_seller_div dsd  on ds.div_ix = dsd.div_ix 
							left join common_member_detail cmd2 on ds.md_mem_ix = cmd2.code
							where  dsd.disp = 1 and ds.div_ix = '$div_ix'  
			...
			for($i=0;$i < $db->total;$i++){
				...
				$sql = "SELECT distinct p.id, p.pcode, p.pname, p.sellprice,
								...
								FROM ".TBL_SHOP_PRODUCT." p, shop_display_Seller_relation cmpr
				...
				*/
				...
				<td class='list_box_td'>".($db->dt[display_type] == "C" ? "분류별전시":"셀러관")."</td>
				...
				*/
				...
				<!--".ChangeDate($db->dt[ds_use_sdate],"Y.m.d")." ~ ".ChangeDate($db->dt[ds_use_edate],"Y.m.d")."-->
				".substr($db->dt[ds_use_sdate],0,10)." ~ ".substr($db->dt[ds_use_edate],0,10)."
				</td>
				<td class='list_box_td'>";
				if(checkMenuAuth(md5($_SERVER["PHP_SELF"]),"U")){
					$mString .= "<a href='seller_display.php?db_ix=".$db->dt[db_ix]."'><img src='../images/".$admininfo["language"]."/bts_modify.gif' border=0></a> ";
				}else{
					$mString .= "<a href=\"".$auth_update_msg."\"><img src='../images/".$admininfo["language"]."/bts_modify.gif' border=0></a> ";
				}

				if(checkMenuAuth(md5($_SERVER["PHP_SELF"]),"D")){
				$mString .= "<a href=\"JavaScript:SellerDisplayDelete('".$db->dt[db_ix]."')\"><img  src='../images/".$admininfo["language"]."/btc_del.gif' border=0></a>";
				}else{
...
//						if(checkMenuAuth(md5($_SERVER["PHP_SELF"]),"C")){
							$mString .= "<a href='seller_display.php'><img src='../images/".$admininfo["language"]."/b_promotionadd.gif' border=0 ></a>";
//						}else{
