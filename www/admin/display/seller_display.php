<?php
	...
	include("./seller_display.lib.php");
	...
	$sql = "SELECT ds_title, ds.ds_ix, ds.display_type, ds.div_ix,  ds_use_sdate , ds_use_edate, ds.disp, ds.cid
				FROM shop_display_seller ds, shop_display_seller_div cmd 
				where ds.div_ix = cmd.div_ix and ds.ds_ix ='$ds_ix' 
				order by ds_use_edate desc limit 1";
	...
	if($db->total){
		...
		$ds_ix = $db->dt[ds_ix];
		$ds_title = $db->dt[ds_title];
		$ds_use_sdate = substr($db->dt[ds_use_sdate],0,10);
		$ds_use_stime = substr($db->dt[ds_use_sdate],11,2);
		$ds_use_smin = substr($db->dt[ds_use_sdate],13,2);

		$ds_use_edate = substr($db->dt[ds_use_edate],0,10);
		$ds_use_etime = substr($db->dt[ds_use_edate],11,2);
		$ds_use_emin = substr($db->dt[ds_use_edate],13,2);
		$disp = $db->dt[disp];
		$cid = $db->dt[cid];
		$depth = $db->dt[depth];
	...
	if($ds_ix=="" || !$ds_ix) $ds_ix="H";
	else $ds_ix=$_GET["ds_ix"];

	$img_pre_text=$ds_ix."_";
	...
	<script type='text/javascript' src='../js/ms_sellerSearch.js'></script>
	...
	<table width='100%' border='0' cellpadding=0 cellspacing=0>
	 <tr>
		<td align='left' colspan=6 > ".GetTitleNavigation("셀러 전시관리", "전시관리 > 셀러 전시관리 ")."</td>
	</tr>
	<tr>
		<td align='left' >								
			...
			<tr>							
				<td class='tab' >
					...
					<tr>
						<th class='box_01'></th>
						<td class='box_02' onclick=\"document.location.href='?display_type=C'\">분류별 셀러 전시</td>
						<th class='box_03'></th>							
					</tr>
					</table>
					...
					<tr>
						<th class='box_01'></th>
						<td class='box_02' onclick=\"document.location.href='?display_type=B'\">셀러관 관리</td>
						<th class='box_03'></th>				
					</tr>
					</table>
				</td>							
	...
	<tr>
		...
		if($display_type == "C"){ 
			$Contents .= "<div  ><b style='font-size:15px;'>분류별 셀러 전시</b> - 상품 분류별로 노출될 셀러를 설정하실 수 있습니다.</div>";
		}else{
			$Contents .= "<div  ><b style='font-size:15px;'>셀러관  전시</b> - 상품 분류별로 노출될 셀러를 설정하실 수 있습니다.</div>";
		}
	  ...
	  <tr>
		<td>
			...
			<input type='hidden' name='ds_ix' value='".$ds_ix."'>
	...
	$Contents .= "
						  <tr height=28>
							<td class='search_box_title'  nowrap> <b>셀러 전시 분류</b></td>
	...
	$Contents .= "
						  <tr height=28>
							<td class='search_box_title' nowrap> <b>셀러 전시 제목</b></td>
							<td class='search_box_item' colspan=3><input class='textbox' type='text' name='ds_title' value='".$db->dt[ds_title]."' validation=true title='전시 제목' maxlength='50' style='width:300px'></td>
						  </tr>
						 <tr height=27>
							  ...
							  <td class='search_box_item'  colspan=3>
								<table cellpadding=0 cellspacing=2 border=0 bgcolor=#ffffff width=100%>
									...
									<tr>
										<td nowrap>
										<input type='text' name='ds_use_sdate' class='textbox' value='".$ds_use_sdate."' style='height:18px;width:70px;text-align:center;' id='start_datepicker'>
										</td>
										<td>일</td>
										<td nowrap>
										<SELECT name=ds_use_stime>";
						for($i=0;$i < 24;$i++){
		$Contents .= "<option value='".$i."' ".($ds_use_stime == $i ? "selected":"").">".$i."</option>";
						}
		$Contents .= "
						</SELECT> 시
						<SELECT name=ds_use_smin>";
						for($i=0;$i < 60;$i++){
		$Contents .= "<option value='".$i."' ".($ds_use_smin == $i ? "selected":"").">".$i."</option>";
						}
		$Contents .= "
						</SELECT> 분
										</td>
										<td align=center> ~ </td>
										<td nowrap>
										<input type='text' name='ds_use_edate' class='textbox' value='".$ds_use_edate."' style='height:18px;width:70px;text-align:center;' id='end_datepicker'>
										</td>
										<td>일</td>
										<td nowrap>
										<SELECT name=ds_use_etime>";
						for($i=0;$i < 24;$i++){
		$Contents .= "<option value='".$i."' ".($ds_use_etime == $i ? "selected":"").">".$i."</option>";
						}
		$Contents .= "
						</SELECT> 시
						<SELECT name=ds_use_emin>";
						for($i=0;$i < 60;$i++){
		$Contents .= "<option value='".$i."' ".($ds_use_emin == $i ? "selected":"").">".$i."</option>";
						}
		$Contents .= "
						</SELECT> 분
										</td>
	...
	$gdb = new MySQL;
	$gdb->query("SELECT * FROM shop_display_seller_group WHERE ds_ix='".$ds_ix."' order by group_code asc ");
	...
	$Contents .= "
						  ...
						  <col width='*'>
							...
							<tr>
							  <td class='search_box_title'><b>셀러 그룹명 이미지</b></td>
	...
	if(file_exists("$DOCUMENT_ROOT".$admin_config[mall_data_root]."/images/display_banner/".$ds_ix."/display_banner_group_".($i+1).".gif")){
		$Contents .= "<img src='".$admin_config[mall_data_root]."/images/display_banner/".$ds_ix."/display_banner_group_".($i+1).".gif'>";
	}
	...
	$Contents .= "						</div><br>
							...
							<!--tr>
							  <td class='search_box_title'><b>셀러 그룹명 이미지링크</b></td>
							  <td class='search_box_item'>
	...
	$Contents .= "
							...
							<tr>
							  ...
							  <div id='goods_manual_area_".($i+1)."'>
								  <a href=\"javascript:\" onclick=\"ms_sellerSearch.show_productSearchBox(event,".($i+1).",'brandList_".($i+1)."');\"><img src='../images/".$admininfo["language"]."/btn_goods_search_add.gif' border=0 align=absmiddle></a><br>
								  <div style='width:100%;padding:5px;' id='group_product_area_".($i+1)."' >".relationEventGroupProductList(($i+1), "clipart",$ds_ix)."</div>
								...
								<table border=0 cellpadding=0 cellspacing=0 width='100%' style='margin-top:5px;padding:5px 10px 5px 10px;border:1px solid silver' >
									...
									<tr>
										<td style='padding-top:5px;'>";
											...
											//	$Contents .= PrintCategoryRelation(($i+1),$ds_ix);
	...
	switch($ds_ix) {
		case ("H") : 
			$P->Navigation = "프로모션/전시 > 셀러 전시관리 > 셀러전시 등록/수정";
			$P->title = "셀러전시 등록/수정";
		break;
		case ("R") : 
			$P->Navigation = "프로모션/전시 > 셀러 전시관리 > 추천도매 셀러관리";
			$P->title = "추천도매 셀러관리";
		break;
		default : 
			$P->Navigation = "프로모션/전시 > 셀러 전시관리 > 셀러전시 등록/수정";
	...
	function PrintCategoryRelation($group_code,$ds_ix="M"){
		global $db ,$admininfo;

		$sql = "select c.cid,c.cname,c.depth, r.mcr_ix, r.regdate  from shop_display_seller_relation r, ".TBL_SHOP_CATEGORY_INFO." c where group_code = '".$group_code."' and c.cid = r.cid AND r.ds_ix='".$ds_ix."' ";
	...
	function relationEventGroupProductList($group_code, $disp_type="",$ds_ix="M"){
		...
		$sql = "SELECT count(*) as total 
						FROM shop_seller mb 					
						LEFT JOIN shop_display_seller_relation br ON mb.b_ix=br.b_ix 
						where mb.b_ix IS NOT NULL and br.ds_ix='".$ds_ix."' and group_code = '".$group_code."'
						 ";
		...
		if ($db->total == 0){
			...
			if($disp_type == "clipart"){
				$mString = '<ul id="brandList_'.$group_code.'" name="brandList" class="brandList"></ul>'."\n";
				$mString .= '<script>'."\n";
				$mString .= 'ms_sellerSearch.groupCode = '.$group_code.";\n";
				...
				for($i=0;$i<$db->total;$i++){
					...
					$mString .= 'ms_sellerSearch._setBrand("brandList_'.$group_code.'", "M", "'.$db->dt['b_ix'].'", "'.$imgPath.'", "'.addslashes(addslashes(trim($db->dt['brand_name']))).'", "'.addslashes(addslashes(trim($db->dt['brand_name']))).'", "'.$db->dt['brand_name'].'");'."\n";
				}
				$mString .= '</script>'."\n";
			}
		}
	...
	CREATE TABLE `shop_display_seller_relation` (
