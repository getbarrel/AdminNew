...
$sql = "select gc.uid from shop_offline_cupon gc left join common_member_detail m on gc.member_id = m.code  $where ";
//echo $sql ;
...
if($mode == "excel"){
	...
	$db->query("select gc.*,m.code
					from (select * from shop_offline_cupon gc $where order by uid desc ) gc left join common_member_detail m on gc.member_id = m.code "); //where uid = '$code'	
	$mstring = "<table border=1>";
	$mstring .= "<tr><td>번호</td><td>상품권구분</td><td>회차</td><td>시리얼</td><td>금액</td><td>사용유효기간</td><td>등록ID</td><td>등록자아이피</td><td>등록일</td><td>발행메모</td><td>생성일</td></tr>";
	for($i=0;$i < $db->total;$i++){
		...
		$mstring .= "<tr><td>".($i+1)."</td>
			<td>$gift_type_str</td>
			<td>".$db->dt[event_gift_num]."</td>
			<td>".$gift_code_str."</td>
			<td>".number_format($db->dt[gift_amount])."원</td><td>".$db->dt[gift_start_date]."~".$db->dt[gift_end_date]."</td><td>".$db->dt[member_id]."</td><td>".$db->dt[member_ip]."</td><td>".$db->dt[chagne_request_date]."</td><td>".$db->dt[memo]."</td><td>".$db->dt[reg_date]."</td></tr>";
	...
	$sql = "select gc.*, m.code
					from (select gc.* from shop_offline_cupon gc left join common_member_detail m on gc.member_id = m.code $where order by uid desc LIMIT $start, $max ) gc left join common_member_detail m on gc.member_id = m.code 
					";//, DATE_FORMAT(m.regdate, '%Y.%m.%d %H:%i;%s') as disp_regdate 

	$sql = "select gc.* , m.code from shop_offline_cupon gc left join common_member_detail m on gc.member_id = m.code $where order by uid desc LIMIT $start, $max";
...
$Contents01 = "
	<table width='100%' cellpadding=3 cellspacing=0 border='0' align='left'>
	  <tr >
		<td align='left' colspan=6 style='padding-bottom:10px;'> ".GetTitleNavigation("상품권 일괄관리", "마케팅관리 > 상품권 일괄관리 ")."</td>
	  </tr>
	  <tr>
			<td>
								...
								<td class='box_05' valign=top>	
									...
									<TR>
										...
										<table cellpadding=2 cellspacing=1 width='100%' class='search_table_box'>
											<tr>
												<th bgcolor='#efefef' class='search_box_title'>상태 : </th>
												<td colspan=2>
											...
											<tr>
												<th bgcolor='#efefef' class='search_box_title'>조건검색 : </th>
												<td colspan=2>
											 ...
											 <tr height=27>
											  <td bgcolor='#efefef' class='search_box_title'><label for='regdate'><b>사용가능일자</b></label><input type='checkbox' name='regdate' id='regdate' value='1' onclick='ChangeRegistDate(document.searchmember);'></td>
...
$Contents02 = "
	<table width='99%' cellpadding=3 cellspacing=0 border='0' align='left' >	  
	  <!--tr>
	  ...
	  <form name=baymoney_list method=post action='member.act.php' onsubmit='return CheckDelete(this)' target='iframe_act'>
		...
		<table width='99%' cellpadding=3 cellspacing=0 border='0' align='left' class='list_table_box' >		
		<tr bgcolor=#efefef align=center height=28>
			...
			<td class='m_td' width=10%>상품권구분 </td>
			<td class='m_td' width=5%>회차 </td>
			<td class='m_td' width=*>상품권번호 </td>
			<td class='m_td' width=12%>사용가능기간</td>
			<!--td class='m_td' width=10%>상품권금액</td-->
			<td class='m_td' width=10% >상태</td>
			<td class='m_td' width=12% >등록일자</td>
			<td class='m_td' width=10% >회원명</td>
			<td class='m_td' width=15% >메모</td>
			<td class='e_td' width=8% >관리 </td>
		</tr>";
...
if($db->total){
	for($i=0;$i  < $db->total; $i++){
		...
		$Contents02 .= "<tr height=28 align=center>
				<td bgcolor='#ffffff'><input type=checkbox class=nonborder id='giftcertificate_id' name=uid[] value='".$db->dt[uid]."'></td>
				<td bgcolor='#efefef'>".$gift_type_str."</td>
				<td bgcolor='#ffffff'>".$db->dt[event_gift_num]."</td>
				...
				<td bgcolor='#ffffff'>".$db->dt[reg_date]."</td>
				<td bgcolor='#efefef'><!-- a href=\"javascript:PoPWindow('baymoney.pop.php?code=".$db->dt[code]."',650,550,'baymoney_pop')\" -->".$db->dt[name]."</a></td>
				<td bgcolor='#ffffff'>".$db->dt[memo]."</td>
				<td bgcolor='#efefef'><a href=\"javascript:DeleteGiftCertificate('".$db->dt[uid]."')\"><img src='../image/btc_del.gif' border=0></a></td>
			</tr>
			<tr hegiht=1><td colspan=10 class='dot-x'></td></tr></table>
	...
	$Contents02 .= "</form>";
		...
		$Contents02 .= "
			<tr height=60><td colspan=10 align=center>상품권 내용이 없습니다.</td></tr>
			";

}

	$Contents02 .= "<tr height=40><td colspan=10 align=left><!--a href=\"JavaScript:SelectDelete(document.forms['baymoney_list']);\"><img  src='../image/bt_all_del.gif' border=0 align=absmiddle ></a--><a href='offline_cupon_write.php'><img  src='../image/btc_giftcard.gif' border=0 align=absmiddle >상품권발급</a></td></tr>";

$Contents02 .= "<tr height=40><td colspan=10 align=center>".page_bar($total, $page, $max,"&code=$code&gift_change_state=$gift_change_state&search_type=$search_type&search_text=".urlencode($search_text)."&FromYY=$FromYY&FromMM=$FromMM&FromDD=$FromDD&ToYY=$ToYY&ToMM=$ToMM&ToDD=$ToDD","")."</td></tr>";
$Contents02 .= "</table>";
...
$P->Navigation = "HOME > 프로모션/전시 > 오프라인 상품권관리";
$P->title = "오프라인 상품권관리";
