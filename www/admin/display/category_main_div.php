...
if($div_ix){
	$mdb->query("SELECT * FROM shop_category_main_div where div_ix = '$div_ix' ");
	$mdb->fetch();
...
$Contents01 = "
	<table width='100%' cellpadding=0 cellspacing=0 border='0' align='left'>
	  <tr >
		<td align='left' colspan=6 > ".GetTitleNavigation("카테고리메인 분류관리", "전시관리 > 카테고리메인 분류관리 ")."</td>
	  </tr>
	   <tr>
...
$Contents01 .= "
	  <tr bgcolor=#ffffff height=25>
	    ...
	    <td class='input_box_item'>
	    	...
	    	<input type=radio name='disp' value='0' ".(($mdb->dt[disp] == 0) ? "checked":"" )." id='disp_0'><label for='disp_0'>사용안함</label>
	    </td>
	  </tr>
	  </table>";
...
$sql = 	"SELECT cmd.*, ci.cname
			FROM shop_category_main_div cmd , shop_category_info ci
			where  cmd.cid = ci.cid ";
*/

$sql = 	"SELECT *	FROM shop_category_main_div order by cid";
...
$Contents = "<form name='div_form' action='category_main_div.act.php' method='post' onsubmit='return CheckFormValue(this)' target='act'><input name='mmode' type='hidden' value='$mmode'>
...
$help_text = "
	...
	$help_text = HelpBox("카테고리메인 분류관리", $help_text);
...
if($mmode == "pop"){
	...
	$P->Navigation = "전시관리 > 분류 전시관리 > 분류별 전시 분류관리";
	$P->NaviTitle = "분류별 전시 분류관리";
