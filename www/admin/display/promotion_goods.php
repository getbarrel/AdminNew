...
<table width='100%' cellpadding=0 cellspacing=0 border='0' align='left'>
 <tr>
	<td align='left' colspan=6 > ".GetTitleNavigation("컨텐츠관리", "프로모션 전시관리 > 프로모션 상품등록")."</td>
</tr>";
if(false){
$Contents .= "
<tr>
		...
		<input type='hidden' name=agent_type value='".$agent_type."'>
			...
			<table class='box_shadow' style='width:100%;' cellpadding='0' cellspacing='0' border='0'>
				...
				<tr>
					...
					$Contents .= "
					  <tr height=28>
						<td class='search_box_title'  nowrap> <b>프로모션전시 분류</b></td>
						<td class='search_box_item'>";
						$Contents .= "".(($div_ix == "" || true) ? getMainDiv($div_ix):getMainDiv($div_ix,"text"))."";
						$Contents .= "
						</td>
						<td class='search_box_title' > <b>노출여부</b> </td>
						<td class='search_box_item'  >
						<input type='hidden' name='pop' value='1'> <input type='radio' name='disp' id='disp_1' value='1' ".CompareReturnValue("1",$disp,"checked")." validation=true title='노출여부'> <label for='disp_1' >노출</label> <input type='radio' name='disp' id='disp_0' value='0' ".CompareReturnValue("0",$disp,"checked")." validation=true title='노출여부'><label for='disp_0' >노출안함</label>
						</td>
					  </tr>
					  <tr height=28>
					  ...
					  <tr style='display:none'>
						  <td class='search_box_title'><b>프로모션전시 이미지</b></td>
						  <td class='search_box_item' style='padding:10px' colspan=3>
						  <input type='file' class='textbox' name='pg_title_img' id='pg_title_img' size=50 value=''> "; 
if(file_exists("$DOCUMENT_ROOT".$admin_config[mall_data_root]."/images/promotion/promotion_title_img_".$pg_ix.".jpg") && $pg_ix){	
					$Contents .= "
						  <input type='checkbox' name='pg_title_img_del' id='pg_title_img_del' size=50 value='Y' style='vertical-align:middle;'><label for='pg_title_img_del'>메인전시제목 이미지 삭제</label><br>";
}
					$Contents .= "
						  <div style='padding:5px 5px 5px 0px;height:100px;width:90%;overflow:auto;max-width:1100px;' id='main_img_area'>";
if(file_exists("$DOCUMENT_ROOT".$admin_config[mall_data_root]."/images/promotion/promotion_title_img_".$pg_ix.".jpg") && $pg_ix){//메인분류추가에 따른 이미지 명 변경 kbk 13/05/09
	$Contents .= "<img src='".$admin_config[mall_data_root]."/images/promotion/promotion_title_img_".$pg_ix.".jpg' style=''>"; 
}
...
$Contents .= "
					  ...
					  <col width='35%'>
						...
						<tr-->
						  ...
						  <td class='search_box_item'>
						  <input type='text' class='textbox numeric' name='product_cnt[".($i+1)."]' id='product_cnt_".($i+1)."' size=10 value='".$gdb->dt[product_cnt]."'>
						  </td>
						</tr>
						<tr>
						  ...
						  <div style='padding:5px 5px 5px 0px;width:460px; height: 200px; overflow:auto' id='group_img_area_".($i+1)."'>";
if(file_exists($_SERVER["DOCUMENT_ROOT"].$admin_config[mall_data_root]."/images/promotion/".$pg_ix."_promotion_group_".($i+1).".gif")){//프로모션분류추가에 따른 이미지 명 변경 kbk 13/05/09
	$Contents .= "<img src='".$admin_config[mall_data_root]."/images/promotion/".$pg_ix."_promotion_group_".($i+1).".gif'>";
} else {
	/*
	if(file_exists("$DOCUMENT_ROOT".$admin_config[mall_data_root]."/images/promotion/promotion_group_".($i+1).".gif")){
		$Contents .= "<img src='".$admin_config[mall_data_root]."/images/promotion/promotion_group_".($i+1).".gif'>";
	}
	*/
}
...
<input type='file' class='textbox' name='group_over_img[".($i+1)."]' id='group_over_img' size=50 value=''> <input type='checkbox' name='group_over_img_del[".($i+1)."]' id='group_over_img_del_".($i+1)."' size=50 value='Y' style='vertical-align:middle;'><label for='group_over_img_del_".($i+1)."'>그룹이미지 삭제</label><br>
						  <div style='padding:5px 5px 5px 0px;width:460px; height: 200px; overflow:auto' id='group_over_img_area_".($i+1)."'>";
if(file_exists($_SERVER["DOCUMENT_ROOT"].$admin_config[mall_data_root]."/images/promotion/".$mg_ix."_promotion_group_over_".($i+1).".gif") && $mg_ix){//메인분류추가에 따른 이미지 명 변경 kbk 13/05/09
	$Contents .= "<img src='".$admin_config[mall_data_root]."/images/promotion/".$mg_ix."_promotion_group_over_".($i+1).".gif'>";
}
...
if($pg_ix && file_exists("$DOCUMENT_ROOT".$admin_config[mall_data_root]."/images/promotion/".$pg_ix."_promotion_group_banner_".($i+1).".gif")){//프로모션분류추가에 따른 이미지 명 변경 kbk 13/05/09
	$Contents .= "<img src='".$admin_config[mall_data_root]."/images/promotion/".$pg_ix."_promotion_group_banner_".($i+1).".gif'>";
/*
} else {
	if($pg_ix && file_exists("$DOCUMENT_ROOT".$admin_config[mall_data_root]."/images/promotion/promotion_group_banner_".($i+1).".gif")){
		$Contents .= "<img src='".$admin_config[mall_data_root]."/images/promotion/promotion_group_banner_".($i+1).".gif'>";
	}
*/
}
...
$Contents .= "						</div><br>
						...
						<tr style='display:none'>
						  ...
						  <td class='search_box_item promotion_types' style='padding:10px 5px;' colspan=3>";
						  
						  $Contents .= "
							  <div class='promotion_type_box' style='float:left;text-align:center;width:138px;margin:30px 0 0 20px;'>
							  	<img src='../images/".$admininfo["language"]."/P_type_1".($gdb->dt[display_type] == "0" ? "_on":"").".png' align=center ><br>
							  	<input type='radio' class='textbox' name='display_type[".($i+1)."]' id='display_type_".($i+1)."_0' value='0' style='border:0px;' ".($gdb->dt[display_type] == "0" ? "checked":"").">
							  	<label for='display_type_".($i+1)."_0'>A-type(10EA)</label>
							  </div>
							  <div class='promotion_type_box' style='float:left;text-align:center;width:138px;margin:30px 0 0 20px;'>
							  	<img src='../images/".$admininfo["language"]."/P_type_2".($gdb->dt[display_type] == "1" ? "_on":"").".png' align=center ><br>
							  	<input type='radio' class='textbox' name='display_type[".($i+1)."]' id='display_type_".($i+1)."_1' value='1' style='border:0px;' ".($gdb->dt[display_type] == "1" ? "checked":"").">
							  	<label for='display_type_".($i+1)."_1'>B-type(8EA)</label>
							  </div>";
						  if($agent_type == "W"){
						  $Contents .= "
							  <div class='promotion_type_box' style='float:left;text-align:center;width:138px;margin:30px 0 0 20px;'>
							  	<img src='../images/".$admininfo["language"]."/P_type_3".($gdb->dt[display_type] == "2" ? "_on":"").".png' align=center ><br>
							  	<input type='radio' class='textbox' name='display_type[".($i+1)."]' id='display_type_".($i+1)."_2' value='2' style='border:0px;' ".($gdb->dt[display_type] == "2" ? "checked":"").">
							  	<label for='display_type_".($i+1)."_2'>C-type(7EA)</label>
							  </div>
							  <div class='promotion_type_box' style='float:left;text-align:center;width:138px;margin:30px 0 0 20px;'>
							  	<img src='../images/".$admininfo["language"]."/P_type_4".($gdb->dt[display_type] == "3" ? "_on":"").".png' align=center ><br>
							  	<input type='radio' class='textbox' name='display_type[".($i+1)."]' id='display_type_".($i+1)."_3' value='3' style='border:0px;' ".($gdb->dt[display_type] == "3" ? "checked":"").">
							  	<label for='display_type_".($i+1)."_3'>D-type(5EA)</label>
							  </div>
							  <div class='promotion_type_box' style='float:left;text-align:center;width:138px;margin:30px 0 0 20px;'>
							  	<img src='../images/".$admininfo["language"]."/P_type_5".($gdb->dt[display_type] == "4" ? "_on":"").".png' align=center ><br>
							  	<input type='radio' class='textbox' name='display_type[".($i+1)."]' id='display_type_".($i+1)."_4' value='4' style='border:0px;' ".($gdb->dt[display_type] == "4" ? "checked":"").">
							  	<label for='display_type_".($i+1)."_4'>E-type(6EA)</label>
							  </div>
							  <div class='promotion_type_box' style='float:left;text-align:center;width:138px;margin:30px 0 0 20px;'>
							  	<img src='../images/".$admininfo["language"]."/P_type_6".($gdb->dt[display_type] == "5" ? "_on":"").".png' align=center ><br>
							  	<input type='radio' class='textbox' name='display_type[".($i+1)."]' id='display_type_".($i+1)."_5' value='5' style='border:0px;' ".($gdb->dt[display_type] == "5" ? "checked":"").">
							  	<label for='display_type_".($i+1)."_5'>F-type(6EA)</label>
							  </div>
							  <div class='promotion_type_box' style='float:left;text-align:center;width:138px;margin:30px 0 0 20px;'>
							  	<img src='../images/".$admininfo["language"]."/P_type_7".($gdb->dt[display_type] == "6" ? "_on":"").".png' align=center ><br>
							  	<input type='radio' class='textbox' name='display_type[".($i+1)."]' id='display_type_".($i+1)."_6' value='6' style='border:0px;' ".($gdb->dt[display_type] == "6" ? "checked":"").">
							  	<label for='display_type_".($i+1)."_6'>G-type(8EA)</label>
							  </div>
							  <div class='promotion_type_box' style='float:left;text-align:center;width:138px;margin:30px 0 0 20px;'>
							  	<img src='../images/".$admininfo["language"]."/P_type_8".($gdb->dt[display_type] == "7" ? "_on":"").".png' align=center ><br>
							  	<input type='radio' class='textbox' name='display_type[".($i+1)."]' id='display_type_".($i+1)."_7' value='7' style='border:0px;' ".($gdb->dt[display_type] == "7" ? "checked":"").">
							  	<label for='display_type_".($i+1)."_7'>I-type(SLIDE)</label>
							  </div>
							  <div class='promotion_type_box' style='float:left;text-align:center;width:138px;margin:30px 0 0 20px;'>
							  	<img src='../images/".$admininfo["language"]."/P_type_9".($gdb->dt[display_type] == "8" ? "_on":"").".png' align=center ><br>
							  	<input type='radio' class='textbox' name='display_type[".($i+1)."]' id='display_type_".($i+1)."_8' value='8' style='border:0px;' ".($gdb->dt[display_type] == "8" ? "checked":"").">
							  	<label for='display_type_".($i+1)."_8'>H-type(9EA)</label>
							  </div>
								 ";
						  }
...
$Contents .= "
					...
					$Contents .= "	</td>
							...
							<select name='display_auto_type[".($i+1)."]'>
								...
								<option value='sale' ".($gdb->dt[display_auto_type] == "sale" ? "selected":"").">세일중</option>
							</select>
						...
						<tr >
						  ...
						  <td class='input_box_item' colspan='3'> ".MDSelect($gdb->dt[md_mem_ix], "md_mem_ix[".($i+1)."]", "md_mem_name[".($i+1)."]",($i+1))."";
						...
						$Contents .= "
						  ...
						  <input type='text' class='textbox' name='display_goal_amount[".($i+1)."]' id='goal_amount_".($i+1)."' size=15 value='".$gdb->dt[goal_amount]."'> 원
						  </td>
						  -->
						</tr>
					  </table><br><br>
...
if($agent_type == "M"){
	if($mmode == "pop"){
		...
		$P->Navigation = "컨텐츠관리 > 프로모션 전시관리 > 프로모션 상품등록";
		$P->title = "프로모션 상품등록";
		$P->NaviTitle = "프로모션 상품등록";
		$P->strContents = $Contents;
		echo $P->PrintLayOut();
	}else{
		$P = new LayOut();
...
//SELECT p.id, p.pcode, p.shotinfo, p.pname, p.sellprice,  p.reserve, ppr_ix, ppr.vieworder, ppr.group_code, p.brand_name
	$sql = "SELECT p.id, p.pcode, p.shotinfo, p.pname, p.listprice, p.sellprice, p.wholesale_sellprice, p.wholesale_price,   
					p.reserve, p.state, p.disp, ppr.vieworder, ppr.group_code, p.brand_name	, p.stock,p.sell_ing_cnt
