...
if($mode == "search"){
	...
	if($admininfo[admin_level] == 9){
		$where = "where p.id Is NOT NULL and p.id = r.pid and p.stock_use_yn = 'Y' and r.basic = 1 AND p.product_type NOT IN (".implode(',',$sns_product_type).")  ";
	}else{
		$where = "where p.id Is NOT NULL and p.id = r.pid and p.stock_use_yn = 'Y' and r.basic = 1 AND p.product_type NOT IN (".implode(',',$sns_product_type).") and admin ='".$admininfo[company_id]."'  ";
	}
...
$innerview = "<ul class='total_cnt_area' style='width:100%;'>
					<li class='front' style='width:100px;'>상품수 : ".number_format($total)." 개</li>
...
if($mode == "search"){
	...
	if($admininfo[admin_level] == 9){
		...
		$sql = "SELECT p.id, p.pname,p.brand, p.sellprice, p.regdate,p.vieworder,p.surtax_yorn, c.com_name,  r.cid,  p.search_keyword,state, p.brand_name,
		p.company, p.pcode, p.coprice, p.listprice, p.disp, p.editdate, p.reserve, p.reserve_rate,p.reserve_yn,
		case when vieworder = 0 then 100000 else vieworder end as vieworder2,p.disp, p.stock, p.safestock, pi.place_name
		FROM ".TBL_SHOP_PRODUCT_RELATION." r, ".TBL_COMMON_COMPANY_DETAIL." c, ".TBL_SHOP_PRODUCT." p left join inventory_place_info pi on p.inventory_info = pi.pi_ix
...
//		echo nl2br($sql);
		...
		$sql = "SELECT p.id, p.pname,p.brand, p.sellprice, p.regdate,p.vieworder,p.surtax_yorn, c.com_name,  r.cid,  p.search_keyword,state, p.brand_name,
		p.company, p.pcode, p.coprice, p.listprice,p.disp, p.editdate, p.reserve, p.reserve_rate,p.reserve_yn,
		case when vieworder = 0 then 100000 else vieworder end as vieworder2, p.stock, p.safestock, pi.place_name
		FROM ".TBL_SHOP_PRODUCT_RELATION." r, ".TBL_COMMON_COMPANY_DETAIL." c, ".TBL_SHOP_PRODUCT." p left join inventory_place_info pi on p.inventory_info = pi.pi_ix
	...
	if ($cid2 == ""){
		if($admininfo[admin_level] == 9){
			...
			$sql = "SELECT  p.id as id, p.pname, p.brand,p.sellprice, p.regdate,p.surtax_yorn, p.vieworder,c.com_name,  r.cid,  p.search_keyword,state, p.brand_name,
			p.company, p.pcode, p.coprice, p.listprice, p.disp, p.editdate, p.reserve, p.reserve_rate,p.reserve_yn,
			case when vieworder = 0 then 100000 else vieworder end as vieworder2,p.disp, p.stock, p.safestock, pi.place_name
			FROM ".TBL_SHOP_PRODUCT_RELATION." r, ".TBL_COMMON_COMPANY_DETAIL." c, ".TBL_SHOP_PRODUCT." p left join inventory_place_info pi on p.inventory_info = pi.pi_ix
			where c.company_id = p.admin and p.id = r.pid and p.stock_use_yn = 'Y' AND p.product_type NOT IN (".implode(',',$sns_product_type).") and r.basic = 1 $where $addWhere $orderbyString LIMIT $start, $max";
			//echo $sql;
			$db->query($sql);
		}else{
			$sql = "SELECT  p.id as id ,p.brand, p.pname, p.sellprice, p.regdate,p.surtax_yorn, p.vieworder,c.com_name,  r.cid,  p.search_keyword,state, p.brand_name,
			p.company, p.pcode, p.coprice, p.listprice, p.disp, p.editdate, p.reserve, p.reserve_rate,p.reserve_yn,
			case when vieworder = 0 then 100000 else vieworder end as vieworder2, p.stock, p.safestock, pi.place_name
			FROM ".TBL_SHOP_PRODUCT_RELATION." r, ".TBL_COMMON_COMPANY_DETAIL." c, ".TBL_SHOP_PRODUCT." p left join inventory_place_info pi on p.inventory_info = pi.pi_ix
			where c.company_id = p.admin and p.id = r.pid and p.stock_use_yn = 'Y' AND p.product_type NOT IN (".implode(',',$sns_product_type).") and r.basic = 1 and admin ='".$admininfo[company_id]."' $where $orderbyString LIMIT $start, $max";
		...
		if($admininfo[admin_level] == 9){
			$sql = "SELECT p.id, p.pname, p.sellprice, p.regdate,c.com_name,p.surtax_yorn, p.vieworder,c.com_name, r.cid, p.search_keyword,state, p.brand, p.brand_name,
...
if($db->total == 0){
	...
	$innerview .= "<tr bgcolor='#ffffff'>
						<td class='list_box_td list_bg_gray' align=center><input type=checkbox class=nonborder id='cpid' name='select_pid[]' value='".$db->dt[id]."'></td>
						<td class='list_box_td point' nowrap>
							<table>
								<tr>
									...
									<a href=\"javascript:PoPWindow3('../product/goods_input.php?mmode=pop&id=".$db->dt[id]."',970,800,'goods_info')\"><b> ".($db->dt[brand_name] ? "[".$db->dt[brand_name]."]":"")." ".$db->dt[pname]."</b></a>
									</td>
								</tr>
							</table>
						</td>
...
$innerview .= "</td>
					<td class='list_box_td' title='".$db->dt[surtax_yorn]."' nowrap>
					 ".($db->dt[surtax_yorn] == "Y" ? "면세(비과세)":"과세")."
					</td>
					<td colspan=9 height=1 bgcolor=#ffffff>".PrintStockByOptionToOrder($db->dt)."</td>
					<!--td class='list_box_td list_bg_gray'>
...
$innerview .= "
					...
					<td class='list_box_td list_bg_gray' style='text-align:center;' nowrap>
						<table align=center>
							<tr>
								<td><a href='cart.php?act=add&id=".$db->dt[id]."&pcount=1' >발주서상품등록</a></td>
							</tr>
						</table>
					</td-->
