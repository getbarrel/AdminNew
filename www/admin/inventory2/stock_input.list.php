...
if($mode == "excel"){
	...
	$accounts_plan_priceXL->getActiveSheet(0)->setCellValue('F' . 1, "보관장소");	
	...
	$accounts_plan_priceXL->getActiveSheet(0)->setCellValue('J' . 1, "입고내용/발주번호");
	...
	for ($i = 0; $i < count($goods_infos); $i++)
	{
		...
		$accounts_plan_priceXL->getActiveSheet()->setCellValue('D' . ($i + 2), $goods_infos[$i][option_name]);
		...
		$accounts_plan_priceXL->getActiveSheet()->setCellValue('J' . ($i + 2), $goods_infos[$i][input_msg]);
...
$Contents .=	"		
			...
			<tr height=10><td  colspan=2 align=center style='padding:20px 0px;'><input type=image src='../images/".$admininfo["language"]."/bt_search.gif' border=0 align=absmiddle> <!--btn_inquiry.gif--></td></td>
			</form>
			<tr>
			<td valign=top >";
...
$innerview = "
			...
			<tr height=40>
				...
				<b >최종입고일</b>
				<a href='?cid=$cid&depth=0&orderby=regdate&ordertype=desc'><img src='../image/orderby_desc.gif' border=0 align=top alt='최근등록순' title='최근등록순'></a>
				<a href='?cid=$cid&depth=0&orderby=regdate&ordertype=asc'><img src='../image/orderby_asc.gif' border=0 align=top alt='등록순' title='등록순'></a>
			...
			<tr bgcolor='#cccccc' height=30 align=center>
				...
				<td class=m_td>입고수량</td>
				<td class=m_td>실입고가</td>
				<td class=m_td style='padding:0px 5px;' nowrap>입고내용/발주번호</td>
				
				<td class=m_td>작성자</td>
				<td class=e_td>".OrderByLink("입고일", "regdate", $ordertype)."<!--a href='?view_type=".$view_type."&orderby=date&company_id=$company_id'>날짜</a--></td>
...
/*
$where = "where  iihd.ihd_ix is not null  and ici.ci_ix = iihd.ci_ix  ";
if($search_text != ""){
	$where .= " and ".$search_type." LIKE '%".$search_text."%' ";
}

if ($startDate != "")	$where .= " and date_format(iihd.regdate, '%Y%m%d') between $startDate and $endDate ";
*/
...
if($db->total == 0){
	$innerview .= "<tr bgcolor=#ffffff height=50><td colspan=10 align=center> 등록된 $page_title이 없습니다.</td></tr>";
}else{
	...
	$innerview .= "<tr height=70>
					...
					<table>
						<tr>
							<td width='50' align=center style='padding:0px 5px;'><img src='".$img_str."' width=50 height=50 style='border:1px solid #eaeaea' align=absmiddle></td>
							...
							<!--a href=\"javascript:PoPWindow3('stock_input_detail.php?idx=".$db->dt[o_ix]."&pid=".$db->dt[pid]."&company_code=".$db->dt[inventory_info]."',820,800,'input_detail_pop')\"--><b>".$db->dt[pname]."</b><!--/a-->
							</td>
						</tr>
					</table>
					</td>
					...
					<td class='list_box_td ' >".$db->dt[input_cnt]."</td>
					<td class='list_box_td list_bg_gray' >".$db->dt[input_price]."</td>
					<td class='list_box_td ' >".($db->dt[input_msg] != "" ?$db->dt[input_msg]."<br>":"")."".$db->dt[oid]."</td>
	...
	$innerview .= "<tr height=30>
					<td class='list_box_td list_bg_gray' colspan=5><b>합계</b></td>					
