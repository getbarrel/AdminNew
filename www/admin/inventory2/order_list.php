													...
													<table cellpaddig='0' cellspacing='0' border='0' width='100%'>
														<tr>
															...
															<select name=search_type>
																...
																<option value='pid' ".CompareReturnValue("pid",$search_type,"selected").">상품아이디</option>
...
function PrintOrderList(){
	...
	if($admininfo[admin_level] == 9){
		$sql = "select COUNT(*) from inventory_order io, inventory_customer_info ici  $where ";
	}else{
		$sql = "select COUNT(*) from inventory_order io, inventory_customer_info ici  $where and company_id = '".$admininfo["company_id"]."' ";
	}
	//echo $sql."<br>";
	$mdb->query($sql);
	$mdb->fetch();
	...
	$mString .= "<tr bgcolor=#efefef style='font-weight:600;'>
		<td class=s_td width='5%' height=27 align='center'>번호</td>
		<td class=m_td width='13%' align='center'>접수일자/접수번호</td>
		<td class=m_td width='*' align='center'>납기일/업체명</td>
		<td class=m_td width='10%' align='center'>발주자</td>
		<td class='m_td' width='10%' align='center'>발주금액 합계</td>
		<td class='m_td' width='10%' align='center'>부가세 합계</td>
		<td class='m_td' width='8%' align='center'>처리상태</td>
		<td class='m_td' width='8%' align='center'>업체담당</td>
		<td class=e_td width='15%' align='center'>관리</td>
		</tr>
		";
	if ($total == 0){
		if($admininfo[admin_level] == 9){
			$mString .= "<tr bgcolor=#ffffff height=50><td class='list_box_td' colspan=9 align=center>발주 내역이 존재 하지 않습니다.</td></tr>";
		}else{
			$mString .= "<tr bgcolor=#ffffff height=50><td class='list_box_td' colspan=9 align=center>발주 내역이 존재 하지 않습니다.</td></tr>";
		}
	}else{
		if($admininfo[admin_level] == 9){
			...
			$sql = "select io.* , ici.customer_name from inventory_order io, inventory_customer_info ici  $where and io.company_id = '".$admininfo["company_id"]."' order by  io.regdate desc limit $start , $max ";
		}
		//echo $sql."<br>";
		$mdb->query($sql);
		...
		for($i=0;$i < $mdb->total;$i++){
			...
			<td class='list_box_td' bgcolor='#ffffff' align=left style='line-height:130%'>
				<table cellpadding='0' cellspacing='0' border='0' width='100%'>
					...
					<tr>
						<td ><a href=\"javascript:PoPWindow3('../inventory/order_detail.php?ioid=".$mdb->dt[ioid]."',950,700,'order_detail')\">".$mdb->dt[ioid]."</a></td>
					</tr>
				</table>
			</td>

			<td class='list_box_td point' bgcolor='#efefef' align=left style='text-align:center;line-height:130%'>
				<table cellpadding='0' cellspacing='0' border='0' width='100%'>
					<tr>
						<td>".$mdb->dt[limit_priod]."</td>
					</tr>
					<tr>
						<td >".$mdb->dt[customer_name]."</td>
					</tr>
				</table>
			</td>
			<td bgcolor='#ffffff'>".$mdb->dt[order_charger]."</td>
			<td bgcolor='#ffffff'>".number_format($mdb->dt[total_price])."</td>
			<td bgcolor='#ffffff'>".number_format($mdb->dt[total_price]*0.1)."</td>
			<td class='list_box_td' bgcolor='#ffffff'> ".$inventory_order_status[$mdb->dt[status]]."</td>
			<td class='list_box_td' bgcolor='#efefef'>".$mdb->dt[incom_company_charger]." </td>
			...
			<a href=\"javascript:PoPWindow3('../inventory/order_detail.php?ioid=".$mdb->dt[ioid]."',950,700,'order_detail')\"><img  src='../images/".$admininfo["language"]."/btc_modify.gif' border=0></a> 
			 ";
			if($mdb->dt[status] != "WC"){
			$mString .= "<a href=\"javascript:PoPWindow3('../inventory/order_detail.php?ioid=".$mdb->dt[ioid]."',950,700,'order_detail')\"><img  src='../images/".$admininfo["language"]."/bts_warehouse.gif' border=0></a>
