							...
							<td class='box_05 align=center'  width='100%'-->
								<table border=0 cellpadding=0 cellspacing=0 width='100%'>
									<tr>
										...
										if(file_exists($_SERVER["DOCUMENT_ROOT"]."".PrintImage($admin_config[mall_data_root]."/images/product", $product_info[id], "s"))){
...
$Contents .= "
					<table border=0 cellpadding=0 cellspacing=0 width='100%' class='list_table_box' >
						...
						<tr height=30px bgcolor=#e5e5e5>
							...
							<td class='m_td' align=center nowrap>옵션명(규격)</td>
						...
						if($db->total){
							$sql = "select vdate, ot.option_stock as stock,ot.option_safestock as safestock,o.option_type,ot.option_div,ot.id,o.option_name ,ot.option_coprice, ot.opn_ix, ot.id as opnd_ix, ot.option_code
										from shop_product_options o , shop_product_options_detail ot left join inventory_product_stockinfo ps on ot.opnd_ix = ps.opnd_ix and ps.pid = '".$id."'
										where o.opn_ix = ot.opn_ix and o.pid = '".$id."' and ot.pid = '".$id."' and o.option_kind = 'b' group by vdate, o.id ";
