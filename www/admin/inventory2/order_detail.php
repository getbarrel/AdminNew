										...
										<tr height='25'>
											...
											<td class='list_box_td' style='text-align:left; padding:3px 10px;'>
												<table cellpadding=0 cellspacing=0 border=0 bgcolor=#ffffff width=100%>
													...
													<tr>
														<TD nowrap>
														<input type='text' name='limit_priod' class='textbox' value='".$inventory_order_infos[limit_priod]."' style='height:20px;width:70px;text-align:center;' id='priod'>
...
function PrintOrderDetail($ioid){
	...
	$where = " where io.ioid = iod.ioid  and iod.pid = sp.id ";
	...
	if($admininfo[admin_level] == 9){
		$sql = "select COUNT(*) from inventory_order io, inventory_order_detail iod , shop_product sp $where ";
	}else{
		$sql = "select COUNT(*) from inventory_order io, inventory_order_detail iod , shop_product sp  $where and company_id = '".$admininfo["company_id"]."' ";
	}
	//echo $sql."<br>";
	$mdb->query($sql);
	$mdb->fetch();
				...
				<table>
					<tr>
						<td bgcolor='#ffffff' align=center style='padding:5px 5px' >
							<a href='../product/goods_input.php?id=".$mdb->dt[id]."' class='screenshot'  rel='".PrintImage($admin_config[mall_data_root]."/images/product", $mdb->dt[id], $LargeImageSize)."'  ><img src='".$img_str."' width=50 height=50 style='border:1px solid #efefef'></a>
						</td>
						<td bgcolor='#ffffff' align=left style='font-weight:normal;line-height:140%;'>
						<!--".getCategoryPathByAdmin($mdb->dt[cid], 4)."<br>-->
						<a href='../product/goods_input.php?id=".$mdb->dt[id]."&mode=$mode&nset=$nset&page=$page&cid2=$cid2&depth=$depth&company_id=$company_id&brand2=$brand2&max=$max&state2=$state2&disp=$disp&search_type=$search_type&search_text=".trim($search_text)."&onew=$onew&best=$best&sale=$sale&event=$event&wnew=$wnew&mnew=$mnew' target='_blank'><b> ".($mdb->dt[brand_name] ? "[".$mdb->dt[brand_name]."]":"")." ".$mdb->dt[pname]."</b>
						".($mdb->dt[option_name] != "" ? "<br><b>옵션명 : ".$mdb->dt[option_name]."</b>":"")."
						</a>
						</td>
					</tr>
				</table>
				
				<!--[".$mdb->dt[pcode]."] ".$mdb->dt[pname]."-->
			</td>
							 ...
							 if($order_type == "direct"){
								...
								$mString .= "
									<select name=change_status style='font-size:20px;height:32px;font-weight:bold;'>
										...
										<option value='OR' ".CompareReturnValue("OR",$status,"selected").">".$inventory_order_status["OR"]."</option><!-- 발주대기 -->
										<option value='WR' ".CompareReturnValue("WR",$status,"selected").">".$inventory_order_status["WR"]."</option><!-- 발주완료(입고대기) -->";
										if(($dc_cnt + $wp_cnt) == 0){
								$mString .= "<option value='OC' ".CompareReturnValue("OC",$status,"selected").">".$inventory_order_status["OC"]."</option><!-- 발주취소 -->";
										}
								$mString .= "
									</select>";
							 }
