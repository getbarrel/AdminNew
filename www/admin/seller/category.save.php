...
if($mode == "infoupdate"){
	...
	$sql = "select * from shop_minishop_category_info where cid = '$cid'  ";
	$db->query($sql);
	//echo $sql;
	if($db->total){
		$db->fetch();
		...
		$('#thisCategoryform').find('input[name=\"category_use\"]').attr('checked',$category_use);
		$('#thisCategoryform').find('input[name=\"category_type[".$category_type_index."]\"]').attr('checked',true);
...
//		parent.document.forms['thisCategoryform'].category_type[".$category_type_index."].checked = true;
		//parent.document.forms['design_subcategory'].this_category_color.value = '".$db->dt[cname_color]."';
		//parent.document.forms['design_subcategory'].this_category_on.value = '".$db->dt[cname_on]."';
		//parent.document.forms['design_subcategory'].this_category_on_color.value = '".$db->dt[cname_on_color]."';
		
		
		$('#thisCategoryform').find('input[name=\"hscode\"]').val('".$db->dt[hscode]."');		
		$('#thisCategoryform').find('input[name=\"basic_weight\"]').attr('".$db->dt[basic_weight]."');
	...
	if($cid){
		...
		$sql = "select
				...
				from
					shop_product_relation
				where
					cid like '".substr($cid,0,3+(3*$depth))."%'";
		$db->query($sql);
		$db->fetch();
...
if ($mode == "modify"){
	...
	if($sub_mode == "design_subcategory"){	// 분류별 디자인설정 
		...
		$sql = "select * from category_img where cid = '$cid' AND company_id = '".$_SESSION["admininfo"]['company_id']."' ";
		$db->query($sql);
		$db->fetch();
		...
		*/
		...
		$sql = "update shop_minishop_category_info set
				...
				where 
					cid = '$cid'
					AND company_id = '".$_SESSION["admininfo"]['company_id']."'";
		//echo $sql;
		//exit;
		$db->query($sql);
		$del_num=0;
		...
		if($del_num>0) {
			$sql="UPDATE shop_minishop_category_info SET ".$del_query." WHERE cid = '$cid'";
			$db->query($sql);
		}
        ...
        */
		...
		if($category_use != "1" && $cid != "000000000000000"){
			$sql = "update shop_minishop_category_info set category_use ='$category_use' where cid LIKE '".substr($cid,0,($this_depth+1)*3)."%' and company_id = '".$_SESSION["admininfo"]['company_id']."' ";
			$db->query($sql);
			...
			/*
			if($this_depth+1 > 0){
				$sql = "update shop_minishop_category_info set category_use ='$category_use' where cid LIKE '".substr($cid,0,($this_depth+1)*3)."%' ";
				$db->query($sql);
			}
			*/
		}
	...
	//exit;
	//updateCategoryXML();
	echo "<script language='JavaScript' src='/admin/_language/language.php'></Script><Script Language='JavaScript'>alert(language_data['category.save.php']['A'][language]);parent.document.location.href='category.php?cid=".$cid."&depth=".$this_depth."';</Script>";
...
function ParentCategoryUseUpdate($mdb, $cid, $this_depth){
	...
	//$mdb->debug = true;
	$sql = "select cid, cname,  depth, category_use from shop_minishop_category_info   $where ";

	$mdb->query($sql);

	$parent_category = $mdb->fetchall();
	for($i=0;$i < count($parent_category);$i++){
		$sql = "update shop_minishop_category_info set category_use ='1' where cid = '".$parent_category[$i][cid]."' ";
		//echo $sql;
		$mdb->query($sql);
	}
...
if ($mode == "del"){
	...
	if (CheckSubCategory($cid,$this_depth)){
		if($sub_cartegory_delete == "1"){
			...
			$sql = "select * from shop_minishop_category_info where cid LIKE '".substr($cid,0,($this_depth+1)*3)."%'";
			$db->query($sql);
			$db->fetch();
			$category_info = $db->dt;

			$sql = "delete from shop_minishop_category_info where cid LIKE '".substr($cid,0,($this_depth+1)*3)."%'";
			$db->query($sql);
			...
			$sql = "delete from shop_minishop_category_info where cid = '$cid'";
			$db->query($sql);
...
if ($mode == "insert"){
	...
	if(trim($sub_cid) != "" && trim($cid) != "") {// 카테고리 정보가 제대로 안넘어 올 경우를 검사 kbk 12/03/22
		$sql = "select * from shop_minishop_category_info where cid = '$cid' and company_id = '".$_SESSION["admininfo"]['company_id']."' ";
		$db->query($sql);
		$db->fetch(0);
		...
		$sql = "insert into shop_minishop_category_info
					(company_id , cid, depth,vlevel1, vlevel2, vlevel3,vlevel4,vlevel5,cname,cname_color,cname_on,cname_on_color,catimg,catimg_on,leftcatimg,leftcatimg_on, subimg,subimg_on, category_use,category_display_type,category_type, category_link,regdate,category_code,is_adult,is_layout_apply)
					values
					('".$_SESSION["admininfo"]['company_id']."' , '$sub_cid', '$sub_depth','$level1','$level2','$level3','$level4','$level5', '$sub_category','$sub_category_color','$sub_category_on','$sub_category_on_color','$category_img_name','$category_img_on_name','$leftcategory_img_name','$leftcategory_img_on_name','$sub_img_name','$sub_img_on_name','$category_use','$category_display_type','$category_type','$category_link',NOW(),'$category_code','$is_adult','$is_layout_apply')";
...
function getMaxlevel($cid,$depth)
{
	...
	$sPos = $depth*3;
	$sql = "select max(vlevel$strdepth)+1 as maxlevel from shop_minishop_category_info where cid LIKE '".substr($cid,0,$sPos)."%' AND company_id = '".$_SESSION["admininfo"]['company_id']."' ";
...
function CheckSubCategory($cid,$depth){
	...
	$endpos = $depth*3+3;
	$this_depth = $depth;
	$sql = "select * from shop_minishop_category_info where depth > $this_depth and cid LIKE '".substr($cid,0,$endpos)."%' AND company_id = '".$_SESSION["admininfo"]['company_id']."' ";
	$db->query($sql);
	//echo "$sql<br>";
...
function updateCategoryXML(){
	...
	$xml = new XmlWriter_();
	$mdb = new Database;
	$mdb->query("select * from shop_minishop_category_info where category_use = 1 order by vlevel1, vlevel2, vlevel3, vlevel4, vlevel5");
