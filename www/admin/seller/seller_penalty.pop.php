...
function DeleteReserve(deposit_ix, uid){
	if(confirm('셀러 판매신용점수 정보를 정말로 삭제하시겠습니까?')){
...
<TABLE cellSpacing=0 cellPadding=0 width='100%' align=center border=0>
	<TR>
		...
		<table border='0' width='100%' cellpadding='0' cellspacing='1' align='center'>
			...
			<tr height=30><td class='p11 ls1' style='padding:0 0 0 0px;text-align:left;' > - 판매신용점수 내용을 수정/ 추가 하실수 있습니다.</td></tr>
			<tr>
				<td align=center style='padding: 0 10 0 10'>
				<form name='deposit_frm' method='post'  action='seller_penalty.act.php'  onSubmit='return checkSellerPoint(this)' target=''>
				<input type='hidden' name='act' value='seller_penalty_insert'>
				<input type='hidden' name='company_id' value='".$company_id."'>
				<input type='hidden' name='penalty_ix' value='".$penalty_ix."'>
				<table border='0' width='100%' cellspacing='1' cellpadding='0'  class='list_table_box' style='margin-bottom:10px;'>
					...
					<tr height='30' align='center'>
						...
						<td class='list_box_td' >
							<input type='text' class=textbox name='penalty' id='penalty' size=10>
						</td>
						<td class='list_box_td' >
...
if($db->dbms_type == "oracle"){
	$db->query("select * from common_seller_penalty as csp  where
			company_id = '$company_id' ");
}else{
	$db->query("select * from common_seller_penalty where
			company_id = '$company_id' ");
}
$total = $db->total;
...
$sql = "select
		...
		where
			company_id = '$company_id' 
			order by edit_date DESC, penalty_ix DESC LIMIT $start, $max";
$db->query($sql);
...
if(count($data_array) > 0){
	...
	$Contents .= "	<tr height=28 align=center>
						...
						<td bgcolor='#fbfbfb'>";
						$Contents .= "<a href=\"javascript:UpdateReserve('".$data_array[$i][penalty_ix]."','".$data_array[$i][etc]."', '".$data_array[$i][penalty]."','".$data_array[$i][state]."','".$data_array[$i][use_state]."','".$data_array[$i][oid]."')\"><font color='".$font_color."'>".number_format($data_array[$i][penalty])."</font></a>
							";
						$Contents .= "</td>
										<td bgcolor='#fbfbfb' class='point'>";
						if($data_array[$i][state]	== RESERVE_STATUS_ORDER_CANCEL){
							$Contents .= "<s>".number_format($data_array[$i][use_penalty])."</s>";
						}else{
							$Contents .= number_format($data_array[$i][use_penalty]);
						}
	$Contents .= "		</td>
		...
		$Contents .= "<tr height=60><td colspan=7 align=center>판매신용점수 내용이 없습니다.</td></tr>";
...
$P->addScript = $Script;
$P->Navigation = "셀러관리 > 판매신용점수 내역보기";
$P->NaviTitle = "보유 판매신용점수 : <span style='color:#ffffff;font-size:20px;'>".number_format(TotalCompanyPenalty($company_id))."</span> 원";
$P->title = "판매신용점수 내역보기";
