...
if($csr_ix){
	$db->query("SELECT * FROM common_support_response where csr_ix ='$csr_ix' ");
	$db->fetch();
	$act = "update";
}else{
	$act = "insert";
...
$Contents01 = "
	...
	<col width='*'>
	<tr bgcolor=#ffffff >
		<td class='input_box_title'> 계시판선택  </td>
		<td class='input_box_item' colspan=3>
			<select name='type' id='type' title='게시판' validation=true>
			<option value=''>게시판을 선택해주세요.</option>
			<option value='Q' ".($db->dt[type] == 'Q' ? "selected" : "").">1:1문의</option>
			<option value='S' ".($db->dt[type] == 'S' ? "selected" : "").">상품제안문의(할인)</option>
			<option value='A' ".($db->dt[type] == 'A' ? "selected" : "").">정산/세무 문의</option>템플릿명
			<option value='C' ".($db->dt[type] == 'C' ? "selected" : "").">관리자 긴급사항</option>
		</td>
	</tr>
	<tr bgcolor=#ffffff >
...
$Contents02 = "
	...
	<tr height=25 bgcolor=#efefef style='font-weight:bold'>
		...
		<td style='width:10%;' align='center' class='m_td'> 게시판명</td>
...
$db->query("SELECT * FROM common_support_response  order by view_order asc   ");
...
if($db->total){
	...
	$Contents02 .= "
		<tr bgcolor=#ffffff height=30>
			...
			<td align='center' class='list_box_td list_bg_gray'>".$type."</td>
		...
		if(checkMenuAuth(md5($_SERVER["PHP_SELF"]),"U")){
			$Contents02.="
				<a href=\"?csr_ix=".$db->dt[csr_ix]."\"><img src='../image/btc_modify.gif' border=0></a>";
		}else{
			$Contents02.="
		...
		if(checkMenuAuth(md5($_SERVER["PHP_SELF"]),"D")){
			$Contents02.="
				<a href=\"javascript:deleteBoardTemplet('delete','".$db->dt[csr_ix]."')\"><img src='../image/btc_del.gif' border=0></a>";
		}else{
			$Contents02.="
...
$Contents = $Contents."<form name='templet_form' action='seller_qna_response.act.php' method='post' onsubmit='return CheckFormValue(this)' target='act'><input name='mmode' type='hidden' value='$mmode'><input name='act' type='hidden' value='".$act."'><input name='csr_ix' type='hidden' value='$csr_ix'>";
...
function deleteBoardTemplet(act, csr_ix){
	if(confirm('해당 게시판 답변 템플릿  정보를 정말로 삭제하시겠습니까?')){
		...
		frm.submit();
		*/
		window.frames['act'].location.href= 'seller_qna_response.act.php?act='+act+'&csr_ix='+csr_ix;
	}
}
...
if($mmode == "pop"){
	...
	$P->Navigation = "HOME > 셀러관리 > 게시판 답변 템플릿";
	$P->NaviTitle = "게시판 답변 템플릿";
