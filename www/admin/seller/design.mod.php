...
if($admin_config[mall_page_type] == "P"){
	...
	$selected_skin = $admin_config[mall_use_templete];
}else{
	$templet_path = $admin_config[mall_data_root]."/mobile_templet/".$admin_config[mall_use_mobile_templete];
	$templet_file_path = $templet_path."/".$mod."/".$page_name;
	$selected_skin = $admin_config[mall_use_mobile_templete];
...
$Contents .="
	<tr height=40>
	    <td align='left' colspan=2> ".GetTitleNavigation("미니샵 레이아웃 디자인", "미니샵 디자인관리 > 미니샵 레이아웃 디자인 <a onClick=\"PoPWindow('/admin/_manual/manual.php?config=".urlencode("몰스토리동영상메뉴얼_레이아웃디자인(090322)_config.xml")."',800,517,'manual_view')\" ><img src='../image/movie_manual.gif'  style='margin:0 0 0px 0'></a>")."</td>
	</tr>
	<tr height=40><td colspan=2 align=right style='padding-bottom:10px;'>
	".colorCirCleBox("#efefef","100%","<div style='padding:5px 5px 5px 15px;'><img src='../image/title_head.gif' ><b> 선택된 페이지 : <a href=\"JavaScript:pview(document.getElementById('page_contents').value, 'aaa', 'bbb');\">".$selected_skin."/$mod/".$page_name."</a> <a href=\"JavaScript:pview(document.getElementById('page_contents').value, 'aaa', 'bbb');\"><img src='../images/".$admininfo["language"]."/btn_page_design_view.gif' border=0 ></a>	</b></div>")."</td></tr>
	<tr height=100>
		<td colspan=2>
			$help_text<br>
		</td>
	...
	<tr height=40>
		...
		if(checkMenuAuth(md5($_SERVER["PHP_SELF"]),"C")){
			$Contents .="<a href=\"javascript:PoPWindow('./addfile.pop.php?pcode=$mod&page_path=$mod',420,170,'fileadd_pop')\"><img src='../images/".$admininfo["language"]."/btn_add_file.gif' border=0 align=absmiddle></a> ";
		}else{
	...
	<tr height=30 >
		<td colspan=2 >
			 <div style='width:98%;height:20px;background-color:#efefef;padding:10px 0px 0px 10px;border:1px solid silver;' class='small' >수정후 CTRL+S 를 누르면 저장됩니다</div>
		</td>
	</tr>
	<tr>
...
<textarea onkeydown=\"textarea_useTab( this, event );\" style='overflow:auto;width:98%;height:300px;font-size:11px;font-family:돋움;' wrap='off' class='tline' name='page_contents' id='page_contents' >
$thisfile
</textarea>
		</td>
	</tr>
...
$Contents .="
	...
	<tr><td colspan=2 align=center style='padding:10px;' id='design_history_area'>".PrintEditPageHistory($mod."/".$page_name)."</td></tr>
</table>
...
if($mmode == "pop"){
	...
	$P->addScript = $Script;
	$P->Navigation = "HOME > 미니샵 디자인관리 > 레이아웃 디자인";
	$P->NaviTitle = "레이아웃 디자인";
	$P->strContents = $Contents;
	echo $P->PrintLayOut();
}else if($mmode == "innerview"){
	echo "<body><div id='design_history_area'>".PrintEditPageHistory($mod."/".$page_name)."</div></body>\n";
	...
	$P->title = "";
	$P->Navigation = "셀러관리 > 미니샵 디자인관리 > 레이아웃관리";
	$P->title = "레이아웃관리";
