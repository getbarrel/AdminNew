...
$mall_page_type = "MI";
$layoutXmlPath = $_SERVER["DOCUMENT_ROOT"] . $admininfo["mall_data_root"] . "/minishop_templet/" . $admin_config["selected_templete_minishop"] . "/layout2.xml";
...
$layoutFindbyPcode = $layoutXml->search("layouts", array("pcode"), array($pcode));
...
$results = $layoutXml->search("layouts", array("skin_type", "pcode", "templet_name"), array($mall_page_type, $pcode, $admin_config[selected_templete]));
...
$unique_file_path = "minishop_templet/".$admin_config[selected_templete]."/".$page_path."/".$page_name; //selec
...
$Contents .="
	<tr height=30>
	    <td align='left'> ".GetTitleNavigation("미니샵 디자인", "미니샵 디자인관리 > 미니샵 디자인 <a onClick=\"PoPWindow('/admin/_manual/manual.php?config=몰스토리동영상메뉴얼_페이지상세디자인(090322)_config.xml',800,517,'manual_view')\" ><img src='../image/movie_manual.gif' align=absmiddle style='margin:0 0 5px 0'></a>")."</td>
	</tr>
	<tr height=30><td align=right style='padding-bottom:10px;'>".colorCirCleBox("#efefef","100%","<div style='padding:5px 5px 5px 15px;text-align:right;'><img src='../image/title_head.gif' ><b> 선택된 페이지 : [$page_title] <a href=\"".$page_path_link."\" target='_blank'>$page_path_string</a> <a href=\"".$page_path_link."\" target='_blank'><img src='../images/".$admininfo["language"]."/btn_page_design_view.gif' border=0 align=absmiddle></a></b></div>")."</td></tr>
	<tr>
	    <td align='left' style='vertical-align:top'>
				...
				<div class='mallstory t_no'>
					...
					<div class='my_box' style='padding:10px;'>
						<table id='design_structure' ".($tab_no == "01" || !$tab_no ? " ":"style='display:none;'") .">
						<tr>
							<td colspan=3>
								".$help_text."<br>
							</td>
				...
				$Contents .="</form>
						...
						<table cellpadding=0 cellspacing=0 id='design_contents'  ".($tab_no == "02" ? " ":"style='display:none;'") ." width='100%'>
						<tr>
						<form name=info_input action='design.act.php' method='post' onsubmit='return SubmitX(this);' target='act'>
					        ...
					        <!--tr>
						<td bgcolor='#ffffff' height='100' colspan='4' style='padding:10px 0 0 0' >

<textarea style=\"background-image: url('../image/text_bg1.gif'); height:140px;\"  class='tline' wrap='off'  onfocus='CheckFocus(this)' basic_message=true name='page_addscript' >
...
//함수 내용을 입력합니다.
						...
						<!--tr>
							<td bgcolor='#ffffff' height='30' colspan='4' style='padding:10 0 0 0'><textarea style=\"background-image: url('../image/text_bg2.gif'); height:50;\" wrap='off' class='tline' onfocus='CheckFocus(this)' basic_message=true name='page_body' >BODY 태그에 입력될 이벤트 및 property를 정의해주세요 onload='functionname();' oncontextmenu='return false'</textarea></td>
						</tr-->
						...
						<tr>
							<td bgcolor='#ffffff' height='30' colspan='4' style='padding:10px 0 5px 0'><textarea style=\"width:815px; height:50px;\" wrap='off' class='tline' onfocus='CheckFocus(this)' ".($page_navi == "" ? "basic_message=true":"basic_message=false")." name='page_navi' >".($page_navi == "" ? "페이지 네비게이션정보를 입력해주세요":$page_navi)."</textarea></td>
						</tr>
						<tr height=30 bgcolor=#efefef>
							<td colspan=3 style='padding:0 0 0 10px;border:1px solid silver;'>
								 <span class='small' >수정후 CTRL+S 를 누르면 저장됩니다</span>
							</td>
						</tr>
						<tr>
							<td bgcolor='#ffffff' height='300' width='100%' colspan='4'>
							<div style='z-index:-1;position:absolute;' id='parent_save_loading'>
...
$Contents .="
<textarea onkeydown=\"textarea_useTab( this, event );\" style='height:300px;width:97%;' wrap='off' class='tline' id='page_contents' name='page_contents' >
$thisfile
</textarea>";
...
<script language='javascript'>
		...
		document.write('<scrip'+'t language=\"javascript\" src=\"'+Editor_Root_Dir+'Editor.js\"></scrip'+'t>');
		}
		-->
		</script>
		...
		<!--
			if(bitUseEditor){
				Editor_New_Generate('page_contents');
			}
		-->
		</script> ";
*/
...
$Contents .="
						...
						<tr>
							<td bgcolor='#ffffff' height='30' colspan='4' style='padding:10px 0 0 0'><!--textarea style='height:50px;width:97%;' wrap='off' onfocus='CheckFocus(this)' class='tline' basic_message=true name='page_change_memo' >
							소스 수정내용을 입력해주세요. 수정된 내용을 확인하시고 추후에 복원하실수 있습니다. </textarea--> ".getTransDiscription(md5($_SERVER["PHP_SELF"]),'B')." </td>
						</tr>
						<tr>
...
$category_str ="<div class=box id=img3  style='width:100%;height:375px;overflow:auto;'>".Category()."</div>";
...
if($mmode == "innerview"){
	...
	$LO->title = "페이지 상세디자인";
	$LO->strLeftMenu = seller_menu("/admin",$category_str);
	$LO->strContents = $Contents;
	$LO->Navigation = "센러관리 > 미니샵 디자인관리 > 페이지 상세디자인";
	$LO->PrintLayOut();
}
...
function PrintEditPageHistory($page_name){
	...
	if ($mdb->total == 0){
		...
		$mString .= "<tr bgcolor=#ffffff height=40><td colspan=4 align=left><a href=\"JavaScript:SelectDelete(document.forms['listform']);\"><img  src='../images/".$admininfo["language"]."/bt_all_del.gif' border=0 align=absmiddle ></a></td></tr>";
	}

	$query_string = str_replace("nset=$nset&page=$page&","",$QUERY_STRING) ;
	//echo $query_string;
	$mString .= "</form>";
	$mString .= "<tr height=50 bgcolor=#ffffff><td colspan=4 align=left>".page_bar($total, $page, $max,"&".$query_string,"")."</td></tr>
					</table>";
