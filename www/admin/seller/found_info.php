...
if(count($status) > 0){
	...
	foreach($status as $key => $value){
		if($i == count($status)){
			$where .= " fs.status = '".$value."'";
		}else{
			$where .= " fs.status = '".$value."' or ";
		}
	$i++;
	}
	$where .=")";
...
if($regdate == '1'){
	if($sdate != "" && $edate !=""){
		$where .= " and date_format(fs.regdate,'%Y-%m-%d') between '".$sdate."' and '".$edate."'";
	}
}
...
if($_SESSION["admininfo"]["admin_level"] != '9'){
	...
	if($company_id != ""){
		$where .= " and fs.company_id = '".$company_id."'";
	}
}
...
$sql = "SELECT 
		...
		FROM
			...
			inner join common_company_detail as ccd on (fs.company_id = ccd.company_id)
		where
			type= '".$type."' $where";
$db->query($sql);
$db->fetch();
...
function deleteMemberInfo(act, code){
 	if(confirm('해당회원 정보를 정말로 삭제하시겠습니까? \\n 삭제시 관련된 모든 정보가 삭제됩니다.')){
		window.frames['iframe_act'].location.href= 'found_info.act.php?act='+act+'&code='+code;
 	}
}

function ProductQnaDelete(fs_ix){
	if(confirm(language_data['product_qna.php']['A'][language])){
	//'제품문의를  정말로 삭제하시겠습니까? '
		window.frames['act'].location.href='../seller/found_info.act.php?act=detail_delete&fs_ix='+fs_ix
		//document.getElementById('iframe_act').src='product_qna.act.php?act=detail_delete&fs_ix='+fs_ix;
	}
}
...
$Contents .= "
<form name='list_frm' method='POST' onsubmit='return BatchSubmit(this);' action='found_info.act.php'  target='act'>
...
<tr height=30 >
	...
	if(checkMenuAuth(md5($_SERVER["PHP_SELF"]),"E")){
		...
		<a href='excel_config.php?".$QUERY_STRING."&info_type=found_qna&excel_type=found_qna_excel' rel='facebox' ><img src='../images/".$admininfo["language"]."/btn_excel_set.gif'></a></span>";
	}else{
		$Contents .= "
...
<tr height='40' bgcolor='#ffffff'>
	...
	<td align='center' class='m_td small' nowrap><font color='#000000'><b>제목<br>작성일</b></font></td>
	<td align='center' class='m_td small' nowrap><font color='#000000'><b>셀러명<br>ID</b></font></td>
	<td align='center' class='m_td small' nowrap><font color='#000000'><b>문의자 명<br>ID</b></font></td>

	<td align='center' class='m_td small' nowrap><font color='#000000'><b>문의자 연락처<br>문의자 메일</b></font></td>
...
if($_SESSION["admininfo"]["admin_level"] == '9'){
$Contents .= "<td align='center' class=m_td><font color='#000000'><b>계시글확인</b></font></td>";
}
$Contents .= "
...
if($db->total == 0){
	...
	for ($i = 0; $i < $db->total; $i++)
	{
		...
		<tr height='45' onMouseOver=\"this.style.backgroundColor='#E8ECF1'; \" onMouseOut=\"this.style.backgroundColor=''\">
			<td class='list_box_td'><input type=checkbox name=fs_ix[] id='code' value='".$db->dt[fs_ix]."' onClick='input_check_num()'></td>
			<td class='list_box_td' >".$image."</td>
			<td class='list_box_td' >".$no."</td>
			<td class='list_box_td point' style='text-align:left;padding:0px 5px;line-height:150%;' nowrap>".$db->dt[title]."<br><span style='font-weight:normal'>".$regdate."</span></td>
			...
			<td class='list_box_td ' style='line-height:150%;'>".$db->dt[mobile]."<br>".$db->dt[mail]."</td>
	...
	if($_SESSION["admininfo"]["admin_level"] == '9'){
		$Contents .="<td class='list_box_td' > ".$check_yn."</td>";
	}
		...
		$Contents .="
			<td class='list_box_td ctr'  style='padding:5px;'>
				...
				if(checkMenuAuth(md5($_SERVER["PHP_SELF"]),"C")){
					$Contents.="<a href=\"javascript:PoPWindow('found_detail.php?mmode=pop&fs_ix=".$db->dt[fs_ix]."&type=$type',900,700,'found_info');\"><img  src='../images/".$admininfo["language"]."/btn_reply.gif' border=0></a>";
				}else{
	...
	$Contents.="<td>";
				if(checkMenuAuth(md5($_SERVER["PHP_SELF"]),"D")){
					$Contents.="
					<a href=JavaScript:ProductQnaDelete('".$db->dt[fs_ix]."')><img  src='../images/".$admininfo["language"]."/btc_del.gif' border=0></a>";
				}else{
					$Contents.="
...
<tr>
	...
	if($_SESSION["admininfo"]["admin_level"] == '9'){
		$Contents .= "
		<img src='../images/icon/btn_write.gif' onClick=\"PopSWindow('../seller/found_detail.php?mmode=pop&code=".$db->dt[code]."&mmode=pop&type=$type',900,710,'member_info')\" style='cursor:pointer;'/>";
	}else{
		if($page_type != 'seller_cs_admin'){
		$Contents .= "
		<img src='../images/icon/btn_write.gif' onClick=\"PopSWindow('../seller/found_detail.php?mmode=pop&code=".$db->dt[code]."&mmode=pop&type=$type',900,710,'member_info')\" style='cursor:pointer;'/>";
		}
	}
