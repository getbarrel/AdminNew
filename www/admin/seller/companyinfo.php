...
function init(){			
	Content_Input();
	Init(document.info_input);
}

function Content_Input(){	
	document.info_input.content.value = document.info_input.mall_companyinfo.value;		
}
...
$db->query("select mall_companyinfo from ".TBL_SHOP_SHOPINFO." where mall_ix = '".$admininfo[company_id]."'");
$db->fetch();
*/
$thisfile = load_template($_SERVER["DOCUMENT_ROOT"]."".$admin_config[mall_data_root]."/templet/".$admin_config[mall_use_templete]."/ms_companyinfo.htm");

$Contents ="
		<table width='100%' border='0' cellspacing='0' cellpadding='0' height='25'>
		<form name=info_input action='companyinfo.act.php' method='post' onsubmit='return SubmitX(this);'>
                    <input type='hidden' name=act value='update'>
                    <input type='hidden' name=mall_ix value='".$admininfo[company_id]."'>	
		    <tr><td colspan=3 align=right style='padding:10px;'>".colorCirCleBox("#efefef","100%","<div style='padding:5px;'><img src='../image/title_head.gif' align=absmiddle><b>회사소개 / 약도</b></div>")."</td></tr>
                    ...
                    <tr><td colspan=3 align=right style='padding:10px;'><input type=image src='../image/b_save.gif' border=0></td></tr>
                    <textarea name='mall_companyinfo'  style='display:none'>".$thisfile."</textarea></form>
                  </table>";
