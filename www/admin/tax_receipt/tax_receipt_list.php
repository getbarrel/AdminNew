...
$mstring ="
		...
		<tr>
			<td align='left' colspan=6 > ".GetTitleNavigation("증빙자료관리", "세무관리 > 증빙자료관리 <a onClick=\"PoPWindow('/admin/_manual/manual.php?config=".urlencode("몰스토리동영상메뉴얼_팝업등록(090108)_config.xml")."',800,517,'manual_view')\"  title='팝업등록 관리 동영상 메뉴얼입니다'><img src='../image/movie_manual.gif' align=absmiddle></a>")."</td>
		</tr>
		<tr>
			<td>";
		$mstring .= "
...
$help_text = HelpBox("<table cellpadding='0' cellspacing='0' border='0'><tr><td valign=top><b>증빙자료 관리</b></td><td style='padding:0px 0px 3px 3px'><a onClick=\"PoPWindow('/admin/_manual/manual.php?config=몰스토리동영상메뉴얼_팝업등록(090108)_config.xml',800,517,'manual_view')\"  title='팝업등록 동영상 메뉴얼입니다' style='cursor:pointer;'><img src='../image/movie_manual.gif' width=26 height=20 style='position:absolute;top:-1px;'></a></td></tr></table>", $help_text,100);
...
/*
	...
	$sql = "select * from tax_receipt $where ";
	$mdb->query($sql);
	$total = $mdb->total;
	...
	$mString = $mString."
		<tr align=center bgcolor=#efefef height=30>
			...
			<td class=m_td width='15%'>업로드일</td>
			<td class=m_td width='15%'>구분</td>
			<td class=e_td width='15%'>담당부서</td>
			<td class=e_td width='15%'>담당자</td>
			<td class=e_td width='15%'>처리현황</td>
			<td class=e_td width='15%'>상세보기</td>
		</tr>";
	if ($mdb->total == 0){
		$mString .= "<tr bgcolor=#ffffff height=70><td colspan=8 align=center>증빙자료가 존재 하지 않습니다.</td></tr>";
		$mString .= "<tr><td colspan=8 class='dot-x'></td></tr>";
		...
		$db->query("select * from tax_receipt  $where order by  regdate desc limit $start, $max");
		...
		for($i=0;$i < $db->total;$i++){
			...
			<a href=\"javascript:PopSWindow('tax_receipt_detail.php?re_ix=".$db->dt[re_ix]."',400,700,'tax_receipt_detail')\" style='cursor:pointer' >보기</a>
			</td>
			</tr>
			";
		}
