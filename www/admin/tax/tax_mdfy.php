<?
	...
	$m_kind = $_GET[m_kind];
	if($m_kind == "1") $modify_title = "기재사항 착오정정";
	if($m_kind == "2") $modify_title = "공급가액 변동";
	if($m_kind == "3") $modify_title = "계약의 해제";
	if($m_kind == "4") $modify_title = "환입";
	if($m_kind == "5") $modify_title = "내국신용장등 사후개설";
	if($m_kind == "6") $modify_title = "착오에 의한 이중발급";
	...
	else
	{
		...
		//echo "SELECT * FROM tax_sales WHERE idx = '$idx'";
		$db->fetch();
		
		if($m_kind == 1){
			//modify_tax_array = array();
			$db2->query("SELECT * FROM tax_sales WHERE idx = '$idx'");
			$db2->fetch(0,"object");
			$modify_tax_array = (array)$db2->dt;
			
			$SQL_S = "SELECT * FROM tax_sales_detail WHERE p_idx = '$idx'";
			$db2->query($SQL_S);
			$modify_tax_detail_array = $db2->fetchall(0,"object");
		}
	
		
		//print_r($modify_tax_array);
		//exit;
		...
		$national_tax_no		= $db->dt[national_tax_no];						// 국세청 승인코드
		$popbill_tax_no		= $db->dt[popbill_tax_no];						// 팝빌 승인코드
	...
	if($m_kind == 6 || $m_kind == 4 || $m_kind == 3){
		$supply_price = $supply_price*-1;
		$tax_price = $tax_price*-1;
		$total_price = $total_price*-1;
	}
	
	if($m_kind == 4 || $m_kind == 2){
		$signdate = date('Y-m-d');
	}
	
	if($m_kind == 2){
		$marking = $db->dt[signdate]." (당초 세금계산서 작성일)";
		$supply_price = 0;
		$tax_price = 0;
		$total_price = 0;
	}
	...
	<script>
		...
		/*  $('input[type=\"radio\"]')
			...
			.css('background','#f0f0f0');
			*/
		  $('textarea[name=\"r_company_name\"]')
			.attr('readonly',true)
			.css('background','#f0f0f0');
		  $('textarea[name=\"r_name\"]')
			.attr('readonly',true)
			.css('background','#f0f0f0');
		  $('textarea[name=\"r_address\"]')
			.attr('readonly',true)
			.css('background','#f0f0f0');
		});
	</script>";
	...
	function show_title()
	{
		var ss_vv = $('input[name=\"m_kind\"]:checked').val();
		if(ss_vv == 1) $('#s_view').html('기재사항의 착오정정');
		if(ss_vv == 2) $('#s_view').html('공급가액변동');
		if(ss_vv == 3) $('#s_view').html('환입');
		if(ss_vv == 4) $('#s_view').html('계약의 해제');
		if(ss_vv == 5) $('#s_view').html('내국신용장 사후 개설');
	}
	</script>
	";
			...
			<tr>
				...
				<td class='tab' align='left'>
					<table id='tab_01' $tab01>
						<tr>
							<th class='box_01'></th>
							<td class='box_02' onclick=\"document.location.href='./tax_mdfy.php?idx=$idx&m_kind=$m_kind&publish_type=1'\">세금계산서</td>
							<th class='box_03'></th>
						</tr>
					</table>
					<table id='tab_02' $tab02>
						<tr>
							<th class='box_01'></th>
							<td class='box_02' onclick=\"document.location.href='./tax_mdfy.php?idx=$idx&m_kind=$m_kind&publish_type=2'\" >역발행</td>
							<th class='box_03'></th>
						</tr>
					</table>
					<table id='tab_03' $tab03>
						<tr>
							<th class='box_01'></th>
							<td class='box_02' onclick=\"document.location.href='./tax_mdfy.php?idx=$idx&m_kind=$m_kind&publish_type=3'\" >위수탁</td>
							<th class='box_03'></th>
						</tr>
					</table>
				</td>
	...
	<!--table border='0' cellspacing='1' cellpadding='0' class='list_table_box'  bgcolor='#CCCCCC' style='margin:0 0 5px 0' width='100%'>
		<tr bgcolor='#FFFFFF' height='35'>
			<td class='input_box_title'>수정사유선택</td>
			<td class='input_box_item'>
			<input type='radio' name='m_kind' id='m_kind' value='1' checked onclick='show_title()'> 기재사항의 착오정정
			<input type='radio' name='m_kind' id='m_kind' value='2' onclick='show_title()'> 공급가액변동
			<input type='radio' name='m_kind' id='m_kind' value='3' onclick='show_title()'> 환입
			<input type='radio' name='m_kind' id='m_kind' value='4' onclick='show_title()'> 계약의 해제
			<input type='radio' name='m_kind' id='m_kind' value='5' onclick='show_title()'> 내국신용장 사후 개설
			<input type='radio' name='m_kind' id='m_kind' value='6' onclick='show_title()'> 착오에 의한 이중발행
			</td>
		</tR>
	</table-->
	...
	<table cellpadding='0' cellspacing='0' border='0' width='100%' style='margin-bottom:20px;'>
		<tbody>
		<tr>
			<td align='left' colspan='6'> <img src='../images/korea/step_modify03.gif' align='absmiddle'></td>
		</tr>
		</tbody>
	</table>
";
...
if($m_kind == "1")
{
	...
	<table width='100%'>
		<tr>
			<td colspan='2' style='padding:10px 10px 10px 10px'>당초 취소분 부(-)의 세금계산서는 자동 작성됩니다.</td>
		</tr>
		<tr>
			<td style='padding:10px 10px 10px 10px'><font color='#339900'>변경분 정(+)의 세금계산서를 작성하여 주십시오</font></td>
			<!--td align='right'><input type='button' value='당초 문서 불러오기'></td-->
		</tr>
	</table>
	";
}
$Contents .= "
	<form name='frm' id='frm' action='./sales_write_act.php' method='POST' target='' enctype='multipart/form-data'><!--PROC-->
	...
	<input type='hidden' name='act' id='act' value='modify'> <!-수정세금계산서 구분 값-->
	<input type='hidden' name='document_type' id='document_type' value='2'> <!-수정세금계산서 구분 값-->
	<input type='hidden' name='m_kind' id='m_kind' value='$m_kind'> <!-수정세금계산서 구분 값-->
	<input type='hidden' name='national_tax_no' id='national_tax_no' value='$national_tax_no'> <!-국세청 승인코드 구분 값-->
	<input type='hidden' name='popbill_tax_no' id='popbill_tax_no' value='$popbill_tax_no'> <!-팝빌 승인코드 구분 값-->
	";
	if($m_kind == 1){
	
	}
	
	if(is_array($modify_tax_array)){
		foreach($modify_tax_array as $key => $value){
			
			if($key == 'supply_price' || $key == 'tax_price' || $key == 'total_price'){
				$value = $value*-1;
			}
		$Contents .= "
			<input type='hidden' name='ori_".$key."'  value='$value'> <!-수정세금계산서 원본정보-->";
			
		}
	}
	
	if(is_array($modify_tax_detail_array)){
		for($i=0 ;$i < count($modify_tax_detail_array); $i++){
			foreach($modify_tax_detail_array[$i] as $key => $value){
			
			if($key == 'price' || $key == 'p_price' || $key == 'tax'){
				$value = $value*-1;
			}
			$Contents .= "
				<input type='' name='ori_detail_".$key."[$i]'  value='$value'> <!-수정세금계산서 원본정보-->";
				
			}
		}
	}
	
//	echo count($modify_tax_detail_array);
	//print_r($_array_tax);
	
//	print_r($modify_tax_detail_array);
...
$Contents .= "
	...
	<tr height='35' bgcolor='white'>
		<td colspan='16' rowspan='2' class='BLine_T' align='center'><b><span style='font-size:24px;'>수정 전자세금계산서</span></b> (<span id='s_view'>$modify_title</span>)</td>
	...
	<tr height='35'>
		...
		<td colspan='8' class='BLine' align='right' style='padding:0px 8px 0px 6px;'><input type='text' class='tb' name='numbering' style='width:95%;IME-MODE:disabled;text-align:right;' valtype='NUM' maxlength='27' value='$numbering'></td>
	</tr>
	<tr height='35'>
		...
		<td colspan='8' align='center' class='RBLine'>
			<input type='text' class='tb NoBorder' name='s_company_number' id='s_company_number' value='$s_company_number' maxlength='14' style='width:100%;text-align:center;' Essential='1' $readOn>
		</td>
		...
		<td colspan='8' align='center' class='RBLine'>
			<input type='text' class='tb'  name='r_company_number' id='r_company_number' value='$r_company_number' maxlength='14' style='width:63%;text-align:center;' Essential='1' BlurEvent='1'>
			<!--img src='./img/search_01.gif' align='absmiddle' onclick='search_company()' style='cursor:hand'-->
			<img src='../images/".$admininfo[language]."/btn_customer_search.gif' onclick='search_company()' style='cursor:pointer' align='absmiddle'>
		</td>
		<td colspan='3' align='center' class='RBLine_T'>종사업장</td>
	...
	<tr height='35'>
		...
		<td colspan='8' align='left' class='RBLine'>
			<textarea class='tb$readOn_style' name='s_company_name' id='s_company_name' style='width:95%;height:26px;overflow:hidden;' Essential='1' $readOn maxlength='70'>$s_company_name</textarea>
		</td>
		...
		<td colspan='3' align='center' class='RBLine_T' style='line-height:16px;'>상호<br>(업체명)</td>
		<td colspan='8' align='left' class='RBLine'>
			<textarea class='tb' name='r_company_name' id='r_company_name' style='width:95%;height:26px;overflow:hidden;' Essential='1' maxlength='70'>$r_company_name</textarea>
		</td>
		<td colspan='1' align='center' class='RBLine_T' style='line-height:16px;'>성<br>명</td>
	...
	<tr height='35'>
		...
		<td colspan='13' align='left' class='RBLine'>
			<textarea class='tb$readOn_style' name='s_address' id='s_address' style='width:97%;height:26px;overflow:hidden;' $readOn maxlength='150'>$s_address</textarea>
		</td>
		<td colspan='3' align='center' class='RBLine_T' style='line-height:16px;'>사업장<br>주소</td>
		<td colspan='13' align='left' class='BLine'>
			<textarea class='tb'  name='r_address' id='r_address' style='width:97%;height:26px;overflow:hidden;' maxlength='150'>$r_address</textarea>
		</td>
	</tr>
	<tr height='35'>
	...
	<tr height='35'>
		...
		<td colspan='2' align='left' class='RBLine'>
			<!--img src='./img/search.gif' id='Select_Contact' class='Buttons' onclick='search_company()' style='cursor:hand'--></td>
		<td colspan='2' align='center' class='RBLine_T' style='padding:0px;'>연락처</td>
		<td colspan='5' align='left' class='BLine'>
	...
	<tr height='35'>
		...
		<td colspan='13' align='left' class='RBLine'>
			<input type='text' class='tb' name='s_email1' id='s_email1' value='$s_email[0]' style='width:130px;' Essential='1'> @ <input type='text' class='tb' name='s_email2' id='s_email2' value='$s_email[1]' style='width:130px;' Essential='1'>
		...
		<td colspan='3' align='center' class='RBLine_T'>이메일</td>
		<td colspan='13' align='left' class='BLine'>
			<input type='text' class='tb' name='r_email1' id='r_email1' value='$r_email[0]' style='width:130px;'> @ <input type='text' class='tb'  name='r_email2' id='r_email2' value='$r_email[1]' style='width:130px;'>
	...
	$Contents .= "
	<tr height='35' bgcolor='white'>
		<td colspan='16' rowspan='2' class='BLine_T' align='center'><b><span style='font-size:24px;'>수정 전자세금계산서</span></b> ($modify_title)</td>
		<td colspan='4' rowspan='2' class='BLine_T' align='center'style='line-height:20px;'>공 급 자<br>(보 관 용)</td>
		<td colspan='2' rowspan='2' class='BLine'>&nbsp;</td>
		<td colspan='4' align='right' class='NoLine_T'>책번호 :</td>
		<td colspan='3' align='right' class='NoLine'><input type='text' class='tb' name='numbering_k' style='width:56px;IME-MODE:disabled;text-align:right;' valtype='NUM' maxlength='4' value='$numbering_k'></td>
	...
	<tr height='35'>
		...
		<td colspan='3' align='center' class='RBLine_T'>등록번호</td>
		<td colspan='8' align='center' class='RBLine'>
			<input type='text' class='tb NoBorder' name='s_company_number' id='s_company_number' value='$s_company_number' maxlength='14' style='width:63%;text-align:center;' Essential='1'>
		...
		<td colspan='8' align='center' class='RBLine'>
			<input type='text' class='tb'  name='r_company_number' id='r_company_number' value='$r_company_number' maxlength='14' style='width:100%;text-align:center;' Essential='1' BlurEvent='1'>
		</td>
		<td colspan='3' align='center' class='RBLine_T'>종사업장</td>
		<td colspan='2' align='center' class='BLine'>
			<input type='text' class='tb' name='r_company_j' id='r_company_j' valtype='NUM' value='' maxlength='4' style='width:95%;IME-MODE:disabled;'>
	...
	<tr height='35'>
		...
		<td colspan='2' align='left' class='RBLine'>
			<!--img src='./img/search.gif' id='Select_Contact' class='Buttons' onclick='search_company(2)' style='cursor:hand'--></td>
		<td colspan='2' align='center' class='RBLine_T' style='padding:0px;'>연락처</td>
		<td colspan='5' align='left' class='RBLine'>
			<input type='text' class='tb' name='s_tel' id='s_tel' value='$s_tel' style='width:100%;' maxlength='20'>
		</td>
	...
	<tr height='35'>
		...
		<td colspan='2' align='left' class='RBLine'>
			<!--img src='./img/search.gif' id='Select_Contact' class='Buttons' onclick='search_company(1)' style='cursor:hand'--></td>
		<td colspan='2' align='center' class='RBLine_T' style='padding:0px;'>연락처</td>
		<td colspan='5' align='left' class='BLine'>
			<input type='text' class='tb' name='r_tel' id='r_tel' value='$r_tel' style='width:100%;' maxlength='20'>
		</td>
	...
	<tr height='35'>
		<td colspan='4' class='RBLine' style='padding:0px 4px 0px 2px;' align='center'><input type='text' name='signdate' id='sdate' value='$signdate' class='tb_readonlycogoodscogoods' style='width:80px;text-align:center' IconYN='0' ></td>
	...
	<tr height='35'>
		...
		<td colspan='29' class='RBLine'><input type='text' class='tb' name='marking' id='marking' maxlength='150' style='width:95%;' value='$marking'></td>
...
if($idx != "")
{
	...
	if($m_kind == 1){
		$modify_tax_detail_array = $db->fetchall();
	}
	...
	for ($i = 0; $i < $db->total; $i++)
	{
		...
		<tr height='35' id='Detail1'>
			...
			<td colspan='3' align='center' class='RBLine'><input type='text' class='tb' name='p_size[$k]' id='p_size[$k]' maxlength='60' style='width:95%;text-align:right;' value='$p_size'></td>
...
{
	$Contents .= "
	<table width='100%' class='table_border' style='margin:10px 0 0 0'>
		<tr>
			<td rowspan='2' align='center' width='150' class='leftmenu'><img src='../image/title_head.gif'> 국세청전송방법</td>
			<td height='30'><input type='radio' name='send_type' id='send_type' value='1' onclick='show_sendview()' $checked1>기본전송 <input type='radio' name='send_type' id='send_type' value='2' onclick='show_sendview()' $checked2>승인후전송 <input type='radio' name='send_type' id='send_type' value='3' onclick='show_sendview()' $checked3>즉시전송</td>
		</tr>
		<tr>
			<td height='30' id='send_view'>
...
<table width='100%' style='margin:10px 0 0 0;' cellpadding='0' cellspacing='0' class='input_table_box'>
	<tr >
		<td class='input_box_title'>문자발송정보</td>
		<td class='input_box_item'>
			<input type='checkbox' name='sms_chk' id='sms_chk' value='Y' $sms_checked> 문자발송여부 
			<input type='text' name='hp1' id='hp1' style='width:50px' value='$sms_number[0]'> - <input type='text' name='hp2' id='hp2' style='width:50px' value='$sms_number[1]'> - <input type='text' name='hp3' id='hp3' style='width:50px' value='$sms_number[2]'>
		</td>
	</tr>
	<tr>
		<td class='input_box_title' >팩스발송정보</td>
		<td class='input_box_item'>
			<input type='checkbox' name='fax_chk' id='fax_chk' value='Y' $fax_checked> 팩스발송여부 
			<input type='text' name='fax1' id='fax1' style='width:50px' value='$fax_number[0]'> - <input type='text' name='fax2' id='fax2' style='width:50px' value='$fax_number[1]'> - <input type='text' name='fax3' id='fax3' style='width:50px' value='$fax_number[2]'>
		</td>
	</tr>
";
...
$Contents .= "
	<tr height='30' bgcolor='#FFFFFF'>
		<td class='input_box_title'>첨부파일</td>
...
$Contents .= "
		<span id='file1' style='padding:10px 5px 10px 5px'><input type='file' class='textbox' name='DataFile[]' id='DataFile[]' style='height:22px; border:1px solid #c5c5c5'> <!--img src='./img/add_.gif' align='absbottom' onclick='add_filearea()'--> </span><span class='file1'><img src='../images/".$admininfo[language]."/btn_add_ico.gif' id='add_btn' style='cursor:pointer' align='absmiddle' onclick='add_filearea()'></span><br>
		</td>
	</tr>
";
...
$Contents .= "
	<tr>
		...
		<td class='input_box_item' height='140' style='padding:5px 5px 5px 15px'>
...
<table width='100%' style='margin-top:15px;'>
	...
	<tr>
		...
		<td align='center'>
			<a href='javascript:frm_submit(1)'><img src='../images/".$admininfo[language]."/bnt_publish_now.gif' align='absmiddle'></a>
			<!--a href='javascript:frm_submit(2)'><img src='../images/".$admininfo[language]."/bnt_publish_waiting .gif' align='absmiddle'></a-->
			<a href='javascript:frm_submit(3)'><img src='../images/".$admininfo[language]."/btn_publish_IRS.gif' align='absmiddle'></a>
		</td>
		<td align='right'>
			<a href='javascript:frm_submit(0)'><img src='../images/".$admininfo[language]."/btn_save_tmp.gif' align='absmiddle'></a>
		</td>
	</tr>
</table>
";
...
$Contents .= "
	...
	$P->jquery_use = true;
	$P->Navigation = "세금계산서관리 > 수정발행 > 수정발행";
	$P->title = "수정발행";
