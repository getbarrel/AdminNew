...
if($_POST['act'] == 'select') {
	...
	$db->fetch();
	if($_POST['select'] == 'Y') {
		echo "<script language='javascript' src='../js/message.js.php'></script><script>show_alert('채택이 정상적으로 처리되었습니다.');self.close();</script>";
	}else {
		echo "<script>self.close();</script>";
	}
}
if($_GET[mode] =='del') {
	$db->query("delete from shop_idea where ix = '".$_GET[ix]."'");
	$db->fetch();
