...
if($excel_type == "stock_report_excel"){
	$excel_name = "기초정보 사원관리 엑셀 ";
	$act = "stock_report_setting";

}else if ($excel_type == 'company_list'){
	$excel_name = "회사 조직도 엑셀";
	$act = "company_list";
	$conf_name = "company_list_".$info_type;
	$conf_name_check = "check_company_list_".$info_type;

}else if ($excel_type == 'member_list'){
	$excel_name = "사원리스트 엑셀";
	$act = "member_list";
	$conf_name = "member_".$info_type;
	$conf_name_check = "check_member_".$info_type;

}else if ($excel_type == 'member_resign'){
	$excel_name = "퇴사사원 엑셀";
	$act = "member_resign";
	$conf_name = "member_resign_".$info_type;
	$conf_name_check = "check_member_resign_".$info_type;

}else if ($excel_type == 'seller_list'){
	$excel_name = "거래처리스트 엑셀";
	$act = "seller_list";
...
$help_text = "
				<table cellpadding=2 cellspacing=0 class='small' >
					...
					<tr>
						<td valign=top>
							<img src='/admin/image/icon_list.gif' ></td><td style='line-height:140%'><b>엑셀필드 순서 정의</b> : 앞쪽부터 상품코드, 상품명 ... 등으로 엑셀에 출력됩니다 순서를 바꾸시길 원하시면 마우스로 드래그해서 순서를 변경하시기 바랍니다</td></tr>
					<tr>
						<td valign=top>
							<img src='/admin/image/icon_list.gif' ></td><td >엑셀에 출력되길 원하는 필드만 선택하시면 됩니다</td></tr>
					<tr>
						<td valign=top>
							<img src='/admin/image/icon_list.gif' ></td><td >저장된 엑셀 설정정보를 삭제하시면 초기 셋팅값이 노출되게 됩니다. 삭제후 재설정하신후 반드시 저장을 하셔야 엑셀 다운로드가 정상적으로 실행됩니다.</td></tr>
				</table>";
