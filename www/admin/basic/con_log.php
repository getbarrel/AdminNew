...
$mstring ="
		...
		<tr>
			<td align='left' colspan=6 > ".GetTitleNavigation("관리자 접속 로그", "사이트관리 > 관리자 접속 로그 ")."</td>
		</tr>
		<tr>
							...
							<tr>
								<td>
								  <select name=search_type>
									<option value='con_id' ".CompareReturnValue("con_id",$search_type,"selected").">접속 ID</option>
...
$help_text = "
	<table cellpadding=1 cellspacing=0 class='small' >
		<col width=8>
		<col width=*>
		<tr><td valign=top><img src='/admin/image/icon_list.gif' ></td><td class='small' >로그인한 관리자의 업체명, 아이디, 로그시간, 로그정보, 접속IP를 확인 할 수 있습니다.</td></tr>

	</table>
";*/
		 $help_text = getTransDiscription(md5($_SERVER["PHP_SELF"]),'A');

$help_text = "<div style='position:relative;z-index:100px;'>".HelpBox("<div style='position:relative;top:4px;'><table><tr><td valign=bottom><b>관리자 접속 로그</b></td><td></td></tr></table></div>", $help_text,170)."</div>";
...
$P->Navigation = "기초정보 > 본사관리 > 관리자 접속 로그";
$P->title = "관리자 접속 로그";
...
function PrintLogList(){
	...
	$sql = "select COUNT(*) as total from con_log $where ";
	...
	<tr align=center bgcolor=#efefef height=25>
		...
		<td class=m_td width='20%'>업체명</td>
		<td class=m_td width='20%'>접속 ID</td>
		<td class=m_td width='20%'>로그시간</td>
		<td class=m_td width='10%'>로그정보</td>
		<td class=e_td width='20%'>접속 IP</td>
	</tr>";
	if ($total == 0){
		$mString .= "<tr bgcolor=#ffffff height=70><td colspan=6 align=center>로그 내역이 존재 하지 않습니다.</td></tr>";
	}else{

		$db->query("select * from con_log  $where order by  log_date desc limit $start, $max");
		...
		for($i=0;$i < $db->total;$i++){
			...
			<td class='list_box_td'>".$db->dt[con_name]."</td>
			<td class='list_box_td point'>".$db->dt[con_id]."</td>
