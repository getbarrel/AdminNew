...
$Script .= "
	...
	function requestPop(idx){
		var url = './admin_log_pop.php?log_ix='+idx;
		var name = idx;
		var option = 'width = 1280, height = 1024, top = 100, left = 200, location = no'
...
<form name=searchmember method='get' >
	<table cellpadding=0 cellspacing=0 width=100% border=0 align=center>
		<tr>
			<td align='left' colspan=6 > ".GetTitleNavigation("관리자 작업 로그", "사이트관리 > 관리자 작업 로그 ")."</td>
		</tr>
		<tr>
						...
						<tr>
							<td class='tab'>
								<table id='tab_01' class='on'>
									<tr>
										<th class='box_01'></th>
										<td class='box_02' onclick=\"document.location.href='admin_log.php'\">관리자 작업 로그</td>
										<th class='box_03'></th>
									</tr>
								</table>
								<table id='tab_02' >
									<tr>
										<th class='box_01'></th>
										<td class='box_02' onclick=\"document.location.href='con_log.php'\" >관리자 접속 로그</td>
										<th class='box_03'></th>
									</tr>
								</table>
							</td>
					...
					<tr>
						...
						<td class='box_05'  valign=top >
							<table width='100%' border='0' cellspacing='0' cellpadding='0' height='25' class='input_table_box'>
								...
								<tr height=27>
									<td class='input_box_title' ><b>CRUD검색</b> </td>
									<td class='input_box_item' >
										<table cellpadding=0 cellspacing=0>
											<tr>
												<td>
													<select name=crud_type>
														<option value='' ".CompareReturnValue("",$crud_type,"selected").">전체</option>
														<option value='C' ".CompareReturnValue("C",$crud_type,"selected").">생성</option>
														<option value='R' ".CompareReturnValue("R",$crud_type,"selected").">읽기</option>
														<option value='U' ".CompareReturnValue("U",$crud_type,"selected").">수정</option>
														<option value='D' ".CompareReturnValue("D",$crud_type,"selected").">삭제</option>
													</select>
												</td>
											</tr>
										</table>
...
$help_text = "<div style='position:relative;z-index:100px;'>".HelpBox("<div style='position:relative;top:4px;'><table><tr><td valign=bottom><b>관리자 작업 로그</b></td><td></td></tr></table></div>", $help_text,170)."</div>";
...
$P->Navigation = "기초정보 > 본사관리 > 관리자 작업 로그";
$P->title = "관리자 작업 로그";
...
function PrintLogList(){
    global $db, $mdb, $page, $search_type, $search_text, $crud_type, $disp_yn, $regdate;
...
function printLogListNew(){
    global $db, $mdb, $page, $search_type,$search_text,$crud_type, $disp_yn;
    ...
    if ($total == 0){
        ...
        for($i=0;$i < $db->total;$i++){
			...
			<tr bgcolor=#ffffff align=center height=30>
				...
				<td class='list_box_td '>".$menu_name."</td>
				<td class='list_box_td list_bg_gray'>".$db->dt[ip]."</td>
				<td class='list_box_td' style='text-align:left;'><span style='text-overflow: ellipsis; overflow: hidden; white-space: nowrap; width: 545px; display: inline-block;'><a href='javascript:requestPop(".$db->dt[log_ix].")'>".$send_data."</a></span></td>
				<td class='list_box_td '>".$crud_div."</td>
				<td class='list_box_td '>".$db->dt[regdate]."</td>
			</tr>
			";
