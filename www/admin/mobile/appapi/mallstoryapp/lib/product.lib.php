<?php
	...
	//상품등록
     function insertproduct($itemdata){
		...
		$pcode = urldecode($itemdata->pcode);
		...
		$company = urldecode($itemdata->company);

		$product_type="6";//모바일등록상품
		$state = "1";
		$disp = "0";
		...
		$stock_use_yn = "Q";
		$free_delivery_yn ="N";
		$delivery_product_policy="3";//3:무료매송 1:유료배송
		...
		if($pname){
			...
			if($_SERVER["HTTP_HOST"]=="dev.forbiz.co.kr"){
				...
				$sql = "INSERT INTO ".TBL_SHOP_PRODUCT."
						(id,  pname,pcode, company,  shotinfo,  buyingservice_coprice, wholesale_price, wholesale_sellprice, listprice,sellprice,  coprice,wholesale_reserve_yn, reserve_yn,  state, disp,product_type,  vieworder, admin, search_keyword,reg_category,  surtax_yorn,delivery_company,one_commission,cupon_use_yn,stock_use_yn, delivery_policy,free_delivery_yn,
						origin, regdate,reg_charger_ix,reg_charger_name,delivery_product_policy)
						values('', '".strip_tags(trim($pname))."','$pcode', '$company', '$shotinfo', '$buyingservice_coprice','$wholesale_price','$wholesale_sellprice','$listprice','$sellprice', '$coprice','$wholesale_reserve_yn', '$reserve_yn', $state, '$disp','$product_type', '$vieworder', '$company_id','$search_keyword','$reg_category','$surtax_yorn','$delivery_company','$one_commission','$cupon_use_yn','$stock_use_yn','$delivery_policy','$free_delivery_yn',
						'$origin',NOW(),'".$_SESSION["admininfo"]["charger_ix"]."','".$_SESSION["admininfo"]["charger"]."(".$_SESSION["admininfo"]["charger_id"].")','$delivery_product_policy') ";
			}else{
			
				//NEWDEV 전용 쿼리
				$sql = "INSERT INTO ".TBL_SHOP_PRODUCT."
						(id,  pname,pcode, company,  shotinfo,  buyingservice_coprice, wholesale_price, wholesale_sellprice, listprice,sellprice,  coprice, reserve_yn,  state, disp,product_type,  vieworder, admin, search_keyword,reg_category,  surtax_yorn,delivery_company,one_commission,stock_use_yn, delivery_policy,free_delivery_yn,
						orgin, regdate,delivery_product_policy)
						values('', '".strip_tags(trim($pname))."','$pcode', '$company', '$shotinfo', '$buyingservice_coprice','$wholesale_price','$wholesale_sellprice','$listprice','$sellprice', '$coprice', '$reserve_yn', $state, '$disp','$product_type', '$vieworder', '$company_id','$search_keyword','$reg_category','$surtax_yorn','$delivery_company','$one_commission','$stock_use_yn','$delivery_policy','$free_delivery_yn',
						'$origin',NOW(),'$delivery_product_policy') ";
			}
	...
	function insertproductprice($itemdata,$pid){
		global $db;
		...
		$sql .= " values('', '".$pid."','$listprice','$sellprice', '$coprice', '$reserve',  '".$_SESSION["admininfo"]["company_id"]."','[".$_SESSION["admininfo"]["company_name"]."] ".$_SESSION["admininfo"]["charger"]."(".$_SESSION["admininfo"]["charger_id"].")', NOW()) ";
		
		
		//$sql = "INSERT INTO ".TBL_SHOP_PRICEINFO." (id, pid, listprice, sellprice, coprice, reserve,  admin, regdate) ";
		//$sql .= " values('', '".$pid."','$listprice','$sellprice', '$coprice', '$reserve',  '".$_SESSION["admininfo"]["company_id"]."', NOW()) ";
	...
	function copyproductimage($itemdata,$pid){
		...
		$uploadpath = $_SERVER["DOCUMENT_ROOT"].'/admin/mobile/appapi/mallstoryapp/tmp/';
	...
	function copyproductbasicinfo($itemdata,$pid){
		...
		//옵션에 등로된 이미지 로직 처리!
		foreach($_FILES as $_fileName_ => $_fileInfo_){
			if(substr_count($_fileName_,"optionimgfile")>0){
				$file=$_fileName_;
				$afile = $_FILES[$file];
				...
				if( $afile['size'] > 0 ) {
					...
					//syslog(LOG_NOTICE, json_encode($image_info[0]));
					//$image_resize_type = "W";
					//syslog(LOG_NOTICE, $file." width : ".$image_info[0]);
