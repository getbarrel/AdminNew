...
$Contents01 = "
	<table width='100%' cellpadding=0 cellspacing=0 border='0' align='left'>
		...
		<col width='*' />
	  <tr >
		<td align='left' colspan=3> ".GetTitleNavigation("부서관리", "관리자설정 > 부서관리 ")."</td>
	  </tr>
	  <tr>
	  ...
	  <tr>
	    <td align='left' colspan=3 style='padding:3px 0px;'> ".colorCirCleBox("#efefef","100%","<div style='padding:5px 5px 5px 15px;'><img src='../image/title_head.gif' align=absmiddle> <b>부서수정하기</b></div>")."</td>
	  </tr>
	 </table>
	...
	<col width='*' />
	  <tr bgcolor=#ffffff >
	    <td class='input_box_title'> <b>부서명 <img src='".$required3_path."'></b> </td>
	    <td class='input_box_item'><input type=text class='textbox' name='dp_name' value='".$db->dt[dp_name]."' style='width:230px;' validation='true' title='부서명'> <span class=small></span></td>
	  </tr>
	  <tr bgcolor=#ffffff >
	    <td class='input_box_title'> 부서 이미지 </td>
	    <td class='input_box_item'>
			<table cellpadding=0 cellspacing=0>
				<tr>
					<td><input type=file class='textbox' name='dp_img' value='' style='width:230px;'></td>
					<td align=left>
						<span class=small><!--부서이미지가 있으실 때 입력해주시기 바랍니다. 109* 27--> ".getTransDiscription(md5($_SERVER["PHP_SELF"]),'A')." </span>
						<div id='dp_img_area' ></div>
					</td>
				</tr>
			</table>
	    </td>
	  ...
	  <tr bgcolor=#ffffff >
	    <td class='input_box_title'> <b>부서등급 <img src='".$required3_path."'></b> </td>
	    <td class='input_box_item'><input type=text class='textbox' name='dp_level' value='".$db->dt[dp_level]."' style='width:60px;' validation='true' title='부서등급'>";
/*
$Contents01 .= "<select name='dp_level' style='font-size:12px;'>";
	for($i=0;$i < 10;$i++){
		$Contents01 .= "<option value='$i' ".($db->dt[dp_level] == $i ? "checked":"").">$i</option>";
	}
$Contents01 .= "</select>";
*/
$Contents01 .= "
...
$Contents02 =colorCirCleBox("#efefef","100%","<div style='padding:5px 5px 5px 15px;'><img src='../image/title_head.gif' align=absmiddle> <b>부서목록</b></div>");
$Contents02 .= "
	<table width='100%' cellpadding=0 cellspacing=0 border='0' align='left' class='list_table_box' style='margin:3px 0px;'>
		<col width='80' />
		<col width='190' />
		<col width='*' />
		<col width='130' />
	  ...
	  <tr height=25 bgcolor=#efefef align=center style='font-weight:bold'>
	    <td class='s_td'> 번호</td>
	    <td class='m_td'> 부서명</td>
	    <td class='m_td'> 부서이미지</td>
	    <td class='m_td'> 부서등급</td>
	    <!--td class='m_td' > 할인율</td-->
	    <td class='m_td'> 사용여부</td>
	    <!--td class='m_td' style='width=170px;'> 등록일자</td-->
	    <td class='e_td'> 관리</td>
	  </tr>";
$db = new Database;


$db->query("SELECT * FROM shop_company_department where company_id = '".$admininfo["company_id"]."'  order by dp_level asc ");
...
if($db->total){
	...
	$Contents02 .= "
		  <tr bgcolor=#ffffff height=30 align=center>
		    ...
		    <td class='list_box_td point'>".$db->dt[dp_name]."</td>
		    <td class='list_box_td list_bg_gray' align=left style='padding:5px;'>";

		    if ($db->dt[dp_img] != '' && file_exists($_SERVER["DOCUMENT_ROOT"]."".$admin_config[mall_data_root]."/images/department/".$db->dt[dp_img])){
			    $Contents02 .= "<img src='".$admin_config[mall_data_root]."/images/department/".$db->dt[dp_img]."' width=109>";
			  }
		    $Contents02 .= "
		    </td>
		    <td class='list_box_td '>".$db->dt[dp_level]."  </td>
		    <!--td class='list_box_td list_bg_gray'>".$db->dt[sale_rate]." %</td-->
		    <td class='list_box_td list_bg_gray'>".($db->dt[disp] == "1" ?  "사용":"사용하지않음")."</td>
		    <td class='list_box_td '>";

				if(checkMenuAuth(md5($_SERVER["PHP_SELF"]),"U")){
					$Contents02 .= "<a href=\"javascript:updateDepartmentInfo('".$db->dt[dp_ix]."','".$db->dt[sale_rate]."','".$db->dt[dp_name]."','".$db->dt[dp_name]."','".$db->dt[dp_id]."','".trim($db->dt[dp_img])."','".$db->dt[dp_level]."','".$db->dt[sale_rate]."','".$db->dt[memberreg_baymoney]."','".$db->dt[use_mall_yn]."','".$db->dt[disp]."','".$db->dt[basic]."')\"><img src='../images/".$admininfo["language"]."/btc_modify.gif' border=0></a>";
				}else{
					$Contents02 .= "
				...
				$Contents02 .= "
					<a href=\"javascript:deleteDepartmentInfo('delete','".$db->dt[dp_ix]."')\"><img src='../images/".$admininfo["language"]."/btc_del.gif' border=0></a>";
				}else{
	...
	$Contents02 .= "
		  <tr bgcolor=#ffffff height=50>
		    <td align=center colspan=10>등록된 부서이 없습니다. </td>
		  </tr>
