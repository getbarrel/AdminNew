...
openlog("phplog", LOG_PID , LOG_LOCAL0);
	...
	if ($act == "excel_input" || true){
		...
		$objPHPExcel = PHPExcel_IOFactory::load($_SERVER["DOCUMENT_ROOT"]."/admin/_language/dic_pack_chinese_desc.xls");
		...
		//SYSLOG(LOG_INFO, "(($objPHPExcel->getActiveSheet()->getCell('A' . $rownum)->getValue() != '') && ($i < 11000))\n");
		while (($objPHPExcel->getActiveSheet()->getCell('B' . $rownum)->getValue() != "") && ($i < 4000)) {
			...
			if($objPHPExcel->getActiveSheet()->getCell('B' . $rownum)->getValue() != ""){
				...
				//$text_trans = $objPHPExcel->getActiveSheet()->getCell('H' . $rownum)->getValue();
				//$desc_trans = $objPHPExcel->getActiveSheet()->getCell('J' . $rownum)->getValue();
				$desc_trans = $objPHPExcel->getActiveSheet()->getCell('I' . $rownum)->getValue();
				//echo "dic_ix : ".$dic_ix;
				//exit;
				if($dic_type == "DESC"){

					$sql = "select * from admin_dic 
							where dic_type = '$dic_type' and menu_div = '$menu_div' and menu_code = '$menu_code' and language_type = 'chinese' 
					...
					if(!$db->total){
						...
						$sql = "insert into admin_dic
								...
								values
								('','$dic_type','$dic_code','$menu_div','$menu_code','chinese','','','$desc_trans','1',NOW()) ";
				...
				}else if($dic_type == "WORD")
				{
					...
					//echo $dic_ix.":::".$db->dt[text_korea].":::".$db->dt[text_trans].":::".$text_trans."<br>\n";
						...
						$sql = "insert into admin_dic
								...
								values
								('','$dic_type','','$menu_div','$menu_code','chinese','".$text_korea."','".$text_trans."','','".$dic_ix."','1',NOW()) ";
