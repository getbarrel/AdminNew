...
//echo "SELECT * FROM shop_payment_config where pg_code = 'nicepay' and mall_ix = '".$mall_ix."'";
$db->query("SELECT * FROM shop_payment_config where pg_code = 'nicepay' and mall_ix = '".$mall_ix."' ");
...
$Contents01 = "<table width='100%' cellpadding=3 cellspacing=0 border='0' align='left'  >
	...
	<table width='100%' cellpadding=3 cellspacing=0 border='0' align='left' class='input_table_box'>
	  ...
	  <tr bgcolor=#ffffff >
	    <td style='width:150px;' class='input_box_title'> 가맹점 ID </td>
	  ...
	  <tr bgcolor=#ffffff >
	    <td style='width:150px;' class='input_box_title'> 가맹점 KEY </td>
	  ...
	  <tr bgcolor=#ffffff >
	    <td style='width:150px;' class='input_box_title'>결제취소 비밀번호 </td>
		<td class='input_box_item'>
			...
			<span ><b>나이스페이</b> 가맹점 관리자의 가맹점 정보메뉴에서 설정한 값을 입력해주세요.</span>
		</td>
	  </tr>
...
$Contents = $Contents."<form name='edit_form' action='nicepay.act.php' method='post' onsubmit='return validate(document.edit_form)' target='act'>
...
$P->Navigation = "HOME > 상점관리 > 결제모듈(나이스페이 TX)";
$P->title = "결제모듈(나이스페이 TX)";
