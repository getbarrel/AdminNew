...
if($cid){
	$sql = 	"SELECT cr.*
			...
			where depth = '$depth' and parent_rg_ix = '$cid'  ";
	//echo $sql;
	$db->query($sql);
}else{
...
if ($db->total){
      ...
      //}
        ...
        for($i=0; $i < $db->total; $i++){
                $db->fetch($i);
				echo "<script type='text/javascript'>
					parent.document.forms['$form'].elements['".$target."'].options[".($i+1)."].text = '".$db->dt[region_name]."';
					parent.document.forms['$form'].elements['".$target."'].options[".($i+1)."].value = '".$db->dt[rg_ix]."';
				</script>";
        }
		exit;
