...
if ($act == "insert")
{
	...
	$sql = "insert into common_region (rg_ix,parent_rg_ix , region_name,depth, vieworder, disp,regdate)
			values
			('','$parent_rg_ix','$region_name','$depth','$vieworder','$disp',NOW())";
	$db->sequences = "COMMON_REGION_SEQ";
	$db->query($sql);
...
if ($act == "update"){
	...
	$sql = "update common_region set region_name='$region_name',vieworder='$vieworder',disp='$disp' where rg_ix='$rg_ix'  ";
...
if ($act == "delete"){
	...
	$sql = "delete from common_region where rg_ix='$rg_ix'";
	$db->query($sql);
