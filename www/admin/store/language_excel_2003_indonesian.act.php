...
openlog("phplog", LOG_PID , LOG_LOCAL0);
	...
	if ($act == "excel_input" || true){
		...
		$objPHPExcel = PHPExcel_IOFactory::load($_SERVER["DOCUMENT_ROOT"]."/admin/_language/laguage_pack_japan_20111015.xls");
		...
		while (($objPHPExcel->getActiveSheet()->getCell('A' . $rownum)->getValue() != "") && ($i < 11000)) {
			...
			if($objPHPExcel->getActiveSheet()->getCell('A' . $rownum)->getValue() != ""){
				...
				$text_trans = $objPHPExcel->getActiveSheet()->getCell('G' . $rownum)->getValue(); // 인도네시아어 필드
				$disp = $objPHPExcel->getActiveSheet()->getCell('H' . $rownum)->getValue();
