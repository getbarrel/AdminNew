...
$Contents01 = "
	<table width='100%' cellpadding=0 cellspacing=0 border='0' align='left'>
	  <tr >
		<td align='left' colspan=6> ".GetTitleNavigation("지역관리", "MD 관리 > 지역관리 ")."</td>
	  </tr>
	  <tr>
	  ...
	  <tr>
	    <td align='left' colspan=4 style='padding:3px 0px;'> ".colorCirCleBox("#efefef","100%","<div style='padding:5px 5px 5px 10px;'><img src='../image/title_head.gif' align=absmiddle> <b class=blk><u>$board_name</u> 지역 추가</b></div>")."</td>
	  </tr>
	  </table>
	  ...
	  <col width = * >
	  <tr bgcolor=#ffffff >
	    <td class='search_box_title' >  지역 타입 </td>
	  ...
	  <tr bgcolor=#ffffff >
	    <td class='search_box_title'> 지역명 : </td>
		<td class='search_box_item'><input type=text class='textbox' name='region_name' value='".$db->dt[region_name]."' style='width:230px;'> <span class=small></span></td>
	  </tr>
	  <tr bgcolor=#ffffff >
...
$Contents02 = "
	<div style='width:100%;'>".colorCirCleBox("#efefef","100%","<div style='padding:5px 5px 5px 10px;'><img src='../image/title_head.gif' align=absmiddle> <b  class=blk><u>$board_name</u>  지역 목록</b></div>")."</div>";
...
$sql = "select cr.* , case when depth = 1 then rg_ix  else parent_rg_ix end as group_order from common_region cr
...
$Contents = "<form name='group_form' action='region.act.php' method='post' onsubmit='return CheckValue(this)' target=act><input name='mmode' type='hidden' value='$mmode'><input name='act' type='hidden' value='insert'><input name='bm_ix' type='hidden' value='$bm_ix'><input name='rg_ix' type='hidden' value=''>";
...
$help_text = "
	<table cellpadding=1 cellspacing=0 class='small' >
		<col width=8>
		<col width=*>
		<tr><td valign=top><img src='/admin/image/icon_list.gif' ></td><td class='small' >MD 관리 지역 설정은 <b>2단계</b>까지 가능합니다</td></tr>
...
$Contents = $Contents.$help_text;
 ...
 function CheckValue(frm){
 	if(frm.depth[1].checked){
 		if(frm.parent_rg_ix.value == ''){
	 		alert('2차 지역을 등록하기 위해서는 1차지역을 반드시 선택하셔야 합니다.');
	 		return false;
 		}
 	}

 	if(frm.region_name.value.length < 1){
 		alert('등록하시고자 하는 MD 관리 지역명을 입력해주세요');
 ...
 function deleteGroupInfo(act, rg_ix){
 	if(confirm('해당지역  정보를 정말로 삭제하시겠습니까?')){
 		var frm = document.group_form;
 		frm.act.value = act;
 		frm.rg_ix.value = rg_ix;
 		frm.submit();
