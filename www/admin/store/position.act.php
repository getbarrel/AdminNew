...
if ($act == "insert")
{
	...
	$db->sequences = "SHOP_COMPANY_POSITION_SEQ";
	$db->query($sql);
	...
	if(!file_exists($_SERVER["DOCUMENT_ROOT"]."".$admin_config[mall_data_root]."/images/position/")){
		...
		chmod($_SERVER["DOCUMENT_ROOT"]."".$admin_config[mall_data_root]."/images/position/",0777);
	}
...
if ($act == "delete"){
	...
	$sql = "delete from shop_company_position where ps_ix='$ps_ix'";
	$db->query($sql);
