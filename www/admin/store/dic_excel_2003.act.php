...
openlog("phplog", LOG_PID , LOG_LOCAL0);
	...
	if ($act == "excel_input" || true){
		...
		if ($excel_file_size > 0){	
			copy($excel_file, $_SERVER["DOCUMENT_ROOT"]."".$admin_config[mall_data_root]."/images/upfile/indonesian_20110929.xls");
		}
		
		//syslog(LOG_INFO, 'GET XL START');
		$objPHPExcel = PHPExcel_IOFactory::load($_SERVER["DOCUMENT_ROOT"]."".$admin_config[mall_data_root]."/images/upfile/indonesian_20110929.xls");
		...
		while (($objPHPExcel->getActiveSheet()->getCell('B' . $rownum)->getValue() != "") && ($i < 11000)) {
			...
			if($objPHPExcel->getActiveSheet()->getCell('B' . $rownum)->getValue() != ""){
				...
				}else if($dic_type == "WORD")
				{
					...
					if(trim($db->dt[text_trans]) != trim($text_trans))
					{
						$text_trans = str_replace("'","\'",$text_trans);
						
						$sql = "update admin_dic set text_trans = '".$text_trans."'  
							where dic_ix = '$dic_ix'  ";
						echo $sql;
						//$db->query($sql);
		...
		//exit;
		echo "<script>alert('영문번역 정보 일괄등록이 완료되었습니다.');</script>";
