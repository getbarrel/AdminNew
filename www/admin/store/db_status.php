...
##  - 2003.01.26 : bug fixed, and upgraded comments, support Database 4.0.x
...
##  - http://www.mysql.com/doc/en/Database_Optimisation.html
##
## [download]
...
class mysql_status
{
  ...
  ##
  var $host	= '';		// string, Database server name or ip-addr
  ...
  ##
  var $stat	= array();	// array, Database server status
  var $vars	= array();	// array, Database server variables
  var $comments	= array();	// array, Database server status comments
  ...
  ##
  function mysql_status($what=0, $db='', $detail=FALSE)
  {
	$this->what = (int)$what;
	$this->db = $db;
  ...
  ## get Database server status and variables
  ##
  ## http://www.mysql.com/doc/en/SHOW_STATUS.html
  ## http://www.mysql.com/doc/en/SHOW_VARIABLES.html
  ## http://www.mysql.com/information/presentations/presentation-oscon2000-20000719/
  ...
  ##
  ## 1) Database caches(all threads shared) [***]
  ...
  ##
  ## 3) Database memory size of INDEX(key)/JOIN/RECORD(read)/SORT/TABLE
  ...
  function get_comments($vars, $stat)
  {
	...
	if($vars['max_connections'])
	{
		...
		if($max['table'] > $vars['table_cache'])
		{
			...
			$stat['refer_vars_table_cache'].'[table_cache]값을 올리세요.'.
			$B['e'];
		} else
	...
	if($vars['skip_locking'] == 'OFF')
	{
		$this->comments['skip_locking'] = $B['n'].',<BR>'.
		'현재 이 Database 서버는 외부 Lock을 사용하는 것으로 설정되어 있습니다. '.
	...
	if($vars['skip_networking'] == 'OFF')
	{
			...
			&& $_SERVER['SERVER_ADDR'] == @gethostbyname($this->host))
		{
			...
			'Database 서버와 클라이언트가 서로 동일한 IP 주소임에도 불구하고 '.
  ...
  function get_vars($vars)
  {
	...
	***/
	...
	'skip_locking' =>
		array($vars['skip_locking'],
		'외부 lock 을 사용한다면 OFF로 설정.'.$this->comments['skip_locking']),
	'skip_networking' =>
  ...
  {
	$r =	'<TABLE BORDER=0 WIDTH=100% CELLPADDING=0 CELLSPACING=0 BGCOLOR='.$color['line'].'>'."\n".
                '<TR BGCOLOR='.$color['line'].' height=2><TD></TD></TR>'."\n".
		'<TR><TD>'."\n".
		'<TABLE BORDER=0 WIDTH=600 CELLPADDING=3 CELLSPACING=1>'."\n".
		'<TR HEIGHT=26 BGCOLOR='.$color['th'].'>'."\n".
  ...
  {
	$r =	'<TABLE BORDER=0 WIDTH=100% CELLPADDING=0 CELLSPACING=0 '.
		'BGCOLOR='.$color['line'].'>'."\n".
                '<TR BGCOLOR='.$color['line'].' height=2><TD></TD></TR>'."\n".
		'<TR><TD>'."\n".
  ...
  { 
	...
	return $months.$days.$hours.$mins.$secs;
     ...
     if(!$detail) $detail = 1; 
     $status = new mysql_status($detail);

     if($detail==1)     $Mess ="<b>MY DATABASE STATUS</b> (connected by ". $status->via .")";
     elseif($detail==2) $Mess ="<b>SERVER STATUS</b> (connected by ". $status->via .") ";
     elseif($detail==4) $Mess ="<b>SERVER VARIABLES</b> (connected by ". $status->via .") ";

     $color = array('line'=>'#959595','th'=>'#EDECE7','td'=>'#FFFFFF');

echo $status->tohtml($color,1);
exit;
     $Result = LoadFromFile("admin/Database/Layout/admin_DbSv_status.html");
     $Result = str_replace("@Status", $status->tohtml($color,1), $Result);
     $Result = str_replace("@Mess", $Mess, $Result);
     $Result = str_replace("[PHP_SELF]", "$PHP_SELF",$Result);     
     include "./admin/_Head.php";  //헤더

?>

<table border="0" cellpadding="0" cellspacing="0" width=100%">
    <tr>
	<td width=170 bgcolor=#f2f4f4 valign=top align=center><?include "./admin/Poll/_Left.php";   //모듈관리 메뉴?></td>
	<td bgcolor=white valign=top style="padding: 0 0 0 15;"><?echo $Result; //STATUS & VARIABLES ?></td>
    </tr>
</table>

<?
     include "./admin/_Foot.php";  //푸터
     mysql_close();
?>