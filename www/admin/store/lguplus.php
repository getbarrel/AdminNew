...
$db->query("SELECT * FROM shop_payment_config where pg_code = 'lguplus' and mall_ix = '".$admininfo[mall_ix]."' ");
...
$Contents01 = "
	...
	<col width='*' />
	  <tr>
	    <td align='left' colspan=3 > ".GetTitleNavigation("결제모듈(lguplus)", "상점관리 > 결제모듈(lguplus)")."</td>
	  </tr>
	  <tr>
	    <td align='left' colspan=3 style='padding:3px 0px;'> ".colorCirCleBox("#efefef","100%","<div style='padding:5px 5px 5px 15px;'><img src='../image/title_head.gif' align=absmiddle> <b>lguplus 결제정보 </b> <span class=small>신용카드 결제 및 기타결제방식은 반드시 대행업체와 계약을 맺으시기 바랍니다 </span></div>")."</td>
	  </tr>
	 </table>
	...
	<col width='*' />
	  <tr bgcolor=#ffffff height=34>
	    <td class='input_box_title'> <b>lguplus 가맹점 코드 <img src='".$required3_path."'></b></td>
		<td class='input_box_item'><input type=text class='textbox' name='lguplus_id' value='".$payment_config[lguplus_id]."' style='width:230px;' validation='true' title='가맹점 코드'> <span class=small><b>lguplus</b> 에서 발급 받은 코드를 입력해주세요 (테스트:lgdacomxpay)</span></td>
	  </tr>
	   <tr bgcolor=#ffffff height=34>
	    <td class='input_box_title'> <b>lguplus 가맹점키 <img src='".$required3_path."'></b></td>
		<td class='input_box_item'><input type=text class='textbox' name='lguplus_key' value='".$payment_config[lguplus_key]."' style='width:230px;' validation='true' title='가맹점 KEY값'> <span class=small><b>lguplus</b> 에서 발급 받은 상점키를 입력해주세요(테스트:/O3g5VfqzfaypzEdeFkMOK/cPAimONPhmkKbRX0hTCEJzZDncxi/xu4y4QbhgzNSMLY/cXKOvrzrCEObK/NHMw==)</span></td>
	  </tr>
	  <tr bgcolor=#ffffff height=34>
		...
		<td class='input_box_item'>
			<select name='SkinType' validation='true' title='스킨타입'>
				...
				<option value='yellow' ".($payment_config[SkinType] == "yellow" ? "selected":"").">YELLOW</option>
				<option value='purple' ".($payment_config[SkinType] == "purple" ? "selected":"").">PURPLE</option>
			</select>
		</td>
	  </tr>
	   <tr bgcolor=#ffffff height=34>
	    <td class='input_box_title'> <b>lguplus 서비스타입 <img src='".$required3_path."'></b></td>
		<td class='input_box_item'><input type='radio' name='lguplus_type' value='test' ".($payment_config[lguplus_type] == "test" ? "checked":"").">TEST <input type='radio' name='lguplus_type' value='service' ".($payment_config[lguplus_type] == "service" ? "checked":"").">SERVICE <span class=small><b>lguplus</b> 에서 테스트키이면 TEST, 실제키를 발급받았으면 SERVICE 를 선택하시기 바랍니다.</span><input type='hidden' name='lguplus_type_befor' value='".$payment_config[lguplus_type]."' /></td>
	  </tr>
	  <tr bgcolor=#ffffff height=34>
		...
		<ul class='paging_area' >
			<li class='front'>
				<input type=text class='textbox' name='lguplus_interest_str' value='".$payment_config[lguplus_interest_str]."' style='width:300px;'><div class=small><br>일반결제시에 할부기간은 2~12개월까지 가능합니다.</div>
			</li>
	  ...
	  <!--tr bgcolor=#ffffff height=54>
	    ...
	    <td class='input_box_item'><input type=text class='textbox' name='lguplus_nointerest_price' value='".$payment_config[lguplus_nointerest_price]."' style='width:130px;'> 원 이상부터는 무이자 할부 적용<br>
	    <div class=small style='line-height:150%'>
	    ☞ 주의) lguplus과 상점과의 별도 계약이 필요합니다.
	    </div>
	    </td>
	  </tr>
	  <tr bgcolor=#ffffff height=34>
		...
		<ul class='paging_area' >
			<li class='front'>
				<input type=text class='textbox' name='lguplus_nointerest_str' value='".$payment_config[lguplus_nointerest_str]."'>
				<div class=small style='line-height:100%'>
				<b>☞ 주의) lguplus와 상점과의 별도 계약이 필요합니다.</b><br>
			...
			<div >
				...
				☞ 신용카드사에서 별도로 진행하는 무이자할부 이벤트는 본 무이자할부와는 상관이 없습니다. 예로 신한카드에서 2-3개월 무이지할부 이벤트를 실시할 경우 lguplus 플러그인을 사용하시는 상점에서 고객이 신한카드 3개월 할부결제를 한다면 화면상에는 일반할부처럼 보이지만 할부이자는 청구되지 않습니다.
				</div>
			</li>
		 </ul>
...
$Contents02 = "
	  ...
	  <tr>
	    <td align='left' colspan=4 style='padding:3px 0px;'> ".colorCirCleBox("#efefef","100%","<div style='padding:5px 5px 5px 15px;'><img src='../image/title_head.gif' align=absmiddle> <b>lguplus 에스크로 </b> <span class=small>신용카드 결제 및 기타결제방식(에스크로 등)은 반드시 대행업체와 계약을 맺으시기 바랍니다 </span></div>")."</td>
	  </tr>
	  </table>
	  ...
	  <tr bgcolor=#ffffff height=34>
	    <td class='input_box_title' style='width:15%;' > Escrow 배송지등록여부 </td><td>
	    <input type=radio  name='escrow_delivery' id='escrow_use_Y' value='Y' ".CompareReturnValue("Y",$payment_config[escrow_delivery],"checked")."><label for='escrow_use_Y'>사용</label>
	    <input type=radio  name='escrow_delivery' id='escrow_use_N' value='N' ".CompareReturnValue("N",$payment_config[escrow_delivery],"checked")."><label for='escrow_use_N'>사용하지 않음</label>
	    </td>
	  </tr>
	  <tr bgcolor=#ffffff height=34>
...
$Contents = $Contents."<form name='edit_form' action='lguplus.act.php' method='post' onsubmit='return CheckFormValue(this)' target='iframe_act'><input name='act' type='hidden' value='update'><input name='mall_ix' type='hidden' value='".$admininfo[mall_ix]."'><input name='pg_code' type='hidden' value='lguplus'>";
...
$P->Navigation = "상점관리 > 결제관련 > 결제모듈(lguplus)";
$P->title = "결제모듈(lguplus)";
