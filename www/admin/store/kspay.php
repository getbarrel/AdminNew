...
$db->query("SELECT * FROM shop_payment_config where pg_code = 'kspay' and mall_ix = '".$admininfo[mall_ix]."' ");
...
$Contents01 = "
	...
	<col width='*' />
	  <tr>
	    <td align='left' colspan=3 > ".GetTitleNavigation("결제모듈(KSPAY)", "상점관리 > 결제모듈(KSPAY)")."</td>
	  </tr>
	  <tr>
	    <td align='left' colspan=3 style='padding:3px 0px;'> ".colorCirCleBox("#efefef","100%","<div style='padding:5px 5px 5px 15px;'><img src='../image/title_head.gif' align=absmiddle> <b>KSPAY 결제정보 </b> <span class=small>신용카드 결제 및 기타결제방식은 반드시 대행업체와 계약을 맺으시기 바랍니다 </span></div>")."</td>
	  </tr>
	 </table>
	...
	<col width='*' />
	  <tr bgcolor=#ffffff height=34>
	    <td class='input_box_title'> <b>KSPAY 가맹점 코드 <img src='".$required3_path."'></b></td>
		<td class='input_box_item'><input type=text class='textbox' name='kspay_id' value='".$payment_config[kspay_id]."' style='width:230px;' validation='true' title='가맹점 코드'> <span class=small><b>KSPAY</b> 에서 발급 받은 코드를 입력해주세요</span></td>
	  </tr>
	   <tr bgcolor=#ffffff height=34>
	    <td class='input_box_title'> <b>KSPAY 가맹점키 <img src='".$required3_path."'></b></td>
		<td class='input_box_item'><input type=text class='textbox' name='kspay_key' value='".$payment_config[kspay_key]."' style='width:230px;' validation='true' title='가맹점 KEY값'> <span class=small><b>KSPAY</b> 에서 발급 받은 상점키를 입력해주세요 (테스트:2999199999)</span></td>
	  </tr>
	  <!--tr bgcolor=#ffffff height=34>
	   ...
	   <tr bgcolor=#ffffff height=34>
	    <td class='input_box_title'> <b>KSPAY 서비스타입 <img src='".$required3_path."'></b></td>
		<td class='input_box_item'><input type='radio' name='kspay_type' value='test' ".($payment_config[kspay_type] == "test" ? "checked":"").">TEST <input type='radio' name='kspay_type' value='service' ".($payment_config[kspay_type] == "service" ? "checked":"").">SERVICE <span class=small><b>KSPAY</b> 에서 테스트키이면 TEST, 실제키를 발급받았으면 SERVICE 를 선택하시기 바랍니다.</span><input type='hidden' name='kspay_type_befor' value='".$payment_config[kspay_type]."' /></td>
	  </tr>
	  <tr bgcolor=#ffffff height=34>
		...
		<ul class='paging_area' >
			<li class='front'>
				<input type=text class='textbox' name='kspay_interest_str' value='".$payment_config[kspay_interest_str]."' style='width:300px;'><div class=small><br>일반결제시에 할부기간은 2~12개월까지 가능합니다.</div>
			</li>
	  ...
	  <tr bgcolor=#ffffff height=54>
	    ...
	    <td class='input_box_item'><input type=text class='textbox' name='kspay_nointerest_price' value='".$payment_config[kspay_nointerest_price]."' style='width:130px;'> 원 이상부터는 무이자 할부 적용<br>
	    <div class=small style='line-height:150%'>
	    ☞ 주의) KSPAY과 상점과의 별도 계약이 필요합니다.
	    </div>
	    </td>
	  </tr>
	  <tr bgcolor=#ffffff height=34>
		...
		<ul class='paging_area' >
			<li class='front'>
				<input type=text class='textbox' name='kspay_nointerest_str' value='".$payment_config[kspay_nointerest_str]."'>
				<div class=small style='line-height:100%'>
				<b>☞ 주의) KSPAY와 상점과의 별도 계약이 필요합니다.</b><br>
			...
			<div >
				...
				☞ 신용카드사에서 별도로 진행하는 무이자할부 이벤트는 본 무이자할부와는 상관이 없습니다. 예로 신한카드에서 2-3개월 무이지할부 이벤트를 실시할 경우 KSPAY 플러그인을 사용하시는 상점에서 고객이 신한카드 3개월 할부결제를 한다면 화면상에는 일반할부처럼 보이지만 할부이자는 청구되지 않습니다.
				</div>
			</li>
		 </ul>
...
$Contents02 = "
	  ...
	  <tr>
	    <td align='left' colspan=4 style='padding:3px 0px;'> ".colorCirCleBox("#efefef","100%","<div style='padding:5px 5px 5px 15px;'><img src='../image/title_head.gif' align=absmiddle> <b>KSPAY 에스크로 </b> <span class=small>신용카드 결제 및 기타결제방식(에스크로 등)은 반드시 대행업체와 계약을 맺으시기 바랍니다 </span></div>")."</td>
	  </tr>
	  </table>
...
$Contents = $Contents."<form name='edit_form' action='kspay.act.php' method='post' onsubmit='return CheckFormValue(this)' target='iframe_act'><input name='act' type='hidden' value='update'><input name='mall_ix' type='hidden' value='".$admininfo[mall_ix]."'><input name='pg_code' type='hidden' value='kspay'>";
...
$P->Navigation = "상점관리 > 결제관련 > 결제모듈(kspay)";
$P->title = "결제모듈(kspay)";
