...
<form name=searchmember method='get' >
	<table cellpadding=0 cellspacing=0 width=100% border=0 align=center>
		<tr>
			<td align='left' colspan=6 > ".GetTitleNavigation("엑셀다운로드 로그", "사이트관리 > 엑셀다운로드 로그 ")."</td>
		</tr>
		<tr>
			<td>
											...
											<tr>
												<td>
													<select name=search_type>
														<option value='id' ".CompareReturnValue("id",$search_type,"selected").">다운로드ID</option>
														<option value='ip' ".CompareReturnValue("ip",$search_type,"selected").">다운로드IP</option>
...
$help_text = "<div style='position:relative;z-index:100px;'>".HelpBox("<div style='position:relative;top:4px;'><table><tr><td valign=bottom><b>엑셀다운로드 로그</b></td><td></td></tr></table></div>", $help_text,170)."</div>";
...
$P->Navigation = "상점관리 > 관리자설정 > 엑셀다운로드 로그";
$P->title = "엑셀다운로드 로그";
...
function printLogListNew(){
    ...
    if($regdate){
        ...
        if($startDate != "" && $endDate != ""){
            $where[] = "h.regDt BETWEEN '$startDate' AND '$endDate'";
        }
    }

    $where = (count($where)) ? " WHERE ".implode(" AND ", $where):'';

    $sql = "SELECT COUNT(*) AS total FROM ig_excel_dn_history h LEFT JOIN common_user u ON h.code = u.code $where ";
    //echo $sql;
    $mdb->query($sql);
    $mdb->fetch();
    $total = $mdb->dt[total];
	...
	<table cellpadding=4 cellspacing=0 border=0 width=100% bgcolor=silver class='list_table_box'>
		...
		<tr align=center bgcolor=#efefef height=30>
			...
			<td class=m_td >다운로드사유</td>
			<td class=m_td >다운로드경로</td>
			<td class=e_td >날짜</td>
		</tr>";
    if ($total == 0){
        ...
        $db->query("SELECT h.ip, h.dn_reason, h.dn_text, h.regDt, u.id FROM ig_excel_dn_history h LEFT JOIN common_user u ON h.code = u.code $where ORDER BY h.regDt DESC LIMIT $start, $max");
        ...
        for($i=0;$i < $db->total;$i++){
			...
			<tr bgcolor=#ffffff align=center height=30>
				...
				<td class='list_box_td' ><span style='text-overflow: ellipsis; overflow: hidden; white-space: nowrap; width: 545px; display: inline-block;'>".$db->dt[dn_reason]."</span></td>
				<td class='list_box_td list_bg_gray'>".$db->dt[dn_text]."</td>
				<td class='list_box_td '>".$db->dt[regDt]."</td>
			</tr>
			";
        }
