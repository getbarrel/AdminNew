...
$db->query("SELECT * FROM shop_payment_config where pg_code = 'tenpay' and mall_ix = '".$mall_ix."' "); //and mall_ix = '".$admininfo[mall_ix]."'
...
$Contents01 = "<table width='100%' cellpadding=3 cellspacing=0 border='0' align='left'  >
	  ...
	  <col width='*'/>  
	 <tr height=40>
	    <td align='left' colspan=4> ".colorCirCleBox("#efefef","100%","<div style='padding:5px;'><img src='../image/title_head.gif' align=absmiddle> <b>텐페이 결제정보 </b> <span class=small>신용카드 결제 및 기타결제방식은 반드시 대행업체와 계약을 맺으시기 바랍니다 </span></div>")."</td>
	  </tr>
	  </table>
	<table width='100%' cellpadding=3 cellspacing=0 border='0' align='left' class='input_table_box'>
	  ...
	  <tr bgcolor=#ffffff >
	    <td style='width:150px;' class='input_box_title'>  ID </td>
		<td class='input_box_item'><input type=text class='textbox' name='mid' value='".$payment_config[mid]."' style='width:230px;'> <span class=small><b>tenpay</b> 에서 발급 받은 아이디를 입력해주세요</span><input type='hidden' name='tenpay_mid_befor' value='".$payment_config[mid]."' /></td>
	  </tr>
	  <tr bgcolor=#ffffff >
	    <td style='width:150px;' class='input_box_title'>  PW </td>
		<td class='input_box_item'><input type=text class='textbox' name='pw' value='".$payment_config[pw]."' style='width:230px;'> <span class=small><b>tenpay</b> 에서 발급 받은 비밀번호를 입력해주세요</span></td>
	  </tr>
	  <tr bgcolor=#ffffff >
	    <td style='width:150px;' class='input_box_title'>  signkey </td>
		<td class='input_box_item'><input type=text class='textbox' name='pay_key' value='".$payment_config[pay_key]."' style='width:230px;'> <span class=small><b>tenpay</b> 에서 발급 받은 사인키를 입력해주세요</span>
	  </tr>
...
$Contents02 = "	  
	  ...
	  <col width='5%'/>
	  <tr height=40>
	    <td align='left' colspan=4> ".colorCirCleBox("#efefef","100%","<div style='padding:5px;'><img src='../image/title_head.gif' align=absmiddle> <b>tenpay 에스크로 </b> <span class=small>신용카드 결제 및 기타결제방식은 반드시 대행업체와 계약을 맺으시기 바랍니다 </span></div>")."</td>
	  </tr>	  	  
	  </table>
	  <table width='100%' cellpadding=3 cellspacing=0 border='0' align='left' class='input_table_box'>
	  <col width='20%'/>
	  ...
	  <tr bgcolor=#ffffff >
		...
		<td class='input_box_item'><input type=text class='textbox' name='escrow_mid' value='".$payment_config[escrow_mid]."' style='width:230px;'> <span class=small><b>tenpay</b> 에서 발급 받은 아이디를 입력해주세요</span></td>
	  </tr>
...
$Contents = $Contents."<form name='edit_form' action='tenpay.act.php' method='post' onsubmit='return validate(document.edit_form)' target='act'><input name='act' type='hidden' value='update'>";
...
$P->Navigation = "HOME > 상점관리 > 결제모듈(텐페이)";
$P->title = "결제모듈(텐페이)";
