...
for($i=0;$i < count($datas);$i++){
	...
	if(!$price && (substr_count($data,iconv("UTF-8","CP949","판매가")) || substr_count($data,"<span class=\"mk_price\">"))){
	...
	*/
	if(($prod_desc_start_line && !$prod_desc_end_line) && substr_count($data,"아이템쓰기")){
	...
	if(!$option2_start_line && substr_count($data,"<select name=spcode2")){
...
if(!$pname){
	preg_match_all("|_pd =_RP\(_S_TAG\(\"(.*)\"\)|U",$results,$pname_tmp, PREG_PATTERN_ORDER);
...
for($i=$prod_desc_start_line;$i < $prod_desc_end_line;$i++){
	...
	if(substr_count($datas[$i],"/design/watoo/dnp_img/pro/model.jpg")){
		$prod_desc_prod .= str_replace("/design/watoo/dnp_img/pro/model.jpg","http://www.iwatoo.com/design/watoo/dnp_img/pro/model.jpg",$datas[$i]);
	}else{
		$prod_desc_prod .= iconv("CP949","UTF-8",$datas[$i]);
	}
}
...
if($options_str){
		...
		preg_match_all("|<font id=\"option1_title\">(.*)</font>|U",$options_str,$option_name_tmp, PREG_PATTERN_ORDER);
		$option_name = $option_name_tmp[1][0];
		//$options_str = substr(<select  id="custom1",$options_str
		//preg_match_all("|<img .*src=\"(.*)\".*>|U",$prod_img_text,$img_out, PREG_PATTERN_ORDER);
		//echo $options1_str;

		preg_match_all("|option .*>(.*)<|U",$options_str,$options, PREG_PATTERN_ORDER);
		//print_r($options);
		
		$_option = $options[1];
		
		$option[0] = trim(str_replace(':',"",iconv("CP949","UTF-8",$option_name)));
...
if($options2_str){
		...
		preg_match_all("|<font id=\"option2_title\">(.*)</font>|U",$options2_str,$option2_name_tmp, PREG_PATTERN_ORDER);
		...
		//exit;
		$_option2 = $options2[1];
		$option2[0] =  trim(str_replace(':',"",iconv("CP949","UTF-8",$option2_name)));
		for($i=1, $j=1;$i < count($_option2);$i++){
			if(iconv("CP949","UTF-8",$_option2[$i]) != "옵션을 선택하세요" && iconv("CP949","UTF-8",$_option2[$i]) != "-----------------"){
				$option2[$j] =  str_replace("&nbsp;","",trim(iconv("CP949","UTF-8",$_option2[$i])));
				$j++;
			}
		}
		//print_r($option2);
