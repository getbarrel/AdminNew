...
///////////////////////////////////////////////////////////////////
//
// 수정 : IE 10 에서 2차카테고리가 선택이 안되는 문제 수정 : 이현우(2013-05-02)
//
///////////////////////////////////////////////////////////////////
...
$db = new Database;
//echo ("SELECT cid, cname FROM standard_category_info where depth ='".($depth+1)."' and cid LIKE '".substr($cid,0,($depth+1)*3)."%' order by vlevel1, vlevel2, vlevel3, vlevel4, vlevel5 <br><br>");
if($cid){
	...
	$db->query("SELECT cid, cname FROM standard_category_info where depth ='".($depth+1)."' and cid LIKE '".substr($cid,0,($depth+1)*3)."%' order by vlevel1, vlevel2, vlevel3, vlevel4, vlevel5 ");
}else{
	//$db->query("SELECT cid, cname FROM standard_category_info where depth ='".($depth+1)."' and cid = '' order by vlevel1, vlevel2, vlevel3, vlevel4, vlevel5 ");
	if($return_type != "json"){
      ...
      //}
		...
		if($childs_num<3) {
			...
			for($i=($childs_num+1);$i<4;$i++) {
				...
				//echo "<script>alert('".$childs_obj."');</script>";
				echo "<script type='text/javascript'>
					top.document.forms['$form'].elements['".$childs_obj."'].length = 1;
					top.document.forms['$form'].elements['".$childs_obj."'].setAttribute('validation','false');
				</script>";
