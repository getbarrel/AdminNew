<?php
	...
	if ($act == "excel_input"){
		...
		while (($objPHPExcel->getActiveSheet()->getCell('A' . $rownum)->getValue() != "") && ($i < 11000)) {
			...
			$brand_name = $objPHPExcel->getActiveSheet()->getCell('A' . $rownum)->getValue();				//브랜드명
			$brand_code = $objPHPExcel->getActiveSheet()->getCell('B' . $rownum)->getValue();				//브랜드코드
			$disp = $objPHPExcel->getActiveSheet()->getCell('C' . $rownum)->getValue();						//사용유무
			$apply_status = $objPHPExcel->getActiveSheet()->getCell('D' . $rownum)->getValue();				//신청상태
			$cid_array = $objPHPExcel->getActiveSheet()->getCell('E' . $rownum)->getValue();				//상품카테고리 배열
			$bd_ix = $objPHPExcel->getActiveSheet()->getCell('F' . $rownum)->getValue();					//브랜드분류
			$shotinfo = $objPHPExcel->getActiveSheet()->getCell('G' . $rownum)->getValue();				//간략설명
			$brand_img = $objPHPExcel->getActiveSheet()->getCell('H' . $rownum)->getValue();				//브랜드 로고파일명
			$brand_banner_img = $objPHPExcel->getActiveSheet()->getCell('I' . $rownum)->getValue();					//브랜드 상단이미지 파일명
			$brand_html = $objPHPExcel->getActiveSheet()->getCell('J' . $rownum)->getValue();					//html적용

			//상품명이 있는경우 db입력 진행.
			if(!empty($brand_name) && !empty($disp) && !empty($apply_status)){
				$brand_name = trim($brand_name);
				//$cid = trim(str_repeat('0',15-strlen($cid)).$cid);
				$brand_code = trim($brand_code);
				$disp = trim($disp);
				$apply_status = trim($apply_status);
				$cid_unit = explode("|",$cid_array);
				$bd_ix = trim($bd_ix);
				$shotinfo = trim($shotinfo);
				$brand_img = trim($brand_img);
				$brand_html = trim($brand_html);
		
				$sql = "insert into shop_brand set
							bd_ix = '".$bd_ix."',
				...
				*/
				...
				if ($brand_img != "") {
					Batch_Images_Processing($sourceDirectory.$brand_img, $b_ix,'logo');
				}

				if ($brand_banner_img != "") {
					Batch_Images_Processing($sourceDirectory.$brand_banner_img, $b_ix,'top');
				}
			}
			$rownum++;
		}
		...
		echo "<script language='javascript' src='../js/message.js.php'></script><script>alert('브랜드등록 완료되었습니다.');document.location.href='./brand_list.php'</script>";
