...
$pname = strip_tags(str_replace(array("&lt;","&quot;","&gt;"),array("<","\"",">"),$pname_tmp[1][0]));//상품명을 잘못 입력해서 태그가 들어가 있는 것 제거 kbk 13/10/15

preg_match_all("|<input type=\"hidden\" name=\"product_no\" value=\"(.*)\">|U",$results,$pcode, PREG_PATTERN_ORDER);
$pcode = $pcode[1][0];

preg_match_all("|<input type=\"hidden\" id=\"product_price\" name=\"product_price\" value=\"(.*)\">|U",$results,$price, PREG_PATTERN_ORDER);
$price = floor(str_replace(",","",$price[1][0]));

$sale_price = "";
$listprice = "";

preg_match_all("|<input type=\"hidden\" id=\"assign_prd_sale_price\" value='(.*)'>|U",$results,$sale_price, PREG_PATTERN_ORDER);
$sale_price = floor(str_replace(",","",$sale_price[1][0]));
if($price > $sale_price) {
	$listprice=$price;
	$price = $sale_price;
}
...
preg_match_all("|<input type='hidden' name='prd_item_img' value='(.*)'>|U",$results,$prod_img_text, PREG_PATTERN_ORDER);
$prod_img_src = $prod_img_text[1][0];
$prod_img_text = $prod_img_text[1][0];

$prod_detail_img_text ="";
preg_match_all("|<input type=\"hidden\" name=\"product_org_img\" value=\"(.*)\">|U",$results,$prod_detail_img_text, PREG_PATTERN_ORDER);
$prod_detail_img_text = $prod_detail_img_text[1][0];
...
for($i=0;$i < count($datas);$i++){
	...
	if(!$prod_desc_prod && substr_count($data,"<tr><td align=\"center\">")){
		if(substr_count($data,$prod_detail_img_text)==0 && substr(str_replace("<tr><td align=\"center\">","",$data),0,7)!="<A HREF") {
			$prod_desc_prod = str_replace(array("<tr><td align=\"center\">","</td></tr>"),array("",""),$datas[$i]);
		}
	...
	*/
	...
	if(!$option_start_line && substr_count($data,"id=\"option0\"")){
		//$option_start_line = $i-3;
		$option_start_line = $i-4;
	}
	...
	if(!$option2_start_line && substr_count($data,"id=\"option1\"")){
		$option2_start_line = $i-4;
	}
...
if($options_str){
		...
		//$option1_spos = strrpos($options_str,"id=\"option0\"");
		...
		preg_match_all("|<td width=\"60px\" align=\"left\">(.*)</td>|U",$options_str,$option_name_tmp, PREG_PATTERN_ORDER);
		...
		$option[0] = $option_name;
 		for($i=1, $j=1;$i < count($_option);$i++){
			if(iconv("CP949","UTF-8",$_option[$i]) != "옵션을 선택해 주세요" && iconv("CP949","UTF-8",$_option[$i]) != "-----------------"){
...
if($options_str2){
		...
		*/
		...
		$options_str2 = str_replace("\\","",$options_str2);
		//echo strrpos($options_str,"<select  id=\"custom1\"")."::::::::::".strpos($options_str,"</select>");
		$option2_spos = strrpos($options_str2,"<select");
		$option2_epos = strpos($options_str2,"</select>");
		//$options1_str = substr($options_str,$option1_spos,$option1_epos+1);
		$options2_str = substr($options_str2,$option2_spos,$option2_epos);

		preg_match_all("|<td width=\"60px\" align=\"left\">(.*)</td>|U",$options_str2,$option2_name_tmp, PREG_PATTERN_ORDER);
		$option2_name = $option2_name_tmp[1][0];
		//$options_str = substr(<select  id="custom1",$options_str
		//preg_match_all("|<img .*src=\"(.*)\".*>|U",$prod_img_text,$img_out, PREG_PATTERN_ORDER);
		//echo $options1_str;
		preg_match_all("|option .*>(.*)<|U",$options2_str,$options2, PREG_PATTERN_ORDER);
		//print_r($options);
		...
		$option2[0] = $option2_name;
 		for($i=1, $j=1;$i < count($_option2);$i++){
			if(iconv("CP949","UTF-8",$_option2[$i]) != "옵션을 선택해 주세요" && iconv("CP949","UTF-8",$_option2[$i]) != "-----------------"){
