...
function getNextCid($cid,$depth)
{
	...
	$sPosA = $depth*3;
	$sPos = $depth*3 + 1;
	$sql = "select max(substr(cid,$sPos,3))+1 as maxid from shop_goods_size_info where cid LIKE '".substr($cid,0,$sPos-1)."%'";
...
<Script Language="JavaScript">
	...
	if('<?=$depth?>' == '0'){
		...
		parent.document.getElementById('selected_category_1').innerHTML = "<?=getLaundryPathByAdmin($cid, $depth)?>";
		parent.document.getElementById('selected_category_3').innerHTML = "<?=getLaundryPathByAdmin($cid, $depth)?>";
	}
