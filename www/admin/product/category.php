...
$db->query("SELECT * FROM ".TBL_SHOP_CATEGORY_INFO." where depth in(0,1,2,3,4)  order by vlevel1, vlevel2, vlevel3, vlevel4, vlevel5");
...
$Contents .= "
				...
				<form name='thisCategoryform' method=\"post\" enctype='multipart/form-data' action='category.save.php' target='calcufrm' style='display:inline;'><!--target='calcufrm'-->
				...
				<col width='32%'>
				<tr bgcolor=#ffffff>
					<td class='input_box_title' nowrap oncontextmenu='init2();return false;'> 	<b>분류 등록 상품수</b></td>
					<td class='input_box_item' nowrap>
						<span id='product_cnt'>30 개</span>
					</td>
					<td class='input_box_title' nowrap oncontextmenu='init2();return false;'> 	<b>하위분류포함 상품수</b></td>
					<td class='input_box_item' nowrap>
						<span id='product_total_cnt'>0 개</span>
						<input type='button' value='상세보기' onclick=\"categoryProduct(this.form)\" />
					</td>
				</tr>
				<tr bgcolor=#ffffff>
				...
				<tr>
					<td class='input_box_title'> 	<b>분류상품정렬 <img src='".$required3_path."'></b></td>
					<td class='input_box_item' nowrap colspan='3'>
						<select name='category_sort' id='category_sort'>
							...
							<option value='viewOrder' selected>배럴추천순</option>
						</select>
				...
				<tr>
					<td class='input_box_title' nowrap oncontextmenu='init2();return false;'><b>카테고리 강조설정 <img src='".$required3_path."'></b></td>
					<td class='input_box_item' nowrap  colspan='3' style='padding-left:5px;'>
						<input type='radio' name='is_layout_emphasis' id='is_layout_emphasis_n' value='N' checked /><label for='is_layout_emphasis_n'> 미설정</label>&nbsp;
						<input type='radio' name='is_layout_emphasis' id='is_layout_emphasis_y' value='Y' ><label for='is_layout_emphasis_y'> 설정</label>&nbsp;
						<!--<span style='font:5pt;'> * 19금 설정시 생년월일을 통한 만 19세이상만 사용가능함</span>-->
					</td>
				</tr>
				</table><br>
				...
				<tr  height=450 style='".$display_yn."'>
...
$Contents .= "
						<tr>
							...
							<td class='input_box_item' ><input type='text' name='etc".($i+1)."_ename' id='etc".($i+1)."_ename' value=''>  </td>
							<td class='input_box_item' ><input type='text' name='etc".($i+1)."'  id='etc".($i+1)."' value=''>  </td>
							<td class='input_box_item' nowrap>
							...
							<td class='input_box_item'  nowrap>
								<input type='text' name='etc".($i+1)."_value' size=50 value=''>
							</td>
						</tr>";
						}
...
$Contents .= "
				...
				<form name='AddPersonForm' method=\"post\" enctype='multipart/form-data' action='category.save.php' target='calcufrm' style='display:inline;'><!--target='calcufrm'-->
				<input type='hidden' name='mode' value=''>
				<input type='hidden' name='cid' value=''>
...
$Contents .= "
				...
				<form name='design_subcategory' method=\"post\" enctype='multipart/form-data' action='category.save.php' target='calcufrm' onsubmit='return SaveAddField(this);' style='display:inline;'>
				<input type='hidden' name='mode' value='modify'>
				<input type='hidden' name='cid' value=''>
...
/**
 * 2017.07.24 sehyun PVF display:none처리 4.구축을 전제로 대부분 커스터마이징으로 진행하기 때문에 불필요
 * 주석 범위 : 1. 기본카테고리 디자인 설정
 *            2. 아이콘 사용
 *            3. 서브이미지 추가
 *            4. 리스트 정렬 table
 */
...
$Contents .= "  <tr>
				...
				<br>";
$Contents .= "
				<table border='0' cellpadding=0 cellspacing=0 width='100%' class='search_table_box' style='margin-bottom:10px; display:none;'>
				<col width='20%'>
				<col width='*'>
...
function categoryProduct(frm){
    var cid = frm.cid.value; 
    var depth = frm.this_depth.value;
    if(cid){
        PoPWindow('category_product.php?cid='+cid+'&depth='+depth,850,550,'category_product');
    }else{
        alert('카테고리를 선택해 주세요')
    }
}
...
$P->strContents = $Contents;
$P->Navigation = "상품관리 > 상품분류관리 > 상품분류설정";
$P->title = "상품분류설정";
$P->PrintLayOut();
}
/*
...
//	}
	...
	node$cid.action = \"setCategory('$cname','$cid',$depth, '$category_display_type','$category_use','$category_access','$category_code','$cname_on','$is_adult','$is_layout_apply', '$is_layout_emphasis','$category_sort')\";
...
///echo "".substr($mcid,0,($_GET["depth"]+1)*3)."==".substr($_GET["cid"],0,($_GET["depth"]+1)*3)."<br><br>";
	...
	$mstring .=  "	groupnode$mcid.tooltip = '$cname';
		...
		groupnode$mcid.action = \"setCategory('$cname','$mcid',$depth, '$category_display_type','$category_use','$category_access','$category_code','$cname_on','$is_adult','$is_layout_apply','$is_layout_emphasis','$category_sort')\";
