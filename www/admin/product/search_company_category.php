...
<col width='50%'>
			...
			<tr>
				<td>
					...
					$db->query("select * from shop_company_div where  depth ='1'");
					$group_array = $db->fetchall();
					for($i=0;$i<count($group_array); $i++){
						$Contents .= "<option value='".$group_array[$i][cd_ix]."'>".$group_array[$i][div_name]."</option>";
					}
	$Contents .= "
					</select>
				</td>
...
if($mode == 'search'){
	if($search_text != ""){
		if(strpos($search_text,',') == false){
			$where = " and bd1.div_name like '%".$search_text."%'";
		}else{
			$search_text_array = explode(",",$search_text);
			$where .=" and (";
			for($i=0;$i<count($search_text_array);$i++){
				if($i == count($search_text_array) -1){
					$where .= " bd1.div_name like '%".$search_text_array[$i]."%' ";
				}else{
					$where .= " bd1.div_name like '%".$search_text_array[$i]."%' or ";
				}
			}
			$where .= ")";
		}
	...
	$sql = "select
			*
			from
				shop_company_div as bd1
			where
				1
				$where
				order by bd1.cd_ix  ASC";
	$db->query($sql);
...
<table cellSpacing=0 cellPadding=0 width='100%' align=center border=0>
	<tr>
		<td align=center >
		<form name='search_frm' action='./search_company_category.php' method='get' onsubmit='return CheckFormValue(this)' enctype='multipart/form-data' style='display:inline;' target=''>
			...
			<td>
				...
				for($i=0;$i<count($data_array); $i++){
					if($data_array[$i][depth] == '2'){
						...
						$Contents .= "<option value='".$data_array[$i][cd_ix]."'>".$div1_name." > ".$data_array[$i][div_name]."</option>";
					}else{
						$Contents .= "<option value='".$data_array[$i][cd_ix]."'>".$data_array[$i][div_name]."</option>";
					}
				}
$Contents .= "
				</select>
	...
	<tr height=30 style='padding:0 0 0 5px;'>
		<td ><span class='small blu'> * 제조사 분류 추가시 해당 분류에 '더블클릭' 하시면 '추가리스트'에 추가됩니다.</td>
	</tr>
	<tr style='padding:0 0 0 5px;'>
		<td ><span class='small blu'> * 한번에 여러 제조사 분류를 검색하시려면 검색어위에 ','를 분여 구분해주시면 됩니다. Ex:)신발,의류,기타 </td>
	</tr>
	<tr height=30>
		<td class='p11 ls1' style='padding:0 0 0 5px;' nowrap><b> > 추가 리스트 </b></td>
		<td class='p11 ls1' style='padding:0 0 0 5px;' nowrap> </td>
...
<!--
	...
	$(document).ready(function(){	//부서 선택
		$('#group_list').dblclick(function(){
			var cd_ix = $('#group_list').find('option:selected').val();
...
function add_department(){
	...
	$('#add_list option').each(function(){
		...
		var add_html = \"<tr style='height:26px;' id='cd_row_\"+dp_ix+\"'><td><input type=hidden name=cd_ix id='cd_ix_\"+dp_ix+\"' value='\"+dp_ix+\"'>\"+dp_text+\"</td><td><a href='javascript:void(0)' onClick='cd_del(\"+dp_ix+\")'><img src='../images/".$_SESSION["admininfo"]["language"]."/btc_del.gif' border=0></a></td></tr>\";
