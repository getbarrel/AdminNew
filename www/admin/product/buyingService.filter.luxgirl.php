...
$pcode = "";
$price = "";
//preg_match_all("|var _HCmz={OZ:\".*\",PC:\"(.*)\",PN:\"(.*)\",PS:\"(.*)\",PT:\".*\"};|U",$results,$pname_tmp, PREG_PATTERN_ORDER);
preg_match_all("|var _HCmz={PC:\"(.*)\",PN:\"(.*)\",PS:\"(.*)\",PT:\".*\"};|U",$results,$pname_tmp, PREG_PATTERN_ORDER);
$pname = iconv("CP949","UTF-8",$pname_tmp[2][0]);
$pcode = $pname_tmp[1][0];
$price = $pname_tmp[3][0];
//print_r($pname_tmp) ;
//exit;
...
for($i=0;$i < count($datas);$i++){
	...
	if(!$prod_img_src && substr_count($data,"class=\"thumb\"")){
		//$image_path = $datas[$i];
	...
	if(!$prod_desc_prod && substr_count($data,"---------- 상세설명끝")){		
	...
	*/
	...
	if(!$option_start_line && substr_count($data,"<select name=\"optionlist[]")){
		$option_start_line = $i;
	}
	...
	if($option_end_line &&!$option2_start_line && substr_count($data,"<select name=\"optionlist[]")){
...
if($options_str){
		...
		preg_match_all("|label=\"(.*)\"|U",$options_str,$option_name_tmp, PREG_PATTERN_ORDER);
		$option_name = iconv("CP949","UTF-8",$option_name_tmp[1][0]);
...
if($options2_str){
		...
		preg_match_all("|label=\"(.*)\"|U",$options2_str,$option_name_tmp, PREG_PATTERN_ORDER);
		$option_name = iconv("CP949","UTF-8",$option_name_tmp[1][0]);

		preg_match_all("|option .*>(.*)<|U",$options2_str,$options, PREG_PATTERN_ORDER);
		$_option2 = $options[1];
		
		$option2[0] = $option_name;
