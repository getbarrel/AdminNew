...
$path = $_SERVER["DOCUMENT_ROOT"] . $admin_config[mall_data_root] . "/BrandBatchUploadImages/";
if(!is_dir($path)){
	mkdir($path, 0777);
}

//auth(8);
$download_excel_file = "../product/brand_batch_sample.xls";
...
$Contents .= "
			...
			<tr>
				...
				$Contents .= "
						...
						5. 브랜드 분류코드 <img src='../v3/images/".$_SESSION["admininfo"]["language"]."/btn_search.gif' align=absmiddle onclick=\"$('#div_list').toggle();\"  style='cursor:pointer;'> <br><br>
						   -브랜드 코드를 찾아 정확히 넣어주세요.<br><br>
						   <div id='div_list' style='display:none;'>
								<table style='width:300px;'>
									<tr><td>브랜드 분류명</td><td>분류코드</td></tr><br>";
								$db->query("select * from shop_brand_div where depth='1' order by parent_bd_ix , bd_ix , depth ASC");
								for($i=0;$i<$db->total;$i++){
									$db->fetch($i);
									$Contents .= "<tr><td> ".$db->dt[div_name]."</td><td>".$db->dt[bd_ix]."</td></tr>";
									$sql = "select * from shop_brand_div where parent_bd_ix = '".$db->dt[bd_ix]."'";
									$db2->query($sql);
									$parent_array = $db2->fetchall();
									if(count($parent_array) > 0){
										for($j = 0;$j<count($parent_array);$j++){
											$Contents .= "<tr><td>".str_repeat('&nbsp;&nbsp;',($db->dt[depth]+1))." ".$parent_array[$j][div_name]."</td><td>".$parent_array[$j][bd_ix]."</td></tr>";
										}
									}
								}
				$Contents .= "
						...
						6. 브랜드 로고파일명  <br><br>
						   - 이미지는 FTP에서 ‘ /www".$_SESSION["admininfo"]["mall_data_root"]."/BatchUploadImages ' 경로에 업로드 해주시면 됩니다.<br><br>
						7. 브랜드 상단이미지 파일명  <br><br>
						   - 이미지는 FTP에서 ‘ /www".$_SESSION["admininfo"]["mall_data_root"]."/BatchUploadImages ' 경로에 업로드 해주시면 됩니다.<br><br>
						8. HTML 적용  <br><br>
						   - HTML 적용 edit 에 작성하실 내용을 입력하시면 됩니다..<br><br>
				</table>
				</td>
			</tr>
			</table>";
...
$P->Navigation = "상품관리 > 브랜드 대량 등록/수정";
$P->title = "브랜드 대량 등록/수정";
