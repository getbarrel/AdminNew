...
$innerview = "
			...
			<table cellpadding=2 cellspacing=0 bgcolor=gray border=0 width=100% class='list_table_box'>
				...
				<tr bgcolor='#cccccc' align=center>
					...
					<td class=m_td>가격비교(네이버)</td>
					<td class=m_td>가격비교(다음)</td>
					<td class=e_td>관리</td>
				</tr>";


if($db->total == 0){
	$innerview .= "<tr bgcolor=#ffffff height=50><td colspan=12 align=center> <!--등록된 상품이 없습니다.--> ".getTransDiscription(md5($_SERVER["PHP_SELF"]),'C')."</td></tr>";
...
$innerview .= "
					...
					<td class='list_box_td ' nowrap>
						".($db->dt['disp_naver'] == 0 ? '미노출'  : '노출')."
					</td>
					<td class='list_box_td ' nowrap>
						".($db->dt['disp_daum'] == 0 ? '미노출'  : '노출')."
					</td>
					<td class='list_box_td' nowrap>
...
<div style='padding:4px 0 4px 0'><img src='../images/dot_org.gif'> <b>상품 노출상태 변경</b> <span class=small style='color:gray'><!--변경하시고자 하는 판매/진열 상태를 선택후 저장 버튼을 클릭해주세요--> ".getTransDiscription(md5($_SERVER["PHP_SELF"]),'B')." </span></div>
	<table width='100%' border=0 cellpadding=0 cellspacing=0 class='input_table_box'>
	<col width=160>
	<col width='*'>
	<tr height=30>
		<td class='input_box_title'> <b>네이버 노출 여부</b></td>
		<td class='input_box_item'>
			<input type='checkbox' name='disp_naver' value='1' validation='true' /> (체크시 노출함으로 선택됩니다.)
		</td>
	</tr>
	<tr height=30>
		<td class='input_box_title'> <b>다음 노출 여부 </b></td>
		<td class='input_box_item'>
			<input type='checkbox' name='disp_daum' value='1' /> (체크시 노출함으로 선택됩니다.)
		</td>
	</tr>
	</table>";
...
<div style='padding:4px 0px 4px 0px'><img src='../images/dot_org.gif'> <b>상품 카테고리 변경</b> <span class=small style='color:gray'><!--변경하시고자 하는 판매/진열 상태를 선택 후 저장 버튼을 클릭해 주세요.--> ".getTransDiscription(md5($_SERVER["PHP_SELF"]),'E')."</span></div>
	...
	<tr height=30>
		<td class='input_box_title'> <b>네이버 노출 여부</b></td>
		<td class='input_box_item'>
			<input type='checkbox' name='naver_disp' value='1' /> (체크시 노출함으로 선택됩니다.)
		</td>
	</tr>
	<tr height=30>
		<td class='input_box_title'> <b>다음 노출 여부 </b></td>
		<td class='input_box_item'>
			<input type='checkbox' name='daum_disp' value='1' /> (체크시 노출함으로 선택됩니다.)
		</td>
	</tr>
	</table>";
if(checkMenuAuth(md5("/admin/product/goods_input.php"),"U") && checkMenuAuth(md5($_SERVER["PHP_SELF"]),"U")){
...
<div style='padding:4px 0px 4px 0px'><img src='../images/dot_org.gif'> <b>마일리지 정보변경</b> <span class=small style='color:gray'><!--변경하시고자 하는 적립금정보를 선택 후 저장 버튼을 클릭해 주세요.--> ".getTransDiscription(md5($_SERVER["PHP_SELF"]),'I')."</span></div>
	<table width='100%' border=0 cellpadding=0 cellspacing=0 class='input_table_box'>
	<col width=160>
	<col width='*'>
	<tr height=30>
	...
	<tr height=30>
		<td class='input_box_title'> <b>개별 마일리지 사용유무 </b></td>
		<td class='input_box_item'>
	...
	<tr height=30>
		<td class='input_box_title'> <b>개별 마일리지 </b></td>
		<td class='input_box_item'>
...
$select = "
	...
	<input type='radio' name='update_kind' id='update_kind_display' value='shopping_all' ".CompareReturnValue("display",$update_kind,"checked")." onclick=\"ChangeUpdateForm('batch_update_display');\"><label for='update_kind_display'>노출일괄 수정</label>
	<!--<input type='radio' name='update_kind' id='update_kind_category' value='shopping_each' ".CompareReturnValue("category",$update_kind,"checked")." onclick=\"ChangeUpdateForm('batch_update_category');\"><label for='update_kind_category'>노출개별 수정</label>-->
	";
