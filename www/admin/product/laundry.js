...
function thisCategorySave(frm,vMode)
{
	if (frm.cid.value.length < 1){
		alert('수정 하시고자 하는 세탁주의관리를 선택해 주세요');
		return false;
	}
	if(CheckFormValue(frm)){
		frm.mode.value = vMode;
...
function SubCategorySave(frm,vMode)
{
	if (frm.title.value.length < 1){
		alert('국문 분류명을 입력하여 주세요.');
		return false;
	}

	if (frm.title_en.value.length < 1){
		alert('영문 분류명을 입력하여 주세요.');
		return false;
	}
...
function setCategory(cname, cid, depth, laundry_use, laundry_use_en, cname_en, contents, contents_en)
{
	
	cname = cname.replace("&quot;","\"");
	cname = cname.replace("&#39;","'");

	cname_en = cname_en.replace("&quot;","\"");
	cname_en = cname_en.replace("&#39;","'");
	...
	contents_en = contents_en.replace("&quot;","\"");
	contents_en = contents_en.replace("&#39;","'");
	contents_en = contents_en.replace("<br>","\r\n");

	document.thisCategoryform.title.value = cname;
	document.thisCategoryform.title_en.value = cname_en;
	document.thisCategoryform.contents.value = contents;
	document.thisCategoryform.contents_en.value = contents_en;

	document.thisCategoryform.cid.value = cid;
	document.thisCategoryform.this_depth.value = depth;

	if(laundry_use == "1"){	//사용
		$('#laundry_use_id').attr('checked',true);
	}else if(laundry_use == "0"){	//미사용
		$('#laundry_use_id_0').attr('checked',true);
	}else{
		$('#laundry_use_id_0').attr('checked',true);
	}

	if(laundry_use_en == "1"){	//사용
		$('#laundry_use_en_id').attr('checked',true);
	}else if(laundry_use_en == "0"){	//미사용
		$('#laundry_use_en_id_0').attr('checked',true);
	}else{
		$('#laundry_use_en_id_0').attr('checked',true);
	}

	document.laundry_order.this_depth.value = depth;
	document.laundry_order.cid.value = cid;

	document.subCategoryform.cid.value = cid;
	document.subCategoryform.sub_depth.value = eval(depth+1);

	document.getElementById("calcufrm").src='calcurate_laundry.php?cid='+cid+'&depth='+eval(depth+1); //eval(depth+1) 이값은 depth 가 0 이 없어서 ...
}
