...
if($cid){
	$sql = 	"SELECT cv.*
			FROM shop_car_vechiletype cv
			where depth = '$depth' and parent_vt_ix = '$cid'  ";
	//echo $sql;
	$db->query($sql);
}else{
...
if ($db->total){
      ...
      //}
        ...
        for($i=0; $i < $db->total; $i++){
                $db->fetch($i);
				echo "<script type='text/javascript'>
					parent.document.forms['$form'].elements['".$target."'].options[".($i+1)."].text = '".$db->dt[vechiletype_name]."';
					parent.document.forms['$form'].elements['".$target."'].options[".($i+1)."].value = '".$db->dt[vt_ix]."';
				</script>";
        }
		exit;
