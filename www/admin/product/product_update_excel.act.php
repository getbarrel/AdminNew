...
if($act == "new_excel_input"){	//대량상품등록 엑셀정보 저장
	...
	*/
	unset($goods_basic_sample);
	$col = 'A';
	for($i=1;$i<=100;$i++){
		if($objPHPExcel->getActiveSheet()->getCell($col."1")->getValue() != ""){
		$goods_basic_sample[$i] = array(
				"code"=>$objPHPExcel->getActiveSheet()->getCell($col."2")->getValue(),
				"text"=>$objPHPExcel->getActiveSheet()->getCell($col."1")->getValue(),
				"desc"=>$objPHPExcel->getActiveSheet()->getCell($col."3")->getValue()
				);
		}
		$col++;
	}
	foreach($goods_basic_sample as $key => $value){
		$upload_excel_data[session_id()][0][$value[code]] = $value[text];//$objPHPExcel->getActiveSheet()->getCell($col . $rownum)->getValue();
		$col++;
	}
	//print_r($goods_basic_sample);exit;
	//print_r($_POST);
	//print_r($_FILES);
	//exit;
	$z=0;
	...
	for($rownum=4;$rownum<=30000;$rownum++){
	...
	if($page_type == 'input'){
		echo "<script language='javascript' src='../js/message.js.php'></script><script>top.location.href='./product_input_excel2.php?up_mode=new_upload'</script>";
	}else{
		echo "<script language='javascript' src='../js/message.js.php'></script><script>top.location.href='./product_update_excel2.php?up_mode=new_upload'</script>";
	}
	exit;
}
...
*/
	...
	if($_SERVER["REMOTE_ADDR"] == '10.62.1.181'){
		...
		/*if($check_infos[update_check_mainimg] == '1'){	//이미지
			if($mainimg !='#'){
				if ($mainimg != "") {
					...
					while ($itemName = readdir($basicHandle)) {
						if($itemName == "." || $itemName == ".."){
						}else{
							if (file_exists($basicDir."/".$itemName)) {
								unlink($basicDir."/".$itemName);
							}
						}
					}

					closedir($basicHandle); // 디렉토리 close

					$addHandle  = opendir($addDir); // 디렉토리 open
					while ($itemName = readdir($addHandle)) {
						if($itemName == "." || $itemName == ".."){
						}else{
							if (file_exists($addDir."/".$itemName)) {
								unlink($addDir."/".$itemName);
							}
						}
					}
...
//print_r($value);
		...
		foreach($value as $_key => $_value){
			...
			if($page_type == 'input'){// 상품 입력일때만 필수값 체크
				//필수값 체크!!!
		...
		$pcode = trim($pcode);	//상품코드앞에 스페이스가 있기에 이부분은 절대 주석처리하면 안됨 2014-10-07 이학봉
		$sql = "select stock_use_yn from shop_product where id = '".$id."'";
		$db->query($sql);
		$db->fetch();
		...
		if(($_SESSION['admininfo']['charger_id'] == 'forbiz' || $_SERVER[REMOTE_ADDR] == '221.151.188.11') && false){	//외부 아이피 : 221.151.188.11 내부 아이피 : 192.168.0.1
			...
			$sql = "select id from shop_product where pname = '".$pname."' and  admin = '".$company_id."' and pcode = '".trim($pcode)."'";
			$db->query($sql);
			$db->fetch();
		...
		//1. 상품타입
		$product_type_array = array ('0','5','12','99');
		...
		// 처음 샘플에 5번으로 되어 있는 경우 때문에 5-->12 공동구매 상품
		if($product_type == '5'){
			$product_type = '12';
		}

		//2. 면세여부
		if($page_type == 'input' || $check_infos[update_check_surtax_yorn] == '1'){// 상품 입력일때만 필수값 체크
		...
		if($page_type == 'input' || $check_infos[update_check_state] == '1'){// 상품 입력일때만 필수값 체크
			//3. 판매상태
		...
		if($page_type == 'input' || $check_infos[update_check_disp] == '1'){// 상품 입력일때만 필수값 체크
			//5. 노출여부
		...
		if($page_type == 'input' || $check_infos[update_check_is_sell_date] == '1'){// 상품 입력일때만 필수값 체크
			//6. 판매기간사용여부
		...
		if($page_type == 'input' || $check_infos[update_check_delivery_coupon_yn] == '1'){// 상품 입력일때만 필수값 체크
			//7. 배송쿠폰사용여부
		...
		if($page_type == 'input' || $check_infos[update_check_coupon_use_yn] == '1'){// 상품 입력일때만 필수값 체크
			//8. 상품쿠폰사용여부
		...
		if($page_type == 'input' || $check_infos[update_check_stock_use_yn] == '1'){// 상품 입력일때만 필수값 체크
			//10. 재고관리 사용여부
		...
		if($page_type == 'input' || $check_infos[update_check_is_adult] == '1'){// 상품 입력일때만 필수값 체크
			//11. 19금 사용여부
		...
		if($page_type == 'input' || $check_infos[update_check_delivery_type] == '1'){// 상품 입력일때만 필수값 체크
			//13. 배송타입선택
	...
	if($admininfo[admin_level] == '9'){	//본사등록시에만 체크
		if($page_type == 'input' || $check_infos[update_check_allow_order_type] == '1'){// 상품 입력일때만 필수값 체크 update_check_allow_order_type
			//12. 한정판매 사용여부
		...
		if($page_type == 'input' || $check_infos[update_check_account_type] == '1'){// 상품 입력일때만 필수값 체크
			//14. 정산방식
		...
		if($page_type == 'input' || $check_infos[update_check_one_commission] == '1'){// 상품 입력일때만 필수값 체크
			//14. 개별수수료 사용여부
		...
		*/
		...
		if($stock_use_yn =="Y" && $stock_options_code !=""){	//WMS사용할경우 가격재고관리 옵션
			...
			for($i=0;$i<count($stock_options_code_array);$i++){
				...
				$stock_options[0][details][$i][listprice] = $db->dt[sellprice];
		...
		if($stock_use_yn == "N" || $stock_use_yn == "Q"){	//WMS사용안할경우
			if($option_div != ""){
				$option_div_array = explode("|",trim($option_div));
				$stock_options_coprice_array = explode("|",trim($stock_options_coprice));
				$stock_options_wholesale_listprice_array = explode("|",trim($stock_options_wholesale_listprice));
				$stock_options_wholesale_price_array = explode("|",trim($stock_options_wholesale_price));
				$stock_options_listprice_array = explode("|",trim($stock_options_listprice));
				$stock_options_sellprice_array = explode("|",trim($stock_options_sellprice));
				$stock_options_stock_array = explode("|",trim($stock_options_stock));
				$stock_options_safestock_array = explode("|",trim($stock_options_safestock));
				$stock_options_code_array = explode("|",trim($stock_options_code));
				$stock_options_barcode_array = explode("|",trim($stock_options_barcode));
				$stock_options_soldout_array = explode("|",trim($stock_options_soldout));
				...
				for($i=0;$i<count($option_div_array);$i++){
					$stock_options[0][details][$i][option_div] = $option_div_array[$i];
					$stock_options[0][details][$i][coprice] = $stock_options_coprice_array[$i];
					$stock_options[0][details][$i][wholesale_listprice] = $stock_options_wholesale_listprice_array[$i];
					$stock_options[0][details][$i][wholesale_price] = $stock_options_wholesale_price_array[$i];
					$stock_options[0][details][$i][listprice] = $stock_options_listprice_array[$i];
					$stock_options[0][details][$i][sellprice] = $stock_options_sellprice_array[$i];
					$stock_options[0][details][$i][sell_ing_cnt] = '';
					$stock_options[0][details][$i][stock] = $stock_options_stock_array[$i];
					$stock_options[0][details][$i][safestock] = $stock_options_safestock_array[$i];
					$stock_options[0][details][$i][code] = $stock_options_code_array[$i];			//WMS 사용안할경우 동일값으로 넣음 2014-08-11 이학봉
					$stock_options[0][details][$i][gid] = $stock_options_code_array[$i];			//WMS 사용안할경우 엑셀에 품목코드가 들어감
					$stock_options[0][details][$i][barcode] = $stock_options_barcode_array[$i];
					$stock_options[0][details][$i][soldout] = $stock_options_soldout_array[$i];		//품절여부

					$option_stock += $stock_options_stock_array[$i];	//총 합산재고수량
					$option_safestock += $stock_options_safestock_array[$i];	//총 합산 안전재고 수량
		...
		if($stock_use_yn=="Y" && $pcode !=""){
			...
			if($db->total){
				...
				//$c_ix = $db->dt[c_ix];			//제조사키값
				$company = $db->dt[company];		//제조사명

				$b_ix = $db->dt[b_ix];				//브랜드키값
				$brand_name = $db->dt[brand];		//브랜드명

				$trade_admin = $db->dt[ci_ix];		//주매입처
				if($db->dt[surtax_div] == '1'){
					$surtax_yorn = 'Y';				//과세(부과세포함)
				}else{
					$surtax_yorn = 'N';				//면세(면세율적용)
				}
		...
		//예외처리
		$english_basicinfo = str_replace("''","",$english_basicinfo);
		$english_basicinfo = str_replace("'","&#39;",$english_basicinfo);
		$english_basicinfo = str_replace('"',"&quot;",$english_basicinfo);
		$english_basicinfo = strip_tags($english_basicinfo,"<img><table><th><col><colgroup><tbody><tr><td><div><a><ul><li><dl><dt><p><h><font><strong><br><span>");

		$english_m_basicinfo = str_replace("''","",$english_m_basicinfo);
		$english_m_basicinfo = str_replace("'","&#39;",$english_m_basicinfo);
		$english_m_basicinfo = str_replace('"',"&quot;",$english_m_basicinfo);
		$english_m_basicinfo = strip_tags($english_m_basicinfo,"<img><table><th><col><colgroup><tbody><tr><td><div><a><ul><li><dl><dt><p><h><font><strong><br><span>");
		*/
		...
		if($insert_yn){
			...
			if($upload_excel_data[session_id()][$key][status]!='C'){
				...
				/*
				//이미지 파일 삭제 초기화
				$zip_goods_img_file_array = explode(".",$goods_img_file);
				$zip_directory = $zip_goods_img_file_array[0]."/";
				$zipfileDirectory = $_SERVER["DOCUMENT_ROOT"] . $admin_config[mall_data_root] . "/BatchUploadImages/".$zip_directory;
				$ziphandle = opendir($zipfileDirectory); // 절대경로
				while ($zipfolder = readdir($ziphandle)) {
					//@unlink($zipfileDirectory.$zipfolder);
				}
				closedir($ziphandle);
				//이미지 파일 삭제 초기화 끝
...
/*
				...
				//추가 이미지);
				if($check_infos[update_check_addimg] == '1'){	//추가 이미지
					if($addimg !='#'){
						if ($addimg != "") {

							//초기화
							$sql = "DELETE FROM shop_addimage WHERE pid = ".$id;
							$db->query($sql);
							$db->fetch();

							$addimg_array = explode("|", trim($addimg));

								//추가이미지 일괄수정시 해당상품번호의 추가이미지를 싹지우고 다시 올린다. (덮어쓰기 아님)
								$adduploaddir = UploadDirText($_SERVER["DOCUMENT_ROOT"].$admin_config[mall_data_root]."/images/addimg", $pid, 'Y');
								$addpath = $_SERVER["DOCUMENT_ROOT"].$admin_config[mall_data_root]."/images/addimg".$adduploaddir."/";
								$handle = opendir($addpath); // 절대경로
				...
				//리스트 이미지
				if($check_infos[update_check_listimg] == '1'){	//리스트 이미지
					if($listimg !='#'){
						if ($listimg != "") {
				...
				//패턴이미지
				if($check_infos[update_check_pattern_image] == '1'){	//패턴이미지
					if($pattern_image !='#'){
						if ($pattern_image != "") {
				...
				if($page_type == 'input'){
					...
					if($m_basicinfo !='#'){
						$m_basicinfo = basicinfoCopy($m_basicinfo, $pid);
					...
					if($check_infos[update_check_english_basicinfo] == '1'){	//상세이미지 글로벌

						//if($basicinfo !='#'){
							$english_basicinfo = basicinfoCopy($english_basicinfo, $pid);
							$db->query("update shop_product_global set basicinfo='".$english_basicinfo."' where id='".$pid."' ");
						//}
					}

					if($check_infos[update_check_english_m_basicinfo] == '1'){	//모바일 상세이미지 글로벌
						//if($m_basicinfo !='#'){
							$english_m_basicinfo = basicinfoCopy($english_m_basicinfo, $pid);
							$db->query("update shop_product_global set m_basicinfo='".$english_m_basicinfo."' where id='".$pid."' ");
						//}
					}

					if($check_infos[update_check_english_coprice] == '1'){
						$english_coprice = getExchangeNationPrice($coprice);
						$db->query("update shop_product_global set coprice='".$english_coprice."' where id='".$pid."' ");
					}

					if($check_infos[update_check_english_listprice] == '1'){
						$english_listprice = getExchangeNationPrice($listprice);
						$db->query("update shop_product_global set listprice='".$english_listprice."' where id='".$pid."' ");
					}

					if($check_infos[update_check_english_sellprice] == '1'){
						$english_sellprice = getExchangeNationPrice($sellprice);
						$db->query("update shop_product_global set sellprice='".$english_sellprice."' where id='".$pid."' ");
					}
...
if($act == "bad_goods_info_excel"){
	...
	if(count($bad_info_excel_data) > 0){
		...
		<!--
			...
			location.href='./product_input_excel2.php?up_mode=new_upload';
		...
		<!--
			alert('등록 실패한 상품이 없습니다.');
			location.href='./product_update_excel2.php?up_mode=new_upload';
...
function Batch_Images_ProcessingNew($process_type,$xlImageFileName, $bpid,$goods_img_file='', $num, $time){
	...
	function Batch_Images_Processing($process_type,$xlImageFileName, $bpid,$goods_img_file=''){
		global $admin_config;
