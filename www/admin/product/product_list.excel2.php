...
$mstring .= "<tr height=40  align=center >
						<td style='background-color:black;color:#fff;font-weight:bold;width:90px;'>상품코드 / 상품이름</td>
						<td style='background-color:black;color:#fff;font-weight:bold;' colspan=3 >사진</td>
						<td style='background-color:black;color:#fff;font-weight:bold;'>옵션</td>
						<td style='background-color:black;color:#fff;font-weight:bold;'>출고가격</td>
						<td style='background-color:black;color:#fff;font-weight:bold;'>재고수량</td>
						<td style='background-color:black;color:#fff;font-weight:bold;'>주문수량</td>
						<td style='background-color:black;color:#fff;font-weight:bold;'>총가격</td>
					</tr>
					";
...
for($i=0;$i<count($goods_infos);$i++){
	...
	$targetDirectory = $basedir . UploadDirText($basedir, $goods_infos[$i][id], 'Y') . '/';

	$sql = "select pod.* from shop_product_options po, shop_product_options_detail pod where po.opn_ix = pod.opn_ix and pod.pid = '".$goods_infos[$i][id]."' and option_kind = 'b' order by pod.id asc ";
			//echo $sql;
	$db->query($sql);
	$goods_options_details = $db->fetchall("object");

	$options_count = count($goods_options_details);
	for($j=0;$j < $options_count;$j++){
		if($j == 0){
	$mstring .= "<tr height=40>
							<td rowspan='".($options_count < 4 ? "4":$options_count)."'>".$goods_infos[$i][pcode]." > / ".$goods_infos[$i][pname]."</td>
							<td rowspan='".($options_count < 4 ? "4":$options_count)."' colspan=3 >
							<table >
								<tr>
									<td colspan=3></td>
								</tr>
								<tr >
									<td></td>
									<td><img src='http://".$HTTP_HOST.PrintImage($admin_config[mall_data_root]."/images/product", $goods_infos[$i][id], "s")."'></td>
									<td></td>
								</tr>
							</table>
							</td>
							<td >
								".$goods_options_details[$j]["option_div"]."
							</td>
							<td >".$goods_options_details[$j]["option_price"]."</td>
							<td >".$goods_options_details[$j]["option_stock"]."</td>
							<td >개</td>
							<td >￦ -</td>
						</tr>
						";
...
for($i=0;$i<$db->total;$i++){
	...
	xlsWriteLabel(($i+1), 14, "<img src='http://".$HTTP_HOST.$admin_config[mall_data_root]."/images/product/b_".$db->dt[id].".gif'>");
