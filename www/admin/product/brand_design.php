...
include("../display/brand_display.lib.php");
	...
	<td align='left'>
		...
		<tr>
			...
			<td align='right'>
				...
				<a href='#;' onclick='location.href=\"../display/display_div.php?display_div=".$display_div."\"'><img src='../images/".$admininfo["language"]."/btn_disp_div.gif' align=absmiddle ></a-->
				<a href='../display/main_goods_category.php'><img src='../images/".$admininfo["language"]."/btn_disp_div.gif' align=absmiddle></a>
			</td>
		</tr>
		</table>
		</div>
...
<tr>
  ...
  <tr>
    <td>
        ...
        <form name='main_frm' method='post' onsubmit='return CheckFormValue(this)'  action='../display/brand_display.act.php' style='display:inline;' enctype='multipart/form-data' target='iframe_act'><!-- onSubmit=\"return SubmitX(this)\"-->
...
$Contents .= "
						...
						<tr>
						  ...
						  <div id='goods_manual_area_".($i+1)."'>
							  <a href=\"javascript:\" onclick=\"ms_brandSearch.show_productSearchBox(event,".($i+1).",'productList_".($i+1)."');\"><img src='../images/".$admininfo["language"]."/btn_goods_search_add.gif' border=0 align=absmiddle></a><br>
...
switch($db_ix) {
	case ("H") : 
		$P->Navigation = "상품관리 > 브랜드 전시관리 > 브랜드전시 등록/수정";
		$P->title = "브랜드전시 등록/수정";
	break;
	case ("R") : 
		$P->Navigation = "상품관리 > 브랜드 전시관리 > 추천도매 브랜드관리";
		$P->title = "추천도매 브랜드관리";
	break;
	default : 
		$P->Navigation = "상품관리 > 브랜드 전시관리 > 브랜드전시 등록/수정";
...
function relationEventGroupProductList($group_code, $disp_type="",$db_ix="M"){
	...
	$sql = "SELECT count(*) as total 
					...
					where mb.b_ix IS NOT NULL and br.db_ix='".$db_ix."' 
					 ";
	...
	if ($db->total == 0){
		if($disp_type == "clipart"){
			$mString = '<ul id="productList_'.$group_code.'" name="productList" class="brandList"></ul>';
		}
	}else{
		$i=0;
		if($disp_type == "clipart"){
			...
			for($i=0;$i<$db->total;$i++){
				...
				$mString .= 'ms_brandSearch._setBrand("productList_'.$group_code.'", "M", "'.$db->dt['b_ix'].'", "'.$imgPath.'", "'.addslashes(addslashes(trim($db->dt['brand_name']))).'", "'.addslashes(addslashes(trim($db->dt['brand_name']))).'", "'.$db->dt['brand_name'].'");'."\n";
			}
			$mString .= '</script>'."\n";
		}
	}
