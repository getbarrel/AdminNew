...
//	echo $data->sheets[0]['numRows'];
	...
	for ($i = 2; $i <= $data->sheets[0]['numRows']; $i++) {		
		...
		//exit;
			...
			$brand = iconv('CP949','UTF-8',$data->sheets[0]['cells'][$i][5+$shift_num]);
			//$pimage = iconv('CP949','UTF-8',$data->sheets[0]['cells'][$i][6+$shift_num]);
			$company = iconv('CP949','UTF-8',$data->sheets[0]['cells'][$i][6+$shift_num]);
			$make_country = iconv('CP949','UTF-8',$data->sheets[0]['cells'][$i][7+$shift_num]);
			$shotinfo = iconv('CP949','UTF-8',$data->sheets[0]['cells'][$i][8+$shift_num]);
			$search_keyword = iconv('CP949','UTF-8',$data->sheets[0]['cells'][$i][9+$shift_num]);
			$category = iconv('CP949','UTF-8',$data->sheets[0]['cells'][$i][10+$shift_num]);
			$basicinfo = iconv('CP949','UTF-8',$data->sheets[0]['cells'][$i][11+$shift_num]);			
			$delivery_company = iconv('CP949','UTF-8',$data->sheets[0]['cells'][$i][12+$shift_num]);			
			$delivery_price1 = iconv('CP949','UTF-8',$data->sheets[0]['cells'][$i][13+$shift_num]);			
			$delivery_price2 = iconv('CP949','UTF-8',$data->sheets[0]['cells'][$i][14+$shift_num]);			
			$delivery_product_policy = iconv('CP949','UTF-8',$data->sheets[0]['cells'][$i][15+$shift_num]);			
			$option_name1 = iconv('CP949','UTF-8',$data->sheets[0]['cells'][$i][16+$shift_num]);
			$option_item1 = iconv('CP949','UTF-8',$data->sheets[0]['cells'][$i][17+$shift_num]);
			$option_name2 = iconv('CP949','UTF-8',$data->sheets[0]['cells'][$i][18+$shift_num]);
			$option_item2 = iconv('CP949','UTF-8',$data->sheets[0]['cells'][$i][19+$shift_num]);
			$option_name3 = iconv('CP949','UTF-8',$data->sheets[0]['cells'][$i][20+$shift_num]);
			$option_item3 = iconv('CP949','UTF-8',$data->sheets[0]['cells'][$i][21+$shift_num]);
			$coprice = $data->sheets[0]['cells'][$i][22+$shift_num];
			$listprice = $data->sheets[0]['cells'][$i][23+$shift_num];			
			$sellprice = $data->sheets[0]['cells'][$i][24+$shift_num];		
			
			$stock = $data->sheets[0]['cells'][$i][25+$shift_num];
			$safestock = $data->sheets[0]['cells'][$i][26+$shift_num];			
			$surtax_yorn = iconv('CP949','UTF-8',$data->sheets[0]['cells'][$i][27+$shift_num]);	
			$reserve = $data->sheets[0]['cells'][$i][28+$shift_num]*$sellprice;
			$reserve_rate = $data->sheets[0]['cells'][$i][29+$shift_num];
		...
		if($pname){			
			...
			$sql = "INSERT INTO ".TBL_SHOP_PRODUCT." 
					(id,  pname, admin,pcode, brand,brand_name, company,make_country,main_material, gift, shotinfo,   listprice,sellprice,  coprice,
					reserve_yn, reserve,reserve_rate,  basicinfo,   state, disp, movie, 
					vieworder, admin,stock,safestock,search_keyword,reg_category, prod_branch_code,
					period_of_circulation,paytype,surtax_yorn,piece,max_buy_number,delivery_company,delivery_method, delivery_product_policy, delivery_price,delivery_zone_type,delivery_zone_exception,pack_method,
					one_commission,commission,etc2, regdate)
					values('', '".strip_tags($pname)."', '$admin','$pcode', '$brand','$brand_name','$company', '$make_country','$main_material','$gift', '$shotinfo', '$listprice','$sellprice', '$coprice','$reserve_yn', '$reserve','$rate1',  '$basicinfo', 
					  $state, 0, '$movie', '$vieworder', '$company_id',
					'$stock','$safestock','$search_keyword','$reg_category','$prod_branch_code',
					'$period_of_circulation','$paytype','$surtax_yorn','$piece','$max_buy_number','$delivery_company','$delivery_method', '$delivery_product_policy', '$delivery_price','$delivery_zone_type',
					'$delivery_zone_exception','$pack_method','$one_commission','$commission','$etc2',NOW()) ";
			...
			if($option_name1 != ""){		
				...
				$option_item1s = split(",",$option_item1);
			...
			if($option_name2 != ""){		
				...
				$option_item2s = split(",",$option_item2);
			...
			if($option_name3 != ""){		
				...
				$option_item3s = split(",",$option_item3);
			...
			if($cid != ""){
				$db->query("insert into ".TBL_SHOP_PRODUCT_RELATION." (rid, cid, pid ,disp ,basic, regdate) values ('','$cid', '$pid','1','$category',NOW());");
			}
	...
	echo "<script>alert('상품등록이 완료되었습니다.');document.location.href='./product_input_excel.php?cid=$cid&depth=$depth&view=innerview'</script>";
