		...
		<div class='tab'>
			...
			<tr>
				<td class='tab'>
					...
					<tr>
						<th class='box_01'></th>
						<td class='box_02'><a href='?info_type=basic&mmode=$mmode&code=$code'>부서 선택</a> </td>
						<th class='box_03'></th>
					</tr>
					</table>
...
if($mode == 'search'){
	if($search_text != ""){
		if(strpos($search_text,',') == false){
			$where = " and (cd.dp_name like '%".$search_text."%' or cg.group_name like '%".$search_text."%')";
		}else{
			...
			for($i=0;$i<count($search_text_array);$i++){
				if($i == count($search_text_array) -1){
					$where .= " cg.group_name like '%".$search_text_array[$i]."%'";
				}else{
					$where .= " or cg.group_name like '%".$search_text_array[$i]."%' or ";
				}
			}
			$where .=")";
		}
...
<table cellSpacing=0 cellPadding=0 width='100%' align=center border=0>
	<tr>
		<td align=center >
		<form name='search_frm' action='./search_department.php' method='get' onsubmit='return CheckFormValue(this)' enctype='multipart/form-data' style='display:inline;' target=''>
		...
		<tr>
			<td>
				...
				for($i=0;$i<count($data_array); $i++){
					$Contents .= "<option value='".$data_array[$i][dp_ix]."'>".$data_array[$i][group_name]." > ".$data_array[$i][dp_name]."</option>";
				}
$Contents .= "
				</select>
			</td>
...
function add_list(){
	...
	$('#department_list option:selected').each(function(){
		$('#add_list').append('<option value='+$(this).val()+' selected>'+$(this).html()+'</option>');
	});
...
function add_department(){
	...
	$('#add_list option').each(function(){
		...
		var add_html = \"<tr style='height:26px;' id='department_row_\"+dp_ix+\"'><td><input type=hidden name=department_list[] id='department_\"+dp_ix+\"' value='\"+dp_ix+\"'></td><td>\"+dp_text+\"</td><td><a href='javascript:void(0)' onClick='department_del(\"+dp_ix+\")'><img src='../images/".$_SESSION["admininfo"]["language"]."/btc_del.gif' border=0></a></td></tr>\";
