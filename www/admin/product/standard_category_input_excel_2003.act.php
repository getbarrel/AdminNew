	...
	*/
	...
	if ($act == "excel_input" || true){
		...
		if ($excel_file_size > 0){	
			copy($excel_file, $_SERVER["DOCUMENT_ROOT"]."".$admin_config[mall_data_root]."/standard_category.xls");
		}
		
		$objPHPExcel = PHPExcel_IOFactory::load($_SERVER["DOCUMENT_ROOT"]."".$admin_config[mall_data_root]."/standard_category.xls");
//print_r($objPHPExcel);
		...
		*/
		...
		while (($objPHPExcel->getActiveSheet()->getCell('A' . $rownum)->getValue() != "") && ($i < 10)) {
			...
			if($objPHPExcel->getActiveSheet()->getCell('A' . $rownum)->getValue() != ""){
                ...
                $category_text[0] = str_replace("\t","",trim($category_text[0]));
				$category_text[1] = str_replace("\t","",trim($category_text[1]));
				$category_text[2] = str_replace("\t","",trim($category_text[2]));
				$category_text[3] = str_replace("\t","",trim($category_text[3]));
				$category_text[4] = str_replace("\t","",trim($category_text[4]));
                ...
                for($i=0;$i < count($category_text);$i++){
				    if(trim($category_text[$i]) != ""){
						...
						if($i == 0){
							$sql = "select * from standard_category_info where cname = '".trim($category_text[$i])."' and depth = '".$i."' ";
						}else{
    						$sql = "select * from standard_category_info where cname = '".trim($category_text[$i])."' and cid LIKE '".substr($cid,0,($depth+1)*3)."%' and depth = '".$i."'";
						}
    					//echo $sql."<br>";
    					$db->query($sql);
    					...
    					if(!$db->total){
    						...
    						$sql = "insert into standard_category_info 
										(cid, depth,vlevel1, vlevel2, vlevel3,vlevel4,vlevel5,cname,category_code, category_use,regdate) 
										values 
										('$sub_cid', '$sub_depth','$level1','$level2','$level3','$level4','$level5', '$sub_category','$category_code','$category_use',NOW());";
    						echo $i." - ".$sql."<br>";
...
function getMaxlevel($cid,$depth)
{
	...
	$sPos = $depth*3;
	$sql = "select IFNULL(max(vlevel$strdepth),0)+1 as maxlevel from standard_category_info where cid LIKE '".substr($cid,0,$sPos-1)."%'";
    echo $sql."<br>";
	$db->query($sql);
	if($strdepth != 1){
		$db->fetch(0);
...
function getNextCid($cid,$depth)
{
	...
	$sPosA = $depth*3;
	$sPos = $depth*3 + 1;
	$sql = "select max(substring(cid,$sPos,3))+1 as maxid from standard_category_info where cid LIKE '".substr($cid,0,$sPos-1)."%'";
	echo $sql."<br><br>";
	$db->query($sql);
	$db->fetch(0);
