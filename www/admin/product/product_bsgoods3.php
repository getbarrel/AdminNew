...
if($total <= $pageging_info["product_bsgoods3.php"]["page"]){
	unset($pageging_info);
	session_unregister("pageging_info");
	$page = 1;
	//echo $pageging_info["product_bsgoods3.php"]["page"];
	//exit;
}

if ($page == ''){
	if($pageging_info["product_bsgoods3.php"]["page"] != ""){
		$page  = $pageging_info["product_bsgoods3.php"]["page"];
		$start = ($page - 1) * $max;
	}else{		
		$page  = 1;
		$start = 0;
	}
	if($pageging_info["product_bsgoods3.php"]["nset"] != ""){
		$nset  = $pageging_info["product_bsgoods3.php"]["nset"];
	}else{
		$nset  = 1;
	}
}else{
...
//echo $page;
//exit;
//http://dev.mallstory.com/admin/product/product_bsgoods3.php?mode=search&cid2=&depth=&bsmode=list&cid0_1=&cid1_1=&cid2_1=&cid3_1=&bs_site=&company_id=&brand_name=&disp=&state2=&search_type=pname&search_text=&max=10&x=64&y=21
...
$Contents .=	"
	<form name='search_form' method='get' action='product_bsgoods3.act.php' onsubmit='return CheckFormValue(this);' target=bsframe>
...
$innerview .= "
				...
				<a href='product_bsgoods3.php?orderby=sellprice&ordertype=desc".$search_query."' target='act'><img src='../image/orderby_desc_".($orderby == "sellprice" && $ordertype ==  "desc" ? "on":"off").".gif' border=0 align=absmiddle title='높은가격순'></a>
				<a href='product_bsgoods3.php?orderby=sellprice&ordertype=asc".$search_query."' target='act'><img src='../image/orderby_asc_".($orderby == "sellprice" && $ordertype ==  "asc" ? "on":"off").".gif' border=0 align=absmiddle title='낮은가격순'></a> |
				<!--b class=small>적립금</b>
				<a href='product_bsgoods3.php?orderby=reserve&ordertype=desc".$search_query."' target='act'><img src='../image/orderby_desc_".($orderby == "reserve" && $ordertype ==  "desc" ? "on":"off").".gif' border=0 align=absmiddle title='높은적립금순'></a>
				<a href='product_bsgoods3.php?orderby=reserve&ordertype=asc".$search_query."' target='act'><img src='../image/orderby_asc_".($orderby == "reserve" && $ordertype ==  "asc" ? "on":"off").".gif' border=0 align=absmiddle title='낮은적립금순'></a> |-->
				<b class=small>상품명</b>
				<a href='product_bsgoods3.php?orderby=pname&ordertype=desc".$search_query."' target='act'><img src='../image/orderby_desc_".($orderby == "pname" && $ordertype ==  "desc" ? "on":"off").".gif' border=0 align=absmiddle title='가나다순'></a>
				<a href='product_bsgoods3.php?orderby=pname&ordertype=asc".$search_query."' target='act'><img src='../image/orderby_asc_".($orderby == "pname" && $ordertype ==  "asc" ? "on":"off").".gif' border=0 align=absmiddle title='가나다역순'></a>
				<b class=small>등록일자</b>
				<a href='product_bsgoods3.php?orderby=r.regdate&ordertype=desc".$search_query."' target='act'><img src='../image/orderby_desc_".((($orderby == "r.regdate" && $ordertype ==  "desc") || ($orderby == "" && $ordertype ==  "")) ? "on":"off").".gif' border=0 align=absmiddle title='최근등록순'></a>
				<a href='product_bsgoods3.php?orderby=r.regdate&ordertype=asc".$search_query."' target='act'><img src='../image/orderby_asc_".($orderby == "r.regdate" && $ordertype ==  "asc" ? "on":"off").".gif' border=0 align=absmiddle title='등록순'></a>
				<b class=small>정렬순</b>
				<a href='product_bsgoods3.php?orderby=vieworder2&ordertype=desc".$search_query."' target='act'><img src='../image/orderby_desc_".($orderby == "vieworder2" && $ordertype ==  "desc" ? "on":"off").".gif' border=0 align=absmiddle title='역순'></a>
				<a href='product_bsgoods3.php?orderby=vieworder2&ordertype=asc".$search_query."' target='act'><img src='../image/orderby_asc_".($orderby == "vieworder2" && $ordertype ==  "asc" ? "on":"off").".gif' border=0 align=absmiddle title='순'></a>
				</td>
				</tr>";
}
$innerview .= "
	...
	if(frm.search_status[0].checked){
		...
		if(bs_act == 'get_goods'){
			...
			if(frm.cid2.value.length < 1){
				alert(language_data['product_bsgoods3.php']['C'][language]);
			...
			if(frm.bs_site.value.length < 1){
				alert(language_data['product_bsgoods3.php']['A'][language]);
				//'구매대행 사이트를 지정해주세요'
				frm.bs_site.focus();
				document.getElementById('save_loading').style.display = 'none';
				obj.innerHTML = \"\";
			...
			if(frm.list_url.value.length < 1){
				alert(language_data['product_bsgoods3.php']['B'][language]);
			...
			if(frm.list_url.value.indexOf(frm.bs_site.value) == -1){
				alert(language_data['product_bsgoods3.php']['D'][language]);
		...
		alert(language_data['product_bsgoods3.php']['E'][language]);
		//'검색 정지중입니다.'
		unloading();
	}
}
...
if($view == "innerview"){
	$pageging_info["product_bsgoods3.php"]["page"] = $page;
	$pageging_info["product_bsgoods3.php"]["nset"] = $nset;
