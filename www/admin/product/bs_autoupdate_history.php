...
$where = "where bsah.bsah_ix is not null ";
if($cid2 != ""){
...
$Contents01 = "
	<table width='100%' cellpadding=0 cellspacing=0 border='0' align='left'>
	  <tr >
			<td align='left' colspan=4> ".GetTitleNavigation("자동업데이트 정보", "상품관리 > 자동업데이트 정보 ")."</td>
	  </tr>
...
$Contents02 = "
	<table width='100%' cellpadding=3 cellspacing=0 border='0' align='left'  class='list_table_box'><!--style='table-layout:fixed;'-->
	  ...
	  <tr bgcolor=#efefef align=center height=25>
			...
			<td class='m_td' align='center' nowrap>업데이트 상품수</td>
			<td class='m_td' >품절상품수</td>
...
if($db->total){
	for($i=0;$i  < $db->total; $i++){
		...
		$Contents02 .= "<tr align=center height=30>
				...
				<td class='list_box_td'>".$db->dt[goods_update_soldout_cnt]."</td>				
	...
	$Contents02 .= "";
		...
		$Contents02 .= "
			<tr height=60><td colspan=9 align=center>자동업데이트  정보가 없습니다.</td></tr>";
...
<table cellpadding=0 cellspacing=0 class='small' >
	...
	<tr><td valign=top><img src='/admin/image/icon_list.gif' ></td><td class='small' >등록된 자동업데이트 정보은 상품 스크래핑 화면에서 자동으로 조회 될수 있습니다.</td></tr>
</table>
";
//$help_text =  getTransDiscription(md5($_SERVER["PHP_SELF"]),'B');

$Contents .= HelpBox("자동업데이트 정보", $help_text)."<br>";
 ...
 function UpdateOrginCategoryInfo(bsah_ix, before_orgin_category_info, change_orgin_category_info){
	...
	if(!($.trim(before_orgin_category_info) == $.trim(change_orgin_category_info))){
		$.ajax({ 
			type: 'GET', 
			data: {'act': 'orgin_category_info_update','bsah_ix': bsah_ix,'orgin_category_info': change_orgin_category_info},
 ...
 function DeleteFavoriteInfo(bsah_ix){
 	if(confirm('구매대행 자동업데이트 정보을 정말로 삭제하시겠습니까?')){//'해당 구매대행 환율/수수료 정보를 정말로 삭제하시겠습니까?'
...
if($mmode == "pop"){
	...
	$P->NaviTitle = "자동업데이트 정보";
	$P->Navigation = "상품관리 > 구매대행 > 자동업데이트 정보";
...
create table shop_buyingservice_autoupdate_history (
	...
	autoupdate_type varchar(20) NOT NULL COMMENT '자동업데이트 타입 new_goods_reg , goods_update ',
