...
function getEstimateGoodsGroupNo(pid){
	...
	$.ajax({
			...
			datatype : 'html',
			async : false,
			data : "act=get_set_group&id="+pid+"&est_ix="+est_ix+"&session_id="+session_id,
	...
	if($('table#minicart tr.order_detail_rows').length > 0){
		...
		if($('#option_kind').val() == "x" || $('#option_kind').val() == "c" || $('#option_kind').val() == "x2" || $('#option_kind').val() == "s2" || add_option_cnt > 0){
			if($("input[name=set_group]").length > 0 && $("input[name=set_group]").val() != ""){
				...
				//getCartGoodsGroupNo 이라는 함수가 이미 존재해서 처음에 선언된 함수 스크립트가 실행된것 같습니다 함수명을 바꾸어야 할것 같아요
				set_group = getEstimateGoodsGroupNo(pid);
				//alert(set_group);
		...
		$('table#minicart tr.order_detail_rows').each(function(j){
			...
			if(opnd_ix != undefined && opnd_ix != ''){
				...
				data['options'][j] = {"opnd_ix_array":opnd_ix,"pcount":pcount,"option_text":$(this).find("span[minicart_id=pname_text]").text(),"delivery_package":$('#delivery_package').val(),"set_group":set_group};
			}
		}); 
