...
if(round($total/$max)  <= $_SESSION["pageging_info"]["product_bsgoods.php"]["page"]){
	unset($pageging_info);
	//session_unregister("pageging_info");
	//$page = 1;
	//echo $_SESSION["pageging_info"]["product_bsgoods.php"]["page"];
	//exit;
}

if ($page == ''){
	if($_SESSION["pageging_info"]["product_bsgoods.php"]["page"] != ""){
		$page  = $_SESSION["pageging_info"]["product_bsgoods.php"]["page"];
		$start = ($page - 1) * $max;
	}else{		
		$page  = 1;
		$start = 0;
	}
	if($_SESSION["pageging_info"]["product_bsgoods.php"]["nset"] != ""){
		$nset  = $_SESSION["pageging_info"]["product_bsgoods.php"]["nset"];
	}else{
		$nset  = 1;
	}
}else{
...
if($mode == "search"){
	...
	if($admininfo[admin_level] == 9){
		$where = "where p.id Is NOT NULL and p.product_type = 1 and p.id = r.pid and r.basic = 1 ";
	}else{
		$where = "where p.id Is NOT NULL and p.product_type = 1 and p.id = r.pid and r.basic = 1 and admin ='".$admininfo[company_id]."'  ";
	}
...
//echo $state;
	...
	if($scid2 != ""){
		//session_register("cid");
		//session_register("depth");
		$where .= " and r.cid LIKE '".substr($scid2,0,$cut_num)."%'";
	}else{
		$where .= "";
	}

	
	$sql = "SELECT count(*) as total FROM ".TBL_SHOP_PRODUCT." p, shop_product_standard_relation r $where  ";
	//echo nl2br($sql);
	$db2->query($sql);
	...
	if ($cid2 == ""){
		if($admininfo[admin_level] == 9){
			$addWhere = "Where p.id = r.pid  and p.product_type = 1 and r.basic = 1";
			if($company_id != ""){
				$addWhere .= " and admin ='".$company_id."'";
			}


			$db2->query("SELECT count(*) as total FROM ".TBL_SHOP_PRODUCT." p, shop_product_standard_relation r  $addWhere ");
		}else{
			$db2->query("SELECT count(*) as total FROM ".TBL_SHOP_PRODUCT." p, shop_product_standard_relation r  where  p.id = r.pid and admin ='".$admininfo[company_id]." '");
		}
		...
		if($admininfo[admin_level] == 9){
			$sql = "SELECT count(*) as total FROM ".TBL_SHOP_PRODUCT." p, shop_product_standard_relation r  where p.id = r.pid and r.basic = 1 and r.cid LIKE '".substr($cid2,0,$cut_num)."%' ";

			$db2->query($sql);

		}else{
			$db2->query("SELECT count(*) as total FROM ".TBL_SHOP_PRODUCT." p, shop_product_standard_relation r  where p.id = r.pid and r.basic = 1 and r.cid LIKE '".substr($cid2,0,$cut_num)."%' and admin ='".$admininfo[company_id]."' ");
		}
...
if($mode == "search"){
	...
	if($admininfo[admin_level] == 9){
		...
		case when vieworder = 0 then 100000 else vieworder end as vieworder2,p.currency_ix ".$product_image_column_str."  
		FROM ".TBL_SHOP_PRODUCT." p right join  shop_product_standard_relation r on p.id = r.pid and r.basic = 1 , ".TBL_COMMON_COMPANY_DETAIL." c
		...
		$sql = "SELECT  p.id, p.product_type, p.pname,p.brand, p.sellprice, p.regdate,p.vieworder,c.com_name,  r.cid,  p.search_keyword,state, p.brand_name, p.bs_goods_url,
		p.company, p.pcode, p.coprice, p.listprice, icons,p.disp, p.editdate, p.reserve, p.reserve_rate,
		case when vieworder = 0 then 100000 else vieworder end as vieworder2 ".$product_image_column_str."  
		FROM ".TBL_SHOP_PRODUCT." p right join shop_product_standard_relation r on p.id = r.pid and r.basic = 1, ".TBL_COMMON_COMPANY_DETAIL." c
	...
	if ($cid2 == ""){
		if($admininfo[admin_level] == 9){
			...
			//$tmp_sql = "create temporary table ".TBL_LOGSTORY_BYPAGE."_tmp ENGINE = MEMORY select vdate, pageid, ncnt, nduration from shop_product_standard_relation where vdate = '$vdate' ";
			...
			p.company, p.pcode, p.coprice, p.listprice,  icons,p.disp, p.editdate, p.reserve, p.reserve_rate,
			case when vieworder = 0 then 100000 else vieworder end as vieworder2, p.currency_ix ".$product_image_column_str."  
			FROM ".TBL_SHOP_PRODUCT." p, shop_product_standard_relation r, ".TBL_COMMON_COMPANY_DETAIL." c
			where c.company_id = p.admin and p.product_type = 1 and p.id = r.pid and r.basic = 1 $where $addWhere $orderbyString LIMIT $start, $max";
			//echo $sql;
			$db->query($sql);
		}else{
			...
			case when vieworder = 0 then 100000 else vieworder end as vieworder2, p.currency_ix ".$product_image_column_str."  
			FROM ".TBL_SHOP_PRODUCT." p, shop_product_standard_relation r, ".TBL_COMMON_COMPANY_DETAIL." c
			where c.company_id = p.admin and p.product_type = 1 and p.id = r.pid and r.basic = 1 and admin ='".$admininfo[company_id]."' $where $orderbyString LIMIT $start, $max";
		...
		if($admininfo[admin_level] == 9){
			$sql = "SELECT p.id as id, p.product_type, p.pname, p.sellprice, p.regdate,c.com_name,p.vieworder,c.com_name, r.cid, p.search_keyword,state, p.brand, p.brand_name, p.bs_goods_url,
				...
				case when vieworder = 0 then 100000 else vieworder end as vieworder2, p.currency_ix ".$product_image_column_str."  
				FROM ".TBL_SHOP_PRODUCT." p, shop_product_standard_relation r, ".TBL_COMMON_COMPANY_DETAIL." c
				where c.company_id = p.admin and p.product_type = 1 and p.id = r.pid and r.basic = 1 and r.cid = '".$cid2."' $where $orderbyString LIMIT $start, $max";
			...
			$sql = "SELECT p.id as id, p.product_type, p.pname, p.sellprice, p.regdate,c.com_name,p.vieworder,c.com_name,  r.cid, p.search_keyword,state, p.brand, p.brand_name, p.bs_goods_url,
				...
				case when vieworder = 0 then 100000 else vieworder end as vieworder2, p.currency_ix ".$product_image_column_str."  
				FROM ".TBL_SHOP_PRODUCT." p, shop_product_standard_relation r, ".TBL_COMMON_COMPANY_DETAIL." c
				where c.company_id = p.admin and p.product_type = 1 and p.id = r.pid and r.basic = 1 and r.cid = '".$cid2."' and admin ='".$admininfo[company_id]."' $where $orderbyString LIMIT $start, $max";
...
//echo $page;
//exit;
//http://dev.mallstory.com/admin/product/product_bsgoods.php?mode=search&cid2=&depth=&bsmode=list&cid0_1=&cid1_1=&cid2_1=&cid3_1=&bs_site=&company_id=&brand_name=&disp=&state2=&search_type=pname&search_text=&max=10&x=64&y=21
...
$Contents .=	"
	...
	<tr>
		<td colspan=2>
			<form name='search_form' method='get' action='product_bsgoods.act.php' onsubmit='return CheckFormValue(this);' target=bsframe>
			<input type='hidden' name='bs_act' value=''>
			<input type='hidden' name='bsui_ix' id='bsui_ix' value='$bsui_ix'>
			<input type='hidden' name='cid2' id='cid2' value='$cid2'>
			<input type='hidden' name='scid2' id='scid2' value='$scid2'>
...
$Contents .=	"
							<tr bgcolor=#ffffff >
								...
								<td class='input_box_item' >
									".getBuyingServiceCurrencyInfo($currency_ix,"select"," validation='true' ")."
							...
							<tr>
								...
								<div style='width:100%;margin:5px;' id=selected_category_path></div>
								".getTransDiscription(md5($_SERVER["PHP_SELF"]),'A')."<br>
								</td>
							</tr>
							<tr>
								<td class='input_box_title'> 카테고리  </td>
								<td class='input_box_item' colspan='3'>
									<table border=0 cellpadding=0 cellspacing=0>
										<tr>"; 
											$Contents .= "
											<td style='padding-right:5px;'>".getCategoryList("대분류", "scid0_1", "scid0_1", "onChange=\"loadCategory(this,'scid1_1',2)\" title='대분류' ", 0, $scid2)."</td>
											<td style='padding-right:5px;'>".getCategoryList("중분류", "scid1_1", "scid1_1", "onChange=\"loadCategory(this,'scid2_1',2)\" title='중분류'", 1, $scid2)."</td>
											<td style='padding-right:5px;'>".getCategoryList("소분류", "scid2_1", "scid2_1", "onChange=\"loadCategory(this,'scid3_1',2)\" title='소분류'", 2, $scid2)."</td>
											<td>".getCategoryList("세분류", "scid3_1", "scid3_1", "onChange=\"loadCategory(this,'scid2',2)\" title='세분류'", 3, $selected_stand_cid)."</td>
											"; 
										$Contents .= "
										</tr>
									</table>
							...
							<tr bgcolor=#ffffff >
							  ...
							  <tr>
								...
								if($admininfo[admin_level] == 9 || $admininfo[admin_id] == 'forbiz'){
								$Contents .=	"
								<tr height=60>
									<td class='input_box_title'>입점업체</td>
									<td class='input_box_item' colspan=3>";
									if(false){
									$Contents .=	"".CompanyList2($company_id,"")." ";
									}
									$Contents .=	"
									".companyAuthList($company_id , "validation=true title='셀러업체' ",'company_id','company_id','com_name','input')."
...
$Contents .=	"
	...
	<tr>
		<td colspan=2>
			...
			<input type='hidden' name='scid2' value='$scid2'>
								...
								<td class='input_box_item' colspan='3'>
									<table border=0 cellpadding=0 cellspacing=0>
										<tr>"; 
											...
											<td style='padding-right:5px;'>".getCategoryList("중분류", "scid1_1", "scid1_1", "onChange=\"loadCategory(this,'scid2_1',2)\" title='중분류'", 1, $scid2)."</td>
											<td style='padding-right:5px;'>".getCategoryList("소분류", "scid2_1", "scid2_1", "onChange=\"loadCategory(this,'scid3_1',2)\" title='소분류'", 2, $scid2)."</td>
											<td>".getCategoryList("세분류", "scid3_1","scid3_1", "onChange=\"loadCategory(this,'scid2',2)\" title='세분류'", 3, $scid2)."</td>
											"; 
											//getCategoryList($category_text ="기본카테고리 선택", $object_name="cid",$id="cid", $onchange_handler="", $depth=0, $cid="")
										$Contents .= "
										</tr>
									</table>
								</td>
...
$Contents .=	"
							...
							<tr>
								...
								<td class='input_box_item'>
									...
									<input type='radio' name='disp'  id='disp_1' value='1' ".($disp == "1" ? " checked":"")."><label for='disp_1'>노출함</label>
									<input type='radio' name='disp'  id='disp_0' value='0' ".($disp == "0" ? " checked":"")."><label for='disp_0'>노출안함</label>
								</td>
								...
								<td class='input_box_item'>
									<table cellpadding=0 cellspacing=0>
										<tr>
											...
											<td style='padding-left:5px;'>
											<INPUT id=search_texts  class='textbox' value='".$search_text."' clickbool='false' style=' FONT-SIZE: 12px; WIDTH: 160px; COLOR: #736357; BACKGROUND-COLOR: #ffffff' name=search_text validation=false  title='검색어'><!--onclick='findNames();'  --><br>
...
$Contents .= "
		...
		<form name=listform method=post action='goods_batch.act.php' onsubmit='return SelectUpdate(this)' target='bsframe'><!--onsubmit='return CheckDelete(this)' target='act'-->
...
if($orderby != "" && $ordertype != ""){
	if($orderby == "regdate" && $ordertype == "asc"){
		$orderbyStringForce = " force index(IDX_MP_REGDATE) ";
		$orderbyStringForceWhere = " p.regdate is not null and ";
	}else{
		$orderbyString = " order by $orderby $ordertype ";
	}
}else{
	//$orderbyString = " order by r.regdate desc ";
	$orderbyString = " order by p.regdate desc ";//박과장님 요청으로 등록일 순으로 정렬 kbk 12/11/23
	$orderbyStringForce = " force index(regdate_desc) ";
	$orderbyStringForceWhere = " p.regdate_desc < 0 and ";
}
...
if($mode == "search"){
	...
	if($admininfo[admin_level] == 9){
		...
		$sql = "SELECT  p.id, p.product_type, p.pname,p.brand, p.buyingservice_coprice, p.sellprice, p.regdate,p.vieworder,c.com_name,  r.cid,  p.search_keyword,state, p.brand_name, p.bs_goods_url,
		p.company, p.pcode, p.coprice, p.listprice, icons,p.disp, p.editdate, p.reserve, p.reserve_rate, p.wholesale_price, p.wholesale_sellprice, 
		case when vieworder = 0 then 100000 else vieworder end as vieworder2,p.disp, p.currency_ix
		FROM ".TBL_SHOP_PRODUCT." p ".$orderbyStringForce." right join  shop_product_standard_relation r on p.id = r.pid and r.basic = 1 , ".TBL_COMMON_COMPANY_DETAIL." c
		where c.company_id = p.admin and p.product_type = 1  $addWhere $where $orderbyString LIMIT $start, $max";
		//echo $sql;
		$slave_db->query($sql);
	}else{
...
//	echo nl2br($sql);
	...
	if ($cid2 == ""){
		if($admininfo[admin_level] == 9){
			...
			//$tmp_sql = "create temporary table ".TBL_LOGSTORY_BYPAGE."_tmp type = heap select vdate, pageid, ncnt, nduration from shop_product_standard_relation where vdate = '$vdate' ";

			$sql = "SELECT p.id as id, p.product_type, p.pname, p.brand, p.buyingservice_coprice, p.sellprice, p.regdate,p.vieworder,c.com_name,  r.cid,  p.search_keyword,state, p.brand_name, p.bs_goods_url,
			p.company, p.pcode, p.coprice, p.listprice,  icons,p.disp, p.editdate, p.reserve, p.reserve_rate,  p.wholesale_price, p.wholesale_sellprice, 
			case when vieworder = 0 then 100000 else vieworder end as vieworder2,p.disp, p.currency_ix
			FROM ".TBL_SHOP_PRODUCT." p ".$orderbyStringForce.", shop_product_standard_relation r, ".TBL_COMMON_COMPANY_DETAIL." c
			where c.company_id = p.admin and p.product_type = 1 and p.id = r.pid and r.basic = 1 $where $addWhere $orderbyString LIMIT $start, $max";
			//echo $sql;
			$slave_db->query($sql);
		}else{
			$sql = "SELECT p.id as id ,p.product_type, p.brand, p.pname, p.buyingservice_coprice,  p.sellprice, p.regdate,p.vieworder,c.com_name,  r.cid,  p.search_keyword,state, p.brand_name, p.bs_goods_url,
			p.company, p.pcode, p.coprice, p.listprice, icons, p.disp, p.editdate, p.reserve, p.reserve_rate,  p.wholesale_price, p.wholesale_sellprice, 
			case when vieworder = 0 then 100000 else vieworder end as vieworder2,p.disp, p.currency_ix
			FROM ".TBL_SHOP_PRODUCT." p ".$orderbyStringForce.", shop_product_standard_relation r, ".TBL_COMMON_COMPANY_DETAIL." c
			where c.company_id = p.admin and p.product_type = 1 and p.id = r.pid and r.basic = 1 and admin ='".$admininfo[company_id]."' $where $orderbyString LIMIT $start, $max";
		...
		if($admininfo[admin_level] == 9){
			$sql = "SELECT p.id as id, p.product_type, p.pname, p.buyingservice_coprice, p.sellprice, p.regdate,c.com_name,p.vieworder,c.com_name, r.cid, p.search_keyword,state, p.brand, p.brand_name, p.bs_goods_url,
				p.company, p.pcode, p.coprice, p.listprice,  icons,p.disp, p.editdate,  p.reserve_rate,  p.wholesale_price, p.wholesale_sellprice, 
				case when vieworder = 0 then 100000 else vieworder end as vieworder2,p.disp, p.currency_ix
				FROM ".TBL_SHOP_PRODUCT." p ".$orderbyStringForce.", shop_product_standard_relation r, ".TBL_COMMON_COMPANY_DETAIL." c
				where c.company_id = p.admin and p.product_type = 1 and p.id = r.pid and r.basic = 1 and r.cid = '".$cid2."' $where $orderbyString LIMIT $start, $max";
...
if($db->total == 0){
	...
	for ($i = 0; $i < count($goods_infos); $i++)
	{
		...
		//	echo $_SERVER["DOCUMENT_ROOT"]."".PrintImage($admin_config[mall_data_root]."images/product", $goods_infos[$i][id], "s", $goods_infos[$i])."<br>";
		if(file_exists($_SERVER["DOCUMENT_ROOT"]."".PrintImage($_SESSION["admin_config"]["mall_data_root"]."/images/product", $goods_infos[$i][id], "s", $goods_infos[$i]))) {
			$img_str = PrintImage($_SESSION["admin_config"]["mall_data_root"]."/images/product", $goods_infos[$i][id], "s", $goods_infos[$i]);
		//	echo $img_str."<br>";
		}else{
	...
	$innerview .= "<tr bgcolor='#ffffff'>
						...
						<td class='list_box_td point' align=left style='padding:5px;'>
							...
							<tr>
								<td width=60 rowspan=2><a href='goods_input.php?id=".$db->dt[id]."' class='screenshot'  rel='".PrintImage($admin_config[mall_data_root]."/images/product", $goods_infos[$i][id], $LargeImageSize, $goods_infos[$i])."'  ><img src='".$img_str."' width=50 ></a></td>
...
$innerview .= "</td>
					...
					<td class='list_box_td list_bg_gray'  nowrap>
						...
						<table align=center>
							...
							<tr>
								<td ><a href='/shop/goods_view.php?cid=".$goods_infos[$i][cid]."&id=".$goods_infos[$i][id]."&depth=3&b_ix=".$goods_infos[$i][brand]."' target='_blank'><img src='../images/".$admininfo["language"]."/bts_preview.gif'></a></td>
							</tr>
						</table>
					</td>
...
if($bsmode == "list"){
		...
		<div style='padding:4px 0 4px 0'><img src='../images/dot_org.gif'> <b><!--구매대행 상품 정보/재고확인-->".getTransDiscription(md5($_SERVER["PHP_SELF"]),'F')."</b> <span class=small style='color:gray'><!--상품/재고 확인을 하고자 하는 상품을 검색/선택 후 재고확인 버튼을 클릭해주세요.-->".getTransDiscription(md5($_SERVER["PHP_SELF"]),'P')."</span></div>
			...
			<tr>
				...
				<td class='input_box_item' style='padding:5px;'>
				<input type='checkbox' name='img_update' id='img_update' value='Y' ><label for='img_update'>기본 이미지 정보를 업데이트 합니다.</label><br>
				<input type='checkbox' name='goods_desc_copy' id='goods_desc_copy' value='1' ><label for='goods_desc_copy'>상세 이미지 정보를 업데이트 합니다.</label><br>
				</td>
			</tr>
			</table>
			<table cellpadding=3 cellspacing=0 width=100% style='border:0px solid silver;'>
...
<!--a href=\"javascript:alert(document.frames['bs_search_frame'].location);\">url 정보 보기</a>
		...
		if($_SERVER["HTTP_HOST"] == "dev.forbiz.co.kr" || $admininfo[admin_id] == "forbiz" || $admininfo[admin_id] == "selladmin" || $admininfo[admin_id] == "dongdong_admin"){ 
...
function loadStandardCategory(obj,target) {
	 
	if(selected_standard_cid){
		var trigger = selected_standard_cid; //selected_standard_cid;//
	}else{
		var trigger = obj.find('option:selected').val(); //selected_standard_cid;//
	}
	...
	$.ajax({
			...
			async: true,
			beforeSend: function(){
				//alert($(obj).parent().parent().parent().parent().parent().parent().parent().parent().parent().parent().parent().parent().parent().parent().parent().html());
				//alert($(obj).closest('form[name=search_form]').html());
				//alert('trigger:'+trigger+',form:'+form);
			},
			...
			success: function(datas){
				...
				if(datas != null){
					...
					var cid = trigger;
					//alert(cid+'::::'+depth);
					//alert(cid.substring(0,6));
					$('input[name=scid2]').val(cid);
					$('input[name=depth]').val(depth);
					

					if(depth == 0){
						$('select[name='+target+']').find('option[value='+cid.substring(0,6)+'000000000]').attr('selected', 'true');
						$('select[name='+target+']').trigger('change');
						//setStandardCategory('scid2_1',cid, 2);			
						
					}else if(depth == 1){
						$('select[name='+target+']').find('option[value='+cid.substring(0,9)+'000000]').attr('selected', 'true');
						$('select[name='+target+']').trigger('change');		
					}else if(depth == 2){
						$('select[name='+target+']').find('option[value='+cid.substring(0,12)+'000]').attr('selected', 'true');
						$('select[name='+target+']').trigger('change');
					}
					
				}else{
					//standard_category_add('select');
				}
			}
		});
	}else{
		//alert(depth);
		var selected_cid = $('select[name^=scid][depth='+(depth-1)+'] option:selected').val();
		//alert(selected_cid);
		$('input[name=scid2]').val(selected_cid);
		$('input[name=depth]').val(depth-1);
	}
}

/*
function loadStandardCategory(sel,target) {

	var trigger = sel.options[sel.selectedIndex].value;
	var form = sel.form.name;

	var depth = $('select[name='+sel.name+']').attr('depth'); 
	window.frames['act'].location.href = '../product/standard_category.load2.php?form=' + form + '&trigger=' + trigger + '&depth='+depth+'&target=' + target;
...
if($view == "innerview"){
	$_SESSION["pageging_info"]["product_bsgoods.php"]["page"] = $page;
	$_SESSION["pageging_info"]["product_bsgoods.php"]["nset"] = $nset;
	...
	$Script .= "<Script Language='JavaScript' src='buyingService_ajax.js?rand=".rand()."'></Script>\n
