...
$Contents01 = "
	...
	<tr>
	  ...
	  <tr >
	    ...
	    <td class='input_box_item'><input type=file class='textbox' name='icon_file' value='' style='width:230px;' title='아이콘파일'> </td>
	  </tr>
	  <tr >
	    <td class='input_box_title'> <b>모바일아이콘파일(30X30)<img src='".$required3_path."'></b> </td>
	    <td class='input_box_item'><input type=file class='textbox' name='mo_icon_file' value='' style='width:230px;' title='모바일아이콘파일'> </td>
	  </tr>
	  <tr  >
...
if($total){
	...
	$Contents02 .= "
			...
			if(checkMenuAuth(md5($_SERVER["PHP_SELF"]),"U")){
				$Contents02 .= "<a href=\"javascript:updateIconInfo('".$db->dt[idx]."','".$db->dt[icon_name]."','".$db->dt[disp]."','".$db->dt[icon_type]."')\"><img src='../images/".$_SESSION["admininfo"]["language"]."/btc_modify.gif' border=0 align=absmiddle></a> ";
			}else{
			...
			if(checkMenuAuth(md5($_SERVER["PHP_SELF"]),"D")){
				$Contents02 .= "<a href=\"javascript:deleteIconInfo('delete','".$db->dt[idx]."')\"><img src='../images/".$_SESSION["admininfo"]["language"]."/btc_del.gif' border=0 align=absmiddle></a> ";
			}else{
	...
	$Contents02 .= "
		  <tr bgcolor=#ffffff height=50>
		    <td align=center colspan=6>등록된 아이콘이 없습니다. </td>
		  </tr>	  ";
}
$Contents02 .= "
...
$Contents = "<form name='bank_form' action='product_icon.act.php' method='post' onsubmit='return CheckFormValue(this)' target='act' enctype='multipart/form-data'><input name='act' type='hidden' value='insert'><input name='idx' type='hidden' value=''>";
...
$Contents = $Contents."</table >";
 ...
 function updateIconInfo(idx,icon_name,disp,icon_type_value){
	...
	$('input[name=icon_type][value='+icon_type_value+']').attr('checked',true);
 ...
 function deleteIconInfo(act, idx){
 	if(confirm('해당 아이콘을 정말로 삭제하시겠습니까?')){
 		var frm = document.bank_form;
 		frm.act.value = act;
 		frm.idx.value = idx;
 		frm.submit();
