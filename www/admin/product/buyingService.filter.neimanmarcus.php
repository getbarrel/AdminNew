...
$pcode_tmp = "";
$pcode = "";
preg_match_all("|<input value=\"(.*)\" type=\"hidden\" name=\"/nm/formhandler/ProdHandler.productId\">|U",$results,$pcode_tmp, PREG_PATTERN_ORDER);
$pcode = $pcode_tmp[1][0];		
$pcode = trim($pcode);
...
$img_tmp = "";
$prod_img_src = "";
preg_match_all("|<meta property=\"og:image\" content=\"(.*)\" />|U",$results2,$img_tmp, PREG_PATTERN_ORDER);
$prod_img_src = $img_tmp[1][0];
$prod_img_src = str_replace("/mb/","/mx/",$prod_img_src);
$prod_img_src = str_replace("mb.jpg","mx.jpg",$prod_img_src);
...
for($i=0;$i < count($datas);$i++){
	...
	if(!$prod_desc_start_line && substr_count($data,"<div class=\"cutline short \">")){
		$prod_desc_start_line = $i+1;
	}
    if(!$prod_desc_end_line && substr_count($data,"<div class=\"gradient\">")){
