...
if($mf_ix){
	$sql = 	"SELECT * FROM shop_car_model where disp=1 and mf_ix = '".$mf_ix."' order by vieworder asc ";
	//echo $sql;
	$db->query($sql);
}else{
...
if ($db->total){
      ...
      //}
        ...
        for($i=0; $i < $db->total; $i++){
                $db->fetch($i);
				echo "<script type='text/javascript'>
					parent.document.forms['$form'].elements['".$target."'].options[".($i+1)."].text = '".$db->dt[model_name]."';
					parent.document.forms['$form'].elements['".$target."'].options[".($i+1)."].value = '".$db->dt[md_ix]."';
				</script>";
        }
		exit;
