...
//$oid = "20140713223034-50403";
//insertProductPoint('1', POINT_USE_STATE_IC, $oid, $order_details[$i]['od_ix'], $point, $order_details[$i]["pid"], '입금완료 상품점수 추가', '', 'ic');
			...
			<tr>
				<td class='box_05' valign=top style='padding:0px;'>
					...
					<tr height=27>
						...
						<td class='search_box_title' bgcolor='#efefef' align=center><b>상품포인트 구분</b></td>
						<td class='search_box_item'  >
							<select name='use_state' id='use_state' style='display:'>
								<option value='' >선택</option>
								<option value='1' ".CompareReturnValue("1",$use_state,"selected").">입금완료</option>
								<option value='2' ".CompareReturnValue("2",$use_state,"selected").">배송완료</option>
								<option value='3' ".CompareReturnValue("3",$use_state,"selected").">구매확정</option>
								<option value='4' ".CompareReturnValue("4",$use_state,"selected").">입금후취소</option>
								<option value='5' ".CompareReturnValue("5",$use_state,"selected").">교환확정</option>
								<option value='6' ".CompareReturnValue("6",$use_state,"selected").">반품확정</option>
								<option value='7' ".CompareReturnValue("7",$use_state,"selected").">배송지연</option>
								<option value='8' ".CompareReturnValue("8",$use_state,"selected").">추가배송지연</option>
								<option value='9' ".CompareReturnValue("9",$use_state,"selected").">기타</option>
								<option value='10' ".CompareReturnValue("10",$use_state,"selected").">클릭</option>
								<option value='11' ".CompareReturnValue("11",$use_state,"selected").">찜하기</option>
								<option value='12' ".CompareReturnValue("12",$use_state,"selected").">장바구니</option>
							</select>
						</td>
					</tr>
					<tr>
						...
						<td class='search_box_item' colspan='3'>
							...
							<tr>
								<td valign='top'>
									...
									<option value='spp.charger_name' ".CompareReturnValue("spp.charger_name",$search_type).">담당자명</option-->
									<option value='spp.oid' ".CompareReturnValue("spp.oid",$search_type).">주문번호</option>
									<option value='spp.pid' ".CompareReturnValue("spp.pid",$search_type).">상품번호</option>
									<option value='od.pname' ".CompareReturnValue("od.pname",$search_type).">상품명</option>
									</select>
									</div>
								</td>
...
if(is_array($state) && count($state)>0){		//노출여부 
	$where.=" AND spp.state IN ('".implode("','",$state)."')";
}else{
	if($state != ""){
		$where .= " and spp.state = '".$state."'";
	}else{
		$state =array();
	}
}
...
//판매신용점수구분
if($use_state != ""){
	$where .= " and spp.use_state = '".$use_state."' ";
}
...
if($slave_db->dbms_type == "oracle"){
	if($search_type != "" && $search_text != ""){
		if($search_type == "jumin"){
			...
			$where .= " and AES_DECRYPT(jumin,'".$slave_db->ase_encrypt_key."') = '$search_text' ";

			$count_where .= " and AES_DECRYPT(jumin,'".$slave_db->ase_encrypt_key."') = '$search_text' ";

		}else if($search_type == "name" || $search_type == "mail" || $search_type == "pcs" || $search_type == "tel" ){
			$where .= " and AES_DECRYPT($search_type,'".$slave_db->ase_encrypt_key."') LIKE  '%$search_text%' ";
			$count_where .= " and AES_DECRYPT($search_type,'".$slave_db->ase_encrypt_key."') LIKE  '%$search_text%' ";
		}else{
	...
	if($search_type != "" && $search_text != ""){
		if($search_type == "cmd.name"){
			$where .= " and AES_DECRYPT(UNHEX($search_type),'".$slave_db->ase_encrypt_key."') LIKE  '%$search_text%' ";
			$count_where .= " and AES_DECRYPT(UNHEX($search_type),'".$slave_db->ase_encrypt_key."') LIKE  '%$search_text%' ";
...
if($search_check == '1'){
	if($sdate != "" && $edate != ""){
		if($slave_db->dbms_type == "oracle"){
			$where .= " and  to_char(spp.regdate, 'YYYYMMDD') between $sdate and $edate ";
		}else{
			$where .= " and  date_format(spp.regdate,'%Y-%m-%d') between '$sdate' and '$edate' ";
		}
	}
}
...
$sql = "select
		...
		from
			shop_product_point as spp 
			inner join common_company_detail as ccd on (ccd.company_id = spp.company_id)			
			left join shop_order_detail as od on (spp.od_ix = od.od_ix)
		where
			1
			$where
			order by spp.edit_date DESC, pp_ix DESC LIMIT $start, $max";

			/*
			cu.id,
			cmd.code,
			AES_DECRYPT(UNHEX(cmd.name),'".$slave_db->ase_encrypt_key."') as name,
...
if($mode == "excel"){	//엑셀다운로드
	...
	$slave_db->query($sql);
	$slave_db->fetch();
	$stock_report_excel = $slave_db->dt[conf_val];
...
//	echo nl2br($sql)."<br><br>";
	$slave_db->query($sql);
	$slave_db->fetch();
	$stock_report_excel_checked = $slave_db->dt[conf_val];
	...
	for($i = 0; $i < count($goods_infos); $i++)
	{
		...
		foreach($check_colums as $key => $value){
			if($key == "regdate"){	//처리일자
				...
				$value_str = $goods_infos[$i][$value];//$slave_db1->dt[$value];
			}
...
$Contents01 .= "
	...
	<tr bgcolor=#efefef style='font-weight:bold' align=center height=33>
		...
		<td class='m_td small' ><b>상품점수구분</b></td>
...
$Contents01 .= "
			<tr height=78 align=center>
				<td class='list_box_td list_bg_gray' ><input type=checkbox class=nonborder id='reserve_id' name=rid[] value='".$data_array[$i][pp_ix]."'></td>
				<td class='list_box_td point' bgcolor='#ffffff'>
					".$data_array[$i][pid]."
				</td>
				<td class='list_box_td' bgcolor='#ffffff' style='padding:10px;line-height:150%;text-align:left;'>
					".$data_array[$i][pname]."<br>
					".$data_array[$i][option_text]."";
					if($data_array[$i][pcnt]){
					$Contents01 .= "
				...
				<td class='list_box_td point' >".$mstate."</td>
				<td class='list_box_td' style='line-height:150%;' >
					...
					".$data_array[$i][oid]."</a><br>".$data_array[$i][etc]."
				</td>
				<td class='list_box_td list_bg_gray'><font color='".$font_colod."'>";
			if($data_array[$i][state]	== '2'){
$Contents01 .= "<font color='red'> -".number_format($data_array[$i][point])."</font>";
			}else{
$Contents01 .= number_format($data_array[$i][point]);
			}
$Contents01 .= "<font>
				</td>
				<td class='list_box_td'><font color='".$font_colod."'>".number_format($data_array[$i][total_point])."<font></td>
				<td class='list_box_td point' bgcolor='#efefef'>".$use_state."</td>
				<td class='list_box_td' bgcolor='#ffffff' style='padding:5px;line-height:150%;' align=left>
					".$regdate."<br>".$data_array[$i][charger_name]."
				</td>
				<td class='list_box_td' style='padding:3px;'>
					<a href=\"javascript:PoPWindow('product_point.pop.php?pid=".$data_array[$i][pid]."',850,550,'product_point_pop')\">
...
$Contents01 .= "
			<tr height=60><td class='list_box_td' colspan=10 align=center>상품 레벨점수 내역이 존재하지 않습니다.</td></tr>";
}
...
/*
drop table shop_product_point;
CREATE TABLE IF NOT EXISTS shop_product_point (
  `pp_ix` int(10) NOT NULL AUTO_INCREMENT,
  `pid` varchar(50) NOT NULL COMMENT '상품키값',
  `pname` varchar(100) NOT NULL COMMENT '상품명',
  `oid` varchar(20) NOT NULL COMMENT '주문번호',
  `od_ix` int(10) NOT NULL COMMENT '주문상세번호',
  `point` int(10) NOT NULL COMMENT '처리 요청 신용점수',
  `total_point` int(10) NOT NULL COMMENT '현재 신용점수',
  `state` varchar(1) NOT NULL COMMENT '1:적립 2:차감',
  `use_state` char(2) NOT NULL COMMENT '1:입금완료 2:배송완료 3:구매확정 4:입금후 취소 5:교환승인 6:반품승인 7:입금후완료일 8:입금후완료일추가 9:기타',
  `etc` text NOT NULL COMMENT '신용점수 내용',
  `charger_ix` varchar(50) NOT NULL COMMENT '담당자코드',
  `charger_name` varchar(50) NOT NULL COMMENT '담당자명',
  `complete_date` datetime NOT NULL COMMENT '적립일자',
  `use_date` datetime NOT NULL COMMENT '차감일자',
  `edit_date` datetime NOT NULL COMMENT '수정일자',
  `regdate` datetime NOT NULL COMMENT '처리일자',
  PRIMARY KEY (`pp_ix`),
  KEY pid_oid_od_ix (`pid`,`oid`,`od_ix`)
) ENGINE=MyISAM  DEFAULT CHARSET=utf8 COMMENT='상품 포인트 관리'
*/
?>
