...
if($excel_type == "brand_list_excel"){
	$excel_name = "브랜드 리스트 ";
	$act = "brand_list";
...
}else if($excel_type == "origin_list_excel"){
	$excel_name = "원산지 리스트 ";
	$act = "origin_list";
	...
	$db->fetch();
	$stock_report_excel_checked = $db->dt[conf_val];
}else if($excel_type == "company_list_excel"){
	$excel_name = "제조사 리스트 ";
	$act = "company_list";
...
$Contents = "<!--img src='../images/0.gif' width='880' height='1'-->
	<table class='layer_box' border=0 cellpadding=0 cellspacing=0 style='width:880px;display:block;' >
		...
		<tr>
			...
			<form name='excel_out_frm' method=\"post\" enctype='multipart/form-data' action='excel_config.act.php' onsubmit='return CheckValue(this);' target='iframe_act' ><!-- -->
...
//print_r($stock_report_excel);
//echo $db->dt[inventory_excel_info];
...
<table cellpadding=2 cellspacing=0 class='small' >
	<col width=8>
	<col width=*>
	<tr><td valign=top><img src='/admin/image/icon_list.gif' ></td><td style='line-height:140%'><b>엑셀필드 순서 정의</b> : 앞쪽부터 등록일, 브랜드 분류 ... 등으로 엑셀에 출력됩니다 순서를 바꾸시길 원하시면 마우스로 드래그해서 순서를 변경하시기 바랍니다</td></tr>
