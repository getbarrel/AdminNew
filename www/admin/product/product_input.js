...
function calcurate(frm){	
	...
	{
		alert(language_data['product_input.js']['C'][language]);	//판매가격이 입력되지 않았습니다.
		return false;
	}
	
	
	if(frm.rate1.value.length < 1)	{
		alert(language_data['product_input.js']['A'][language]);	//현금사용시 적립율이 입력되지 않았습니다.
		return false;
	}else{
		rate1 = rate1/100
	}
	...
	if(frm.rate2.value.length < 1)	{
		alert(language_data['product_input.js']['B'][language]);	//카드사용시 적립율이 입력되지 않았습니다.
		return false;
	}else{
		rate2 = rate2/100
	}
...
//alert(sellprice);
	...
	if(frm.card_pay.value.length < 1)	{
		alert(language_data['product_input.js']['D'][language]);	//카드수수료가 입력되지 않았습니다.
		return false;
	}else{
...
function ProductInput(frm,act)
{
	...
	if (frm.pname.value.length < 1){
		alert(language_data['product_input.js']['E'][language]);	//제품명이 입력되지 않았습니다.
		return false;	
	}
	/*
	if (frm.sellprice.value != frm.bsellprice.value){
		alert("가격에 대한 정보가 변경되었습니다.");
	...
	if(frm.coprice.value.length < 1)
	{
		alert(language_data['product_input.js']['G'][language]);	//공급가격이 입력되지 않았습니다.
		return false;
	}
	/*
	if (frm.shotinfo.value.length < 1){
		alert('제품소개가 입력되지 않았습니다.');
		return false;	
	}
	*/
	frm.submit();	
...
function dpCheckOptionData(frm){
	if(frm.dp_title.value.length < 1){
		alert(language_data['product_input.js']['I'][language]);	//옵션구분값을 입력해주세요.
		frm.option_div.focus();
		return false;
	}
	
	if(frm.dp_desc.value.length < 1){
		alert(language_data['product_input.js']['J'][language]);	//옵션별 비회원가를 입력해주세요.
...
function CheckOptionData(frm){
	...
	if(option_kind == 'b' || option_kind == 'p'){
		if(frm.opn_ix.value == ""){
			alert(language_data['product_input.js']['K'][language]);	//옵션이름을 선택해주세요.
			frm.option_div.focus();
			return false;
		}
		...
		if(frm.option_m_price.value.length < 1){
			alert(language_data['product_input.js']['L'][language]);	//옵션별 회원가를 입력해주세요.
			frm.option_m_price.focus();
			return false;
		}
		
		if(frm.option_d_price.value.length < 1){
			alert(language_data['product_input.js']['M'][language]);	//옵션별 딜러가를 입력해주세요.
			frm.option_d_price.focus();
			return false;
		}
		
		if(frm.option_a_price.value.length < 1){
			alert(language_data['product_input.js']['N'][language]);	//옵션별 대리점가를 입력해주세요.
...
function num_check() {
        ...
        if ((keyCode < 48 || keyCode > 57) && keyCode != 8){
                alert(language_data['product_input.js']['O'][language]+"["+keyCode+"]")
...
//Base = i 이면 소숫점위의 자릿수에서, f 이면 소숫점아래의 자릿수에서 반올림
	...
	return Math.round(Num); 
	                ...
	                alert(language_data['product_input.js']['P'][language]); //"자릿수는 정수로만 구분합니다."
	                return false;
	 }
...
function commaSplit(srcNumber) {
	...
	if (isNaN(txtNumber) || txtNumber == "") {
		//alert(language_data['product_input.js']['Q'][language]); //"숫자만 입력 하세요"
		return 0;
	}
	else {
...
function copyPrice(frm, copy_price, step){
	...
	if(step == 1){
		if(copy_price == ""){
			alert(language_data['product_input.js']['R'][language]);	//'구매단가(공급가)를 입력후 복사 버튼을 클릭해주세요'
			return false;
		}
		...
		if(copy_price == ""){
			alert(language_data['product_input.js']['S'][language]);	//'정가를 입력후 복사 버튼을 클릭해주세요'
			return false;
		}
		frm.sellprice.value = copy_price;
		/*frm.prd_member_price.value = copy_price;
	...
	/*else if(step == 3){
		if(copy_price == ""){
			alert(language_data['product_input.js']['T'][language]);	//'회원가를 입력후 복사 버튼을 클릭해주세요'
			return false;
		}
		frm.prd_member_price.value = copy_price;
		frm.prd_dealer_price.value = copy_price;
		...
		if(copy_price == ""){
			alert(language_data['product_input.js']['U'][language]);	//'딜러가를 입력후 복사 버튼을 클릭해주세요'
			return false;
		}
		frm.prd_agent_price.value = copy_price;	
	}*/
