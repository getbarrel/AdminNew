...
$innerview .= "	
	...
	<tr>
		...
		<td valign=top style='padding:0px;padding-top:0px;' id=product_list>
			...
			<form name=listform method=post action='product_list.act.php' onsubmit='return CheckDelete(this)' target='iframe_act'>
...
$innerview .= "
			...
			<tr bgcolor='#ffffff' align=center height=30>
				...
				<td width='42%' class=m_td>".OrderByLink("상품정보", "pname", $ordertype)." </td>
				<td width='*' class=m_td>".OrderByLink("가격", "sellprice", $ordertype)."</td>
...
if(count($goods_datas) == 0){
	...
	//$script_time[loop_start] = time();
	//for ($i = 0; $i < $db->total; $i++){
	for ($i = 0; $i < count($goods_datas); $i++){

		//$db->fetch($i);

		//if(file_exists($_SERVER["DOCUMENT_ROOT"]."".PrintImage($admin_config[mall_data_root]."/images/product", $goods_datas[$i][id], "s", $db->dt)) || $image_hosting_type=='ftp'){
		...
		if($layout_config[mall_use_inventory] == "Y" && $admininfo["mall_type"] != "O"){
			$slave_db->query("select h.pi_ix,h.place_name from inventory_place_info h , shop_product p where h.pi_ix = p.inventory_info and id = '".$goods_datas[$i][id]."' ");
			if($slave_db->total){
				$slave_db->fetch();
...
$innerview .= "	<tr bgcolor='#ffffff'>
					<td class='list_box_td list_bg_gray' bgcolor='#efefef' align=center>
						<input type=checkbox class=nonborder id='cpid' name=cpid[] value='".$goods_datas[$i][id]."'>
					</td>
	...
	$innerview .= "<br><a href='/shop/goods_view.php?id=".$goods_datas[$i][id]."' target='_blank' class='screenshot'  rel='".PrintImage($admin_config[mall_data_root]."/images/product", $goods_datas[$i][id], $LargeImageSize, $goods_datas[$i])."'><img src='".$img_str."' width=50 height=50></a><br>";
	...
	$innerview .= "
							...
							<td style='padding:10px 0px 0px 10px'>
								...
								<tr>
									...
									<td align='center'>
											...
											<img src='../images/".$admininfo["language"]."/customer_icon.gif' align=absmiddle style='cursor:pointer;' title='문의' alt='문의'>
									</td>
									<td align='left'>
			...
			$innerview .= "
					...
					<td class='list_box_td list_bg_gray' bgcolor='#efefef' align=left style='padding:5px;' valign='top'>
...
$innerview .= "
							<tr>
								<td align=left style='padding:0px 0px 0px 5px'>소매가 / 할인가</td>
								<td>:</td>
								<td nowrap align=left colspan='4' align='left'> ".$currency_display[$admin_config["currency_unit"]]["front"]." 
...
$innerview .= "
							<tr>
								...
								<td align=left style='padding:0px 0px 0px 5px'><!--평균출고일--></td>
								<td><!--:--></td>
								<td align=left><!--b>".(AvergeDliveryDate($goods_datas[$i][id]) > 0?AvergeDliveryDate($goods_datas[$i][id]).' 일':'0 일')."</b--></td>
							</tr>
							<!--
							<tr>
					...
					<td class='list_box_td' style='padding:10px;' nowrap valign='top'>
						<table align=center style='padding:10px;text-align:left;' border=0>
							<tr>
								<td>
									...
									상품 MD : ".($goods_datas[$i][md_name] != ""?$goods_datas[$i][md_name]:'미지정')."<br><br> 
								</td>
							</tr>
							<tr>
								<td>
