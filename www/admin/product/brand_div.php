...
$sql = "select cr.* , case when depth = 1 then bd_ix  else parent_bd_ix end as group_order from shop_brand_div cr where bd_ix = '".$bd_ix."'   ";
$db->query($sql);
if($db->total){
	$db->fetch();
...
$Contents01 = "
	<table width='100%' cellpadding=0 cellspacing=0 border='0' align='left'>
	  ...
	  <col width='35%'>
	  <tr >
		<td align='left' colspan=6 style='padding-bottom:10px;'> ".GetTitleNavigation("브랜드 분류관리", "상품분류관리 > 브랜드 분류관리 ")."</td>
	  </tr>
	  <tr>
	  ...
	  <tr>
	    <td align='left' colspan=4 style='padding:3px 0px;'> ".colorCirCleBox("#efefef","100%","<div style='padding:5px 5px 5px 10px;'><img src='../image/title_head.gif' align=absmiddle> <b><u>$board_name</u> 브랜드 분류 추가/수정</b></div>")."</td>
	  </tr>
	  </table>
	  ...
	  <col width='*'>
	  <tr bgcolor=#ffffff >
	    <td class='input_box_title'> <b> 브랜드 분류 : </b></td>
	    <td class='input_box_item'>
	    	<input type=radio name='depth' value='1' id='depth_1' onclick=\"document.getElementById('parent_bd_ix').disabled=true;\" checked><label for='depth_1'>1차 브랜드 분류</label>
	    	<input type=radio name='depth' value='2' id='depth_2' onclick=\"document.getElementById('parent_bd_ix').disabled=false;\" ><label for='depth_2'>2차 브랜드 분류</label>
	    	".getFirstDIV($db->dt[bd_ix])." <span class='small'><!--2차 브랜드 분류 등록하기 위해서는 반드시 1차브랜드 분류를 선택하셔야 합니다.--> ".getTransDiscription(md5($_SERVER["PHP_SELF"]),'A')." </span>
	    </td>	  
	    <td class='input_box_title'> <b>브랜드 분류명 :</b> </td>
...
$Contents02 = "
	<div style='width:100%;'>".colorCirCleBox("#efefef","100%","<div style='padding:5px 5px 5px 10px;'><img src='../image/title_head.gif' align=absmiddle> <b><u>$board_name</u>  브랜드 분류 목록</b></div>")."</div>";
...
$sql = "select cr.* , case when depth = 1 then bd_ix  else parent_bd_ix end as group_order from shop_brand_div cr  order by  group_order asc ,depth asc , vieworder asc ";
$db->query($sql);
...
$Contents02 .= "
		    ...
		    <td class='list_box_td '>
		    	<!--a href=\"javascript:updateGroupInfo('".$db->dt[bd_ix]."','".$db->dt[div_name]."','".$db->dt[depth]."','".$db->dt[vieworder]."','".$db->dt[group_order]."','".$db->dt[disp]."')\"><img src='../images/".$admininfo["language"]."/btc_modify.gif' border=0></a-->
				<a href=\"?bd_ix=".$db->dt[bd_ix]."\"><img src='../images/".$admininfo["language"]."/btc_modify.gif' border=0></a>
	    		<a href=\"javascript:deleteGroupInfo('delete','".$db->dt[bd_ix]."')\"><img src='../images/".$admininfo["language"]."/btc_del.gif' border=0></a>
		    </td>
		  </tr> ";
	}
}else{
	$Contents02 .= "
		  <tr bgcolor=#ffffff height=50>
		    <td align=center colspan=6>등록된 브랜드 분류이 없습니다. </td>
		  </tr>";
}
$Contents02 .= "
...
$Contents = $Contents."<form name='brand_form' action='brand_div.act.php' method='post' onsubmit='return CheckValue(this)' enctype='multipart/form-data' target='act'>
<input name='mmode' type='hidden' value='$mmode'>
<input name='act' type='hidden' value='".$act."'>
<input name='bd_ix' type='hidden' value='".$bd_ix."'>";
...
$help_text = getTransDiscription(md5($_SERVER["PHP_SELF"]),'D');
	...
	$help_text = HelpBox("브랜드 분류관리", $help_text);
...
function deleteImage(imagetype, bd_ix){
 ...
 function CheckValue(frm){
 	if(frm.depth[1].checked){
 		if(frm.parent_bd_ix.value == ''){
	 		alert(language_data['region.php']['A'][language]);
			//'2차 브랜드 분류을 등록하기 위해서는 1차브랜드 분류을 반드시 선택하셔야 합니다.'
	 		return false;
 		}
 	}
 	...
 	if(frm.div_name.value.length < 1){
		...
		//'등록하시고자 하는 상품분류관리 브랜드 분류명을 입력해주세요'
 		frm.div_name.focus();
 		return false;
 	}

 }
 function updateGroupInfo(bd_ix,div_name,depth, vieworder,group_order,disp){
 	var frm = document.brand_form;

 	frm.act.value = 'update';
 	frm.bd_ix.value = bd_ix;
	...
	if(disp=='1') {
 	...
 	if(depth == '1'){
		...
		document.getElementById('parent_bd_ix').disabled=true;
		document.getElementById('parent_bd_ix').options[0].selected='selected';
 	}else{
 		frm.depth[1].checked = true;
		document.getElementById('parent_bd_ix').disabled=false;
		document.getElementById('parent_bd_ix').value=group_order;
 	}

}

 function deleteGroupInfo(act, bd_ix){
...
if($mmode == "pop"){
	...
	$P->Navigation = "상품관리 > 상품분류관리 > 브랜드 분류관리";
	$P->NaviTitle = "브랜드 분류관리";
...
CREATE TABLE IF NOT EXISTS `shop_brand_div` (
  `bd_ix` int(4) unsigned NOT NULL AUTO_INCREMENT COMMENT '인덱스',
  `parent_bd_ix` int(4) unsigned DEFAULT NULL COMMENT '상위인덱스값',
  ...
  PRIMARY KEY (`bd_ix`)
) ENGINE=MyISAM  DEFAULT CHARSET=utf8 COMMENT='브랜드 분류정보'
