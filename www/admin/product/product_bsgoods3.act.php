...
if($admininfo[company_id] == ""){
	...
	if($bs_site == "izabel"){
        ...
        $search_list_max_regxp = "|<li class=\"last\"><a class=\"standard\" href=\".*\">(.*)</a></li>|U";	
		$GoodsListPageName = "/c/";
		...
		$search_detail_regxp = "|<a href='(.*)'>|U";
		$search_list_max_regxp = "|javascript: document.getElementById\('page_no'\).value='(.*)';|U";
...
if($bs_act == "get_goods"){
	...
	if(!substr_count($list_url,$PageParamName)){
        if($bs_site == "toryburch" || $bs_site =="zappos" || $bs_site =="disneystore" || $bs_site == "toryburch" || $bs_site == "saksfifthavenue"){ }
        else{
	...
	*/
	...
	if(goods_detail_link.length > 0 && this_pagenum > 0){
		if(start <= this_pagenum &&  this_pagenum <= end ){
...
if($bs_act == "bsgoods_pcode_update" || $bs_act == "bsgoods_one_reg" || $bs_act == "bsgoods_one_update"  || $bs_act == "bsgoods_one_stock_update"){
			...
			if($bs_site == "barneys"){
                ...
                $__bs_url = split("[&]",$bs_url);
				$__pcode =str_replace("productId=","",$__bs_url[1]);
				$__pcode=explode("_",$__pcode);
				$pcode=$__pcode[0]."_".$__pcode[1];
				//print_r($pcode);
            }else if($bs_site == "toryburch"){
								...
								if($options[$i]["option_name"]){
									...
									for($j=0;$j < count($options[$i]["details"]);$j++){
										if($options[$i][details][$j][option_div]){
                                                /**
                                                 * 업데이트일때 가격재고 옵션의 환율적용 12.4.16 배광호
                                                 */
												 /*
                                                $bs_fee_array = BuyingServicePriceCalcurate($options[$i][details][$j][price],$buying_service_currencyinfo);
                                                $round_result_array = PriceRoundUpOrDown($round_type,$round_precision,$bs_fee_array[buyingservice_coprice],$bs_fee_array[bs_fee]);
                                                $options[$i][details][$j][price] = $round_result_array[sellprice];
                                                */
