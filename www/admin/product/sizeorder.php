...
function getlevel($cid,$depth){
	...
	$levelnum = $depth+1;
	$sql = "select vlevel$levelnum as level from shop_goods_size_info where depth = $depth and cid = '$cid'";
	$db->query($sql);
	$db->fetch(0);
...
function getTargetCid($depth,$cid,$select_level,$num){
	...
	$sql = "select cid from shop_goods_size_info where depth = $depth and vlevel$levelnum = '".($select_level+$num)."' and cid LIKE '".substr($cid,0,$sPos)."%'";
	$db->query($sql);
	if ($db->total){
		$db->fetch(0);
		return $db->dt[cid];
...
function Pluslevel($cid,$depth,$selectlevel,$target_cid)
{
	...
	if ($depth == 0){
		$sql = "UPDATE shop_goods_size_info SET vlevel$levelnum = '".($selectlevel)."' where vlevel$levelnum = '".($selectlevel-1)."' and cid LIKE '".substr($target_cid,0,$sPos)."%'";
		$db->query($sql);
	}else{
		$sql = "UPDATE shop_goods_size_info SET vlevel$levelnum = '".($selectlevel)."' where vlevel$levelnum = '".($selectlevel-1)."' and cid LIKE '".substr($target_cid,0,$sPos)."%'";
		$db->query($sql);
	...
	$sql = "UPDATE shop_goods_size_info SET vlevel$levelnum = '".($selectlevel - 1)."' where vlevel$levelnum = '$selectlevel'  and cid LIKE '".substr($cid,0,$sPos)."%'";
