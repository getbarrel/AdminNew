	...
	*/
	...
	if ($db->total == 0){
		...
		if($disp_type == "clipart"){
			...
			for($i=0;$i<$db->total;$i++){
				$db->fetch($i);
				$mString .= 'ms_productSearch._setProduct("productSetList_'.$group_code.'", "M", "'.$db->dt['id'].'", "'.PrintImage($admin_config[mall_data_root]."/images/product", $db->dt['id'], "c").'", "'.addslashes(addslashes(trim($db->dt['pname']))).'", "'.addslashes(addslashes(trim($db->dt['brand_name']))).'", "'.$db->dt['sellprice'].'");'."\n";
	...
	if ($db->total == 0){
		...
		if($disp_type == "clipart"){
			...
			for($i=0;$i<$db->total;$i++){
				...
				$mString .= 'ms_productSearch._setProduct("productList_1", "M", "'.$db->dt['id'].'", "'.PrintImage($admin_config[mall_data_root]."/images/product", $db->dt['id'], "c").'", "'.addslashes(addslashes(trim($db->dt['pname']))).'", "'.addslashes(addslashes(trim($db->dt['brand_name']))).'", "'.$db->dt['sellprice'].'","", "", "", "", "", "'.$db->dt[disp].'", "'.$db->dt[state].'");'."\n";
    ...
    if ($db->total == 0){
        ...
        if($disp_type == "clipart"){
            ...
            for($i=0;$i<$db->total;$i++){
                $db->fetch($i);
                //function(obj, mode, pID, imgSrc, pName, bName, pPrice, listprice, reserve,coprice,wholesale_price,wholesale_sellprice, disp, state, dcprice, vieworder, viewcnt, regdate)
                $mString .= 'ms_productSearch._setProduct("productList_2", "M", "'.$db->dt['id'].'", "'.PrintImage($admin_config[mall_data_root]."/images/product", $db->dt['id'], "c").'", "'.addslashes(addslashes(trim($db->dt['pname']))).'", "'.addslashes(addslashes(trim($db->dt['brand_name']))).'", "'.$db->dt['sellprice'].'","", "", "", "", "", "'.$db->dt[disp].'", "'.$db->dt[state].'");'."\n";
    ...
    if ($db->total == 0){
        ...
        if($disp_type == "clipart"){
            ...
            for($i=0;$i<$db->total;$i++){
                $db->fetch($i);
                //function(obj, mode, pID, imgSrc, pName, bName, pPrice, listprice, reserve,coprice,wholesale_price,wholesale_sellprice, disp, state, dcprice, vieworder, viewcnt, regdate)
                $mString .= 'ms_productSearch._setProduct("productList_3", "M", "'.$db->dt['id'].'", "'.PrintImage($admin_config[mall_data_root]."/images/product", $db->dt['id'], "c").'", "'.addslashes(addslashes(trim($db->dt['pname']))).'", "'.addslashes(addslashes(trim($db->dt['brand_name']))).'", "'.$db->dt['sellprice'].'","", "", "", "", "", "'.$db->dt[disp].'", "'.$db->dt[state].'");'."\n";
    ...
    if ($db->total == 0){
        ...
        if($disp_type == "clipart"){
            ...
            for($i=0;$i<$db->total;$i++){
                $db->fetch($i);
                //function(obj, mode, pID, imgSrc, pName, bName, pPrice, listprice, reserve,coprice,wholesale_price,wholesale_sellprice, disp, state, dcprice, vieworder, viewcnt, regdate)
                $mString .= 'ms_productSearch._setProduct("productList_4", "M", "'.$db->dt['id'].'", "'.PrintImage($admin_config[mall_data_root]."/images/product", $db->dt['id'], "c").'", "'.addslashes(addslashes(trim($db->dt['pname']))).'", "'.addslashes(addslashes(trim($db->dt['brand_name']))).'", "'.$db->dt['sellprice'].'","", "", "", "", "", "'.$db->dt[disp].'", "'.$db->dt[state].'");'."\n";
	...
	*/
	...
	if ($db->total == 0){
		...
		if($disp_type == "clipart"){
			...
			for($i=0;$i<$db->total;$i++){
				$db->fetch($i);
				$mString .= 'ms_productSearch._setProduct("productList_2", "M", "'.$db->dt['id'].'", "'.PrintImage($admin_config[mall_data_root]."/images/product", $db->dt['id'], "c").'", "'.str_replace("\"","&quot;",trim($db->dt['pname'])).'", "'.str_replace("\"","&quot;",trim($db->dt['brand_name'])).'", "'.$db->dt['sellprice'].'");'."\n";
    ...
    if ($db->total == 0){
        ...
        if($disp_type == "clipart"){
            ...
            for($i=0;$i<$db->total;$i++){
                $db->fetch($i);
                //function(obj, mode, pID, imgSrc, pName, bName, pPrice, listprice, reserve,coprice,wholesale_price,wholesale_sellprice, disp, state, dcprice, vieworder, viewcnt, regdate)
                $mString .= 'ms_productSearch._setProduct("gift_1", "M", "'.$db->dt['id'].'", "'.PrintImage($admin_config[mall_data_root]."/images/product", $db->dt['id'], "c").'", "'.addslashes(addslashes(trim($db->dt['pname']))).'", "'.addslashes(addslashes(trim($db->dt['brand_name']))).'", "'.$db->dt['sellprice'].'","", "", "", "", "", "'.$db->dt[disp].'", "'.$db->dt[state].'","","","","","'.$db->dt[one_commission].'","'.$db->dt[product_type].'","'.$db->dt[gid].'");'."\n";
            }
            $mString .= '</script>'."\n";
        }
    }
