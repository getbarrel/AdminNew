...
$fp = fopen($_SERVER["DOCUMENT_ROOT"]. $this->mall_data_root . '/sellertool/gsshop/log/API_LOG_' . date('Ym') . '.log', 'a');
$fh = $result.'
';
fwrite($fp, $fh);
fclose($fp);
	...
	private function makeAddItemArray($pid = '', $add_info_id = '' , $modGbn = '', $prdCd = ''){
		...
		if($pinfo['state'] != '1' || $pinfo['disp'] != '1' || $pinfo['is_sell_date']=='1'){
			$saleEndDtm = date('YmdHis',strtotime('+1 minutes'));

			if( $pinfo['is_sell_date']=='1' && $regGbn == 'U' ){// 수정일때
				$saleStrDtm = substr($saleStrDtm,0,-2) . rand(10,59);
			}
		}
