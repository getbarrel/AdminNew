...
$db->query("select count(*) as total from ".TBL_SHOP_COMPANY."  where disp='1' $search_str ");
$db->fetch();

$total = $db->dt[total];


$sql = "select * from ".TBL_SHOP_COMPANY." where disp='1' $search_str order by company_name limit $start, $max";
//echo $sql;
$db->query($sql);
...
<TABLE cellSpacing=0 cellPadding=0 width='100%' align=center border=0>
	<TR>
		...
		<table border='0' width='100%' cellpadding='0' cellspacing='0' align='center'>
					...
					<table width='100%' border='0' cellspacing='0' cellpadding='0' >
						<tr>
							<td width='10%' height='31' valign='middle' style='font-family:굴림;font-size:16px;font-weight:bold;letter-spacing:-2px;word-spacing:0px;padding-left:10px;' nowrap>
								<img src='/admin/images/icon/sub_title_dot.gif' align=absmiddle> 제조사 검색
							</td>
			...
			<tr >
				<td align='left' colspan=2> ".GetTitleNavigation("제조사 검색", "제조사 검색", false)."</td>
			</tr>
									...
									<tr height='40' valign='middle'>
										<td align='center'  colspan=2 class='input_box_title'><b>원산지검색</b>
											<select name='search_type'>
												...
												<option value='c_ix'> 제조사 코드(key)</option>
											</select>
										</td>
	...
	<tr height=30>
		<td class='p11 ls1' style='padding:0 0 0 5px;' nowrap> ".($search_text == "" ? "제조사 전체 목록입니다.":"'".$search_text ."' 로 검색된 결과 입니다.")."</td>
	...
	<tr><form name='send_mail' method='post' action='sms.act.php' onsubmit='return CheckSMS(this);' ><input type=hidden name='act' value='send_mail' >
		...
		<tr height='28' bgcolor='#ffffff'>
			<td width='20%' align='center' class='m_td'><font color='#000000'><b>제조사 코드</b></font></td>
			<td width='*' align='center' class=m_td><font color='#000000'><b>제조사 명</b></font></td>
...
if($db->total){
	for($i=0;$i  < $db->total; $i++){
		...
		$Contents .= "<tr height=25 style='text-align:center;cursor:pointer;' >
								<td class='list_box_td '>".$db->dt[c_ix]."</td>
								<td class='list_box_td point' onclick=\"SelectBrand('".$db->dt[c_ix]."','".$db->dt[company_name]."');\">".$db->dt[company_name]."</td>
