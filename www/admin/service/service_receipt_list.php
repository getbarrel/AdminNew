...
$mstring ="
		...
		<tr>
			<td align='left' colspan=6 > ".GetTitleNavigation("현금영수증 발행내역", "서비스 관리 > 현금영수증 발행내역")."</td>
		</tr>
		<tr>
			<td>
...
function deleteReceipt(oid){
	if(confirm(language_data['receipt_list.php']['A'][language])){//현금영수증 신청을 삭제 하시겠습니까?
		window.frames['act'].location.href='service_receipt.act.php?oid='+oid;
	}
}
</script>
";
...
function PrintEventList(){
	...
	global $search_type, $search_text,$receipt_yn,$FromYY,$FromMM,$FromDD,$ToYY,$ToMM,$ToDD,$orderby,$ordertype;
	...
	$sql = "select o.oid, count(*) as order_detail_cnt ,
		...
		from ".TBL_SHOP_ORDER." o, ".TBL_COMMON_USER." m , ".TBL_COMMON_MEMBER_DETAIL." md , service_order_detail od
		where receipt_y = 'Y' and taxsheet_yn not in ('Y','C') and o.oid = od.oid
		and o.code = m.code AND m.code=md.code  $where $where1
	...
	$sql = "select o.oid, count(*) as order_detail_cnt ,
		...
		sum(case when od.status in ('CC','RC','FC') then 1 else 0 end) as cancel_cnt,
		sum(case when od.status in ('IR','IC','DR','DI','EA','EI','ED','EC','FA','RA','RI','RD','CA') then 1 else 0 end) as ing_cnt
		from service_order o,service_receipt b, ".TBL_COMMON_USER." m , ".TBL_COMMON_MEMBER_DETAIL." md , service_order_detail od
	...
	//	$sql = "select a.*,b.*,m.code,m.name from receipt b left join receipt_result a on b.order_no = a.oid , common_member_detail cmd where b.id = m.id $where $orderby_str limit $start , $max";
		...
		sum(case when od.status in ('IC') then 1 else 0 end) as complet_cnt,
		sum(case when od.status in ('CC','RC','FC') then 1 else 0 end) as cancel_cnt,
		sum(case when od.status in ('IR','DR','DI','EA','EI','ED','FA','RA','RI','RD','CA') then 1 else 0 end) as ing_cnt
		from service_order o, service_receipt b, ".TBL_COMMON_USER." m , ".TBL_COMMON_MEMBER_DETAIL." md , service_order_detail od
		where o.oid = b.order_no and o.oid = od.oid
		and o.code = m.code AND m.code=md.code
		$where $where1
		group by o.oid having cancel_cnt != order_detail_cnt
		...
		for($i=0;$i < $db->total;$i++){
			...
			$sql = "select * from service_receipt_result where oid = '".$db->dt[oid]."'";
			$db2->query($sql);
			if($db2->total){
				$db2->fetch();
				...
				$sql = "select * from service_receipt where order_no = '".$db->dt[oid]."' ";
				$db2->query($sql);
				$db2->fetch();
			...
			<td class='list_box_td' ><a href='service_orders.edit.php?oid=".$db->dt[oid]."' target='_blank' title='클릭:주문내역보기'><font color='#3333FF'>".$db->dt[order_detail_cnt]."</font></a></td>
			<td class='list_box_td list_bg_gray' >".$db->dt[cancel_cnt]."</td>
			<td class='list_box_td' >".($db->dt[complet_cnt]-$db->dt[cancel_cnt])."</td>
			<td class='list_box_td list_bg_gray' ><a href='service_orders.edit.php?oid=".$db->dt[oid]."' target='_blank' title='클릭:주문내역보기'><font color='#FF0033'>".$db->dt[ing_cnt]."</font></a></td>
			...
			if($auth_code == ""){
				if($db->dt[ing_cnt] > 0){
					$mString = $mString."<img src='../images/".$admininfo["language"]."/btn_auth_ok.gif' onclick=\"alert(language_data['receipt_list.php']['B']['".$admininfo["language"]."']);\" style='cursor:pointer'> <img src='../images/".$admininfo["language"]."/btc_del.gif' border=0 onclick=\"deleteReceipt('".$db->dt[oid]."')\" style='cursor:pointer'>";//'아직 주문완료 처리되지 않은 상품이 있습니다. 확인후 주문완료 처리후 현금영수증을 발행하실수 있습니다.'
				}else{
					$mString = $mString."<a href=\"javascript:PoPWindow('service_receipt_apply.php?oid=".$db->dt[oid]."','660','440','service_receipt_apply')\"><img src='../images/".$admininfo["language"]."/btn_auth_ok.gif' align=absmiddle></a> <img src='../images/".$admininfo["language"]."/btc_del.gif' border=0 onclick=\"deleteReceipt('".$db->dt[oid]."')\" style='cursor:pointer' align=absmiddle>";
				}
			}else{
				$mString = $mString."<a href=\"javascript:PoPWindow('/admin/service/service_receipt_view.php?oid=".$db->dt[oid]."','500','370','service_receipt_view')\"><img src='../images/".$admininfo["language"]."/btn_auth_com.gif ' align=absmiddle></a> <a href=\"javascript:PoPWindow('service_receipt_mod.php?oid=".$db->dt[oid]."','560','440','')\"><img src='../images/".$admininfo["language"]."/btc_cancle.gif' align=absmiddle></a>";
			}
