...
<tr>
    <td align='left'> ".GetTitleNavigation("서비스주문정보수정", "서비스관리 > 서비스주문정보수정 ")."</td>
		...
		$sql = "SELECT status FROM service_mall_order_detail od WHERE od.oid = '".$oid."' ";
		$db2->query($sql);
		//$db2->fetchall();
		...
		if($admininfo[admin_level] == 9 || $admininfo[admin_level] == 8){
				...
				$sql = "SELECT cu.id,o.*,UNIX_TIMESTAMP(o.date) AS date,(select sum(delivery_price) from shop_order_delivery where o.oid = oid and delivery_pay_type in ('1','3') group by oid) as delivery_price1,(select sum(delivery_price) from shop_order_delivery where o.oid = oid and delivery_pay_type = 2 group by oid) as delivery_price2
								FROM service_mall_order o left join ".TBL_COMMON_USER." cu on o.uid=cu.code , ".TBL_COMMON_MEMBER_DETAIL." cmd
								WHERE o.oid = '".$oid."' and cu.code = cmd.code ";
				*/
				// 회원이나 상품 테이블을 조인하면 안됨... 2011.09.13
					$sql = "SELECT o.*, cmd.*, UNIX_TIMESTAMP(o.date) AS date
								FROM service_mall_order o 
								left join ".TBL_COMMON_COMPANY_DETAIL." cmd on o.company_id = cmd.company_id
								WHERE o.oid = '".$oid."' ";
		}
...
$Contents = $Contents."
      ...
      <div id='TG_order_edit' style='position: relative;width:100%;'>
		<form name='order_info_edit' method='post' onSubmit='return CheckFormValue(this)'  action='service_mall_orders.act.php' >
						...
						<tr>
							<td >
								...
								<col width='35%' />
									...
									<tr bgcolor='#ffffff' >
										<td class='input_box_title' >주문자업체(쇼핑몰)</td>
										<td class='input_box_item'>&nbsp;<input type='text' size=25 name='com_name' class='textbox' value='".$db2->dt[com_name]."' validation='true' title='주문자이름' ></td>
									...
									<tr>
										...
										<td class='input_box_item' colspan=3 >&nbsp;<input type='text' size='12' maxlength=8 name='bank_input_date' class='textbox' value='".$db2->dt[bank_input_date]."' onkeyup='onlyEditableNumber(this)' style='ime-mode:disabled;' validation='false' title='입금확인일자'> 예) 20090806</td>
									</tr>
									<tr >
										...
										if($admininfo[admin_level] == 9){
											$sql = "select os.regdate, os.status, os.status_message, od.pname
															from service_mall_order_status os
															left join service_mall_order_detail od on os.pid = od.pid and od.oid ='$oid'
											...
											$sql = "select os.regdate, os.status, os.status_message, od.pname
															from service_mall_order_status os, service_mall_order_detail od
															where  os.oid ='$oid' and os.status not in ('SR')
															union
...
".OrderGoodsList($db2, $oid, "cancel")."
<div style='height:20px;'></div>
		<table border='0' cellspacing='1' cellpadding='15' width='100%' bgcolor='#F8F9FA' bordercolor='#black'>
			<tr>
			  <td  style='padding:10px;'>
				<form name=order_memo_frm method=post action='service_mall_orders_memo.act.php'  target='iframe_act'>
...
function memoDelete(oid, om_ix){
	if(confirm(language_data['service_mall_orders.edit.php']['A'][language])){//해당 상담내역을 정말로 삭제 하시겠습니까?
		window.frames['iframe_act'].location.href='service_mall_orders_memo.act.php?act=memo_delete&oid='+oid+'&om_ix='+om_ix;
	}
}
...
$P->addScript = $Script."<script language='javascript' src='service_mall_orders.js'></script>";
...
function OrderGoodsList($db2, $oid, $list_type = "general"){
	...
	if($admininfo[admin_level] == 9){//quick, ptprice, od.option_text, po.option_etc1,invoice_no, od.status , od.coprice,od.delivery_type, od.ac_date, od.ac_ix, od.dc_date
		...
		$sql = "SELECT od.od_ix,od.pid, od.pname,  psprice, ptprice, od.status ,od.coprice FROM service_mall_order_detail od WHERE od.oid = '".$oid."' $addWhere ";
	}else if($admininfo[admin_level] == 8){
		/*
		$sql = "SELECT od.od_ix,od.pid,od.product_type, od.pname, od.reserve, pcnt, psprice, ptprice, od.option_text, od.option_etc, od.status , od.use_coupon, od.use_coupon_code,
						...
						FROM service_mall_order_detail od LEFT JOIN shop_order_detail_deliveryinfo odd ON od.od_ix=odd.od_ix
...
$Contents .= "
<form name='order_edit_".$list_type."' method='post' onSubmit=\"return orderStatusUpdate(this)\"  action='service_mall_orders.act.php' target='act'><!--target='act'-->
<input type=hidden name=oid value='$oid'>
...
function PrintOrderMemo($oid){
	...
	$sql = "select count(*) as total from service_mall_order_memo where oid ='$oid'    ";
	$mdb->query($sql);
	$mdb->fetch();
	$total = $mdb->dt[total];
	...
	$mString = $mString."
				<form name=listform method=post action='service_mall_orders.act.php' onsubmit='return CheckDelete(this)' target='iframe_act'>
				<input type='hidden' name='act' value='memo_insert'>
				<input type='hidden' name='oid' value='$oid'>
				<col width='15%'>
				<col width='10%'>
	...
	if ($total == 0){
		...
		$mdb->query("select * from service_mall_order_memo where oid ='$oid' order by regdate desc   limit $start , $max");
...
function printStatusInfo($oid){
	...
	$mdb->query("select * from service_mall_order_status where oid = '$oid' ");
