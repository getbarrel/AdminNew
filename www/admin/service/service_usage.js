...
function init_date_edit(FromDate,ToDate) {
	var frm = document.order_info_edit;
...
function onLoadEdit(FromDate, ToDate, frm) {
	if(!frm){
		var frm = document.order_info_edit;
	}
	...
	init_date_edit(FromDate,ToDate);
...
function act(act, si_ix)
{
	...
	if (act == "update")
	{
		...
		form.action = 'service_usage.act.php?act='+act+'&si_ix='+si_ix;
		form.submit();
	}
	...
	if (act == "delete")
	{
		if(confirm(language_data['orders.js']['I'][language])){//카테고리를 선택해주세요
			window.frames["iframe_act"].location.href= 'service_usage.act.php?act='+act+'&si_ix='+si_ix;
		}
	}
}
...
function order_delete(act, oid){
	if (act == "delete")
	{
		if(confirm(language_data['orders.js']['B'][language])){//[카드결제]의 경우는 승인취소후 삭제해주세요. 해당 주문을  정말로 삭제하시겠습니까?
			window.frames["iframe_act"].location.href= 'service_usage.act.php?act='+act+'&oid='+oid;
		}
	}
}
