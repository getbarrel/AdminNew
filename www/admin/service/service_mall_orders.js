...
function ReturnOK(oid){
	...
	if(confirm(language_data['orders.js']['A'][language])){//정말로 반품 처리 하시겠습니까?
		document.location.href='service_mall_orders.act.php?act=stateupdate&oid='+oid;
	}else{
...
function PrintMemberOrder(oid){
	PrintWindow("./service_mall_orders.print.php?oid="+oid,700,430,'OrderPrint');		
}

function PrintDealingsSheet(oid){
	PrintWindow("./service_mall_orders.dealings.php?oid="+oid,700,430,'OrderPrint');		
}

function PrintOrderDetail(oid){
	PrintWindow("./service_mall_orders.view.php?oid="+oid,700,430,'OrderPrint');		
}
...
function act(act, oid)
{
	...
	if (act == "update")
	{
		...
		form.action = 'service_mall_orders.act.php?act='+act+'&oid='+oid;
		form.submit();
	}
	...
	if (act == "delete")
	{
		if(confirm(language_data['orders.js']['I'][language])){//카테고리를 선택해주세요
			window.frames["iframe_act"].location.href= 'service_mall_orders.act.php?act='+act+'&oid='+oid;
		}
	}
}
