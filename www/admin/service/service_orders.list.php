...
//	$sDate = date("Y/m/d");
	...
	$sDate2 = $FromYY."/".$FromMM."/".$FromDD;
	$eDate2 = $ToYY."/".$ToMM."/".$ToDD;
	$startDate2 = $FromYY.$FromMM.$FromDD;
	$endDate2 = $ToYY.$ToMM.$ToDD;
...
if ($FromYY != "")	$where .= "and date_format(od.end_date,'%Y%m%d') between $startDate2 and $endDate2 ";

if($search_type && $search_text){
		if($search_type == "combi_name"){
			$where .= "and (bname LIKE '%".trim($search_text)."%'  or bank_input_name LIKE '%".trim($search_text)."%') ";	//service_order 에는 or rname LIKE '%".trim($search_text)."%' 없어서 삭제했음 2014-03-28 이학봉
		}else{
...
if($parent_service_code) {
	$where .= "and od.s_kind = '$parent_service_code' ";
	if($service_code) {
		$where .= "and od.s_type = '$service_code' ";
	}
}
...
<table width='100%' cellpadding='0' cellspacing='0' border=0>
	<tr>
		<td align='left' colspan=6 > ".GetTitleNavigation($title_str, "서비스 주문관리 > $title_str ")."</td>
	</tr>
	<!--tr>
...
$Contents .= "
												...
												<td class='search_box_item' colspan=3>
													...
													<tr>
														...
														<select name='search_type' style='font-size:11px;'>
															...
															<!--기존 검색 조건인데 없는 컬럼값들이 있어서 에러뜸. 없는 컬럼검색 조건은 일단 삭제했음 2014-03-28 이학봉
															<option value='combi_name' ".CompareReturnValue('combi_name',$search_type,' selected').">주문자이름+입금자명+수취인명</option>
															<option value='bname' ".CompareReturnValue('bname',$search_type,' selected').">주문자이름</option>
															<option value='pname' ".CompareReturnValue('pname',$search_type,' selected').">상품이름</option>
															<option value='od.oid' ".CompareReturnValue('od.oid',$search_type,' selected').">주문번호</option>
				...
				$v3monthago = date("Y/m/d",mktime(0,0,0,substr($vdate,4,2)-3,substr($vdate,6,2)+1,substr($vdate,0,4)));
											...
											<tr height=33>
												<th class='search_box_title' >
												서비스만료일
												<input type='checkbox' name='enddate' id='enddate' value='1' onclick='ChangeEndDate(document.search_frm);' ".CompareReturnValue('1',$enddate,' checked')."></th>
												<td class='search_box_item'  colspan=3>
													<table cellpadding=3  cellspacing=1 border=0 bgcolor=#ffffff>
														<tr>
...
<form name=listform method=post action='service_orders.act.php' onsubmit='return CheckStatusUpdate(this)' target='act'><!--target='act'-->
...
<table width='100%' border='0' cellpadding='3' cellspacing='0' align='center' >
 <tr>";
	...
	$sql = "SELECT o.oid, o.payment_price, o.code
					FROM service_order o LEFT JOIN service_order_detail od ON o.oid=od.oid
					$where
...
$Contents .= " <a href='service_orders_excel2003.php?".$QUERY_STRING."'><img src='../images/".$admininfo["language"]."/btn_excel_save.gif' border=0></a>&nbsp;&nbsp;";
/*
}else if($admininfo[admin_level] == 8){
...
$Contents .= "<a href='service_orders.excel.php?".$QUERY_STRING."'><img src='../images/".$admininfo["language"]."/btn_excel_save.gif' border=0></a>&nbsp;&nbsp;";
...
$Contents .= "
  ...
  <table width='100%' border='0' cellpadding='3' cellspacing='0' align='center' class='list_table_box'>
	<tr height='25' >
		...
		<td width='7%' align='center'  class='m_td' nowrap><font color='#000000' class=small><b>주문시회원그룹</b></font></td>
		...
		<td width='7%' align='center' class='m_td' nowrap><font color='#000000' class=small><b>서비스만료일</b></font></td>
...
if($db1->total){
	for ($i = 0; $i < $db1->total; $i++)
	{
		...
		if($admininfo[admin_level] == 9){
			...
			$sql = "SELECT o.oid, o.code, o.mem_group, od.pname, od.regdate, od.psprice, od.ptprice,od.pid, bname,
						tid, od.status, method, total_price, UNIX_TIMESTAMP(date) AS date,receipt_y, od.end_date
						FROM service_order o, service_order_detail od
	...
	for($j=0;$j < $ddb->total;$j++){
		...
		if($ddb->dt[oid] != $b_oid){
			...
			$Contents .= $ddb->dt[regdate]."<br>
											<a href=\"service_orders.edit.php?oid=".$ddb->dt[oid]."&pid=".$ddb->dt[pid]."\" style='color:#007DB7;font-weight:bold;' class='small'>".$ddb->dt[oid]."</a><br>

										</td>
								<td ".($ddb->dt[oid] != $b_oid ? "rowspan='".($od_count+$od_count_plus)."'":"")." align='center'  nowrap>".$ddb->dt[mem_group]."</td>";
			$Contents .= "<td ".($ddb->dt[oid] != $b_oid ? "rowspan='".($od_count+$od_count_plus)."'":"")." style='line-height:140%' align=center >
			".($ddb->dt[code] != "" ? "<a href=\"javascript:PopSWindow('../member/member_view.php?code=".$ddb->dt[code]."',950,500,'member_info')\" >".$ddb->dt[bname]."</a>":$ddb->dt[bname])."<br>
		...
		$Contents .= "
			...
			$Contents .= cut_str($ddb->dt[pname],30)."<br><b>".$ddb->dt[option_text]."</b>
						...
						<td class=dot-x align=center>".substr($ddb->dt[end_date],0,10)."</td>
		...
		if($ddb->dt[oid] != $b_oid){
			...
			if(checkMenuAuth(md5($_SERVER["PHP_SELF"]),"U")){
				$Contents .= "<a href=\"service_orders.edit.php?oid=".$ddb->dt[oid]."&pid=".$ddb->dt[pid]."\"><img src='../images/".$admininfo["language"]."/btn_invoice.gif' border=0 align=absmiddle style='margin:3px;'><!--btc_modify.gif--></a> ";
			}else{
		...
		if(($od_count-1) == $j && $admininfo[admin_level] == 9){
			$Contents .= "<tr >
							...
							<td class=dot-x  style='background-color:#efefef;height:30px;font-weight:bold;padding-left:5px;'  colspan=5>
							<span>현금영수증 : ".$receipt_y."</span>
							</td>
							<td class=dot-x  style='background-color:#efefef;height:30px;font-weight:bold;'  colspan=3 align='right'>
...
$Contents .= HelpBox("서비스주문리스트", $help_text);
$P = new LayOut();
$P->OnloadFunction = "onLoad('$sDate','$eDate','$sDate2','$eDate2');ChangeOrderDate(document.search_frm);ChangeEndDate(document.search_frm);";//MenuHidden(false);
$P->addScript = "<script language='javascript' src='service_orders.js'></script>\n<script language='javascript' src='../include/DateSelect.js'></script>\n".$Script;
$P->strLeftMenu = service_menu();
$P->Navigation = "서비스관리 > 서비스주문리스트";
$P->title = "서비스주문리스트";
...
function OrderSummary($status, $title){
	...
	$sql = "select '구분      ','입금예정 ', '입금예정금액','입금확인건수(카드결제 미포함)', '입금확인금액 '
			...
			from service_order_detail where regdate between '".date("Y-m-d 00:00:00")."' AND '".date("Y-m-d 23:59:59")."'
			union
			Select '최근1개월',
			IFNULL(sum(case when status = '".ORDER_STATUS_INCOM_READY."'  then 1 else '0' end),0) as incom_ready_cnt,
			IFNULL(sum(case when status in ('".ORDER_STATUS_INCOM_READY."') then ptprice else '0' end),0) as incom_ready_price,
			IFNULL(sum(case when status = '".ORDER_STATUS_INCOM_COMPLETE."'  then 1 else '0' end),0) as incom_complete_cnt,
			IFNULL(sum(case when status in ('".ORDER_STATUS_INCOM_COMPLETE."')  then ptprice else '0' end),0) as incom_complete_price
			from service_order_detail where regdate between '".date("Y-m-d 00:00:00",mktime(0,0,0,substr($vdate,4,2),substr($vdate,6,2)-30,substr($vdate,0,4)))."' and '".date("Y-m-d 23:59:59")."' ";
	}else if($status == "CA"){
