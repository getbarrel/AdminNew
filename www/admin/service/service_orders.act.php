...
if ($act == "orderinfo_update"){
	...
	if($admininfo[admin_level] == 9){
		$sql = "UPDATE service_order SET bank_input_date='$bank_input_date'
						WHERE oid='$oid'";
		$db->query($sql);
	}else{
		$sql = "UPDATE service_order SET bank_input_date='$bank_input_date'
	...
	if($admininfo[admin_level] == 9){
		...
		//if($bstatus != $status){ // 현재상태정보와 변경상태가 틀릴경우만 상태변경정보 추가
			...
			if($status == ORDER_STATUS_INCOM_READY || $status == ORDER_STATUS_INCOM_COMPLETE){
				if($status == ORDER_STATUS_INCOM_COMPLETE){
					$db->query("select oid, code, bank_input_date from service_order  WHERE oid='".$oid."'");
					$db->fetch();
					$order = $db->dt;
					
					if($order[bank_input_date] == ""){
						$db->query("update service_order set bank_input_date = '".date("Ymd")."' WHERE oid='".$oid."'");
						$db->fetch();
					}

					$sql = "select oid, status from service_order_detail od WHERE oid='".$oid."'";
					//echo $sql;
					$db->query($sql);
			...
			if ($status == ORDER_STATUS_DELIVERY_ING){
				...
				$db->query("select * from service_order  WHERE oid='".$oid."'");
				$order = $db->fetch();

				$db->query("select * from service_order_detail WHERE oid='$oid'");
				...
				*/
				$db->query("select * from service_order_detail WHERE oid='$oid'");
				//echo "select * from service_order_detail WHERE oid='$oid'";
				$order_details = $db->fetchall();
				...
				//[Start]날짜를 롤백해준다. 또는 그룹을 변경시켜준다.
				//1번 프로세스는 없어도 될거 같다...
				/*
				//1. service_order_detail의 apply_type이 N 이면 신규 프리미엄회원이다. 이때에는 그룹을 변경해준다.
				$apply_type = $order_details[0][apply_type];
				if($apply_type == "N")
				{
					//1.1 그룹 변경
					$sql="UPDATE ".TBL_COMMON_MEMBER_DETAIL." SET gp_ix='1' WHERE code='".$code."' ";
					$db->query($sql);

					//1.2 service_info 의 정보를 수정.
					$sql = "UPDATE service_info SET end_date = '".date("Y-m-d H:i:s")."' WHERE code='".$code."' ";
					$db->query($sql);
				...
				{*/
					//2.apply_type == 'E' 때는 연속결제 또는 중복 결제 이므로 조건을 봐서 결정해야한다.
					
					//현재 시점이 가장 최근에 구매한 날짜 사이에 있는지 확인한다.
					if( (strtotime(date("Y-m-d H:i:s")) - strtotime($order_details[0][start_date])) >= 0 && (strtotime(date("Y-m-d H:i:s")) - strtotime($order_details[0][end_date])) <= 0 )
					{
						//2.1 취소시점이 프리미엄 기간에 들어가 있는 경우
												
						//2.1.1 그룹 변경
						$sql="UPDATE ".TBL_COMMON_MEMBER_DETAIL." SET gp_ix='1' WHERE code='".$code."' ";
						$db->query($sql);

						//2.1.2 service_info 의 정보를 수정.
						$sql = "UPDATE service_info SET end_date = '".date("Y-m-d H:i:s")."' WHERE code='".$code."' ";
						$db->query($sql);

						//2.1.3 상태정보변경
						$STATUS_MESSAGE = "관리자 - 취소 완료(회원등급변경 [프리미엄회원]->[일반회원])";
						$db->query("insert into service_order_status (os_ix, oid, pid, status, status_message, regdate ) VALUES 								('','".$oid."','".$order_details[0][pid]."','CC','".$STATUS_MESSAGE."',NOW())");
						$db->query($sql);

					}
					else
					{
						//2.2 취소시점이 프리미엄 기간 전인경우

						//2.2.1 취소전 가장 최근의 결제정보를 가져온다.
						$sql = "SELECT * from service_order so inner join service_order_detail sod
									ON so.oid = sod.oid AND so.code = '".$code."' WHERE sod.status = 'IC' ORDER BY sod.od_ix DESC LIMIT 1";
						$db->query($sql);
						$db->fetch();

						//2.2.2 service_info 의 정보를 수정.
						$sql = "UPDATE service_info SET end_date = '".$db->dt[end_date]."' WHERE code='".$code."' ";
						$db->query($sql);
		...
		if ($product_status_change){// 상품상태 변경 체크시 각각의 상품에 대한 상태 변경이 된다
			...
			for($i=0;$i < count($od_ix);$i++){
				$db->query("select * from service_order_detail where  od_ix ='".$od_ix[$i]."' ");//oid='$oid' and
				$db->fetch();
				...
				$s_kind = $db->dt[s_kind];//서비스 종류 대분류
				$s_type = $db->dt[s_type];//서비스 종류 중분류
				...
				if($db->dt[status] != $status && $status == ORDER_STATUS_DELIVERY_COMPLETE){
					...
					if($order["escrow_yn"]=="Y") {//에스크로 사용 시 pg 상점관리자에 자동으로 배송시작 업데이트 kbk 12/03/12
						$db2->query("SELECT COUNT(od_ix) AS od_ix_cnt FROM service_order_detail WHERE oid='".$oid."' AND invoice_no!='' ");
						$db2->fetch();
					...
					if($stock_use_yn == "Y"){
						if($select_option_id != 0){
							...
							if($udb->total){
								...
								//주문취소시 판매진행중 재고만 감소하기 때문에 실재 재고는 변경이 없다. 따라서 옵션 재고여부도 변경되지 않는다.
					...
					$udb->query("update service_order set order_cancel_price = order_cancel_price + ".$ptprice." where oid = '$oid' ");
					...
					if($_SESSION["layout_config"]["mall_use_inventory"] == "Y"){
						...
						//InventoryInputHistory($udb, $db->dt,"CC","주문취소 및 반품");
					}
					*/
					...
					$udb->query("UPDATE ".TBL_SHOP_PRODUCT."  Set stock = stock + ".$pcnt." $stock_update WHERE id = '".$db->dt[pid]."'");
					$udb->query("update service_order set order_return_price = order_return_price + ".$ptprice." where oid = '$oid' ");
					/////////////////////////////////////////////////////////////////////////////////////////////////////재고관리 사용할때 출고 리스트에 재고 정보 남기기//////////////////////////////////////////////////////////////////////////////////////////
					if($_SESSION["layout_config"]["mall_use_inventory"] == "Y"){
						InventoryInputHistory($db->dt,"RC","반품완료후 입고");
					}
					*/

				}else if($db->dt[status] != $status && $status == ORDER_STATUS_INCOM_COMPLETE){//입금확인 시 서비스 상태 변경 kbk
					
					$sql2="SELECT si_ix,unix_timestamp(end_date) AS end_date FROM service_info WHERE si_status NOT IN ('SR','CC') AND code='".$order["code"]."' ";
					$db2->query($sql2);

					//service_info에 정보가 있는지 없는지 확인 후 작업.
					if(!$db2->total) {
						$apply_type="N";//신규
						$sdate=date('Y-m-d H:i:s');
						$edate = date('Y-m-d H:i:s',strtotime('+'.$period.' month'));
						$sql="INSERT service_info SET code='".$order["code"]."', name='".$order["bname"]."', pname='".$pname."', s_kind='".$s_kind."', s_type='".$s_type."', start_date='".$sdate."', end_date='".$edate."', si_status='SI', regdate=NOW() ";
						$db->query($sql);
						$in_si_ix=mysql_insert_id();

						$sql="UPDATE service_order_detail SET si_ix='".$in_si_ix."', start_date='".$sdate."', end_date='".$edate."' WHERE od_ix='".$od_ix[$i]."' ";
						$db->query($sql);

					} else {
						$apply_type="E";//연장
						$sql="SELECT UNIX_TIMESTAMP(end_date) AS out_end_date FROM service_info WHERE si_ix='".$si_ix."' ";
						$db->query($sql);
						$db->fetch();
						...
						if($now_date>=$out_end_date) {
							...
							$end_timestamp=$out_end_date+(strtotime("+".$period." month") - time());
							$edate=date("Y-m-d H:i:s",$end_timestamp);
						}
						
						$sql="UPDATE service_info SET end_date='".$edate."' WHERE si_ix='".$si_ix."' ";
					...
					$sql="UPDATE service_order_detail SET start_date='".$sdate."', end_date='".$edate."' WHERE od_ix ='".$od_ix[$i]."' ";
					$db->query($sql);

					if($s_kind=="MS") {//회원 등급 변경 kbk 13/08/22
						
						if($s_type=="M") $gp_ix="2";//프리미엄 등급 (cowell)							
						
						$sql="SELECT gp_name FROM ".TBL_SHOP_GROUPINFO." WHERE gp_ix='".$gp_ix."' ";
						$db->query($sql);
						$db->fetch();
						$gp_name=$db->dt["gp_name"];
						//$user_gp_name=$_SESSION["user"][gp_name];
						$sql="SELECT g.gp_name, cmd.code FROM service_order o LEFT JOIN ".TBL_COMMON_MEMBER_DETAIL." cmd ON o.code=cmd.code LEFT JOIN ".TBL_SHOP_GROUPINFO." g ON cmd.gp_ix=g.gp_ix WHERE o.oid='".$oid."' ";
						$db->query($sql);
						$db->fetch();
						$user_gp_name=$db->dt["gp_name"];
						$user_code=$db->dt["code"];

						$sql="UPDATE ".TBL_COMMON_MEMBER_DETAIL." SET gp_ix='".$gp_ix."' WHERE code='".$user_code."' ";
						$db->query($sql);

						$add_order_status=" (회원등급변경 [".$user_gp_name."]->[".$gp_name."])";
					}
					...
					if($db->dt[status]==ORDER_STATUS_INCOM_COMPLETE) {
						if($apply_type=="N") {//신규
							$sql="UPDATE service_info SET si_status='CC' WHERE si_ix='".$si_ix."' ";
							$db->query($sql);
						} else if($apply_type=="E") {//연장
							$sql="SELECT period FROM service_order_detail od WHERE od_ix ='".$od_ix[$i]."' AND od.status='IC' ";
							$db->query($sql);
							$db->fetch();
							$pre_period=$db->dt["period"];
							$pre_edate=date('Y-m-d H:i:s',strtotime('-'.$pre_period.' month'));
							$sql="UPDATE service_info SET end_date='".$pre_edate."' WHERE si_ix='".$si_ix."' ";
							$db->query($sql);
						}
				...
				$db->query("UPDATE service_order_detail SET status = '$status' WHERE od_ix ='".$od_ix[$i]."' ");//oid='$oid' and
				$status_message = $admininfo[charger]."(".$admininfo[charger_id].")";

				if($status != ORDER_STATUS_INCOM_READY || $status != ORDER_STATUS_INCOM_COMPLETE){
					$db->query("insert into service_order_status (os_ix, oid,pid, status, status_message, regdate ) values ('','$oid','".$pid."','$status','$status_message".$add_order_status."',NOW())");
				}else{
					$dc_date_str = "";
				}
			}
			...
			if($status == ORDER_STATUS_CANCEL_COMPLETE){
					...
					from service_order_detail WHERE oid='$oid'";
					$db->query($sql);
					$db->fetch();
					...
					if(count($od_ix) == $whole_total || $whole_total == $status_total){
						$udb->query("UPDATE service_order SET status = '$status'  WHERE oid ='".$oid."' ");
						//$db->query("UPDATE ".TBL_SHOP_RESERVE_INFO."  Set state = '".RESERVE_STATUS_ORDER_CANCEL."' WHERE oid = '$oid'");
					}
			}
			...
			for($i=0;$i < count($od_ix);$i++){
				//echo ("UPDATE service_order_detail SET invoice_no='$deliverycode' WHERE oid='$oid' and pid ='".$pid[$i]."' ");
				$db->query("select * from service_order_detail where  od_ix ='".$od_ix[$i]."' ");//oid='$oid' and
				$db->fetch();
				$pid = $db->dt[pid];
				$ptprice = $db->dt[ptprice];
				...
				/*}else if($db->dt[status] != $status && $status == ORDER_STATUS_REFUND_APPLY){
					$db2->query("select * from service_order_detail where  od_ix ='".$od_ix[$i]."' ");
				...
				*///주석 처리 함 kbk 12/02/28
				...
				$db->query("insert into service_order_status (os_ix, oid,pid, status, status_message, regdate ) values ('','$oid','".$pid."','$status','$status_message',NOW())");
...
/*	문제없으면 추후 삭제
	if ($status == ORDER_STATUS_DELIVERY_ING){
		...
		for($i=0;$i < $db->total;$i++){
			$db->fetch($i);
			$udb->query("UPDATE ".TBL_SHOP_PRODUCT."  Set stock = stock + ".$pcnt." WHERE id = '".$db->dt[id]."'");

			if($select_option_id != 0){
				// 옵션 코드값이 있는 경우에 옵션이 가격/재고 관리 옵션인지 'b' 판단해서  가격/재고 관리 옵션일경우 옵션의 재고수량도 증가시켜준다
				$udb->query("select po.id from  ".TBL_SHOP_PRODUCT_OPTIONS." pos, ".TBL_SHOP_PRODUCT_OPTIONS_DETAIL." po  WHERE id = '".$select_option_id."' and pos.opn_ix = po.opn_ix and pos.option_kind = 'b'");
				if($udb->total){
					$udb->fetch($i);
					$udb->query("UPDATE ".TBL_SHOP_PRODUCT_OPTIONS_DETAIL."  Set option_sellstock = option_sellstock + ".$pcnt." WHERE id = '".$udb->dt[id]."'");
				}
			}
		}
...
if ($act == "stateupdate"){
	$db->query("UPDATE service_order SET status='".ORDER_STATUS_RETURN_COMPLETE."' WHERE oid='$oid'");
...
//	$db->query("INSERT INTO ".TBL_SHOP_RESERVE_INFO."  (id,uid,oid,pid,ptprice,payprice,reserve,state,regdate) VALUES ('','".$user[code]."','$oid','$pid','$sum','".($sum-$order[reserve_price])."','-".$order[reserve_price]."','4',NOW())");
	...
	// 해당 주문에 대한 상태 정보를 저장한다.
	$db->query("insert into service_order_status (os_ix, oid,pid, status, status_message, regdate ) values ('','$oid','".$pid."','$status','카드구매',NOW())");

	echo("<script language='javascript' src='../js/message.js.php'></script><script>show_alert('반품처리가 완료되었습니다.');document.location.href='service_orders.list.php'</script>");
}
...
if ($act == "select_status_update"){
	...
	if($admininfo[admin_level] == 9){
		...
		for($j=0;$j < count($oid);$j++){
			...
			if($status == ORDER_STATUS_INCOM_READY || $status == ORDER_STATUS_INCOM_COMPLETE){
						/*
				$db->query("insert into service_order_status (os_ix, oid, status, status_message, regdate ) values ('','$oid','$status','',NOW())");
				$db->query("SELECT os_ix FROM service_order_status WHERE os_ix=LAST_INSERT_ID()");
				$db->fetch();
				$os_ix = $db->dt[os_ix];
				...
				*/
				if($status == ORDER_STATUS_INCOM_COMPLETE){
					$db->query("select oid, code from service_order  WHERE oid='".$oid[$j]."'");
					$db->fetch();
					$order = $db->dt;

					$sql = "select oid, status from service_order_detail od WHERE oid='".$oid[$j]."'";
					//echo $sql;
					$db->query($sql);
					for($i=0;$i < $db->total;$i++){
			...
			if ($status == ORDER_STATUS_DELIVERY_ING){
				...
				$db->query("select * from service_order  WHERE oid='".$oid[$j]."'");
				$order = $db->fetch();

				$db->query("select * from service_order_detail WHERE oid='".$oid[$j]."'");
				$order_details = $db->fetchall();
			...
			for($i=0;$i < count($od_ix);$i++){
				$db->query("select * from service_order_detail where  od_ix ='".$od_ix[$i]["od_ix"]."' ");//oid='$oid' and
				$db->fetch();
				$order_detail_info = $db->dt;
				$pid = $db->dt[pid];
				$oid_d = $db->dt[oid];
				...
				$db->query("UPDATE service_order_detail SET status = '$status' WHERE od_ix ='".$od_ix[$i]["od_ix"]."' ");//oid='$oid' and
				$status_message = $admininfo[charger]."(".$admininfo[charger_id].")";

				/*if($status != ORDER_STATUS_INCOM_READY || $status != ORDER_STATUS_INCOM_COMPLETE){
					$db->query("insert into service_order_status (os_ix, oid, pid, status, status_message, company_id, regdate ) values ('','$oid','".$pid."','$status','$status_message','".$admininfo[company_id]."',NOW())");
				}*/
			}
			...
			if($status == ORDER_STATUS_CANCEL_COMPLETE){
					...
					sum(case when status = '$status' then 1 else 0 end) as status_total
					from service_order_detail WHERE oid='".$oid[$j]."'";
					$db->query($sql);
					$db->fetch();
					$whole_total = $db->dt[whole_total];
					$status_total = $db->dt[status_total];
					...
					if(count($od_ix) == $whole_total || $whole_total == $status_total){
						$udb->query("UPDATE service_order SET status = '$status'  WHERE oid ='".$oid[$j]."' ");
	...
	/*if($admininfo[admin_level] == 9){
		for($i=0;$i < count($oid);$i++){
			...
			if($status == ORDER_STATUS_INCOM_READY || $status == ORDER_STATUS_INCOM_COMPLETE){
				...
				$db->query("UPDATE service_order SET status='$status' WHERE oid='".$oid[$i]."'");
				$db->query("insert into service_order_status (os_ix, oid, status, status_message, regdate ) values ('','".$oid[$i]."','$status','일괄배송처리',NOW())");

				if($status == ORDER_STATUS_INCOM_COMPLETE){
					$db->query("select oid, uid, rmobile from service_order  WHERE oid='".$oid[$i]."'");
					$db->fetch();
					$order = $db->dt;

					$db->query("select oid, hotcon_event_id, hotcon_pcode, pcnt from service_order_detail od WHERE oid='".$oid[$i]."'");
					for($i=0;$i < $db->total;$i++){
						$db->fetch($i);
						if($db->dt[hotcon_event_id] && $db->dt[hotcon_pcode]){
							CallHotCon($order[uid], $order[oid], $db->dt[pid], $db->dt[hotcon_event_id], $db->dt[hotcon_pcode], $db->dt[pcnt], $order[rmobile]);
			...
			if($status == ORDER_STATUS_DELIVERY_READY){
				$db->query("select oid, pid from service_order_detail  WHERE oid='".$oid[$i]."' and status in ('".ORDER_STATUS_INCOM_COMPLETE."')");
				$o_details = $db->fetchall();
				for($j=0;$j < count($o_details);$j++){
					$db->query("UPDATE service_order_detail SET status='".$status."' WHERE oid='".$o_details[$j][oid]."' and pid='".$o_details[$j][pid]."' ");
					$db->query("insert into service_order_status (os_ix, oid, pid, status, status_message, regdate ) values ('','".$o_details[$j][oid]."','".$o_details[$j][pid]."','$status','$status_message',NOW())");
				}
			}else{
				$db->query("UPDATE service_order_detail SET status='".$status."' WHERE oid='".$oid[$i]."' ");
			}
		...
		for($i=0;$i<count($oid);$i++){
			...
			if($status == ORDER_STATUS_DELIVERY_READY){
				$db->query("select oid, pid from service_order_detail  WHERE oid='".$oid[$i]."' and status in ('".ORDER_STATUS_INCOM_COMPLETE."') and company_id ='".$admininfo[company_id]."'");
				$o_details = $db->fetchall();
...
if ($act == "delete"){
	$db->query("DELETE FROM service_order WHERE oid='$oid'");
	$db->query("DELETE FROM service_order_detail WHERE oid='$oid'");
	$db->query("DELETE FROM service_order_status WHERE oid='$oid'");
	$db->query("DELETE FROM service_order_memo WHERE oid='$oid' ");
	echo("<script>top.location.href = 'service_orders.list.php?page=$page';</script>");
}
...
if ($act == "send_mail"){
	...
	/*$db->query("Select bmail, bname, uid FROM service_order WHERE oid = '".$oid."'");
	$db->fetch();
