function deleteProduct(act,id, addQuery){	
	if(confirm(language_data['product_list.js']['J'][language])){//'정말로 삭제하시겠습니까?'
		...
		document.getElementById('iframe_act').src='./service_product_list.act.php?act='+act+'&id='+id+addQuery;//kbk
	}
}
...
function loadCategory(sel,target) {
	...
	window.frames["act"].location.href = 'service_category.load2.php?form=' + form + '&trigger=' + trigger + '&depth='+depth+'&target=' + target;
...
function setCategory(cname,cid,depth,pid){
	...
	document.getElementById('act').src='./service_product_list.php?cid='+cid+'&depth='+depth+'&view=innerview';
}
...
function CopyData(frm_name, pid){
	...
	if(confirm(" ' "+pname+" ' "+ language_data['product_list.js']['C'][language])){	//해당 상품에 대한 정보를 수정하시겠습니까?
		...
		*/
		...
		sfrm.action = "service_product_list.act.php";
		sfrm.submit();
	}
}
...
function SelectUpdateLoading(){
		...
		obj.innerHTML = "<table height=100%><tr><td valign=middle><img src='/admin/images/indicator.gif' border=0 width=32 height=32 align=absmiddle> 상품정보를 변경중입니다. 잠시만 기다려주세요.</td></tr></table>";
