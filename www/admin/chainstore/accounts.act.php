...
if($act == "account_confirm"){
			...
			for($j=0;$j < count($accounts);$j++){
				...
				$sql = "UPDATE ".TBL_SHOP_ORDER_DETAIL." SET status = '".ORDER_STATUS_ACCOUNT_COMPLETE."' WHERE ac_ix ='".$ac_ix."' and od_ix = '".$accounts[$i][od_ix]."' ";
				//echo $sql."<br>";
				$db->query($sql);	
				$sql = "insert into shop_order_status(oid,pid, status,status_message,regdate) values ('".$accounts[$i][oid]."','".$accounts[$i][pid]."','".ORDER_STATUS_ACCOUNT_COMPLETE."','정산확인',NOW())";
				$db->query($sql);	
			}
			//$db->fetch();
...
if($act == "account"){
	...
	if($admininfo[admin_level] == 9){		
		if(is_array($company_id)){
			$company_id_str = " and p.admin in ('".join("','",$company_id)."') ";
		}else if($company_id != ""){
		...
		$company_id_str = " and p.admin = '".$admininfo[company_id]."' ";
	}
		...
		for($i=0;$i < count($accounts);$i++){
			$ac_price = $accounts[$i][sell_total_coprice];
			//echo $ac_price;
			//exit;
			// 위 정산정보를 정산테이블에 입력한다
			$sql = "insert into ".TBL_SHOP_ACCOUNTS." (ac_ix,company_id,ac_date,month, week, ac_cnt,ac_price,sell_total_price,sell_fee,card_fee,pre_shipping_cnt, shipping_fee,status, regdate) 
				values
				('','".$accounts[$i][company_id]."','".$eDate."','".substr($eDate,4,2)."','".$week."','".$accounts[$i][sell_cnt]."','".$ac_price."','".$accounts[$i][sell_total_ptprice]."','".$accounts[$i][sell_fee]."','".$accounts[$i][card_fee]."','".$accounts[$i][pre_shipping_cnt]."','".$accounts[$i][shipping_price]."','".ORDER_STATUS_ACCOUNT_READY."',NOW())";
			$db->query($sql);
