...
if ($act == "update")
{
	...
	$sql = "update shop_mail_box set 
			mail_title='$mail_title',mail_text='$mail_text',disp='$disp'
	...
	echo("<script>top.location.href = 'email_manage_write.php?mail_ix=$mail_ix';</script>");
}
...
if ($act == "insert"){
	...
	$mail_use_sdate = $FromYY.$FromMM.$FromDD;
	$mail_use_edate = $ToYY.$ToMM.$ToDD;
	$db->query("insert into shop_mail_box (mail_ix,mail_title,mail_text,disp,regdate) values('$mail_ix','$mail_title','$mail_text','$disp',NOW())");
	...
	$db->query("UPDATE shop_mail_box SET mail_text = '$data_text' WHERE mail_ix='$LAST_ID'");
	echo("<script>top.location.href = 'email_manage.php';</script>");
}
