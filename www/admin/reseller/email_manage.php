...
function mailDelete(mail_ix){
	...
	{
		window.frames['act'].location.href= 'email_manage.act.php?act=delete&mail_ix='+mail_ix;
	}
}
</script>";
...
$help_text = HelpBox("이메일설정", $help_text);
...
function PrintMailList(){
	global $db, $mdb, $admininfo, $start, $max;
	...
	if ($total == 0){
		...
		$sql = "select m.mail_ix, m.mail_title, m.regdate, m.disp, count(*) as send_cnt,
		...
		for($i=0;$i < $mdb->total;$i++){
			...
			<td class='list_box_td point' style='text-align:left;padding-left:10px;'><a href='email_manage_write.php?mail_ix=".$mdb->dt[mail_ix]."'>".$mdb->dt[mail_title]."</a></td>
		...
		$mString .= "<tr>
					...
					<td colspan=0 align=right><a href='email_manage_write.php'><img src='../images/".$admininfo["language"]."/btn_mailadd.gif' border=0 ></a></td>
				</tr>";
	}
