...
*/
	$sql = "SELECT  count(*) as total
			FROM ".TBL_COMMON_USER." cu left join ".TBL_COMMON_MEMBER_DETAIL." cmd on cu.code = cmd.code
 			where cu.company_id = '".$company_id."'   $search_str  ";
$db->query($sql);
$db->fetch();
...
$sql = "select ccd.company_id, ccd.com_name, ccd.com_phone, ccd.com_ceo, csd.shop_name
			from ".TBL_COMMON_SELLER_DETAIL." csd, ".TBL_COMMON_COMPANY_DETAIL." ccd
			where ccd.com_type = 'S' and csd.company_id = ccd.company_id and ccd.seller_auth = 'Y' $search_str
			limit $start, $max";
...
function SearchCharger(charger_ix, charger_name){
...
<TABLE cellSpacing=0 cellPadding=0 width='100%' align=center border=0>
	<TR>
		...
		<table border='0' width='100%' cellpadding='0' cellspacing='0' align='center'>
					...
					<table width='100%' border='0' cellspacing='0' cellpadding='0' >
						<tr>
							<td width='10%' height='31' valign='middle' style='font-family:굴림;font-size:16px;font-weight:bold;letter-spacing:-2px;word-spacing:0px;padding-left:10px;' nowrap>
								<img src='/admin/images/icon/sub_title_dot.gif' align=absmiddle> 담당자 검색
							</td>
			...
			<tr >
				<td align='left' colspan=2> ".GetTitleNavigation("담당자 검색", "담당자 검색", false)."</td>
			</tr>
	...
	<tr height=30>
		<td class='p11 ls1' style='padding:0 0 0 5px;' nowrap> ".($search_text == "" ? "담당자 전체 목록입니다.":"'".$search_text ."' 로 검색된 결과 입니다.")."</td>
	...
	<tr><form name='send_mail' method='post' action='sms.act.php' onsubmit='return CheckSMS(this);' ><input type=hidden name='act' value='send_mail' >
		...
		<tr height='28' bgcolor='#ffffff'>
			...
			<td width='40%' align='center' class=m_td><font color='#000000'><b>이메일</b></font></td>
...
if($db->total){
	for($i=0;$i  < $db->total; $i++){
		...
		$Contents .= "<tr height=25 style='text-align:center;cursor:pointer;' >
						...
						<td class='list_box_td point' onclick=\"SearchCharger('".$db->dt[code]."','".$db->dt[name]."');\">".$db->dt[name]."</td>
