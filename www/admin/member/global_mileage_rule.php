...
$shmop = new Shared("global_mileage_rule");
$shmop->filepath = $_SERVER["DOCUMENT_ROOT"].$admininfo["mall_data_root"]."/_shared/";
$shmop->SetFilePath();
$reserve_data = $shmop->getObjectForKey("global_mileage_rule");
				...
				<td style='padding:5px;'>
					<div style='background-color: #efefef;padding: 10px;'>
						<img src='../image/emo_3_15.gif' align=absmiddle > <b>사용 적립금 제외설정 가이드</b></br></br>
							...
							<span>-적립 비율을 제외한 금액은 소수점 두자리 반올림 처리하여 계산 합니다.</span>
					</div>
				</td>
			</tr>
			</table>            
...
$sql = "SELECT gi.gp_ix, gi.gp_name 
							...
							where disp='1' and use_reserve_yn='Y' and mall_ix = '20bd04dac38084b2bafdd6d78cd596b2' ";
$db->query($sql);
$basic_groups = $db->fetchall("object");

$group_detail = "";
$group_detail2 = "";
if(is_array($basic_groups)){
    ...
    $group_detail .= "<td class='list_box_td' rowspan='2'>그룹을 설정해주세요</td>";
}
...
$Contents01 .= "
	...
	<tr bgcolor='#ffffff'>
		...
		<td class='input_box_item'>
			...
			<input type='radio' name='deliveryprice' id='deliveryprice_y' value='Y' ".($reserve_data[deliveryprice] =="Y" ? "checked":"")."> <label for='deliveryprice_y'>배송비 결제 가능 </label>
		</td>
	</tr>
	<tr bgcolor='#ffffff'>
		...
		<td class='input_box_item'>
			...
			<tr height=30>
				<td style='padding-left:5px;'> 
					- 상품 구매금액 합계액이 <input type=text class='textbox number' name='total_order_price' value='".$reserve_data[total_order_price]."' style='width:60px;' title='Mileage In-Use제한 설정'> $ 이상일 경우 마일리지 사용 가능 
				</td>
			</tr>
			<tr>
				<td style='padding:5px;'>
...
<input type='hidden' name='act' value='global_mileage'>";
