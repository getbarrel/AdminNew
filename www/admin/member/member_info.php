	...
	if($info_type == "member" || $info_type == "c_member"){
		...
		if($db->dbms_type == "oracle"){
			$sql = "SELECT cmd.code,AES_DECRYPT(jumin,'".$db->ase_encrypt_key."') as jumin
						 ...
						 , birthday, birthday_div, tel_div, info, sms, collection, nick_name, job, cmd.date_ as regdate2, cmd.file_, recent_order_date
			...
			$sql = "SELECT cmd.code
						 ...
						 , cmd.mem_card, ccd.company_id, cmd.level_ix, ccd.com_mobile, cmd.level_msg, cmd.customs_clearance_number
						 , birthday, birthday_div, tel_div, info, sms, collection, nick_name, job, cmd.date as regdate2, cmd.file, recent_order_date
						 , recom_id, gp_ix, sex_div, mem_level, branch, team, department, position, black_list
						 , add_etc1, add_etc2, add_etc3, add_etc4, add_etc5, add_etc6
	...
	<tr>
		...
		<table border='0' width='100%' cellpadding='0' cellspacing='0' align='center'>
			...
			<tr>
				...
				<form name='EDIT_".$db->dt[code]."' method='post' onsubmit='return wel_chkPW(\"".$act."\", this);' action='member.act.php'  target='act'>
...
$Contents .= "
								<tr>
									...
									<td class='input_box_item'><input type='password' class='textbox' name='pass' size='16' maxlength='30' autocomplete='off'  value='' readonly style='background-color:#eee;'>
									...
									<div style='font-size: 11px;color:red;'>※SNS 회원의 경우 비번 변경 시 로그인 불가 합니다.<br>비밀번호수정 체크시 임시패스워드가 이메일로 발송됩니다.</div>


									</td>
									<td class='input_box_title' nowrap> 비번확인</td>
									<td class='input_box_item'><input type='password' class='textbox' name='again' size='16' maxlength='30' autocomplete='off' value='' readonly style='background-color:#eee;'>
									</td>
								</tr>";
		}
...
$Contents .= "
					...
					$Contents .= "
								<tr>
									...
									<td class='input_box_title' nowrap> 개인정보 수신여부</td>
									<td class='input_box_item'>
										<input type='radio' name='collection' id='collection_1' value='1' ".($db->dt[collection] == "1" ? "checked":"")." style='border:0px;'>
										<label for='collection_1'>수신함</label>
										<input type='radio' name='collection' id='collection_0' value='0' ".($db->dt[collection] == "0" ? "checked":"")." style='border:0px;'>
										<label for='collection_0'>수신안함</label>
									</td>
								</tr>
								<tr>
					...
					$Contents .= "	</td>
									<td class='input_box_title' nowrap> Email 수신여부 <img src='".$required3_path."'></td>
									<td class='input_box_item'>
...
?>
	<script type="text/javascript">
		//  welshoop pw 추가
		function wel_chkPW(s_act, ig_thisData){
...
<script type="text/javascript">
	...
	$("input[name=pass]").val(new_PW);
	$("input[name=again]").val(new_PW);
</script>