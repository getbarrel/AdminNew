...
if($info_type == "list" or $info_type == ""){
	...
	$sum_reserve = $total_reserve - abs($total_use); // 총누적마일리지 - 총사용마일리지
...
if($db->dbms_type == "oracle"){
	$where = " where cmd.code = r.uid_ and cmd.code = cu.code ";
}else{
	$where = " where  cmd.code = r.uid and cu.code = cmd.code ";
}
...
if($ust_status_add !=""){
	$where .= " and r.use_state = '".$ust_status_add."' ";
}

if($ust_status_cancel !=""){
	$where .= " and r.use_state = '".$ust_status_cancel."' ";
}
...
if($db->dbms_type == "oracle"){
	$sql = "select 
			...
			from 
				".TBL_SHOP_RESERVE." as r 
				inner join ".TBL_SHOP_RESERVE_INFO." as ri on (r.reserve_id = ri.reserve_id),
  ...
  <tr height=30 >
	...
	if($mmode != "personalization"){ 
		if(checkMenuAuth(md5($_SERVER["PHP_SELF"]),"E")){
			$Contents02 .= "<a href='reserve_excel2003.php?".$QUERY_STRING."'><img src='../images/".$admininfo["language"]."/btn_excel_save.gif' border=0></a>";
		}else{
...
if($mmode != "personalization"){ 
$Contents02 .= "
				<td class='list_box_td point' bgcolor='#efefef'><a href=\"javascript:PoPWindow('reserve.pop.php?code=".$db->dt[code]."',750,550,'reserve_pop')\">".$db->dt[name]."</a></td>
				<td class='list_box_td point' bgcolor='#efefef'><a href=\"javascript:PoPWindow('reserve.pop.php?code=".$db->dt[code]."',750,550,'reserve_pop')\">".$db->dt[member_id]."</a></td>
...
$Contents02 .= "
	...
	$Contents02 .= "<font></td>
				...
				if(checkMenuAuth(md5($_SERVER["PHP_SELF"]),"D")){
					...
					<a href=\"javascript:PoPWindow('reserve.pop.php?code=".$db->dt[code]."&reserve_id=".$db->dt[reserve_id]."',800,550,'reserve_pop')\"><img src='../images/".$admininfo["language"]."/btc_modify.gif' border=0></a>
	...
	$Contents02 .= "</form>";
		...
		$Contents02 .= "
			<tr height=60><td class='list_box_td' ".($mmode == "personalization" ? "colspan=8":"colspan=11")." align=center>적립금 내용이 없습니다.</td></tr>";
}
...
<table cellpadding=0 cellspacing=0 class='small' >
	<col width=8>
	<col width=*>
	<tr><td valign=top><img src='/admin/image/icon_list.gif' ></td><td class='small' >고객들에게 지급됐거나 고객들이 사용한 적립금 내역입니다 </td></tr>
	<tr><td valign=top><img src='/admin/image/icon_list.gif' ></td><td class='small' >회원아이디를 클릭하시면 해당 회원에 대한 적립금을 확인하실수 있습니다</td></tr>
	<tr><td valign=top><img src='/admin/image/icon_list.gif' ></td><td class='small' >삭제를 원하시는 적립금 내역을 선택하신후 일괄정보 삭제를 클릭하시면 적립금이 삭제됩니다</td></tr>
	<tr><td valign=top><img src='/admin/image/icon_list.gif' ></td><td class='small' >적립금을 직접 지급 하고자 하실 경우 회원 이름을 클릭하여 입력하시면 됩니다.</td></tr>
</table>
";
*/
