...
if($db->dbms_type == "oracle"){
	...
	$where = " where  cmd.code = ri.uid and u.code = cmd.code ";
}
...
if($regdate == '1'){	//신청일
	...
	if($db->dbms_type == "oracle"){
		...
		//$db->query("select ri.id,ri.state,ri.etc,ri.reserve,ri.oid,ri.uid_,cmd.code,  AES_DECRYPT(IFNULL(cmd.name,'-'),'".$db->ase_encrypt_key."') as name , DATE_FORMAT(regdate, '%Y.%m.%d %H:%i:%s') as disp_regdate from ".TBL_SHOP_RESERVE_INFO." ri ,".TBL_COMMON_MEMBER_DETAIL." cmd  $where order by regdate desc"); //where uid = '$code'
	}else{
		$sql = "select 
			r.*,
			ri.etc,
		...
		//$db->query("select ri.*,cmd.code,  AES_DECRYPT(UNHEX(IFNULL(cmd.name,'-')),'".$db->ase_encrypt_key."') as name , DATE_FORMAT(regdate, '%Y.%m.%d %H:%i;%s') as disp_regdate from ".TBL_SHOP_RESERVE_INFO." ri ,".TBL_COMMON_MEMBER_DETAIL." cmd  $where order by regdate desc"); //where uid = '$code'
	}
