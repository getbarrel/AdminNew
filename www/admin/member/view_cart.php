...
<table width='100%' cellpadding='0' cellspacing='0' border=0>
	<tr>
		<td align='left' colspan=6 > ".GetTitleNavigation("장바구니 보기", "회원관리 > 장바구니 보기 ")."</td>
	</tr>
</table>
<table width='100%' cellpadding='0' cellspacing='0' border=0>
	<tr>
...
$where = " c.delivery_package in ('Y','N') ";

if($mmode == "personalization"){
	$where .= " and c.mem_ix = '".$mem_ix."' ";
}
...
if($startDate && $endDate){
	$where .= " and  date_format(c.regdate, '%Y%m%d') between '".str_replace('-','',$startDate)."' and '".str_replace('-','',$endDate)."' ";
}

$sql="select count(*) as total 
		from shop_cart c 
		left join common_member_detail cmd on c.mem_ix = cmd.code 
		left join common_user cu on c.mem_ix = cu.code 
		where ".$where." ";
//echo nl2br($sql);
$db->query($sql);
$db->fetch();
...
$Contents .= "
  ...
  <table width='100%' border='0' cellpadding='3' cellspacing='0' align='center' class='list_table_box' style='table-layout:fixed;display:table;'>
	...
	if($mmode != "personalization"){
$Contents .= "
		<td width='7%' align='center' class='m_td' nowrap><font color='#000000' ><b>회원명</b></font></td>";
	}
$Contents .= "
		<td width='*' align='center' class='m_td' nowrap><font color='#000000' ><b>제품명</b></font></td>		
		<td width='9%' align='center' class='m_td' nowrap><font color='#000000' ><b>상품금액</b></font></td>
		<td width='7%' align='center' class='m_td' nowrap><font color='#000000' ><b>수량</b></font></td>
		<td width='9%' align='center' class='m_td' nowrap><font color='#000000' ><b>주문금액</b></font></td>
		<td width='10%' align='center' class='m_td' nowrap><font color='#000000' ><b>예상 적립금</b></font></td>
		<td width='10%' align='center' class='m_td' nowrap><font color='#000000' ><b>등록일자</b></font></td>
...
if($total){
	...
	$sql="select c.*, AES_DECRYPT(UNHEX(cmd.name),'".$db->ase_encrypt_key."') as name 
	from shop_cart c 
	left join common_member_detail cmd on c.mem_ix = cmd.code 
	...
	if($mmode != "personalization"){
$Contents .= "
		<td  align='center'  >".($carts[$j][name] ? $carts[$j][name]:"비회원")."</td>";
	}
$Contents .= "
		<td  align='left'  >
			<table cellpadding=3>
			<tr>
				<td>
				<img src='".PrintImage($admin_config[mall_data_root]."/images/product", $carts[$j][id], "c")."' width=40 align=left>
				</td>
				<td>
		...
		<td align='center'  >".number_format($carts[$j][dcprice])."</td>
		<td align='center'  >".$carts[$j][pcount]."</td>
		<td align='center' class='point' >".number_format($carts[$j][totalprice])."</td>
	...
	if($mem_ix != ""){
		$Contents .= "<tr height=50><td colspan=7 align=center>장바구니 목록이 존재하지 않습니다.</td></tr>";
	}else{
		$Contents .= "<tr height=50><td colspan=8 align=center>장바구니 목록이 존재하지 않습니다.</td></tr>";
	}
}
...
$Contents .= HelpBox("장바구니 보기", $help_text);
