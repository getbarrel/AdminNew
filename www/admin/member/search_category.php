...
<table cellSpacing=0 cellPadding=0 width='100%' align=center border=0>
	<tr>
		<td align=center colspan=2>
			...
			<tr>
				<td>
					<select name='category_list1' id='category_list1' style='border:solid 1px #ddd;width:98%;height:148px;font-size:12px;background:#fff;' class='participation' multiple>";
					
					if($group_code	==	"member"){
	
						$sql = "SELECT * FROM ".TBL_SHOP_GROUPINFO." where disp=1 order by gp_level asc ";
						$db->query($sql);
						$category_array = $db->fetchall();

						for($i=0;$i<count($category_array); $i++){
						$Contents .= "<option value='".$category_array[$i][gp_ix]."'>".$category_array[$i][gp_name]."</option>";
						}
				
					}else if($group_code	==	"event"){
						
						$today	=	strtotime(date("Ymd"));
			
						$sql = "select se.* from ".TBL_SHOP_EVENT." se left join shop_event_relation ser on (se.er_ix=ser.er_ix) WHERE event_use_edate > $today";
						$db->query($sql);

						$category_array = $db->fetchall();

						for($i=0;$i<count($category_array); $i++){
						$Contents .= "<option value='".$category_array[$i][event_ix]."'>".$category_array[$i][event_title]." (".date("Y.m.d",$category_array[$i][event_use_sdate])."~".date("Y.m.d",$category_array[$i][event_use_edate]).")</option>";
						}
				...
				    success: function(args){
						...
						if(args){
							...
							$.each(args, function(index, entry){
								$('#'+target_id).arppend(\"<option value=\"+index+\">\"+entry+\"</option>\");
							});
						}else{
							$('#'+target_id).empty();
							//alert('하위 분류가 없습니다.');
...
function add_list(){
	...
	$('#category_list1 option:selected').each(function(){
		var cid = $(this).val();
		var text =$(this).text();
		var option_cnt	=	$('#add_list option').length;
		if($('#add_list option:last').val() != cid){
				$('#add_list').append('<option value='+cid+' selected>'+text+'</option>');
		}
	});
}
...
function add_department(){
	if('$group_code' == 'member'){
		var tbody = $('#objMember', opener.document);
		console.log(tbody);
		var thisRow = tbody.find('tr[depth^=1]:last');
		$('#add_list option').each(function(){
			var dp_ix = $(this).val();
			var dp_text = $(this).html();
			var add_html = '<tr style=\"height:26px;\" id=\"row_'+dp_ix+'\"><td><input type=hidden name=gid[] id=\"cid_'+dp_ix+'\" value=\"'+dp_ix+'\">'+dp_text+'</td><td><a href=\"javascript:void(0)\" onClick=\"cid_del(\''+dp_ix+'\')\"><img src=\"../images/".$_SESSION["admininfo"]["language"]."/btc_del.gif\" border=0></a></td></tr>';
			tbody.append(add_html);
		});
	}else if('$group_code' == 'event'){
		var tbody = $('#objEvent tbody',opener.document);
		var thisRow = tbody.find('tr[depth^=1]:last');
		$('#add_list option').each(function(){
			var dp_ix = $(this).val();
			var dp_text = $(this).html();
			var add_html = '<tr style=\"height:26px;\" id=\"row_'+dp_ix+'\"><td><input type=hidden name=eid[] id=\"cid_'+dp_ix+'\" value=\"'+dp_ix+'\">'+dp_text+'</td><td><a href=\"javascript:void(0)\" onClick=\"cid_del(\''+dp_ix+'\')\"><img src=\"../images/".$_SESSION["admininfo"]["language"]."/btc_del.gif\" border=0></a></td></tr>';
			tbody.append(add_html);
		});
	}
	self.close();
