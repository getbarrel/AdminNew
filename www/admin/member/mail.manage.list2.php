...
$Script = "	<script language='javascript'>
		...
		function mailDelete(mc_ix){
			if(confirm(language_data['mail.manage.list2.php']['A'][language])){
			//'해당 메일 목록을 정말로 삭제하시겠습니까? 메일 목록을  삭제 하시면 관련 데이타 모두가 삭제 됩니다.'
...
$db->query("SELECT * FROM shop_mailsend_config $where");
...
$mstring ="<form name=poll action='board.manage.act.php'><input type=hidden name=act value=insert>
		...
		<tr>
			...
			<table cellpadding=3 cellspacing=0 width='100%' class='list_table_box'>
				<tr height=27 align=center style='font-weight:bold;'>
					<td width='10%' class='s_td' nowrap> NO.</td>
					<td width='40%' class='m_td' nowrap>메일/SMS 발송 관리목록 </td>
					<!--td width='35%' class='m_td'>메일발송제목 </td-->
					<td width='10%' class='m_td' nowrap>관리자 발송(메일) </td>
					<td width='10%' class='m_td' nowrap>사용자 발송(메일) </td>
					<td width='10%' class='m_td' nowrap>관리자 발송(SMS) </td>
					<td width='10%' class='m_td' nowrap>사용자 발송(SMS) </td>
...
$mstring .="		<tr height=27 align=center>
					<td class='list_box_td' align=center bgcolor=#efefef>
						".($i+1)."
					</td>
					<td class='list_box_td point' style='text-align:left;'>
						<div style='margin-left:20px;'><a href='mail.manage.php?mc_ix=".$db->dt[mc_ix]."'>".$db->dt[mc_title]."</a></div>
					</td>
					...
					<td  class='list_box_td' bgcolor=#efefef align=left style='padding-left:10px;' nowrap>
					<a href='mail.manage.php?mc_ix=".$db->dt[mc_ix]."'><img src='../images/".$admininfo["language"]."/bts_modify.gif' border=0></a> ";


$mstring .="			<a href=\"JavaScript:mailDelete('".$db->dt[mc_ix]."')\"><img src='../images/".$admininfo["language"]."/btn_del.gif' border=0></a>";
...
$mstring .="		<tr height=50 align=center >
					<td class='list_box_td' align=center colspan=9 bgcolor=#ffffff>
						게시판이 존재 하지 않습니다.
					</td>
				</tr>
				";
...
$help_text = "
<table width=100% cellpadding=0 cellspacing=0 border=0 align=left>	 <tbody><tr height=40><td colspan=8 align=left style=text-align:left;></td><td colspan=2 align=right>
                
    <a href=mail.manage.php><img src=../images/btm_reg.gif border=0 align=absmiddle></a></td></tr></tbody></table>
