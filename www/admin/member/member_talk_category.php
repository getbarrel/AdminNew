	    ...
	    <div class='tab'>
			...
			<tr>
				<td class='tab'>
					...
					<tr>
						<th class='box_01'></th>
						<td class='box_02'  ><a href='member_talk_list.php?info_type=list'>전체리스트</a> </td>
						<th class='box_03'></th>
					</tr>
					</table>
					...
					<tr>
						...
						<td class='box_02' >";
							...
							$Contents02 .= "<a href='member_talk_list.php?info_type=vip'>VIP리스트</a>";
					...
					<tr>
						...
						<td class='box_02' >";
							...
							$Contents02 .= "<a href='member_talk_category.php?info_type=category'>문의분류관리</a>";
...
if($tc_ix){
	$sql = "select * from shop_member_talk_category where tc_ix = '".$tc_ix."'";
	$db->query($sql);
	$db->fetch();
...
$Contents02 .= "
	<table width='100%' cellpadding=3 cellspacing=0 border='0'  class=''>
	  <tr>
		<td align='left' colspan=2 style='padding:3px 0px;'> ".colorCirCleBox("#efefef","100%","<div style='padding:5px 5px 5px 15px;'><img src='../image/title_head.gif' align=middle><b> 문의분류 설정</b><span class=small> &nbsp;&nbsp;&nbsp;<font color=#5B5B5B>".getTransDiscription(md5($_SERVER["PHP_SELF"]),'A')."</font> </span></div>")."</td>
	  </tr>
	</table>";

$Contents02 .= "
<form name='edit_form' action='member_talk_category.act.php' method='post' onsubmit='return CheckFormValue(this)' enctype='multipart/form-data' style='display:inline;' target=''><!--target='iframe_act' -->
<input name='act' type='hidden' value='$act'>
<input name='tc_ix' type='hidden' value='$tc_ix'>
	...
	<col width='35%' />
	  <tr bgcolor=#ffffff  height='30' >
	    <td class='search_box_title' width='16%'> <b>문의분류코드 <img src='".$required3_path."'></b> </td>
	    <td class='search_box_item' >
		<input type=text class='textbox' name='tc_code' value='".$db->dt[tc_code]."' style='width:80px;' maxlength=2 size=2 validation='true' title='문의분류코드'> * 2자리사용</td>
	  ...
	  <tr bgcolor=#ffffff  height='30' >
	    <td class='search_box_title' width='16%'> <b>문의분류명 <img src='".$required3_path."'></b> </td>
	    <td class='search_box_item' colspan='3'><input type=text class='textbox' name='tc_name' value='".$db->dt[tc_name]."' style='width:300px;' validation='true' title='문의분류명'></td>
	  </tr>
	  <tr bgcolor=#ffffff height=110>
		<td class='input_box_title'><b>문의분류설명</b></td>
		<td class='input_box_item' colspan=3><textarea type=text class='textbox' name='tc_msg' value='".$db->dt[tc_msg]."' style='width:98%;height:85px;padding:2px;'>".$db->dt[tc_msg]."</textarea></td>
	  </tr>
	</table>
	<table><br><br>";
...
<table width='100%' cellpadding=3 cellspacing=0 border='0'  class=''>
  <tr>
	<td align='left' colspan=2 style='padding:3px 0px;'> ".colorCirCleBox("#efefef","100%","<div style='padding:5px 5px 5px 15px;'><img src='../image/title_head.gif' align=middle><b> 문의분류 리스트</b><span class=small> &nbsp;&nbsp;&nbsp;<font color=#5B5B5B>".getTransDiscription(md5($_SERVER["PHP_SELF"]),'A')."</font> </span></div>")."</td>
  </tr>
</table>";
...
<table width='100%' cellpadding=0 cellspacing=0 class='list_table_box'>
	...
	<tr bgcolor=#efefef align=center height=27>
		<td class='s_td'>번호</td>
		<td class='m_td'>문의분류코드</td>
		<td class='m_td'>문의분류명</td>
		<td class='m_td'>문의분류설명</td>
		<td class='m_td'>사용유무</td>
		<td class='m_td'>등록일</td>
		<td class='e_td'>관리</td>
		</tr>";

$sql = "select * from shop_member_talk_category where 1 order by tc_ix asc";
...
if(count($dropmember_array) > 0){
	for($i=0;$i<count($dropmember_array); $i++){
		...
		$Contents02 .="<tr height=32 align=center>
					...
					<td class='list_box_td '>".$dropmember_array[$i][tc_code]."</td>
					<td class='list_box_td list_bg_gray'>".$dropmember_array[$i][tc_name]."</td>
					<td class='list_box_td point' style='padding-left:10px; text-align:left;'>".$dropmember_array[$i][tc_msg]."</td>
			...
			if(checkMenuAuth(md5($_SERVER["PHP_SELF"]),"U")){
				$Contents02 .="<a href='member_talk_category.php?tc_ix=".$dropmember_array[$i][tc_ix]."&info_type=".$info_type."'><img src='../images/".$admininfo["language"]."/btc_modify.gif' border=0></a> ";
			}else{
				$Contents02 .="<a href=\"".$auth_update_msg."\"><img src='../images/".$admininfo["language"]."/btc_modify.gif' border=0></a> ";
			}

			if(checkMenuAuth(md5($_SERVER["PHP_SELF"]),"D")){
				$Contents02 .="<a href='member_talk_category.act.php?tc_ix=".$dropmember_array[$i][tc_ix]."&act=delete&info_type=".$info_type."'><img src='../images/".$admininfo["language"]."/btc_del.gif' border=0></a>";
			}else{
	...
	$Contents02 .= "
				<tr height=50><td colspan=10 align=center style='padding-top:10px;'>등록된 문의분류 정보가 없습니다.</td></tr>";
}
$Contents02 .= "</table>";
$Contents02 .= "<table>";
...
$Contents .= HelpBox("회원상담내역 문의분류", $help_text);
