<?
include($_SERVER[DOCUMENT_ROOT]."/admin/member/point_new.pop.php");
exit;
...
function DeleteReserve(id, uid){
	if(confirm('포인트 정보를 정말로 삭제하시겠습니까?')){
		window.frames['act'].location.href='member.act.php?act=point_delete&id='+id+'&uid='+uid;
	}
}
...
<TABLE cellSpacing=0 cellPadding=0 width='100%' align=center border=0>
	<TR>
		...
		<table border='0' width='100%' cellpadding='0' cellspacing='1' align='center'>
			...
			<tr height=30><td class='p11 ls1' style='padding:0 0 0 20px;text-align:left;' > - 포인트 내용을 수정/ 추가 하실수 있습니다.</td></tr>
			<tr>
				...
				<td align=center style='padding: 0 10 0 10'>
					<table cellspacing=0 cellpadding=0 width=100% class='list_table_box'>
						<tr bgcolor=#efefef align=center height=28>
							...
							<td class='m_td' width=12% >사용 포인트 </td>
							<td class='m_td' width=10% >상태 </td>
							<td class='m_td' width=13% >사용구분</td>
							<td class='e_td' width=10% >주문번호 </td>
...
if($reserve_id !=""){
	$where = " and r.reserve_id = '".$reserve_id."'";
}

if($db->dbms_type == "oracle"){
	$db->query("select id from ".TBL_SHOP_POINT." r  inner join ".TBL_SHOP_POINT_INFO." pi on (r.reserve_id = pi.reserve_id) where r.uid_ = '$code' $where");
}else{
	$db->query("select id from ".TBL_SHOP_POINT." r  inner join ".TBL_SHOP_POINT_INFO." pi on (r.reserve_id = pi.reserve_id)  where r.uid = '$code' $where");
}

$total = $db->total;

if($db->dbms_type == "oracle"){
	$db->query("select r.cancel_reserve, ri.id,r.uid_ as user_id ,r.oid,r.reserve,r.state,r.use_state,ri.etc, DATE_FORMAT(ri.regdate,'%Y.%m.%d %H:%i:%s') as disp_regdate from ".TBL_SHOP_POINT_INFO." as ri,  ".TBL_SHOP_POINT." as r where ri.uid_ = '$code' $where and r.reserve_id = ri.reserve_id  order by r.regdate desc, ri.id desc LIMIT $start, $max");
}else{
	$db->query("select r.cancel_reserve, ri.id,r.uid as user_id ,r.oid,r.reserve,r.state,r.use_state,ri.etc, DATE_FORMAT(ri.regdate, '%m.%d %H:%i:%s') as disp_regdate from ".TBL_SHOP_POINT_INFO." as ri, ".TBL_SHOP_POINT." as r  where ri.uid = '$code' $where and r.reserve_id = ri.reserve_id  order by r.regdate desc, ri.id desc LIMIT $start, $max");
}
	...
	for($i=0;$i  < $db->total; $i++){
		...
		$Contents .= "<tr height=28 align=center>
						...
						if($db->dt[state]	== RESERVE_STATUS_ORDER_CANCEL){
							$Contents .= "<s><font color='".$font_color."'>".number_format($db->dt[reserve])."</font></s>";
						}else{
							$Contents .="<font color='".$font_color."'>". number_format($db->dt[reserve])."</font>";
						}
						$Contents .= "</td>
										<td bgcolor='#fbfbfb'>";
						if($db->dt[state]	== RESERVE_STATUS_ORDER_CANCEL){
		...
		$Contents .= "<tr height=60><td colspan=7 align=center>포인트 내용이 없습니다.</td></tr>";
...
if($db->dbms_type == "oracle"){
	$sql = " select 
			sum(if(r.state = '2',r.reserve,0)) use_reserve,
			sum(if(r.state = '1',r.reserve,0))  total_reserve
			from
				shop_point  r 
				inner join shop_point_info  ri on (r.reserve_id = ri.reserve_id )
			where
				1
				and r.auto_cancel = 'N'
				and r.state in ('2','1')
