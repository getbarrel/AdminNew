...
$shmop->SetFilePath();
$reserve_data = $shmop->getObjectForKey("b2c_point_rule");
...
<table width='100%' cellpadding=0 cellspacing=0 border='0' align='left' style='table-layout:fixed;' class='input_table_box'>
	...
	<tr bgcolor='#ffffff'>
		...
		<input type='radio' name='order_use_yn' id='order_use_n' value='N' ".($reserve_data[order_use_yn] =="N" ? "checked":"")."> <label for='order_use_n'>미사용</label>
		</td>
	</tr>-->
	<tr bgcolor='#ffffff'>
	...
	<tr bgcolor='#ffffff'>
		...
		<td class='input_box_item' colspan='3'>
		<input type='radio' name='point_login_yn' id='point_login_yn_y' value='Y' ".($reserve_data[point_login_yn] == "Y" ? "checked":"")."><label for='point_login_yn_y'> 사용 </label>
		<input type='radio' name='point_login_yn' id='point_login_yn_n' value='N' ".($reserve_data[point_login_yn] =="N" ? "checked":"")."> <label for='point_login_yn_n'> 미사용</label>
		&nbsp;&nbsp;&nbsp;
...
//포인트 및 마일리지는 등록된 항목에 대한 업데이트 규정이 제외되어 대기상태가 빠졌기 때문에 해당 기능은 사용하지 않음 JK160405
/*
$Contents01 .= "
...
<table width='100%' cellpadding=0 cellspacing=0 border='0' align='left' style='table-layout:fixed;margin-top:30px;'>
	...
	<tr bgcolor='#ffffff' height='50' style='line-height:150%'>
		...
		<td class='input_box_item'>
			...
			<tr>
				<td>
					<input type=radio name='point_one_use_type' id='point_one_use_type_2' value='2' ".($reserve_data[point_one_use_type] == "2" ? "checked":"")." > 1회 사용한도 상품 구매 합계액의 <input type=text class='textbox' name='max_goods_sum_rate' value='".$reserve_data[max_goods_sum_rate]."' style='width:60px;' id='max_goods_sum_rate' ".($reserve_data[point_one_use_type] == "2" ? "validation='false'":"validation='false'")." title='Mileage 1회 In-Use 한도'>%  까지만 사용 가능 * 100%시 전액 적립금 사용 가능합니다.
				</td>
			</tr>
			</table>
		</td>
...
<input type='hidden' name='act' value='b2c_point'>";
