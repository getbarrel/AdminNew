<?php
//if(!(substr_count($_SERVER["HTTP_HOST"],'forbiz.co.kr') > 0)){
...
if($act == "verify"){
	//session_start();
   /* if($_SERVER['HTTP_HOST'] == 'dev.forbizkorea.co.kr'){
		if(strpos($_SERVER["REMOTE_ADDR"],'221.151.188.') === false && TRIM($id) == 'forbiz'){
            echo "<script type='text/javascript'>
					alert('접속하실 수 없습니다.');
        ...
        if( $id != 'forbiz'
            && $_SERVER["REMOTE_ADDR"] != '221.151.188.10'
			&& $_SERVER["REMOTE_ADDR"] != '221.151.188.11'
										...
										$db->query($ig_admin_loginChk_SQL);
											...
											$db->fetch();
													...
													if(trim($db->dt[fail_count]) >= "10") {
														...
														echo"
															<script>alert('지속된 실패 8회로 50분후 재시도 가능합니다.');
															document.location.href='./'</script>
															";
														exit;
