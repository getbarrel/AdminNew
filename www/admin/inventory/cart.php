...
<script  id='dynamic'></script>
	<tr>
		<td align='left' colspan=4> ".GetTitleNavigation("청구요청 목록", "재고관리 > 발주(사입)관리 > 청구요청 목록")."</td>
	</tr>";
...
$Contents .= "
						...
						<div class='tab'>
							...
							<tr>
								<td class='tab'>
									...
									<tr>
										<th class='box_01'></th>
										<td class='box_02' onclick=\"document.location.href='?order_list_type='\">업체별 청구요청 리스트</td>
										<th class='box_03'></th>
									</tr>
									</table>
									...
									<tr>
										<th class='box_01'></th>
										<td class='box_02' onclick=\"document.location.href='?order_list_type=P'\">담당자별 청구요청 리스트</td>
										<th class='box_03'></th>
									</tr>
									</table>
									";
		$Contents .= "
								...
								카트 품목수 : ".number_format(count($db->total))." 개
								</td>
							</tr>
							</table>
							</div>
...
if($order_list_type == "P"){
	...
	$sql = "select distinct odt.ci_ix, ici.customer_name
				from inventory_customer_info ici , inventory_order_detail_tmp odt
				where ici.ci_ix = odt.ci_ix and odt.company_id = '".$_SESSION["admininfo"]["company_id"]."'  ";
...
if($view == "innerview"){
	...
	$P->addScript = $Script;
	$P->Navigation = "재고관리 > 발주(사입)요청관리 > 청구요청 목록";
	$P->title = "청구요청 목록";
...
if($order_list_type == "P"){
	...
	*/
	$sql = "select odt.*, ici.customer_name, g.cid
				...
				inventory_customer_info ici
				where ici.ci_ix = odt.ci_ix and odt.ci_ix = '".$supply_company_info[ci_ix]."'
				and odt.gid = g.gid and charger_ix = '".$_SESSION["admininfo"]["charger_ix"]."'
	...
	$sql = "select odt.*,  listprice, coprice, sellprice , r.cid, ici.customer_name, pod.option_price
				...
				inventory_customer_info ici
				where ici.ci_ix = odt.ci_ix and odt.ci_ix = '".$supply_company_info[ci_ix]."'
				and odt.pid = sp.id and odt.company_id = '".$_SESSION["admininfo"]["company_id"]."'
				order by odt.regdate asc , pid , opnd_ix";
	*/
		$sql = "select odt.*, ici.customer_name, g.cid
				from  inventory_order_detail_tmp odt left join inventory_goods_item gi on odt.gid = gi.gid and odt.gi_ix = gi.gi_ix ,
				...
				inventory_customer_info ici
				where ici.ci_ix = odt.ci_ix and odt.ci_ix = '".$supply_company_info[ci_ix]."'
				and odt.gid = g.gid and odt.company_id = '".$_SESSION["admininfo"]["company_id"]."'
				order by odt.regdate asc , odt.gid , odt.gi_ix";
}
$mdb->query($sql);
...
<tr>
	<td colspan='2' height='25' style='padding:0px 0px;'>
		<img src='../images/dot_org.gif' align=absmiddle> <b class='middle_title'>".$supply_company_info[customer_name]." 청구요청 목록 </b>
	</td>
</tr>
</table>
...
if($mdb->total){
	...
	for($i=0;$i<$mdb->total;$i++){
		$mdb->fetch($i);
			...
			$gid = $mdb->dt[gid];
			$gname = $mdb->dt[gname];
			$gname_str .= $mdb->dt[gname];
			$order_cnt    = $mdb->dt[order_cnt];
			$item_name    = $mdb->dt[item_name];
			...
			if(file_exists($_SERVER["DOCUMENT_ROOT"].InventoryPrintImage($admin_config[mall_data_root]."/images/product", $pid, "c"))) {
				$img_str = InventoryPrintImage($admin_config[mall_data_root]."/images/product", $pid, "c");
			}else{
					...
					<td height='55' style='padding:5px;' nowrap>
						<table>
							<tr>
								...
								<span class='small'>".getIventoryCategoryPathByAdmin($mdb->dt[cid], 4)."</span><br>
								<b>$gname </b>";
...
*/
		$mstring .="
					...
					<td height='55' align=center>".$item_name."</td>
					<td height='55' nowrap>
...
$mstring .="
				<tr height=50><td colspan=8 align=center>청구요청 품목내역이  존재 하지 않습니다.</td></tr>
				";
...
$mstring .="
				<tr bgcolor=#ffffff height=35 >
					...
					<td colspan='5' class=m_td><b><font color='#333333'>총합계</font></b></td>
