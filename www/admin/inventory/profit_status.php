...
if($db->dbms_type == "oracle"){
	$sql = "select iohd.*, sum(iohd.delivery_cnt)as delivery_cnt, sum(iohd.delivery_price)/count(iohd.iohd_ix) as delivery_price, iohd.charger_ix, ipi.place_name, ici.customer_name, AES_DECRYPT(cmd.name,'".$db->ase_encrypt_key."') as name,
			(SELECT ROUND(SUM(input_cnt*input_price)/SUM(input_cnt),0) FROM inventory_input_history_detail WHERE gid=iohd.gid AND gi_ix=iohd.gi_ix) AS cast_price
				from inventory_output_history_detail iohd
				left join inventory_place_info ipi on ipi.pi_ix = iohd.pi_ix
				left join inventory_customer_info ici on ici.ci_ix = iohd.ci_ix and ici.customer_type = 'D'
				left join common_member_detail cmd on iohd.charger_ix = cmd.code
				$where group by iohd.gi_ix  $orderbyString	";
}else{
	$sql = "select iohd.*, sum(iohd.delivery_cnt)as delivery_cnt, sum(iohd.delivery_price)/count(iohd.iohd_ix) as delivery_price, iohd.charger_ix, ipi.place_name, ici.customer_name, AES_DECRYPT(UNHEX(cmd.name),'".$db->ase_encrypt_key."') as name,
			(SELECT ROUND(SUM(input_cnt*input_price)/SUM(input_cnt),0) FROM inventory_input_history_detail WHERE gid=iohd.gid AND gi_ix=iohd.gi_ix) AS cast_price
				from inventory_output_history_detail iohd
...
if($mode == "excel"){
	...
	$accounts_plan_priceXL->getActiveSheet(0)->setCellValue('B' . 1, "재고상품아이디");
	$accounts_plan_priceXL->getActiveSheet(0)->setCellValue('C' . 1, "재고상품명");
	$accounts_plan_priceXL->getActiveSheet(0)->setCellValue('D' . 1, "단품명");
	$accounts_plan_priceXL->getActiveSheet(0)->setCellValue('E' . 1, "판매수량");
	$accounts_plan_priceXL->getActiveSheet(0)->setCellValue('F' . 1, "판매단가");
	$accounts_plan_priceXL->getActiveSheet(0)->setCellValue('G' . 1, "판매금액");
	$accounts_plan_priceXL->getActiveSheet(0)->setCellValue('H' . 1, "입고단가");
	$accounts_plan_priceXL->getActiveSheet(0)->setCellValue('I' . 1, "입고금액");
	$accounts_plan_priceXL->getActiveSheet(0)->setCellValue('J' . 1, "이익단가");
	$accounts_plan_priceXL->getActiveSheet(0)->setCellValue('K' . 1, "이익금액");
	$accounts_plan_priceXL->getActiveSheet(0)->setCellValue('L' . 1, "이익률");
	...
	for ($i = 0; $i < count($goods_infos); $i++)
	{
		...
		$accounts_plan_priceXL->getActiveSheet()->setCellValue('B' . ($i + 2), $goods_infos[$i][gid]);
		$accounts_plan_priceXL->getActiveSheet()->setCellValue('C' . ($i + 2), $goods_infos[$i][gname]);
		$accounts_plan_priceXL->getActiveSheet()->setCellValue('D' . ($i + 2), $goods_infos[$i][item_name]);
		$accounts_plan_priceXL->getActiveSheet()->setCellValue('E' . ($i + 2), $goods_infos[$i][delivery_cnt]);
		$accounts_plan_priceXL->getActiveSheet()->setCellValue('F' . ($i + 2), number_format($goods_infos[$i][delivery_price]));
		$accounts_plan_priceXL->getActiveSheet()->setCellValue('G' . ($i + 2), number_format($goods_infos[$i][delivery_cnt] * $goods_infos[$i][delivery_price])) ;
		$accounts_plan_priceXL->getActiveSheet()->setCellValue('H' . ($i + 2), number_format($goods_infos[$i][cast_price]));
		$accounts_plan_priceXL->getActiveSheet()->setCellValue('I' . ($i + 2), number_format($goods_infos[$i][cast_price] * $goods_infos[$i][delivery_cnt]));
		$accounts_plan_priceXL->getActiveSheet()->setCellValue('J' . ($i + 2), number_format($profit));
		$accounts_plan_priceXL->getActiveSheet()->setCellValue('K' . ($i + 2), number_format($profit_price));
		$accounts_plan_priceXL->getActiveSheet()->setCellValue('L' . ($i + 2), $profit_margin);
	...
	header('Content-Disposition: attachment;filename="profit_status.xls"');
...
$Contents .=	"				<col width=20%>
									...
									<tr>
										...
										<td class='input_box_item'>
										 ".SelectInventoryInfo($pi_ix, 'pi_ix',"select","false")."
										</td>
									...
									<tr>
										...
										<td class='input_box_item'  colspan=3 align=left  style='padding-right:5px;padding-top:2px;'>
											...
											<tr>
												...
												<option value='iohd.gi_ix'>단품아이디(key)</option>
												</select></td>
...
$innerview = "
			...
			<tr height=30>
				<td align=left>
				<b>상품명</b>
				<a href='?cid=$cid&depth=0&orderby=gname&ordertype=asc'><img src='../image/orderby_desc.gif' border=0 align=top alt='가나다순' title='가나다순'></a>
				<a href='?cid=$cid&depth=0&orderby=gname&ordertype=desc'><img src='../image/orderby_asc.gif' border=0 align=top alt='가나다역순' title='가나다역순'></a>
				<b>출고날짜</b>
				<a href='?cid=$cid&depth=0&orderby=iohd.regdate&ordertype=desc'><img src='../image/orderby_desc.gif' border=0 align=top alt='최근등록순' title='최근등록순'></a>
				<a href='?cid=$cid&depth=0&orderby=iohd.regdate&ordertype=asc'><img src='../image/orderby_asc.gif' border=0 align=top alt='등록순' title='등록순'></a>
				...
				if(checkMenuAuth(md5($_SERVER["PHP_SELF"]),"E")){
					if ($mode == ""){
						$innerview .= "<a href='profit_status.php?".str_replace("mode=", "mode=excel",$search_query)."'><img src='../images/".$admininfo["language"]."/btn_excel_save.gif' border=0></a>";
					}else{
						$innerview .= "<a href='profit_status.php?".str_replace("mode=search", "mode=excel",$search_query)."'><img src='../images/".$admininfo["language"]."/btn_excel_save.gif' border=0></a>";
					}
				}else{
...
$innerview .= "
			...
			<tr bgcolor='#cccccc' height=30 align=center>
			  <td width='7%' class=m_td class=m_td  rowspan=2>재고단품코드</td>
			  <td width='*' class=m_td rowspan=2>이미지/재고상품명</td>
			  <td width='10%' class=m_td rowspan=2>단품명</td>
			  <td width='15%' class=m_td colspan=3>판매(출고가-평균)</td>
			  <td width='15%' class=m_td colspan=2>원가(입고가-평균)</td>
			  <td width='15%' class=m_td colspan=2>이익</td>
			  <td width='7%' class=e_td rowspan=2>이익율</td>
			</tr>
			<tr>
			  <td width='7%' class=m_td height=25px>수량</td>
			  <td width='7%' class=m_td>단가</td>
			  <td width='7%' class=m_td>금액</td>
			  <td width='7%' class=m_td>단가</td>
			  <td width='7%' class=m_td>금액</td>
			  <td width='7%' class=m_td>단가</td>
			  <td width='7%' class=m_td>금액</td>
...
if($db->dbms_type == "oracle"){
	/*
	$sql = "select iohd.gi_ix, sum((SELECT ROUND(SUM(input_cnt*input_price)/SUM(input_cnt),0) FROM inventory_input_history_detail WHERE gid=iohd.gid AND gi_ix=iohd.gi_ix)) AS cast_price,
				sum(iohd.delivery_cnt) as delivery_cnt, sum(iohd.delivery_price)/count(iohd.iohd_ix) as delivery_price
				from inventory_output_history_detail iohd
				left join inventory_place_info ipi on ipi.pi_ix = iohd.pi_ix
				left join inventory_customer_info ici on ici.ci_ix = iohd.ci_ix and ici.customer_type = 'D'
				left join common_member_detail cmd on iohd.charger_ix = cmd.code
				$where group by iohd.gi_ix  $orderbyString	";
	$sql = " select g.gid, gi.gi_ix, sum(iohd.delivery_cnt) as delivery_cnt, sum(iohd.delivery_price)/count(iohd.iohd_ix) as delivery_price, ROUND(SUM(ih.input_cnt*ih.input_price)/SUM(ih.input_cnt),0) as cast_price from inventory_goods g
		  left join inventory_goods_item gi on g.gid = gi.gid
		  left join inventory_out_history_dt iohd on gi.gid = iohd.gid and gi.gi_ix = iohd.gi_ix
		  left join inventory_in_history_dt ih on gi.gid = ih.gid and gi.gi_ix = ih.gi_ix
		  , inventory_place_info pi, inventory_customer_info ici, common_member_detail cmd
		  $where  and  iohd.pi_ix = pi.pi_ix and iohd.ci_ix = ici.ci_ix and iohd.charger_ix = cmd.code

		 group by g.gid, gi.gi_ix $orderbyString ";
		 */
	$sql = "  select iohd.gid, iohd.gi_ix,iohd.gname,iohd.item_name, iohd.iohd_ix, sum(iohd.delivery_cnt) as delivery_cnt, sum(iohd.delivery_price)/count(iohd.iohd_ix) as delivery_price, ROUND(SUM(ih.input_cnt*ih.input_price)/SUM(ih.input_cnt),0) as cast_price
	from inventory_output_history_detail iohd left join inventory_input_history_detail ih on iohd.gid = ih.gid and iohd.gi_ix = ih.gi_ix
		  , inventory_place_info pi, inventory_customer_info ici, common_member_detail cmd
		   $where  and iohd.pi_ix = pi.pi_ix and iohd.ci_ix = ici.ci_ix and iohd.charger_ix = cmd.code
		 group by iohd.gid,iohd.gi_ix,iohd.gname,iohd.item_name,iohd.iohd_ix $orderbyString  ";
}else{
	$sql = "select iohd.*, sum(iohd.delivery_cnt)as delivery_cnt, sum(iohd.delivery_price)/count(iohd.iohd_ix) as delivery_price, iohd.charger_ix, ipi.place_name, ici.customer_name, AES_DECRYPT(UNHEX(cmd.name),'".$db->ase_encrypt_key."') as name,
			(SELECT ROUND(SUM(input_cnt*input_price)/SUM(input_cnt),0) FROM inventory_input_history_detail WHERE gid=iohd.gid AND gi_ix=iohd.gi_ix) AS cast_price
				from inventory_output_history_detail iohd
...
if($db->total == 0){
	...
	$innerview .= "<tr height=70>
					<td class='list_box_td list_bg_gray'>".$db->dt[gid]."</td>
					<td class='list_box_td point' style='padding:5px 5px;' >
					<table>
						<tr>
					...
					<td class='list_box_td list_bg_gray'> ".$db->dt[item_name]."</td>
					<td class='list_box_td'>".$db->dt[delivery_cnt]."</td>
					<td class='list_box_td list_bg_gray'>".number_format($db->dt[delivery_price])."</td>
					<td class='list_box_td' style='padding:0px 5px;' >".number_format($total_delivery_price)."</td>
...
";
	...
	$innerview .= "<tr height=30>
					...
					<td class='list_box_td list_bg_gray' ></td>
					<td class='list_box_td  ' >".number_format($sum_profit_price)."</td>
					<td class='list_box_td list_bg_gray ' >".number_format($sum_profit_margin)."%</td>
