...
*/
	if($startDate != "" && $endDate != ""){
		$where .= " and  date_format(io.regdate,'%Y%m%d') between  '$search_startDate' and '$search_endDate' ";
	}



	if($admininfo[admin_level] == 9){
		$sql = "select COUNT(*) as total from inventory_order io
					left join inventory_order_detail iod
...
*/
	...
	if($admininfo[admin_level] == 9){
		$sql = "select COUNT(*) from inventory_order  io
					left join inventory_order_detail  iod
					on io.ioid = iod.ioid  $where ";
	}else{
		/*
