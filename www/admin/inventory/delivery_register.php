...
$Contents ="
<form  name='input_frm' method='post' onsubmit='return CheckFormValue(this)' action='./register.act.php'   target='iframe_act'><!--action='./purchase.act.php' -->
...
$Contents .= "
						...
						<tr height='30'>
							...
							<td class='input_box_item' >
								<table cellpadding=0 cellspacing=0 border=0 bgcolor=#ffffff width=100%>
									...
									<tr>
										<TD nowrap>
											<input type='text' name='vdate' class='textbox' value='".$db->dt[vdate]."' style='height:20px;width:70px;text-align:center;' id='end_datepicker' validation='true' title='".$sub_title."일'>
										</TD>
									</tr>
								</table>
							</td>
							...
							".CompayChargerSearch($_SESSION["admininfo"]["company_id"] ,$_SESSION["admininfo"]["charger_ix"])."
							</td>
						</tr>
						<tr height='30'>";

	$Contents .= "						
							<td class='input_box_title'>".$sub_title."창고</td>
							<td class='input_box_item' >								
								".SelectEstablishment($move_company_id,"move_company_id","select","true","onChange=\"loadPlace(this,'move_pi_ix')\" ")."
								".SelectInventoryInfo($move_company_id,$db->dt[move_pi_ix],'move_pi_ix','select','true', "validation=true title='이동창고' onChange=\"loadPlaceSection(this,'move_ps_ix')\"  ")."
								".SelectSectionInfo($db->dt[move_pi_ix],$db->dt[move_ps_ix],'move_ps_ix',"select","true"," title='보관장소' ")." 
							</td>";
	...
	$Contents .= "
						<tr height='30'>
							<td class='input_box_title'> 출고처 <img src='".$required3_path."'></td>
...
$Contents .= "
						<tr>
							...
							<td class='input_box_title'>".$sub_title." 상태</td>
							<td class='input_box_item' >";
if($page_type == 'stocked'){
		$Contents .= "";
}else{
...
$Contents .= "
								".getInventoryStatus($db->dt[status], "status"," onchange='changeMoveStatus(this);' ")." 
							</td>
						</tr>";
}
$Contents .= "
...
<tr>
	...
	<td align=right><a href=\"javascript:PopGoodsSelect();\"><img src='../images/".$admininfo["language"]."/btc_goods_add.gif' border='0' align='absmiddle'  style='cursor:pointer;'></a></td>
</tr>
<tr>
...
$Contents .= "
	<table cellpadding=3 cellspacing=0 border=0 bgcolor=#ffffff width=100% onselect='return false;' align=center class='list_table_box' id='warehouse_move_apply_list'>
				...
				<tr align=center height=30 style='font-weight:bold;' >
					...
					<td class=m_td rowspan=2 nowrap>제조일</td>
					<td class=m_td colspan=3>사업장/보관장소</td>
					<td class=m_td rowspan=2>재고</td>
					<td class=m_td rowspan=2 nowrap>수량</td>
					<td class=m_td rowspan=2  nowrap>출고가격</td>
				...
				<tr bgcolor=#ffffff height=30 >
					...
					<td class=m_td><b id='amount_sum'></b></td>
					<td align=center class=m_td colspan='2'><b> <font class='blk' id='font_sum_price' style='font-size:12px;font-family:arial;'>".number_format($order_totalprice)." </font></b><font class='blk'> 원</font></td>
				</tr>-->";
...
$Contents .="
				<tr height=30 depth=1 ondblclick=\"javascript:PopGoodsSelect();\"  help_height='35' help_html='더블클릭시 품목을 선택할 수 있는 창이 노출되게 됩니다.' style='cursor:pointer;'>
					<td align=center>
						<input type=hidden class='nonborder' id='wmd_ix'  name=item_infos[".$i."][wmd_ix] value='".$db->dt[wmd_ix]."'>
					...
					<td align=center><span  id='expiry_date_text'>".$db->dt[expiry_date]."</span><input type=hidden class='textbox numeric' name='item_infos[".$i."][expiry_date]' id='expiry_date' value='".$db->dt[expiry_date]."'> </td>
...
$Contents .="
				<tr height=30 depth=1 ondblclick=\"javascript:PopGoodsSelect()\"  help_height='35' help_html='더블클릭시 품목을 선택할 수 있는 창이 노출되게 됩니다.' style='cursor:pointer;'>
					...
					<td align=center><span  id='expiry_date_text'>".$db->dt[expiry_date]."</span><input type=hidden class='textbox numeric' name='item_infos[0][expiry_date]' id='expiry_date' value='$expiry_date'> </td>
					<td align=center ><span id='company_name'>".$company_name."</span></td>
					...
					<td style='text-align:center;'>
						<input type=text class='textbox numeric amount' name='item_infos[0][amount]'  id='amount'
						onkeyup=\"if($(this).val()=='0'){alert('수량 0을 입력하실수 없습니다.');$(this).val('1')}$(this).parent().parent().find('#total_price').html( $(this).val()*$(this).parent().parent().find('#price').val());
						$(this).parent().parent().find('#in_total_price').val( $(this).val()*$(this).parent().parent().find('#price').val());change_totalprice()\"
						value='$amount' size=8 title='수량' ".(($move_status == "" || $move_status == "MA") ? " validation=true  style='width:80%;text-align:right;padding:0 5px 0 0'":"style='border:0px;width:80%;text-align:right;padding:0 5px 0 0' readonly ")." > 
					</td>
					
					<td align=center id='td_price'>".$price." <input type=text class='textbox numeric price' name='item_infos[0][price]'  id='price' 
					onkeyup=\"$(this).parent().parent().find('#total_price').html( $(this).val()*$(this).parent().parent().find('#amount').val());
					$(this).parent().parent().find('#in_total_price').val( $(this).val()*$(this).parent().parent().find('#amount').val());change_totalprice()\"
					value='$price' size=8 title='출고단가' ".(($move_status == "" || $move_status == "MA") ? " validation=true  style='width:80%;text-align:right;padding:0 5px 0 0'":"style='border:0px;width:80%;text-align:right;padding:0 5px 0 0' readonly ")." > </td>
					<td align=center id='total_price'>".number_format($totalprice)."</td>
					<input type='hidden' name='in_total_price_".$db->dt[gid]."' id='in_total_price' value='0'>
				</tr>";
					...
					<td colspan=4>
						<table cellpadding=10 cellspacing=0 border=0 bgcolor=#ffffff width=100% align=center>
							<tr>
								...
								총 주문금액 : <span class='blk' id='sum_price' style='font-size:12px;font-family:arial;'>".number_format($order_totalprice + $total_delivery_price)."</span> 원</td>
							</tr>
...
function GoodsSelect(gid, gname, unit,unit_text, standard, price, company_name, place_name, section_name, pi_ix, ps_ix, vdate, expiry_date,stock,offline_wholesale_price,sellprice){
   var tbody = $('#warehouse_move_apply_list tbody');  	
   var total_rows = tbody.find('tr[depth^=1]').length;  
   var rows = tbody.find('tr[depth^=1]').length;  
	...
	if(thisRow.find('#gid_text').html() == ''){ 
		...
		thisRow.find('#price').val(sellprice);
		...
		$.ajax({ 
			...
			success: function(data){ 
				...
				thisRow.find('#price').html(data.price);
		...
		*/
		...
		newRow.find('#price').val(sellprice);
...
function change_totalprice(){
	...
	var sum_price = 0 ;
	$('input[name^=in_total_price]').each(function(){
			sum_price += parseInt($(this).val());
	});
	//sumprice = Number(sum_price);
	$('#sum_price').html(sum_price);
	//$('#font_sum_price').html(sum_price);
}

function PopGoodsSelect(){
	ShowModalWindow('goods_select.php?page_type=".$page_type."&type_div=".$type_div."',1000,800,'goods_select');		
...
CREATE TABLE IF NOT EXISTS `inventory_warehouse_move_detail` (
  ...
  `amount` int(8) DEFAULT NULL COMMENT '이동요청수량',
