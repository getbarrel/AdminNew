...
include('../inventory/exception_stocked_info.lib.php');
...
if($act == "new_excel_input"){	//대량상품등록 엑셀정보 저장 
	...
	$shmop = new Shared("exception_stocked_input_excel_".$page_type."_".$_SESSION["admininfo"]["charger_ix"]);
	$shmop->filepath = $_SERVER["DOCUMENT_ROOT"].$admin_config[mall_data_root]."/_shared/";
	//echo $shmop->filepath;
	$shmop->SetFilePath();
	$upload_excel_data = $shmop->getObjectForKey("exception_stocked_input_excel_".$page_type."_".$_SESSION["admininfo"]["charger_ix"]);
	...
	$shmop->setObjectForKey($upload_excel_data, "exception_stocked_input_excel_".$page_type."_".$_SESSION["admininfo"]["charger_ix"]) ;

	if($page_type == 'input'){
		echo "<script language='javascript' src='../js/message.js.php'></script><script>top.location.href='./exception_stocked_input_excel.php?up_mode=new_upload'</script>";
	}else{
		echo "<script language='javascript' src='../js/message.js.php'></script><script>top.location.href='./exception_stocked_input_excel.php?up_mode=new_upload'</script>";
	}
...
if($act == "single_goods_reg" && strlen($p_no)>0){		//메모리에 저장된 대량상품등록 정보로 상품추가 부분 2014-06-18 이학봉 
	...
	foreach($select_excel_goods_infos as $key => $value){
		...
		*/
		...
		//1. 입고유형
		$h_type_array = array('01','02','03','04','05','06','07','08','17');
		if(!in_array($h_type,$h_type_array)){
			$insert_yn=false;
			$result_massage .= "<span class='red'>입고유형 값이 정상적이지 않습니다.</span><br/>";
		}
		...
		if($insert_yn){
			if($upload_excel_data[session_id()][$key][status] !='C'){
				...
				$upload_excel_data[session_id()][$key][status_message] = "입고 등록완료";
				echo "<span class='blue'>입고 등록완료</span>";
			}else{
...
if($act == "bad_goods_info_excel"){
	...
	if(count($bad_info_excel_data) > 0){
		...
		header('Content-Type: application/vnd.ms-excel');
		header('Content-Disposition: attachment;filename=bad_exception_stocked_info_excel.xls');
		header('Cache-Control: max-age=0');
		...
		if($page_type == 'input'){
			...
			<!--
				location.href='./exception_stocked_input_excel.php?up_mode=new_upload';
