...
$Contents .=	"
					...
					<table width='100%' border=0 cellpadding=0 cellspacing=1 class='input_table_box'>
						...
						<tr height=30 align=center>
							...
							<td class='input_box_item' id='select_category_path3' align=left style='padding-left:10px;' colspan=3>
								<a href='batch_stock_adjustment_input_excel.xls'><img src='../images/".$admininfo["language"]."/btn_sample_excel_save.gif' align=absmiddle></a>
							</td>
						</tr>
						<tr height=30 align=center>
							<td class='input_box_title'  ><b>기타 코드</b></td>
							<td class='input_box_item' id='select_category_path3' align=left style='padding-left:10px;' colspan=3>
								<a href='stock_adjustment_input_excel.act.php?act=ect_code_down'><img src='../images/".$admininfo["language"]."/btn_sample_excel_save.gif' align=absmiddle></a>
	...
	if(checkMenuAuth(md5($_SERVER["PHP_SELF"]),"C") || true){
		...
		if($admininfo[mall_type] == "BW"){
			$download_excel_file = "batch_stock_adjustment_input_excel.xls";
		}else{
			$download_excel_file = "batch_stock_adjustment_input_excel.xls";
		}
...
$Contents .="
			<tr>
				...
				<form name='excel_input_form' method='post' action='stock_adjustment_input_excel.act.php' enctype='multipart/form-data' onsubmit='return CheckFormValue(this)' target=''><!--iframe_act-->
...
$Contents .="
				<tr height=30 align=center>
					...
					<td class='input_box_item'>
						...
						* 	batch_stock_adjustment_input_excel.xls ( 엑셀 저장시 97~03년 양식으로 저장하시고 등록하세요.)
					</td>
				</tr>
				<!--
				<tr height=30 align=center>
...
function UploadExcelGoodsRegAjax(total_no,now_no){
	...
	$.ajax({
		...
		url: './stock_adjustment_input_excel.act.php?page_type=input',
		dataType: 'html',
		async: true,
		page_type :'input',
		beforeSend: function(){
			...
			try{
				if(total_no > now_no){
					...
					if(confirm('등록완료되었습니다. 등록이 실패한 품목정보를 엑셀로 다운받으시겠습니까?')){
						location.href='./stock_adjustment_input_excel.act.php?act=bad_goods_info_excel&page_type=input';
					}
				}
			}catch(e){
				alert('123');
...
function MakeUploadExcelData(){
	...
	//auth(8);
	$shmop = new Shared("stock_adjustment_input_excel_input_".$_SESSION["admininfo"]["charger_ix"]);
	//	$shmop->clear();
	$shmop->filepath = $_SERVER["DOCUMENT_ROOT"].$_SESSION["admininfo"]["mall_data_root"]."/_shared/";
	$shmop->SetFilePath();
	$upload_excel_data = $shmop->getObjectForKey("stock_adjustment_input_excel_input_".$_SESSION["admininfo"]["charger_ix"]);
