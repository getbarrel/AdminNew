...
if($excel_type == "stock_report_excel"){
	$excel_name = "실시간 재고현황 엑셀 ";
	$act = "stock_report_setting";
...
}else if($excel_type == 'purchase_ready'){
	$excel_name = "발주리스트 엑셀 ";
	$act = "purchase_list_setting";
	$sql = "select conf_val from inventory_config where charger_ix='".$admininfo[charger_ix]."' and conf_name='inventory_excel_".$info_type."' ";
...
<form name='excel_out_frm' method=\"post\" enctype='multipart/form-data' action='excel_config.act.php' onsubmit='return CheckValue(this);' target='act' ><!-- -->
<input type='hidden' name='act' value='".$act."'><input type='hidden' name='info_type' value='".$info_type."'>
...
$Contents .= "
		<tr>
			...
			if($db->dt[conf_val] && $stock_report_excel){
				$Contents .= "<div  class='point_color' style='width:791px;padding:10px;text-align:center;border:1px silver solid'><b style='font-size:15px;'>$excel_name 설정정보 입니다.</b> - 내용을 변경 하실 수 있습니다..</div>";
			}else{
				$Contents .= "<div  class='point_color' style='width:791px;padding:10px;text-align:center;border:1px silver solid'><b style='font-size:15px;'>$excel_name 설정 정보가 없습니다.</b> - 원하는 항목과 노출순서를 정하신후 저장후 엑셀 다운로드를 하실 수 있습니다.</div>";
			}
...
$check_colums = unserialize(stripslashes($stock_report_excel_checked));
    ...
    for($i=0;$i < count($sortlist);$i++){
		//if($check_colums[$sortlist[$i]]){
			$Contents .= "<li class='ui-state-default leftmenu' id='".$sortlist[$i]."'  style='float:left;height:25px;width:19%;padding:4px 0px 0px 3px;margin:1px;'>
	...
	foreach($colums as $key => $value){
			$Contents .= "<li class='ui-state-default leftmenu' id='".$key."' style='float:left;height:25px;width:19%;padding:4px 0px 0px 3px;margin:1px;' >
							<table width=150 border=0>
							<col width=10>
							<col width=25>
							<col width=*>
