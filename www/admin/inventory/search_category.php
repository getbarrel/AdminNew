		...
		<td align=center colspan=2>
			...
			<tr>
				<td>
					...
					$db->query("select * from ".$tb." where depth='0' order by cid ASC");
...
<!--
	...
	function get_cagetory_info(id,target_id){
		...
		$('#'+id).click(function(){
			...
			if(cid){
				$.ajax({
				    url : './inventory_category.save.php',
				    type : 'POST',
				    data : {cid:cid,
...
function add_department(){
	...
	$('#add_list option').each(function(){
		...
		var add_html = '<tr style=\"height:26px;\" id=\"row_'+dp_ix+'\"><td><input type=hidden name=cid[] id=\"cid_'+dp_ix+'\" value=\"'+dp_ix+'\">'+dp_text+'</td><td><a href=\"javascript:void(0)\" onClick=\"cid_del(\''+dp_ix+'\')\"><img src=\"../images/".$_SESSION["admininfo"]["language"]."/btc_del.gif\" border=0></a></td></tr>';
