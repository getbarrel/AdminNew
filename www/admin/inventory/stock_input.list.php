...
$where = "where ihd_ix is not null ";
...
if($ci_ix != ""){
	$where .= "and iihd.ci_ix = '".$ci_ix."' ";
}

if($pi_ix != ""){
	$where .= "and iihd.pi_ix = '".$pi_ix."' ";
}
if ($vFromYY != "")	$where .= " and date_format(iihd.regdate, '%Y%m%d') between $startDate and $endDate ";

$sql = "select count(*) as total
			from inventory_input_history_detail iihd
			left join inventory_customer_info ici	on ici.ci_ix = iihd.ci_ix	and ici.customer_type = 'E'
			left join inventory_place_info ipi on ipi.pi_ix = iihd.pi_ix
			$where ";
...
if($db->dbms_type == "oracle"){
	$sql = "select iihd.*,  ipi.place_name, ici.customer_name , AES_DECRYPT(cmd.name,'".$db->ase_encrypt_key."') as name
			from inventory_input_history_detail iihd
			left join inventory_customer_info ici on ici.ci_ix = iihd.ci_ix and ici.customer_type = 'E'
			left join inventory_place_info ipi on ipi.pi_ix = iihd.pi_ix
			left join common_member_detail cmd on iihd.charger_ix = cmd.code
			$where $orderbyString";
}else{
	$sql = "select iihd.*,  ipi.place_name, ici.customer_name , AES_DECRYPT(UNHEX(cmd.name),'".$db->ase_encrypt_key."') as name
			from inventory_input_history_detail iihd
			left join inventory_customer_info ici on ici.ci_ix = iihd.ci_ix and ici.customer_type = 'E'
			left join inventory_place_info ipi on ipi.pi_ix = iihd.pi_ix
			left join common_member_detail cmd on iihd.charger_ix = cmd.code
...
if($mode == "excel"){
	...
	$accounts_plan_priceXL->getActiveSheet(0)->setCellValue('E' . 1, "입고처");
	$accounts_plan_priceXL->getActiveSheet(0)->setCellValue('F' . 1, "창고");
	$accounts_plan_priceXL->getActiveSheet(0)->setCellValue('G' . 1, "입고수량");
	$accounts_plan_priceXL->getActiveSheet(0)->setCellValue('H' . 1, "실입고가");
	$accounts_plan_priceXL->getActiveSheet(0)->setCellValue('I' . 1, "총액");
	$accounts_plan_priceXL->getActiveSheet(0)->setCellValue('J' . 1, "입고유형/비고");
	$accounts_plan_priceXL->getActiveSheet(0)->setCellValue('K' . 1, "작성자");
	$accounts_plan_priceXL->getActiveSheet(0)->setCellValue('L' . 1, "입고일");
	...
	for ($i = 0; $i < count($goods_infos); $i++)
	{
		...
		$accounts_plan_priceXL->getActiveSheet()->setCellValue('E' . ($i + 2), $goods_infos[$i][customer_name]);
		$accounts_plan_priceXL->getActiveSheet()->setCellValue('F' . ($i + 2), $goods_infos[$i][place_name]);
		$accounts_plan_priceXL->getActiveSheet()->setCellValue('G' . ($i + 2), number_format($goods_infos[$i][input_cnt]));
		$accounts_plan_priceXL->getActiveSheet()->setCellValue('H' . ($i + 2), number_format($goods_infos[$i][input_price])) ;
		$accounts_plan_priceXL->getActiveSheet()->setCellValue('I' . ($i + 2), number_format($goods_infos[$i][input_cnt] * $goods_infos[$i][input_price]));
		$accounts_plan_priceXL->getActiveSheet()->setCellValue('J' . ($i + 2), selectDeliveryType($goods_infos[$i][input_type],'','I','text')."/".$goods_infos[$i][input_msg]);
	...
	header('Content-Disposition: attachment;filename="stock_input.list.xls"');
...
$Contents =	"<table cellpadding=0 cellspacing=0 width='100%'>
			 <tr>
			    <td align='left' colspan=4 > ".GetTitleNavigation("입고내역", "재고관리 > $page_title")."</td>
			</tr>
...
$Contents .=	"
									...
									<col width=30%>
									<tr>
										<td class='input_box_title'>   <label for='regdate'>입고일자 </label><input type='checkbox' name='regdate' id='regdate' value='1' onclick='ChangeIncomeDate(document.search_frm);' ".CompareReturnValue("1",$regdate,"checked")."></td>
										<td class='input_box_item' colspan=3>
											<table border=0 cellpadding=0 cellspacing=0>
												<TD nowrap>
													...
													<SELECT onchange=javascript:onChangeDate(this.form.vFromYY,this.form.vFromMM,this.form.vFromDD) name=vFromMM></SELECT> 월 <SELECT name=vFromDD></SELECT>
												</TD>
									...
									<tr>
										...
										<td class='input_box_item' colspan=3 style='padding-right:5px;padding-top:2px;'>
											...
											<tr>
												...
												<INPUT id=search_texts onkeyup='findNames();' class='textbox' value='' onclick='findNames();'  clickbool='false' style=' FONT-SIZE: 12px; WIDTH: 360px; COLOR: #736357; BACKGROUND-COLOR: #ffffff' name=search_text autocomplete='off' validation=false  title='검색어'><!--onFocusOut='clearNames()'--><br>

												<DIV id=popup style='display: none; width: 368px; POSITION: absolute; height: 150px; backgorund-color: #fffafa' ><!--onmouseover=focusOutBool=false;  onmouseout=focusOutBool=true;-->
												...
												<td colspan=2 style='padding-left:5px;'><span class='small'>* 상품명의 일부를 입력하시면 자동검색됩니다. 2자 이상 입력해주세요</span></td>
											</tr>
											</table>
										</td>
									</tr>
...
if(checkMenuAuth(md5($_SERVER["PHP_SELF"]),"E")){
		...
		<a href='excel_config.php?info_type=stock_input&".$QUERY_STRING."' rel='facebox' ><img src='../images/".$admininfo["language"]."/btn_excel_set.gif'></a></span>";
	}else{
		$innerview .= "
...
if(checkMenuAuth(md5($_SERVER["PHP_SELF"]),"E")){
$innerview .= "
				".($view_type == "list" ? "<a href='stock_input.list.php?".str_replace("view_type=list", "mode=excel",$_SERVER["QUERY_STRING"])."'><img src='../images/".$admininfo["language"]."/btn_excel_save.gif' border=0></a>" : "")."";
}else{
$innerview .= "
...
$innerview .= "
			...
			<tr bgcolor='#cccccc' height=30 align=center>
				...
				<td class=m_td  rowspan=2>".OrderByLink("상품명", "gname", $ordertype)."</td>
				<td class=m_td  rowspan=2>규격</td>
				<td class=m_td  rowspan=2>입고처</td>
				<td class=m_td colspan=3 nowrap>현위치</td>
				<td class=m_td  rowspan=2>단위</td>
				<td class=m_td  rowspan=2>입고유형</td>
				<td class=m_td  rowspan=2 nowrap>입고수량</td>
				<td class=m_td  rowspan=2>실입고가</td>
				<td class=m_td  rowspan=2 style='padding:0px 5px;' nowrap>발주번호<br/> or 주문번호</td>
				<td class=m_td  rowspan=2>작성자</td>
				<td class=m_td  rowspan=2>".OrderByLink("입고일", "regdate", $ordertype)."<!--a href='?view_type=".$view_type."&orderby=date&company_id=$company_id'>날짜</a--></td>
...
if($orderby != "" && $ordertype != ""){
	...
	$orderbyString = " order by iihd.regdate desc ";
}
...
if($db->total == 0){
	$innerview .= "<tr bgcolor=#ffffff height=50><td colspan=12 align=center> 등록된 $page_title이 없습니다.</td></tr>";
}else{
	...
	for ($i = 0; $i < $db->total; $i++)
	{
		...
		if($db->dt[input_type] == "DI"){
			$input_text = "일반입고";
		}else if($db->dt[input_type] == "CC"){
			$input_text = "주문취소";
		}
		...
		$innerview .= "
					...
					<td class='list_box_td list_bg_gray'>".$db->dt[company_name]."</td>
					...
					<td class='list_box_td '>".selectDeliveryType($db->dt[input_type],'','I','text')."</td>
					...
					<td class='list_box_td list_bg_gray' nowrap>".$db->dt[regdate]."</td>
					<td class='list_box_td '>".$db->dt[input_msg]."</td>
					</tr>
					";
	...
	$innerview .= "<tr height=30>
					...
					<td class='list_box_td ' >".$sum_input_cnt."</td>
					<td class='list_box_td ' colspan=5></td>
					</tr>
					";
	}
