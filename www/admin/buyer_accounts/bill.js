...
function CheckStatusUpdate(frm){
	...
	if(frm.act.value == 'order_bill' || frm.act.value == 'preiod_ready'){
		for(i=0;i < frm.oid.length;i++){
			if(frm.oid[i].checked){
				checked_bool = true;
			}
		}
	}else if (frm.act.value == 'preiod_bill'){
...
function tax_compute(index,total_price){
	//alert(total_price);
	//$("input[name='coprice["+index+"]']").val(0);
	$("input[name='tax["+index+"]']").val(total_price);
}

function tax_compute2(index,date,total_price){
	//alert(total_price);
	//$("input[name='coprice["+index+"]["+date+"]']").val(0);
	$("input[name='tax["+index+"]["+date+"]']").val(total_price);
}
