...
function SearchCharger(user_name,user_id,user_tel,user_phone,level,gp,ucode){
	
	user_tel	=	user_tel.split('-');
	user_phone	=	user_phone.split('-');
	$('#user_name',opener.document).val(user_name);
	$('#user_id',opener.document).val(user_id);
	$('#user_code',opener.document).val(ucode);
	$('#tel_1',opener.document).val(user_tel[0]);
	$('#tel_2',opener.document).val(user_tel[1]);
	$('#tel_3',opener.document).val(user_tel[2]);
	$('#phone_1',opener.document).val(user_phone[0]);
	$('#phone_2',opener.document).val(user_phone[1]);
	$('#phone_3',opener.document).val(user_phone[2]);
	$('#user_group',opener.document).val(gp);
...
if($db->total){
	for($i=0;$i  < $db->total; $i++){
		...
		$Contents .= "<tr height=25 style='text-align:center;cursor:pointer;' onclick=\"if($(this).children().css('background-color').replace(/\s/g,'')=='rgb(249,222,209)'){SearchCharger('".$db->dt[name]."','".$db->dt[id]."','".$db->dt[tel]."','".$db->dt[pcs]."','".$db->dt[level_ix]."','".$db->dt[gp_ix]."','".$db->dt[code]."');}else{changeBgColor(this)}\">
