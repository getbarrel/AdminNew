...
if($mall_ix != ""){
	$search_str .= " and mall_ix = '".$mall_ix."' ";
}

if($sdate != "" && $edate != ""){
	$sdate = str_replace("/","",$sdate);
	$edate = str_replace("/","",$edate);
	//$search_str .= " and  MID(replace(e.regdate ,'-',''),1,8) between  $sdate and $edate ";
	$search_str .= " and date_format(e.regdate,'%Y-%m-%d') between  '".$sdate."' and '".$edate."' ";
}
...
if(count($estimate_div) > 0){
	$search_str .= " and e.estimate_div in (";
...
$Contents .=	"				<col width=20%>
										...
										<table cellpadding=0 cellspacing=2 border=0 bgcolor=#ffffff width=100%>
											...
											<tr>
												...
												<TD style='padding:0px 10px'>
													...
													<a href=\"javascript:select_date('$voneweeklater','$today',1);\"><img src='../images/".$admininfo[language]."/btn_1week.gif'></a>
													<a href=\"javascript:select_date('$v15later','$today',1);\"><img src='../images/".$admininfo[language]."/btn_15days.gif'></a>
													<a href=\"javascript:select_date('$vonemonthlater','$today',1);\"><img src='../images/".$admininfo[language]."/btn_1month.gif'></a>
													<a href=\"javascript:select_date('$v2monthlater','$today',1);\"><img src='../images/".$admininfo[language]."/btn_2months.gif'></a>
													<a href=\"javascript:select_date('$v3monthlater','$today',1);\"><img src='../images/".$admininfo[language]."/btn_3months.gif'></a>
												</TD>
											</tr>
										</table>
									  </td>
									...
									<tr height='30' id='move_status_tr'>
										...
										<td class='input_box_item' colspan=3>
											<input type=checkbox name='estimate_type[]' id='estimate_type_1' value='1'><label for='estimate_type_1'>가이드 견적서</label>
											<input type=checkbox name='estimate_type[]' id='estimate_type_2' value='2'><label for='estimate_type_2'>셀프 견적서</label>
											<input type=checkbox name='estimate_type[]' id='estimate_type_3' value='3'><label for='estimate_type_3'>전문가 견적서</label>
											<input type=checkbox name='estimate_type[]' id='estimate_type_4' value='4'><label for='estimate_type_4'>프리 견적서</label>
										</td>
									</tr>-->
									<tr height='30' id='move_status_tr'>
										...
										<td class='input_box_item'>
											...
											<input type=checkbox name='estimate_div[]' id='estimate_div_2' value='2' ".$estimate_div_2."><label for='estimate_div_2'>대량구매문의</label>
										</td>
									...
									<tr height='30' id='move_status_tr'>
										...
										<td class='input_box_item' colspan=3>
											<input type=checkbox name='estimate_status[]' id='estimate_status_1' value='1'><label for='estimate_status_1'>전체</label>
											<input type=checkbox name='estimate_status[]' id='estimate_status_2' value='2'><label for='estimate_status_2'>견적대기</label>
											<input type=checkbox name='estimate_status[]' id='estimate_status_3' value='3'><label for='estimate_status_3'>견적취소</label>
											<input type=checkbox name='estimate_status[]' id='estimate_status_4' value='4'><label for='estimate_status_4'>견적진행중</label>
											<input type=checkbox name='estimate_status[]' id='estimate_status_5' value='5'><label for='estimate_status_5'>견적기간만료</label>
											<input type=checkbox name='estimate_status[]' id='estimate_status_7' value='7'><label for='estimate_status_7'>구매확정</label>
										</td>
									</tr>-->
									</tr>
										...
										<td class='input_box_item'  align=left  style='padding-right:5px;padding-top:2px;'>
											...
											<tr>
												<td>
													...
													<option value='e.estimate_title'>견적제목</option>
													<option value='e.estimate_code'>견적번호</option>
													</select>
												</td>
...
function EstimateApplyList($ecid, $depth,$search=''){
	...
	$sql = "select e.est_ix from ".TBL_SHOP_ESTIMATES." as e where 1 $where $search";
	$db->query($sql);
	$total = $db->total;
	...
	$mString .= "
		...
		<td align='center' class='m_td' style='background-color:#fff7da;' nowrap=''><font color='#000000' class='small'><b>사이트 구분</b></font></td>
	...
	if ($db->total == 0){
		...
		for($i=0;$i<$db->total;$i++){
			...
			if($db->dt[mall_ix]){
				...
				$mdb->query($sql);
				$mdb->fetch();
				$mall_domain = $mdb->dt[mall_domain];
			}else{
				$mall_domain = "전체사이트";
			}
						...
						<td class='' style='background-color:#fff7da;height:30px;font-weight:bold;' colspan='12'>
							<table width='100%' border='0' cellpadding='0' cellspacing='0' align='center'>
								...
								<col width='6%'>
									...
									<td align='center' style='background-color:#fff7da;' nowrap=''><font color='blue' class='small'><b><a href='/admin/estimate/guide_estimate.php?est_ix=".$db->dt[est_ix]."' style='color:#007DB7;font-weight:bold;' class='small'>".$db->dt[estimate_code]."</a></b></font></td>
									<td align='center' style='background-color:#fff7da;' nowrap=''><font color='#000000' class='small'><b>".$mall_domain."</b></font></td>
									...
									<td align='center' style='background-color:#fff7da;' class='small'>
									<a href='./guide_estimate.php?est_ix=".$db->dt[est_ix]."'><img src='../images/".$admininfo["language"]."/bts_modify.gif' border=0></a>&nbsp;
									<a href=\"JavaScript:deleteEstimate('delete','".$db->dt[est_ix]."','guide')\"><img src='../images/".$admininfo["language"]."/btn_del.gif' border=0></a>
									</td>
								</tr>
							</tbody></table>
						</td>
