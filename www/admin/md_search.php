...
/*
$sql = "select count(*) as total from ".TBL_COMMON_USER." cu, ".TBL_COMMON_MEMBER_DETAIL." cmd 
			where ccd.com_type = 'S' and csd.company_id = ccd.company_id and ccd.seller_auth = 'Y' $search_str $code_str  ";
*/
	$sql = "SELECT  count(*) as total
			FROM ".TBL_COMMON_USER." cu inner join ".TBL_COMMON_MEMBER_DETAIL." cmd on cu.code = cmd.code
 			where 1 $where and cu.mem_div = 'MD'  $search_str  ";
$db->query($sql);
$db->fetch();
...
<TABLE cellSpacing=0 cellPadding=0 width='100%' align=center border=0>
	<TR>
		...
		<table border='0' width='100%' cellpadding='0' cellspacing='0' align='center'>
					...
					<table width='100%' border='0' cellspacing='0' cellpadding='0' >
						<tr>
							<td width='10%' height='31' valign='middle' style='font-family:굴림;font-size:16px;font-weight:bold;letter-spacing:-2px;word-spacing:0px;padding-left:10px;' nowrap>
								<img src='/admin/images/icon/sub_title_dot.gif' align=absmiddle> MD담당자 검색
							</td>
			...
			<tr >
				<td align='left' colspan=2> ".GetTitleNavigation("MD담당자 검색", "MD담당자 검색", false)."</td>
			</tr>
							...
							<td class='box_05' align=right style='padding: 0 20 0 20'>
								<table border='0' width='100%' cellspacing='0' cellpadding='0' class='input_table_box'>
									...
									<tr height='40' valign='middle'>
										<td align='center'  colspan=2 class='input_box_title'><b>MD담당자 검색</b>
											<select name='search_type'>
												<option value='' ".CompareReturnValue("",$search_type,"selected")."> MD담당자 검색</option>
												...
												<option value='pcs' ".CompareReturnValue("pcs",$search_type,"selected")."> 전화번호 </option>
											</select>
										</td>
	...
	<tr height=30>
		<td class='p11 ls1' style='padding:0 0 0 5px;' nowrap> ".($search_text == "" ? "MD담당자 전체 목록입니다.":"'".$search_text ."' 로 검색된 결과 입니다.")."</td>
	...
	<tr><form name='send_mail' method='post' action='sms.act.php' onsubmit='return CheckSMS(this);' ><input type=hidden name='act' value='send_mail' >
		...
		<tr height='28' bgcolor='#ffffff'>
			...
			<td width='15%' align='center' class=m_td><font color='#000000'><b>이름</b></font></td>
...
if($db->total){
	for($i=0;$i  < $db->total; $i++){
		...
		$Contents .= "<tr height=25 style='text-align:center;cursor:pointer;' onclick=\"if($(this).children().css('background-color').replace(/\s/g,'')=='rgb(249,222,209)'){SearchCharger('".$db->dt[code]."','".$db->dt[name]."');}else{changeBgColor(this)}\">
		...
		$Contents .= "<tr><td colspan=4 align=center height=100>MD 담당자 정보가 존재하지 않습니다.</td></tr>";
}
...
$P->addScript = $Script;
$P->Navigation = "MD담당자 검색";
$P->NaviTitle = "MD담당자 검색";
