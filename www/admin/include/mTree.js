...
function treeNodeToHTML() {
	...
	szHtml += "<table border=0 cellspacing=0 oncontextmenu='return false;' cellpadding=0 id=" + this.id + " style='background-color:" + this.workAreaBgColor + ";color:" + this.color + 
				";border-width:0";
	/*
	...
	*/
	...
	if(this.depth() != 0) { 
		...
		szHtml += "<td nowrap valign=middle><img width=19 height=16  id=" + this.id + "_signimage src=" + this.getSignImage() +
					" onMouseOver=\"treeNodeOnMouseOver('" + this.id + "',event);return false;\"" + // 호환성을 위해 event 추가 2011-04-04 kbk
					" onMouseOut=\"treeNodeOnMouseOut('" + this.id + "',event);return false;\"" + // 호환성을 위해 event 추가 2011-04-04 kbk
					" onClick=\"treeNodeOnClick('" + this.id + "',event);return false;\""+	 // 호환성을 위해 event 추가 2011-04-04 kbk				
					"></td>";
	...
	szHtml += "<td nowrap><div id=" + this.id + "_text title='" + this.tooltip + "'" +  
					...
					" onMouseOut=\"treeNodeOnMouseOut('" + this.id + "',event);return false;\"" + // 호환성을 위해 event 추가 2011-04-04 kbk
					" onClick=\"treeNodeOnClick('" + this.id + "',event);return false;\"" + // 호환성을 위해 event 추가 2011-04-04 kbk
					" onDblClick=\"treeNodeOnDblClick('" + this.id + "',event);return false;\"" + // 호환성을 위해 event 추가 2011-04-04 kbk
					" oncontextmenu=\"compManager.get('" + this.id + "').getRootTree().selectedNode.deselect();compManager.get('" + this.id + "').select();execScript(compManager.get('" + this.id + "').action);init2('" + this.id + "');return false;\" "+
					">" + this.text + 
					"</div></td>";
...
function treeNodeOnClick(id,evt) {
	...
	if(target.id==id+"_text") {
		...
		if(nodeComp.action != null) {
			...
			if(window.execScript) { // execScript() 함수는 익스(크롬) 전용, firefox에서는 안먹힘 kbk
				execScript(nodeComp.action); // 익스,크롬 kbk
			} else {
	...
	} else if(target.id == id + "_signimage" ||	target.id == id + "_folderimage") { 
	...
	//event.cancelBubble = nodeComp.cancelBubble;
	evt.cancelBubble = nodeComp.cancelBubble; // 호환성을 위해 event 추가 2011-04-04 kbk
	
	return false;
	
}
// 내가 추가한 함수 글 읽을때 트리 열리게 하는 함수
function UsptreeNodeOnClick(id,evt) {
...
function treeNodeOnContextMenu(id) {
	...
	if(objNodeComp.context) {
		...
		if(objNodeComp.action && objNodeComp != objNodeComp.getRootTree().selectedNode) {
			...
			if(window.execScript) { // execScript() 함수는 익스(크롬) 전용, firefox에서는 안먹힘 kbk
				window.execScript(nobjNodeComp.action); // 익스,크롬 kbk
			} else {
...
function checkBoxOnClick(obj){	//카테고리 상품고시정보 체크박스 

	var _depth = parseInt(obj.getAttribute("cdepth"));
	...
	for(i=0; i < cobj.length;i++){
		...
		if(obj.value == cobj[i].value){
			...
			//$('#alert').val((_depth+1)*3);
			//$('#alert').val(obj.value.substring(0,(_depth+1)*3) +'=='+ cobj[i].value.substring(0,(_depth+1)*3));
			if(obj.value.substring(0,(_depth+1)*3) == cobj[i].value.substring(0,(_depth+1)*3)){
				//$('#alert').val('222');
				if(obj.checked && cobj[i].getAttribute("is_layout_apply") == "N"){
					cobj[i].checked = true;
				}else{
					cobj[i].checked = false;
				}
