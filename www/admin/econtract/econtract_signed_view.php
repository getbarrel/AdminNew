...
//$total = $db->total;
	...
	if ( ! empty($com_signature) || ! empty($contractor_signature) )
	{
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, 'http://crosscert.forbiz.co.kr/get_verify_data.php');
		curl_setopt($ch, CURLOPT_POST, 1);
		if ( ! empty($contractor_signature) ) {
			curl_setopt($ch, CURLOPT_POSTFIELDS, 'signed_data=' . $contractor_signature);
		} else {
			curl_setopt($ch, CURLOPT_POSTFIELDS, 'signed_data=' . $com_signature);
		}
...
<tr>
	...
	<div style="height: 800px; padding: 10px; overflow-y: scroll;line-height:150%;" id="contract_detail">
	' . $server_output . '	 
	</div>
	</td>
</tr>
