...
<tr>
	...
	<div style="height: 500px; padding: 10px; overflow-y: scroll;line-height:150%;" id="contract_detail">
	' . $contract_detail . '	
	</div>
	</td>
</tr>
...
<tr>
	<td colspan="6" style="text-align: left; padding: 30px;">';
...
for( $i = 0; ($i < $db->total); $i++) {
	...
	$Contents .= '<tr height=30>
		...
		<td>
			<input type="hidden" class="textbox" name="relation_files[' . $i . '][file_text]" id="relation_file" style="vertical-align: middle;" value="' . $db->dt['file_text'] . '" /> 
			<input type="file" class="textbox"  name="relation_files[' . $i . '][attach_file]" id="relation_file" style="vertical-align: middle;" value="" />
		</td>
	</tr>';
}
$Contents .= '</table>';
...
<tr>
	<td colspan="6" style="text-align: center; padding: 30px;">
		<table width="70%" border="0" cellpadding="0" cellspacing="0" align="center" class="input_table_box">
		<colgroup>
			<col width="30%" />
			<col width="35%" />
			<col width="35%" />
		...
		<tr>
			<td class="input_box_title">
				<input type="hidden" id="com-ceo-contents" value="' . $com_ceo . '" />
				<input type="hidden" id="com-reg-no" value="' . $com_reg_no . '" />
				<b>계약자 (갑)</b>
			</td>
			<td class="input_box_item">계약자명 <b>' . $com_ceo . '</b></td>
			<td class="input_box_item">' ;
			$Contents .= $signature_date ;
			if($_SESSION["admininfo"]["company_id"] == $company_id && !$com_signature){	
				$Contents .= ' <button id="com-ceo-sign">서명</button>';
			}
			$Contents .= '
			</td>
		</tr>
		<tr>
			<td class="input_box_title">
				<input type="hidden" id="contractor-ceo-contents" value="' . $contractor_ceo . '" />
				<input type="hidden" id="contractor-reg-no" value="' . $contractor_reg_no . '" />
				<b>계약자 (을)</b>
			</td>
			<td class="input_box_item">계약자명 <b>' . $contractor_ceo . '</b></td>
			<td class="input_box_item">';
				$Contents .= $contractor_signature_date ;
				if($_SESSION["admininfo"]["company_id"] == $contractor_id && !$contractor_signature){	
					$Contents .= ' <button id="contractor-ceo-sign">서명</button>';
				}
				$Contents .= '</td>
		</tr>
		</table>
...
function printArea(ei_ix) {

	window.open ("./econtract_print.php?mmode=print&ei_ix="+ei_ix, "newwindow", "height=800, width=800, top=0, left=0, toolbar=no, menubar=no, scrollbars=yes, resizable=no,location=no, status=no");
