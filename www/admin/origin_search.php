...
$db->query("select count(*) as total from common_origin where disp='1' $search_str ");
$db->fetch();

$total = $db->dt[total];


$sql = "select * from common_origin where disp='1' $search_str order by origin_name limit $start, $max";
//echo $sql;
$db->query($sql);

$str_page_bar = page_bar($total, $page,$max, "&view=innerview&max=$max&buying_status=$buying_status&sdate=$sdate&edate=$edate&input_id=$input_id&select_id=$select_id","");
...
function SelectBrand(og_ix, origin_name){
	//alert('".$input_id."');
	//alert($('#company_id',opener.document).parent().html());
	$('#".$input_id."',opener.document).val(og_ix);
	$('#".$select_id."',opener.document).val(origin_name);
	self.close();
}
</Script>";
...
<TABLE cellSpacing=0 cellPadding=0 width='100%' align=center border=0>
	<TR>
		...
		<table border='0' width='100%' cellpadding='0' cellspacing='0' align='center'>
					...
					<table width='100%' border='0' cellspacing='0' cellpadding='0' >
						<tr>
							<td width='10%' height='31' valign='middle' style='font-family:굴림;font-size:16px;font-weight:bold;letter-spacing:-2px;word-spacing:0px;padding-left:10px;' nowrap>
								<img src='/admin/images/icon/sub_title_dot.gif' align=absmiddle> 원산지검색
							</td>
			...
			<tr >
				<td align='left' colspan=2> ".GetTitleNavigation("원산지검색", "원산지검색", false)."</td>
			</tr>
			<tr height=30><td class='p11 ls1' style='padding:0 0 0 5px;'  align='left' > - 찾으실 원산지명 또는 원산지코드을 입력하세요.</td></tr>
			<tr>
							...
							<td class='box_05' align=right style='padding: 0 20 0 20'>
								<table border='0' width='100%' cellspacing='0' cellpadding='0' class='input_table_box'>
									...
									<tr height='40' valign='middle'>
										<td align='center'  colspan=2 class='input_box_title'><b>원산지검색</b>
											<select name='search_type'>
												<option value='origin_name'> 원산지명</option>
												<option value='og_ix'> 원산지코드(key)</option>
											</select>
										</td>
	...
	<tr><form name='send_mail' method='post' action='sms.act.php' onsubmit='return CheckSMS(this);' ><input type=hidden name='act' value='send_mail' >
		...
		<tr height='28' bgcolor='#ffffff'>
			<td width='20%' align='center' class='m_td'><font color='#000000'><b>원산지 코드</b></font></td>
			<td width='*' align='center' class=m_td><font color='#000000'><b>원산지 명</b></font></td>
...
if($db->total){
	for($i=0;$i  < $db->total; $i++){
		...
		$Contents .= "<tr height=25 style='text-align:center;cursor:pointer;' >
								<td class='list_box_td '>".$db->dt[origin_code]."</td>
								<td class='list_box_td point' onclick=\"SelectBrand('".$db->dt[og_ix]."','".$db->dt[origin_name]."');\">".$db->dt[origin_name]."</td>
