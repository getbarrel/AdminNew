...
$sms_design = new SMS;
	...
	//메일구분 검색 0:본문 1:서명
	if($mail_type == "0"){
		$where .= " and sms_type =  '0' ";
	}else if($mail_type == "1"){
		$where .= " and sms_type =  '1' ";
	}
	...
	// 전체 갯수 불러오는 부분
	$sql = "select count(*) as total from shop_sms_box m where 1 $where";
	$mdb->query($sql);
	$mdb->fetch();
	...
	//실제 데이터
	$sql	=	"select * from shop_sms_box m where 1 $where order by regdate desc limit $start , $max";
	$mdb->query($sql);

		if($mdb->dbms_type == "oracle"){
			$sql = "select m.sms_ix,m.sms_code,m.sms_type,m.sms_send_type, m.sms_title, m.regdate, m.disp, count(*) as send_cnt, 
				sum(case when sms_open = '1' then 1 else 0 end) as sms_open, sum(case when sms_click = '1' then 1 else 0 end) as sms_click
				from shop_sms_box m
				left join shop_smsling_history mh on m.sms_ix = mh.sms_ix
				group by m.sms_ix, m.sms_title, m.regdate, m.disp order by m.regdate desc where 1 $where limit $start, $max ";
		}else{
			$sql = "select *
				from shop_sms_box m
				where 1 $where group by sms_ix order by m.regdate desc limit $start, $max ";
		}
		$mdb->query($sql);
...
function smsDelete(sms_ix){
	if(confirm('해당 SMS를 정말로 삭제하시겠습니까?'))
	{
		window.frames['act'].location.href= 'sms.act.php?act=delete&sms_ix='+sms_ix;
	}
}
...
<table class='box_shadow' style='width:100%;' cellpadding='0' cellspacing='0' >
	...
	<tr>
		...
		<form name=searchmember method='get' style='display:inline;'><!--SubmitX(this);'-->
		    ...
		    <tr>
		      <td class='search_box_title'>문자그룹</td>
						...
						<tr>
							<td>
							  <select name=search_type>
										<option value='user_name' ".CompareReturnValue("user_name",$search_type,"selected").">제목</option>
										<option value='mobile' ".CompareReturnValue("mobile",$search_type,"selected").">담당자</option>
										<option value='email' ".CompareReturnValue("email",$search_type,"selected").">메일코드</option>
							  </select>
							 </td>
  ...
  <tr height=50>
    	<td style='padding:10px 20px 20px 20px' colspan=4 align=center>
			<input type=image src='../images/".$admininfo["language"]."/bt_search.gif' align=absmiddle  >
		</td>
  </tr></form>
</table>
<table style='width:100%'>
	<td colspan=0 align=left>전체 : ".$total."개</td>
	<td colspan=0 align=right><a href=\"javascript:PopSWindow('sms_write.php?mmode=pop',880,600,'member_info')\"><img src='../images/".$admininfo["language"]."/btn_mailadd.gif' border=0 ></a></td>
</table>
...
<table width='100%' border='0' cellpadding='0' cellspacing='0'  class='list_table_box'>
  ...
  <tr height='28' bgcolor='#ffffff'>
    ...
    <td align='center' class='m_td'><font color='#000000'><b>문자구분</b></font></td>
    <td align='center' class='m_td'><font color='#000000'><b>문자그룹</b></font></td>
    <td align='center' class='m_td'><font color='#000000'><b>발송구분</b></font></td>
    <td align='center' class='m_td'><font color='#000000'><b>문자코드</b></font></td>
...
$Contents = $Contents."
  <tr height='30' align=center onMouseOver=\"this.style.backgroundColor='#E8ECF1'; \" onMouseOut=\"this.style.backgroundColor=''\">
    <td class='list_box_td'><input type=checkbox name=ab_ix[] id='ab_ix' value='".$mdb->dt['sms_ix']."'></td>
    <td class='list_box_td list_bg_gray' style='padding:5px;'>".$no."</td>
	 <td class='list_box_td' nowrap>".substr($mdb->dt['regdate'],0,10)."</td>
    <td class='list_box_td point' style='text-align:center;padding:5px;'><span >".($mdb->dt['sms_type'] == "0" ? "마케팅":"프로모션")."</span></td>
    <td class='list_box_td  list_bg_gray' >".$mdb->dt['sms_group']."</td>
	<td class='list_box_td point' ></td>
    <td class='list_box_td' nowrap>".$mdb->dt['sms_code']."</td>
    <td class='list_box_td point' >".$mdb->dt['sms_title']."</td>
    <td class='list_box_td' nowrap>".($mdb->dt['disp'] == "1" ? "O":"X")."</td>
    <td class='list_box_td list_bg_gray' nowrap>";
    if(checkMenuAuth(md5($_SERVER["PHP_SELF"]),"C")){
        $Contents.="
    	<a href=\"javascript:PopSWindow('sms_write.php?mmode=pop&sms_ix=".$mdb->dt['sms_ix']."',880,600,'sms_info')\"><img src='../images/".$admininfo["language"]."/btc_modify.gif' border=0></a>";
    }else{
        $Contents.="
    ...
    if(checkMenuAuth(md5($_SERVER["PHP_SELF"]),"D")){
        $Contents.="
	    <a href=\"JavaScript:smsDelete('".$mdb->dt[sms_ix]."')\"><img  src='../images/".$admininfo["language"]."/btc_del.gif' border=0></a>";
    }else{
        $Contents.="
	    <a href=\"JavaScript:smsDelete('".$mdb->dt[sms_ix]."')\"><img  src='../images/".$admininfo["language"]."/btc_del.gif' border=0></a>";
    }
