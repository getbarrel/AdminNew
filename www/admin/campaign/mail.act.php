...
if ($act == "update")
{
	...
	$sql = "update shop_mail_box set
			mail_title='$mail_title',mail_text='$mail_text',disp='$disp',mail_type='$mail_type',mail_code='$mail_code',mail_send_type='$mail_send_type'
			where mail_ix='$mail_ix'";
	...
	$path = $_SERVER["DOCUMENT_ROOT"]."".$admin_config[mall_data_root]."/images/mail/$LAST_ID/";
	...
	for($i=0;$i < count($out);$i++){
		for($j=0;$j < count($out[$i]);$j++){
			...
			$img = ClearText($img);
				...
				if(substr_count($img,$admin_config[mall_data_root]."/images/mail/$LAST_ID/") == 0){// 이미지 URL 이 이벤트 관련 폴더에 있지 않으면 ...

					if(substr_count($img,"$HTTP_HOST")>0){	// 자사 사이트에 있는 이미지면
						//$img = str_replace("http://$HTTP_HOST/",$_SERVER["DOCUMENT_ROOT"]."",$img);
						//echo $img."<br>";
						if(@copy("$img",$_SERVER["DOCUMENT_ROOT"]."".$admin_config[mall_data_root]."/images/mail/$LAST_ID/".returnFileName($img))){
						...
						@copy("$img",$_SERVER["DOCUMENT_ROOT"]."".$admin_config[mall_data_root]."/images/mail/$LAST_ID/".returnFileName($img));
					}
				}

				$data_text = str_replace($img,"http://$HTTP_HOST".$admin_config[mall_data_root]."/images/mail/$LAST_ID/".returnFileName($img),$data_text);
		}
	}


	$db->query("UPDATE shop_mail_box SET mail_text = '$data_text' WHERE mail_ix='$LAST_ID'");
	if($mmode =='pop'){
	echo("<script>alert('수정되었습니다');top.location.href = 'mail_write.php?mail_ix=$mail_ix&mmode=pop';</script>");
	}else{
	echo("<script>top.location.href = 'mail_write.php?mail_ix=$mail_ix';</script>");
	}
}
...
if ($act == "insert"){
	...
	$db->sequences = "SHOP_MAIL_BOX_SEQ";
	$db->query("insert into shop_mail_box (mail_ix,mail_code,mail_type,mail_send_type,mail_title,mail_text,disp,regdate) values('$mail_ix','$mail_code','$mail_type','$mail_send_type','$mail_title','$mail_text','$disp',NOW())");

	if($db->dbms_type == "oracle"){
		$LAST_ID = $db->last_insert_id;
	}else{
		$db->query("SELECT mail_ix FROM shop_mail_box WHERE mail_ix=LAST_INSERT_ID()");
		$db->fetch();
		$LAST_ID = $db->dt[mail_ix];
	}
	...
	for($i=0;$i < count($out);$i++){
		for($j=0;$j < count($out[$i]);$j++){
			...
			$img = ClearText($img);
				...
				$data_text = str_replace($img,$admin_config[mall_data_root]."/images/mail/$LAST_ID/".returnFileName($img),$data_text);
		}
	}
...
if ($act == "delete")
{
	...
	$db->query("DELETE FROM shop_mail_box WHERE mail_ix='$mail_ix'");
	$db->query("DELETE FROM shop_mailling_history WHERE mail_ix='$mail_ix'");
	echo("<script>top.location.href = 'mail_box.php';</script>");
	exit;
}
