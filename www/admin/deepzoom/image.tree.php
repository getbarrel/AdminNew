...
if($mode == "image_group"){
	$sql = "select dig.* , case when group_depth = 1 then group_ix  else parent_group_ix end as group_order from deepzoom_image_group dig where group_depth = 1 and company_id ='".$admininfo["company_id"]."' order by  group_order asc , vieworder asc ";
	$db->query($sql);
	...
	for($i=0;$i < count($datas);$i++){
		$sql = "select dig.* , case when group_depth = 1 then group_ix  else parent_group_ix end as group_order 
				from deepzoom_image_group dig where group_depth = 2 and parent_group_ix = '".$datas[$i]["group_ix"]."'
