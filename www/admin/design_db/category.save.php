...
if(file_exists($_SERVER["DOCUMENT_ROOT"].$admininfo["mall_data_root"]."/templet/".$selected_templete_text."/layout2.xml")) {//layout2.xml 파일이 있을 때만 kbk 130412
	...
	// 	$db->query($sql);
		...
		if(count($resultFindByPcode)){
			...
			parent.document.getElementById('down').innerHTML = '<a href=\"download.php?path=".$resultFindByPcode[0]->path."&cid=$cid\">파일다운로드</a>';
	...
	if ($mode == "del"){
		...
		if (CheckSubCategory($cid,$this_depth)){
			if($sub_cartegory_delete == "1"){
				...
				if(is_dir($dir_path) && $dir_path != $DOCUMENT_ROOT.$admin_config[mall_data_root]."/templet/".$admin_config[selected_templete]){		
...
// 		echo("xxxxxxxxxxxxxxxxxxxxxxxxxx");
		...
		$parent_page_path = $results[0]->path;
...
// 		exit;
		...
		$layoutXml->layouts[$count]->vlevelf = $sub_cid;
					/*
					sprintf("%03d%03d%03d%03d%03d"
	                                                        ,$level1
	                                                        ,$level2
	                                                        ,$level3
	                                                        ,$level4
...
// 		exit;
		...
		$layoutXml->layouts[$count]->pcode = $layoutXml->layouts[$count]->vlevelf;
...
// 		exit;
		...
		$layoutXml->layouts[$count]->page_path = $parent_page_path."/".$path;
		...
		//exit;
		//$dir_path = "$DOCUMENT_ROOT".$admin_config[mall_data_root]."/templet/".$admin_config[selected_templete]."/$page_path";
		if($admin_config[mall_page_type] == "P"){		
			$dir_path = "$DOCUMENT_ROOT".$admin_config[mall_data_root]."/templet/".$admin_config[selected_templete]."/".trim($page_path);			
		}else if($admin_config[mall_page_type] == "M"){
			$dir_path = "$DOCUMENT_ROOT".$admin_config[mall_data_root]."/mobile_templet/".$admin_config[selected_templete]."/".trim($page_path);
...
//echo "<br>maxlevel:".getMaxlevel($cid,$sub_depth);
	...
	if($mode == "infoupdate"){
		...
		if($db->total){
			...
			parent.document.forms['thisCategoryform'].category_use.checked = $category_use;
			parent.document.forms['thisCategoryform'].is_layout_apply.checked = $is_layout_apply;
			parent.document.forms['thisCategoryform'].path.value = '".$db->dt[path]."';
			parent.document.forms['thisCategoryform'].basic_link.value = '".$db->dt[basic_link]."';
			parent.document.getElementById('down').innerHTML = '<a href=\"download.php?path=".$db->dt[path]."&cid=$cid\">파일다운로드</a>';
	...
	if ($mode == "modify"){
		if ($category_img_size > 0){
			copy($category_img, $_SERVER["DOCUMENT_ROOT"]."".$admin_config[mall_data_root]."/images/category/".trim($category_img_name));
			$setString = ", catimg = '$category_img_name'";
		}

		if ($leftcategory_img_size > 0){
			copy($leftcategory_img, $_SERVER["DOCUMENT_ROOT"]."".$admin_config[mall_data_root]."/images/category/".trim($leftcategory_img_name));
		...
		$page_path = getDesignTempletPath($cid, $this_depth);
		if($admin_config[mall_page_type] == "P"){
			$dir_path = $_SERVER["DOCUMENT_ROOT"]."".$admin_config[mall_data_root]."/templet/".$admin_config[selected_templete]."/".trim($page_path);
		}else if($admin_config[mall_page_type] == "M"){
			$dir_path = $_SERVER["DOCUMENT_ROOT"]."".$admin_config[mall_data_root]."/mobile_templet/".$admin_config[selected_templete]."/".trim($page_path);
		}else if($admin_config[mall_page_type] == "MI"){
			$dir_path = $_SERVER["DOCUMENT_ROOT"]."".$admin_config[mall_data_root]."/minishop_templet/".$admin_config[selected_templete]."/".trim($page_path);
		}
		...
		if($admin_config[mall_page_type] == "MI"){
			$sql = "update ".TBL_SHOP_MINISHOP_LAYOUT_INFO." set
			cname = '$this_category', path = '$path',basic_link = '$basic_link', category_top_view = '$category_top_view', category_use ='$category_use',
			category_display_type ='$category_display_type', is_layout_apply = '$is_layout_apply', bbs_name ='$bbs_name' $setString
			where cid = '$cid'";
		}else{
			$sql = "update ".TBL_SHOP_LAYOUT_INFO." set
			cname = '$this_category', path = '$path',basic_link = '$basic_link', category_top_view = '$category_top_view', category_use ='$category_use',
			category_display_type ='$category_display_type', is_layout_apply = '$is_layout_apply', bbs_name ='$bbs_name' $setString
			where cid = '$cid'";
		}
		...
		if($admin_config[mall_page_type] == "P"){
			...
			$change_dir_path = $_SERVER["DOCUMENT_ROOT"]."".$admin_config[mall_data_root]."/templet/".$admin_config[selected_templete]."/$change_page_path";
		}else if($admin_config[mall_page_type] == "M"){
			...
			$change_dir_path = $_SERVER["DOCUMENT_ROOT"]."".$admin_config[mall_data_root]."/mobile_templet/".$admin_config[selected_templete]."/$change_page_path";
		}else if($admin_config[mall_page_type] == "MI"){
			...
			$change_dir_path = $_SERVER["DOCUMENT_ROOT"]."".$admin_config[mall_data_root]."/minishop_templet/".$admin_config[selected_templete]."/$change_page_path";
		}
	...
	//	echo $sql ;
		...
		$page_path = getDesignTempletPath($sub_cid, $sub_depth);
		//$dir_path = $_SERVER["DOCUMENT_ROOT"]."".$admin_config[mall_data_root]."/templet/".$admin_config[selected_templete]."/$page_path";
		if($admin_config[mall_page_type] == "P"){
			$dir_path = $_SERVER["DOCUMENT_ROOT"]."".$admin_config[mall_data_root]."/templet/".$admin_config[selected_templete]."/".trim($page_path);
		}else if($admin_config[mall_page_type] == "M"){
			$dir_path = $_SERVER["DOCUMENT_ROOT"]."".$admin_config[mall_data_root]."/mobile_templet/".$admin_config[selected_templete]."/".trim($page_path);
		...
		if($category_display_type == "P" || $category_display_type == "B"){
			...
			if(!$db->total){
				$sql = "insert into ".TBL_SHOP_DESIGN." (pcode,mall_ix,page_type,page_path, layout,header1,header2,leftmenu,contents,rightmenu,footer1,footer2,templet_name,regdate)
					select '$sub_cid' as pcode, '".$admininfo[mall_ix]."' as mall_ix , 'A' page_type, '$page_path' as page_path , layout,header1,header2,leftmenu, '".$path.".htm' as contents ,rightmenu,footer1,footer2, '".$admin_config[selected_templete]."' as templet_name , NOW() as regdate
					from ".TBL_SHOP_DESIGN." where pcode = 'basic' and mall_ix='".$admininfo[mall_ix]."' and templet_name = '".$admin_config[selected_templete]."' ";
