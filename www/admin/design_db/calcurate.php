...
<?
	...
	function getNextCid($cid,$depth)
	{
		...
		$sPos = $depth*3 + 1;
		if($admin_config[mall_page_type] == "MI"){
			$sql = "select max(substr(cid,$sPos,3))+1 as maxid from ".TBL_SHOP_MINISHOP_LAYOUT_INFO." where cid LIKE '".substr($cid,0,$sPos-1)."%'";
		}else{
			$sql = "select max(substr(cid,$sPos,3))+1 as maxid from ".TBL_SHOP_LAYOUT_INFO." where cid LIKE '".substr($cid,0,$sPos-1)."%'";
		}
		$db->query($sql);
		$db->fetch(0);
