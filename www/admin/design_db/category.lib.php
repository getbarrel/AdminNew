<?php
	...
	// 		echo("<br />");
		...
		/*	Create Root node	*/
			...
			rootnode.action = \"setCategory('product category','000000000000000',-1,'".$id."')\";
			rootnode.expanded = true;";
		//echo print_r($admin_config);
		//echo $admininfo[mall_type];
		//exit;
		if($admin_config[mall_page_type] == "MI"){
			$sql = "SELECT * FROM ".TBL_SHOP_MINISHOP_LAYOUT_INFO." where 1=1 order by vlevel1, vlevel2, vlevel3, vlevel4, vlevel5";
		}else{
			if($admininfo[mall_type] == "H"){
				$sql = "SELECT * FROM ".TBL_SHOP_LAYOUT_INFO." where cid NOT LIKE '001%' and cid NOT LIKE '006%' and cid NOT LIKE '012%' and cid NOT LIKE '011%' and cid NOT LIKE '015%' order by vlevel1, vlevel2, vlevel3, vlevel4, vlevel5";
			}else{
				$sql = "SELECT * FROM ".TBL_SHOP_LAYOUT_INFO." where 1=1 order by vlevel1, vlevel2, vlevel3, vlevel4, vlevel5";
			}
		}
			
		$mdb->query($sql);
		 //cid NOT LIKE '013%' and cid NOT LIKE '014%' 
		$total = $mdb->total;
