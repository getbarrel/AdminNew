	...
	if(file_exists($_SERVER["DOCUMENT_ROOT"].$admininfo["mall_data_root"]."/templet/".$selected_templete_text."/layout2.xml")) {//layout2.xml 파일이 있을 때만 kbk 130412
		...
		// 	print_r($layoutXml->photoSkins);
			...
			if ($design_act == "bbs_update"){
				...
				//$file_path = "$DOCUMENT_ROOT".$admin_config[mall_data_root]."/bbs_templet/".$templet."/".$page_name;	
				$file_path = "$DOCUMENT_ROOT"."/bbs_templet/".$templet."/".$page_name;	
		...
		// 			$db->query($sql);
					...
					if($design_act == "pop_update"){
						...
						if($design_backup){
							...
							/*echo "<script>
										...
										</script>";*/ //리로드 에러 로 기존 로딩 방식으로 변경 jk121123
			...
			if ($layout_act == "update"){
				$results = $layoutXml->search("layouts", array("pcode", "mall_ix", "skin_type", "templet_name")
						, array($pcode , $mall_ix, $admin_config[mall_page_type], $admin_config[selected_templete]));
				//print_r($_POST);
		...
		if ($layout_act == "insert"){
			//print_r($_POST);
			$sql = "select pcode from ".TBL_SHOP_DESIGN." where pcode ='$pcode' and skin_type = '".$admin_config[mall_page_type]."' and templet_name = '".$admin_config[selected_templete]."' and mall_ix ='$mall_ix' ";
			//echo $sql."<br>";
			//exit;
			$db->query($sql);
			
			if($db->total){
				echo("<script language='javascript' src='../_language/language.php'></script><script>alert(language_data['design.act.php']['C'][language]);</script>"); //해당코드는 이미 등록된 코드입니다. 페이지코드를 바꿔주시기 바랍니다.
				exit;
			}else{
				if($page_type == "") $page_type  = "B";
				
				$sql = "insert into ".TBL_SHOP_DESIGN." (pcode,mall_ix, skin_type, page_type, page_path, page_link,page_title,page_help,page_addscript,page_body,page_desc, layout, header1,header2,leftmenu,contents,contents_add,rightmenu,footer1,footer2, caching, caching_time, templet_name ,regdate) 
						values
						('$pcode','$mall_ix','".$admin_config[mall_page_type]."','$page_type','$page_path','$page_link','$page_title','$page_help','$page_addscript','$page_body','$page_desc','$layout','$header1','$header2','$leftmenu','$contents','$contents_add','$rightmenu','$footer1','$footer2','$caching','$caching_time','".$admin_config[selected_templete]."',NOW())";
				//echo $sql;
				//exit;
				$db->query($sql);
				...
				if($basic_design){
					...
					$db->query("select * from ".TBL_SHOP_DESIGN." where pcode='basic' and skin_type = '".$admin_config[mall_page_type]."' and templet_name = '".$admin_config[selected_templete]."' and mall_ix ='$mall_ix' ");
					
					if($db->total){
						$sql = "	update ".TBL_SHOP_DESIGN." set 
								layout='$layout',header1='$header1',header2='$header2',leftmenu='$leftmenu',rightmenu='$rightmenu',footer1='$footer1',footer2='$footer2',caching='$caching',caching_time='$caching_time'
								where pcode='basic' and skin_type = '".$admin_config[mall_page_type]."' and templet_name = '".$admin_config[selected_templete]."' and mall_ix ='$mall_ix'	";
						
						$db->query($sql);
					}else{
						$sql = "insert into ".TBL_SHOP_DESIGN." (pcode,mall_ix,skin_type,page_type,layout,header1,header2,leftmenu,contents,rightmenu,footer1,footer2,caching, caching_time, regdate) 
						values('basic','$mall_ix','".$admin_config[mall_page_type]."','B','$layout','$header1','$header2','$leftmenu','$contents','$rightmenu','$footer1','$footer2','$caching','$caching_time',NOW())";
		...
		/*
			$templet_path = $admin_config[mall_data_root]."/templet/".$admin_config[selected_templete];
			$templet_file_path = $templet_path."/".$page_path."/".$contents;
			$thisfile = load_template($DOCUMENT_ROOT.$templet_file_path);
			//echo("<script>alert(\$_SERVER["DOCUMENT_ROOT"]."$templet_file_path\");</script>");
		echo "<html><meta http-equiv='Content-Type' content='text/html; charset=euc-kr'>
		<body >
		...
		alert(document.getElementById('page_contents').innerHTML);
		parent.document.forms['info_input'].page_contents.value = document.getElementById('page_contents').innerHTML;
		</script>";
		*/
