...
$db->query("select count(*) as total from ".TBL_COMMON_SELLER_DETAIL." csd, ".TBL_COMMON_COMPANY_DETAIL." ccd where ccd.com_type = 'G' and csd.company_id = ccd.company_id and ccd.seller_auth = 'Y' $search_str $code_str  ");
$db->fetch();

$total = $db->dt[total];


$sql = "select ccd.company_id, ccd.com_name, ccd.com_phone, ccd.com_ceo, csd.shop_name,ccd.com_div
			from ".TBL_COMMON_SELLER_DETAIL." csd, ".TBL_COMMON_COMPANY_DETAIL." ccd
			where ccd.com_type = 'G' and csd.company_id = ccd.company_id and ccd.seller_auth = 'Y' $search_str
			limit $start, $max";
//echo $sql;
...
function SelectMember(company_id, com_name){
	...
	$('#".$input_id."',opener.document).val(company_id);
...
<TABLE cellSpacing=0 cellPadding=0 width='100%' align=center border=0>
	<TR>
		...
		<table border='0' width='100%' cellpadding='0' cellspacing='0' align='center'>
					...
					<table width='100%' border='0' cellspacing='0' cellpadding='0' >
						<tr>
							<td width='10%' height='31' valign='middle' style='font-family:굴림;font-size:16px;font-weight:bold;letter-spacing:-2px;word-spacing:0px;padding-left:10px;' nowrap>
								<img src='/admin/images/icon/sub_title_dot.gif' align=absmiddle> 거래처검색
							</td>
			...
			<tr >
				<td align='left' colspan=2> ".GetTitleNavigation("거래처검색", "거래처검색", false)."</td>
			</tr>
							...
							<td class='box_05' align=right style='padding: 0 20 0 20'>
								<table border='0' width='100%' cellspacing='0' cellpadding='0' class='input_table_box'>
									...
									<tr height='40' valign='middle'>
										<td align='center'  colspan=2 class='input_box_title'><b>거래처검색</b>
											<select name='search_type'>
												<option value=''> 거래처검색</option>
												<option value='com_name'> 상호명</option>
												<option value='com_ceo'> 대표자 </option>
												<option value='com_phone'> 전화번호 </option>
											</select>
										</td>
	...
	<tr height=30>
		<td class='p11 ls1' style='padding:0 0 0 5px;' nowrap> ".($search_text == "" ? "거래처 전체 목록입니다.":"'".$search_text ."' 로 검색된 결과 입니다.")."</td>
	...
	<tr><form name='send_mail' method='post' action='sms.act.php' onsubmit='return CheckSMS(this);' ><input type=hidden name='act' value='send_mail' >
		...
		<tr height='28' bgcolor='#ffffff'>
			<td width='20%' align='center' class='m_td'><font color='#000000'><b>사업자유형</b></font></td>
			<td width='*' align='center' class=m_td><font color='#000000'><b>상호명</b></font></td>
			<td width='24%' align='center' class=m_td><font color='#000000'><b>대표자</b></font></td>
...
if($db->total){
	for($i=0;$i  < $db->total; $i++){
		...
		if($db->dt[com_div]=="R"){
			$com_div="법인사업자";
		}elseif($db->dt[com_div]=="P"){
			$com_div="개인사업자";
		}elseif($db->dt[com_div]=="S"){
			$com_div="간이과세자";
		}elseif($db->dt[com_div]=="E"){
			$com_div="면세사업자";
		}

		$Contents .= "<tr height=25 style='text-align:center;cursor:pointer;' >
								<td class='list_box_td list_bg_gray'>".$com_div."</td>
								<td class='list_box_td point' onclick=\"SelectMember('".$db->dt[company_id]."','".$db->dt[com_name]."');\">".$db->dt[com_name]."</td>
