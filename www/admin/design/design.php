...
if($mmode != "innerview"){
    $P = new LayOut;
    if(empty($admin_config["mall_use_templete"])){
...
// 	echo("<br />");
	...
	//"mall_ix",
	//$admininfo[mall_ix]  ,
	$results = $layoutXml->search("layouts", array("skin_type", "pcode", "templet_name")
										   , array($admin_config[mall_page_type]
										   , $pcode
...
//echo $layout->layout;
	$Contents .="
		...
		if(substr(trim($page_path),0,4)=="main" || trim($page_path)=="") {
			$page_path_link="/index.php";
		} else {
			$page_path_link=$page_path.".php";
		}
		if(is_file($_SERVER["DOCUMENT_ROOT"].$page_path_link)) {//페이지 미리보기를 실제 페이지로 연결함(php함수 호출 에러때문에) kbk 13/12/30
			$Contents .=colorCirCleBox("#efefef","100%","<div style='padding:5px 5px 5px 15px;text-align:right;'><img src='../image/title_head.gif' ><b> 선택된 페이지 : [$page_title] <a href=\"".$page_path_link."\" target='_blank'>$page_path_string</a> <a href=\"".$page_path_link."\" target='_blank'><img src='../images/".$admininfo["language"]."/btn_page_design_view.gif' border=0 align=absmiddle></a></b></div>");
		} else {
			$Contents .=colorCirCleBox("#efefef","100%","<div style='padding:5px 5px 5px 15px;text-align:right;'><img src='../image/title_head.gif' ><b> 선택된 페이지 : [$page_title] <a href=\"JavaScript:pview_pop('/main/page.php?pgid=".$_GET["pcode"]."', 'aaa', 'bbb');\">$page_path_string</a> <a href=\"JavaScript:pview_pop('/main/page.php?pgid=".$_GET["pcode"]."', 'aaa', 'bbb');\"><img src='../images/".$admininfo["language"]."/btn_page_design_view.gif' border=0 align=absmiddle></a></b></div>");
		}
		$Contents .="</td></tr>
		<tr>
								...
								<td valign=top>
						         ...
						         <td align=left style='padding:10px;'>
						                	<table cellpadding=0 cellspacing=0 border=0 width=100%>
						                		<tr>
													<td colspan=3 align=left style='padding:3px 0px;'>";
													if(is_file($_SERVER["DOCUMENT_ROOT"].$page_path_link)) {//페이지 미리보기를 실제 페이지로 연결함(php함수 호출 에러때문에) kbk 13/12/30
														$Contents .=colorCirCleBox("#efefef","100%","<div style='padding:2px 5px 2px 15px;'><b> 선택된 페이지 : [$page_title]  <a href=\"".$page_path_link."\" target='_blank'><img src='../images/".$admininfo["language"]."/btn_page_design_view.gif' border=0 align=absmiddle></a></b></div>");
													} else {
														$Contents .=colorCirCleBox("#efefef","100%","<div style='padding:2px 5px 2px 15px;'><b> 선택된 페이지 : [$page_title]  <a href=\"JavaScript:pview_pop('/main/page.php?pgid=".$_GET["pcode"]."', 'aaa', 'bbb');\"><img src='../images/".$admininfo["language"]."/btn_page_design_view.gif' border=0 align=absmiddle></a></b></div>");
													}
													$Contents .="</td>
												</tr>
											</table>
	...
	$category_str ="<div class=box id=img3  style='width:195px;height:375px;overflow:auto;'>".Category()."</div>";
