...
function topmenu($default_path='/admin/basic', $return_type=""){
...
<div class = 'containerOuter'>
	<div class = 'container'>
		<div class = 'iosSliderContainer'>
			<div class = 'iosSlider'>
				<div class='top_new_warp slider'>
					";//<div class='top_shadow' style='padding-left:30px; display:block;float:left; width:0px; height:74px;'>&nbsp;</div>";
	$mdb = new Database;
	//print_r($admininfo);
if($admininfo[use_work] != "1"){
	...
	if($_SESSION['admininfo']['admin_level'] == 8){
		$ecommerce_menu = " or (div_name='logstory/commerce' and menu_div='logstory/commerce') ";
	}

	if($admininfo[mall_type] == "O"){// 오픈마켓형
		//$sql = 	"SELECT * FROM admin_menu_div where disp = 1 and gnb_use_biz = 'Y' ".$inventory_str." order by vieworder asc ";
		$sql = "SELECT distinct am.menu_div, amd.div_name, amd.gnb_name, amd.basic_link,amd.vieworder
				...
				and am.use_openmarket = 'Y' $inventory_str $ecommerce_menu
				order by amd.vieworder asc ";
	}else if($admininfo[mall_type] == "E"){// 오픈마켓형
		//$sql = 	"SELECT * FROM admin_menu_div where disp = 1 and gnb_use_biz = 'Y' ".$inventory_str." order by vieworder asc ";
		$sql = "SELECT distinct am.menu_div, amd.div_name, amd.gnb_name, amd.basic_link,amd.vieworder
				FROM admin_menus am 
				left join admin_auth_templet_detail aatd 
				on am.menu_code = aatd.menu_code
				and auth_templet_ix = '".$admininfo[charger_roll]."'
				and am.disp_auth = 'Y' , admin_menu_div amd
				WHERE amd.div_name = am.menu_div and amd.gnb_use_enterprise = 'Y' and disp_auth = 'Y' and auth_read = 'Y'
				and am.use_openmarket = 'Y' $inventory_str $ecommerce_menu
				order by amd.vieworder asc ";
		...
		$sql = "SELECT distinct am.menu_div, amd.div_name, amd.gnb_name, amd.basic_link
				...
				and auth_templet_ix = '".$admininfo[charger_roll]."'
				and am.disp_auth = 'Y' , admin_menu_div amd
				where amd.div_name = am.menu_div and amd.gnb_use_biz = 'Y' 
				and disp_auth = 'Y' and auth_read = 'Y'
				and am.use_business = 'Y' $inventory_str
				order by amd.vieworder asc ";
	}else if($admininfo[mall_type] == "BW"){// 비즈형 도매형
		//$sql = 	"SELECT * FROM admin_menu_div where disp = 1 and gnb_use_biz = 'Y' ".$inventory_str." order by vieworder asc ";
		$sql = "SELECT distinct am.menu_div, amd.div_name, amd.gnb_name, amd.basic_link
				from admin_menus am left join admin_auth_templet_detail aatd on am.menu_code = aatd.menu_code
				and auth_templet_ix = '".$admininfo[charger_roll]."'
				and am.disp_auth = 'Y' , admin_menu_div amd
				where amd.div_name = am.menu_div and amd.gnb_use_wholesale = 'Y' 
				and disp_auth = 'Y' and auth_read = 'Y'
				and am.use_business = 'Y' $inventory_str
				order by amd.vieworder asc ";
	...
	if(!$mdb->total){
		...
		for($i=0;$i < $mdb->total;$i++){
			...
			}else if(($admininfo[admin_level] == 8 || $admininfo[mem_type] == "S")){
				$gnb_name = str_replace("셀러관리","상점관리",$mdb->dt[gnb_name]);
			}else if(($admininfo[admin_level] == 8 || $admininfo[mem_type] == "CS")){
				$gnb_name = str_replace("셀러관리","상점관리",$mdb->dt[gnb_name]);
			}
			...
			$mstring .= "
					...
					if($admininfo[language]=="english") {
						$mstring .= "class='gnb_flo_version_eng'";
					}else{
						$mstring .= "class='gnb_flo'";
					}
					$mstring .= "
					ID=gnb_link_".str_replace("/","_",$mdb->dt[div_name])."  onmouseover=\"showSubMenuLayer('".str_replace("/","_",$mdb->dt[div_name])."');\"  onmouseout=\"hideSubMenuLayer('".str_replace("/","_",$mdb->dt[div_name])."');\"   ".(strlen($mdb->dt[gnb_name]) > 18 ? "":"")." align=center>
						<div align=center  STYLE='text-align:center;position:relative;' onmouseover=\"MM_swapImage('Image".($i+1)."','','$default_path/".$menu_on_src."',1)\" onmouseout='MM_swapImgRestore()'>
						<A HREF='".$mdb->dt[basic_link]."' STYLE='position:relative;text-decoration:none;'>
		...
		if($admininfo[mall_type] == "E" && $admininfo[admin_level] == 9 && $_SERVER['HTTP_HOST'] != 'daisomall.co.kr'){
			//엔터일때 SNS 분석 링크아이콘 추가 2014.10.08 bgh
			/*
			$mstring .= "
						<div  
						";
						...
						$mstring .= "
						   align=center>
							<div align=center  STYLE='text-align:center;position:relative;'>
								<a href='https://socialenable.co' STYLE='position:relative;text-decoration:none;' target='_blank'>
									<div style='display:block;'>
										<img class='top_menu_img' src='$default_path/sns_report.gif' vspace=2 border=0>
									</div>
									<div>
		...
		$mstring .= "
			<td style='padding:5px 5px;vertical-align:top;'>
							<div style='display:block;'>
							<a href='/store/main.php'><img src='".$default_path."/etc.gif' class='top_menu_img'  alt='상점세팅' title='상점세팅'  class='menu_title' ".($_COOKIE[TOP_MENU_HIDDEN] == "N" ? "style='display:none'":"")."/>
							</div>
							<strong>상점세팅</strong></a>
						</td>";
						*/
	}
...
/*
	...
	//}
		if(substr_count ($PHP_SELF, "/campaign/")){
			$menu_src = "campaign_on.gif";
		}else{
			$menu_src = "campaign.gif";
		}
		$mstring .= "
				<div style='text-align:center; width:80px; float:left;padding:5px 5px;vertical-align:top;' nowrap>
					...
					<A HREF='/admin/campaign/addressbook_list.php' onmouseover=\"MM_swapImage('Image13','','$default_path/campaign_on.gif',1)\" onmouseout='MM_swapImgRestore()'>
						<div style='display:block;'>
						<img class='top_menu_img' src='$default_path/$menu_src' border=0 ID='gnb_link_text_campaign' name=Image13 ".($_COOKIE[TOP_MENU_HIDDEN] == "N" ? "style='display:none'":"").">
						</div>
						<strong>메일링/SMS</strong>
					</A>
					</div>
				</div>";

		if(substr_count ($PHP_SELF, "/work/")){
			$menu_src = "work_on.gif";
		}else{
			$menu_src = "work.gif";
		}
		$mstring .= "
				<div style='text-align:center; width:80px; float:left;padding:5px 5px;vertical-align:top;' nowrap >
					...
					<A HREF='/admin/work/work_list.php?list_type=weekly' onmouseover=\"MM_swapImage('Image11','','$default_path/work_on.gif',1)\" onmouseout='MM_swapImgRestore()'>
						<div style='display:block;'>
						<img class='top_menu_img'  src='$default_path/$menu_src' border=0 ID='gnb_link_text_work' name=Image11 ".($_COOKIE[TOP_MENU_HIDDEN] == "N" ? "style='display:none'":"").">
						</div>
						<strong>업무관리</strong>
					</A>
					</div>
	...
	if(($admininfo[charger_id] == "sigi1074" || $admininfo[charger_id] == "tech9" )){
		...
		$mstring .= "
				<div style='text-align:center; width:70px; float:left;padding:5px 5px;vertical-align:top;' nowrap>
					...
					<A HREF='/admin/kms/' onmouseover=\"MM_swapImage('Image11','','$default_path/estimate_on.gif',1)\" onmouseout='MM_swapImgRestore()'>
						<div style='display:block;'>
						<img class='top_menu_img' src='$default_path/$menu_src' border=0 ID='gnb_link_text_kms' name=Image11 ".($_COOKIE[TOP_MENU_HIDDEN] == "N" ? "style='display:none'":"").">
						</div>
						<strong>KMS 관리</strong>
						</A>
					</div>
				</div>";

		if(substr_count ($PHP_SELF, "/deepzoom/")){
			$menu_src = "cscenter_on.gif";
		}else{
			$menu_src = "cscenter.gif";
		}
		$mstring .= "
				<div style='text-align:center; width:70px; float:left;padding:5px 5px;vertical-align:top;' nowrap>
					...
					<A HREF='/admin/cms/' onmouseover=\"MM_swapImage('Image11','','$default_path/estimate_on.gif',1)\" onmouseout='MM_swapImgRestore()'>
						<div style='display:block;'>
						<img class='top_menu_img'  src='$default_path/$menu_src' border=0 ID='gnb_link_text_deepzoom' name=Image11 ".($_COOKIE[TOP_MENU_HIDDEN] == "N" ? "style='display:none'":"").">
						</div>
		...
		$mstring .= "
				<div class='item' style='text-align:center; width:95px; float:left;padding:5px 5px;vertical-align:top;' nowrap>
					<div ID=gnb_link_tax STYLE='position:relative' ONMOUSEOVER=\"javascript:showSubMenuLayer('tax');/*relationOnMouseOut();*/\" ONMOUSEOUT=\"javascript:hideSubMenuLayer('tax')\" >
					<A HREF='/admin/tax/' onmouseover=\"MM_swapImage('Image11','','$default_path/estimate_on.gif',1)\" onmouseout='MM_swapImgRestore()'>
						<div style='display:block;'>
							<img class='top_menu_img' src='$default_path/$menu_src' border=0 ID='gnb_link_text_tax' name=Image11 ".($_COOKIE[TOP_MENU_HIDDEN] == "N" ? "style='display:none'":"").">
						</div>
						<strong>전자세금계산서</strong>
					</A>
					</div>
				</div>";
...
function service_manage_menu($default_path='/admin'){
global $admin_config, $admininfo;

	if($admininfo[mall_type] == "H"){
	$mstring = "
	<table cellpadding=0  cellspacing=0 width=156 border=0>
			<tr><td align=center style='padding-bottom:5px;'><img src='$default_path/v3/images/".$admininfo[language]."/left_title_site.gif'></td></tr>
	</table>";
	}else{
	$mstring = "
	<table cellpadding=0  cellspacing=0 width=156 border=0>
			<tr><td align=center style='padding-bottom:5px;'><img src='$default_path/v3/images/".$admininfo[language]."/left_title_SM.gif'></td></tr>
	</table>";
	}

	$menus = getMenuData("service_manage");
...
global $admin_config, $admininfo;
	...
	$mstring = "
	<table cellpadding=0  cellspacing=0 width=156 border=0>
			<tr><td align=center style='padding-bottom:5px;'><img src='$default_path/v3/images/".$admininfo[language]."/left_title_store.gif'></td></tr>
	</table>";
	}
...
function chainstore_menu($default_path='/admin'){
global $admin_config, $admininfo;

	$mstring = "
	<table cellpadding=0  cellspacing=0 width=156 border=0>
			<tr><td align=center style='padding-bottom:5px;'><img src='$default_path/v3/images/".$admininfo[language]."/left_title_".($admininfo[admin_level] == 9 ? "chainstore":"chainstore").".gif'></td></tr>
	</table>";

	$menus = getMenuData("chainstore");
...
function seller_accounts_menu($default_path='/admin'){
global $admin_config, $admininfo;

	$mstring = "
	<table cellpadding=0  cellspacing=0 width=156 border=0>
			<tr><td align=center style='padding-bottom:5px;'><img src='$default_path/v3/images/".$admininfo[language]."/left_title_".($admininfo[admin_level] == 9 ? "buyer_accounts":"buyer_accounts").".gif'></td></tr>
	</table>";

	$menus = getMenuData("seller_accounts");
	$mstring .= getnerateLeftMenu($default_path, $menus);

	return $mstring;
}

function buyer_accounts_menu($default_path='/admin'){
global $admin_config, $admininfo;

	$mstring = "
	<table cellpadding=0  cellspacing=0 width=156 border=0>
			<tr><td align=center style='padding-bottom:5px;'><img src='$default_path/v3/images/".$admininfo[language]."/left_title_".($admininfo[admin_level] == 9 ? "Customer_Receipt":"Customer_Receipt").".gif'></td></tr>
	</table>";

	$menus = getMenuData("buyer_accounts");
	$mstring .= getnerateLeftMenu($default_path, $menus);

	return $mstring;
}

function seller_menu($default_path='/admin', $category_tree=""){
global $admin_config, $admininfo;

	$mstring = "
	<table cellpadding=0  cellspacing=0 width=156 border=0>
			<tr><td align=center style='padding-bottom:5px;'><img src='$default_path/v3/images/".$admininfo[language]."/left_title_".($admininfo[admin_level] == 9 ? "seller":"store").".gif'></td></tr>
	</table>";
...
global $admin_config, $admininfo;
	...
	$mstring = "
	<table cellpadding=0  cellspacing=0 width=156 border=0>
			<tr><td align=center style='padding-bottom:5px;'><img src='$default_path/v3/images/".$admininfo[language]."/left_title_".($admininfo[admin_level] == 9 ? "basic":"store").".gif'></td></tr>
	</table>";
...
global $admin_config, $admininfo;
	...
	$mstring = "
	<table cellpadding=0  cellspacing=0 width=156 border=0>
			<tr><td align=center style='padding-bottom:5px;'><img src='$default_path/v3/images/".$admininfo[language]."/left_title_".($admininfo[admin_level] == 9 ? "taxbill":"store").".gif'></td></tr>
	</table>";

	$menus = getMenuData("tax");
	$mstring .= getnerateLeftMenu($default_path, $menus);
...
function product_menu($default_path='/admin', $category_tree=""){
	...
	$mstring = "
	<table cellpadding=0  cellspacing=0 width=156 border=0>
			<tr><td align=center style='padding-bottom:5px;'><img src='$default_path/v3/images/".$admininfo[language]."/left_title_product.gif'></td></tr>
	</table>";
...
MenuTreeView('leftmenu_tree', 'product');
</script>
		";
...
function display_menu($default_path='/admin', $category_tree=""){
	global $admininfo;
	if(substr_count($_SERVER["REQUEST_URI"],"/b2b_display/") > 0){
	$mstring = "
	<table cellpadding=0  cellspacing=0 width=196 border=0>
			<tr><td align=center style='padding-bottom:5px;'><img src='$default_path/v3/images/".$admininfo[language]."/left_title_display_b2b.gif'></td></tr>
	</table>";
	}else{
	$mstring = "
	<table cellpadding=0  cellspacing=0 width=196 border=0>
			<tr><td align=center style='padding-bottom:5px;'><img src='$default_path/v3/images/".$admininfo[language]."/left_title_display_b2c.gif'></td></tr>
	</table>";
	}

	if(substr_count($_SERVER["REQUEST_URI"],"/b2b_display/") > 0){
		$menus = getMenuData("b2b_display");
	}else{
		$menus = getMenuData("display");
	}
	$mstring .= getnerateLeftMenu($default_path, $menus, $category_tree, 196);
...
function promotion_menu($default_path='/admin', $category_tree=""){
	global $admininfo;
 
	$mstring = "
	<table cellpadding=0  cellspacing=0 width=196 border=0>
			<tr><td align=center style='padding-bottom:5px;'><img src='$default_path/v3/images/".$admininfo[language]."/left_title_marketting.gif'></td></tr>
	</table>"; 
	 
	$menus = getMenuData("promotion");
...
function mshop_menu($default_path='/admin', $category_tree=""){
	global $admininfo;
 
	$mstring = "
	<table cellpadding=0  cellspacing=0 width=196 border=0>
			<tr><td align=center style='padding-bottom:5px;'><img src='$default_path/v3/images/".$admininfo[language]."/left_title_mShop.gif'></td></tr>
	</table>"; 
	 
	$menus = getMenuData("mShop");
...
function sns_menu($default_path='/admin', $category_tree=""){
	...
	$mstring = "
	<table cellpadding=0  cellspacing=0 width=156 border=0>
			<tr><td align=center style='padding-bottom:5px;'><img src='$default_path/v3/images/".$admininfo[language]."/left_title_social.gif'></td></tr>
	</table>";
...
function estimate_menu($default_path='/admin', $category_tree=""){
	...
	$mstring = "
	<table cellpadding=0  cellspacing=0 width=156 border=0>
			<tr><td align=center style='padding-bottom:5px;'><img src='$default_path/v3/images/".$admininfo[language]."/left_title_estimation.gif'></td></tr>
	</table>";
...
function offline_order_menu($default_path='/admin', $category_tree=""){
	global $admininfo;
	$mstring = "<div style='position:relative; width:186px; margin:0px auto;'>
	<table cellpadding=0  cellspacing=0 width=100% border=0>
			<tr><td align=center style='padding-bottom:5px;'><img src='$default_path/v3/images/".$admininfo[language]."/left_title_offline_order.gif'></td></tr>
	</table>";
	$menus = getMenuData("offline_order");
	$mstring .= getnerateLeftMenu($default_path, $menus, $category_tree);
	$mstring .= "</div>";
	/*
	<table cellpadding=0 bgcolor='#c0c0c0' cellspacing=1 width=156 border=0 style='border-collapse:separate; border-spacing:1px;'>";
...
function pos_order_menu($default_path='/admin', $category_tree=""){
	global $admininfo;
	$mstring = "
	<table cellpadding=0  cellspacing=0 width=156 border=0>
			<tr><td align=center style='padding-bottom:5px;'><img src='$default_path/v3/images/".$admininfo[language]."/left_title_pos_order.gif'></td></tr>
	</table>";
	$menus = getMenuData("pos_order");
	$mstring .= getnerateLeftMenu($default_path, $menus, $category_tree);
	/*
	<table cellpadding=0 bgcolor='#c0c0c0' cellspacing=1 width=156 border=0 style='border-collapse:separate; border-spacing:1px;'>";
	if(substr_count ($admininfo[permit], "09-01")){
...
function design_menu22($default_path='/admin', $category_tree=""){
	...
	$mstring = "
	<table cellpadding=0  cellspacing=0 width=156 border=0>
			<tr><td align=center style='padding-bottom:5px;'><img src='$default_path/v3/images/".$admininfo[language]."/left_title_order.gif'></td></tr>
	</table>";
...
";
$mstring .= 	"<table cellpadding=0  cellspacing=0 width=196 border=0>
					<tr><td align=center style='padding-bottom:5px;'><img src='$default_path/v3/images/".$admininfo[language]."/left_title_design.gif'></td></tr>
			</table>
...
$mstring = $mstring."
		  <TR>
		    <TD align=left>
		      ...
		      <DIV class=SM_c11464 id=SM1146430Sub30 onmouseover=\"SMcs11464(this, 'SM_co11464', '')\" onmouseout=\"SMcs11464(this, 'SM_c11464', '')\"><A href='$default_path/design/design.mod.php?mod=layout&page_name=layout.htm&SubID=SM1146411Sub' ".($_GET["mod"] == "layout" ? "style='color:red;font-weight:bold;'":"")."><img src='".$default_path."/images/icon/left_dot.gif' border=0 align=absmiddle> 쇼핑몰 레이아웃</a> <!--a href=\"javascript:PoPWindow('design.mod.php?mod=layout&page_name=layout.htm&mmode=pop',960,600,'design')\"'><img src='/admin/image/btn_spop_manage.gif'  valign=bottom'></a--> </DIV>
		      <DIV class=SM_c11464 id=SM1146419Sub19 onmouseover=\"SMcs11464(this, 'SM_co11464', '')\" onmouseout=\"SMcs11464(this, 'SM_c11464', '')\"><a href='$default_path/design/design.mod.php?mod=layout/header&page_name=header_top.htm&SubID=SM1146411Sub' ".($_GET["mod"] == "layout/header" ? "style='color:red;font-weight:bold;'":"")."><img src='".$default_path."/images/icon/left_dot.gif' border=0 align=absmiddle> 상단(header)</A></DIV>
		      <DIV class=SM_c11464 id=SM1146419Sub19 onmouseover=\"SMcs11464(this, 'SM_co11464', '')\" onmouseout=\"SMcs11464(this, 'SM_c11464', '')\"><A href='$default_path/design/design.mod.php?mod=layout/leftmenu&page_name=ms_community_leftmenu.htm&SubID=SM1146411Sub' ".($_GET["mod"] == "layout/leftmenu" ? "style='color:red;font-weight:bold;'":"")."><img src='".$default_path."/images/icon/left_dot.gif' border=0 align=absmiddle> 좌측메뉴(leftmenu)</A></DIV>
		      <DIV class=SM_c11464 id=SM1146419Sub19 onmouseover=\"SMcs11464(this, 'SM_co11464', '')\" onmouseout=\"SMcs11464(this, 'SM_c11464', '')\"><A href='$default_path/design/design.mod.php?mod=layout/contents_add&page_name=contents_add.htm&SubID=SM1146411Sub' ".($_GET["mod"] == "layout/contents_add" ? "style='color:red;font-weight:bold;'":"")."><img src='".$default_path."/images/icon/left_dot.gif' border=0 align=absmiddle> 추가컨텐츠</A></DIV>
		      <DIV class=SM_c11464 id=SM1146430Sub31 onmouseover=\"SMcs11464(this, 'SM_co11464', '')\" onmouseout=\"SMcs11464(this, 'SM_c11464', '')\"><A href='$default_path/design/design.mod.php?mod=layout/rightmenu&page_name=today_history2.htm&SubID=SM1146411Sub' ".($_GET["mod"] == "layout/rightmenu" ? "style='color:red;font-weight:bold;'":"")."><img src='".$default_path."/images/icon/left_dot.gif' border=0 align=absmiddle> 오른쪽메뉴(rightmenu)</A></DIV>
		      <DIV class=SM_c11464 id=SM1146430Sub32 onmouseover=\"SMcs11464(this, 'SM_co11464', '')\" onmouseout=\"SMcs11464(this, 'SM_c11464', '')\"><A href='$default_path/design/design.mod.php?mod=layout/footer&page_name=footer_menu.htm&SubID=SM1146411Sub' ".($_GET["mod"] == "layout/footer" ? "style='color:red;font-weight:bold;'":"")."><img src='".$default_path."/images/icon/left_dot.gif' border=0 align=absmiddle> 하단설명(footer)</A></DIV>
		      <DIV class=SM_c11464 id=SM1146430Sub32 onmouseover=\"SMcs11464(this, 'SM_co11464', '')\" onmouseout=\"SMcs11464(this, 'SM_c11464', '')\"><A href='$default_path/design/design.mod.php?mod=css&page_name=mallstory.css&SubID=SM1146411Sub' ".($_GET["mod"] == "css" ? "style='color:red;font-weight:bold;'":"")."><img src='".$default_path."/images/icon/left_dot.gif' border=0 align=absmiddle> 스타일시트(css)</A></DIV>
		      <DIV class=SM_c11464 id=SM1146430Sub32 onmouseover=\"SMcs11464(this, 'SM_co11464', '')\" onmouseout=\"SMcs11464(this, 'SM_c11464', '')\"><A href='$default_path/design/design.mod.php?mod=js&page_name=mallstory.js&SubID=SM1146411Sub' ".($_GET["mod"] == "js" ? "style='color:red;font-weight:bold;'":"")."><img src='".$default_path."/images/icon/left_dot.gif' border=0 align=absmiddle> 자바스크립트(js)</A></DIV>
		      <DIV class=SM_c11464 id=SM1146430Sub32 onmouseover=\"SMcs11464(this, 'SM_co11464', '')\" onmouseout=\"SMcs11464(this, 'SM_c11464', '')\"><A href='$default_path/design/design.mod.php?mod=etc&page_name=category_sbulayer.htm&SubID=SM1146411Sub' ".($_GET["mod"] == "etc" ? "style='color:red;font-weight:bold;'":"")."><img src='".$default_path."/images/icon/left_dot.gif' border=0 align=absmiddle> 레이아웃기타</A></DIV>
		      </DIV>
		      </TD>
		  </TR>";
...
$mstring = $mstring."
		  <TR>
		    <TD align=left >
		      ...
		      <A href='$default_path/design/design_layout.php?SubID=SM22464243Sub' ".(substr_count($_SERVER["PHP_SELF"],"design_layout.php") ? "style='color:red;font-weight:bold;'":"").">
		      ...
		      <A href='$default_path/design/design.html.php?SubID=SM22464243Sub' ".(substr_count($_SERVER["PHP_SELF"],"design.html.php") ? "style='color:red;font-weight:bold;'":"").">
		      ...
		      <!--A href='$default_path/design/design_title.php?SubID=SM22464243Sub' ".(substr_count($_SERVER["PHP_SELF"],"design_title.php") ? "style='color:red;font-weight:bold;'":"").">
		      ...
		      <A href='$default_path/design/design_bbs_templet.php?SubID=SM22464243Sub' ".(substr_count($_SERVER["PHP_SELF"],"design_bbs_templet.php") ? "style='color:red;font-weight:bold;'":"").">
			  ...
			  <A href='$default_path/design/product_icon.php?SubID=SM22464243Sub' ".(substr_count($_SERVER["PHP_SELF"],"product_icon.php") ? "style='color:red;font-weight:bold;'":"").">
...
function order_menu($default_path='/admin'){
	...
	$mstring = "<div class='btn_workset'>
	<table cellpadding=0  cellspacing=0 width=180 border=0 height='5%' >
			<tr><td align=center style='padding-bottom:5px;'><img src='$default_path/v3/images/".$admininfo[language]."/left_title_order.gif' ></td></tr>
	</table>
	</div>"
	;
	...
	$mstring .= "<table cellpadding=0  cellspacing=0 width=196 border=0>
							<tr height=23 bgcolor='#FFFFFF'>
								...
								<div style='float:left;border:1px solid gray;background-color:#efefef;width:44%;text-align:center;padding:4px;'><a href='#' id='btnExpandAll'>전체열기</a></div>
								<div style='float:left;border:1px solid gray;background-color:#efefef;width:44%;text-align:center;padding:4px;margin-left:2px;'><a href='#' id='btnCollapseAll'>전체닫기</a></div>
									<div id='leftmenu_tree'  style='overflow:auto;width:190px;height:950px;border: 0px solid silver;text-align:left;'></div>
								</td>
							</tr>";
	$mstring.= "</table>	
<link href='/admin/lib/dynatree/skin/ui.dynatree.css' rel='stylesheet' type='text/css' id='skinSheet'>
<script src='/admin/lib/dynatree/jquery.dynatree.js' type='text/javascript'></script>
<script language='javascript'>
MenuTreeView('leftmenu_tree', 'order');
</script>
		";
	/*
	$mstring .= "<div class='svs_menu i_scroll'>
						<div style='position:relative; width:190px; margin:0px auto;'>
							<div class='list_menu'>";
...
function member_menu($default_path='/admin'){
	...
	$mstring = "
	<table cellpadding=0  cellspacing=0 width=156 border=0>
			<tr><td align=center style='padding-bottom:5px;'><img src='$default_path/v3/images/".$admininfo[language]."/left_title_member.gif'></td></tr>
...
function database_menu($default_path='/admin'){
	...
	$mstring = "
	<table cellpadding=0  cellspacing=0 width=156 border=0>
			<tr><td align=center style='padding-bottom:5px;'><img src='$default_path/v3/images/".$admininfo[language]."/left_title_data.gif'></td></tr>
	</table>";
...
global $admin_config, $admininfo;
	...
	$mstring = "
	<table cellpadding=0  cellspacing=0 width=156 border=0>
			<tr><td align=center style='padding-bottom:5px;'><img src='$default_path/v3/images/".$admininfo[language]."/left_title_cscenter.gif'></td></tr>
	</table>";
	...
	$mdb = new Database;
	$mdb->query("select bmc.* , bg.div_ix, bg.div_name as group_name from bbs_manage_config bmc , bbs_group bg where bmc.board_group = bg.div_ix and disp = 1 and bmc.board_style = 'bbs' and bbs_templet_dir != 'basic_after'");
	$bbs_menes = $mdb->fetchall();
	
	$mstring .= "<table cellpadding=0 cellspacing=0 width=156 border=0 style='border:1px solid silver;'>";
	for($j=0;$j<count($bbs_menes);$j++){
		$mstring .= "<tr height=25 bgcolor='#FFFFFF'><td align=left style='padding-left:15px;'> <img src='/admin/images/icon/dot_orange.gif' align=absmiddle> <a href='./bbs.php?mode=list&board=".$bbs_menes[$j][board_ename]."' >".$bbs_menes[$j][board_name]."</a></td></tr>";
	}
	$mstring.= "</table>";
...
global $admin_config, $admininfo, $admin_root;
	$mdb = new Database;

	$mstring = "
	<table cellpadding=0  cellspacing=0 width=156 border=0>
			<tr><td align=center style='padding-bottom:5px;'><img src='$default_path/v3/images/".$admininfo[language]."/left_title_bbsmanage.gif'></td></tr>
	</table>";
...
//	$mstring .= "<table cellpadding=0 class='line_color' cellspacing=0 width=156 border=0 style='border-collapse:collapse; border-spacing:1px;'>";//class='table_border'
/*
	...
	for($i=0;$i < count($menus);$i++){
		...
		$menu_link = str_replace("/admin",$admin_root,$menus[$i][menu_link]);
		...
		if(count($navis[1]) >= 1){
			...
			$depth1_name = $navis[0];
			$mstring .= "<tr height=20 bgcolor='#FFFFFF'><td align=left class='".$selectedClass."' style='padding-left:20px;'> <img src='/admin/images/icon/dot_orange.gif' align=absmiddle> <a href='".$menu_link."' class='menu_style1_a'>".(count($navis[0]) >= 1 ? $navis[1]:$menus[$i][menu_name])."</a></td></tr>";


		}else{
			if("b3844808846874c7072c75b170558590" == $menus[$i][menu_code]){
				$mstring.= "<tr height=20 bgcolor='#FFFFFF'><td align=left class='".$selectedClass."'><IMG id=SM114641I src='".$default_path."/images/icon/dot_orange_triangle.gif' border=0>&nbsp;<a href='".$menu_link."' class='menu_style1_a'>".$menus[$i][menu_name]."</a></td></tr>";
...
function goodss_menu($default_path='/admin'){
global $admin_config, $admininfo;

	$mstring = "
	<table cellpadding=0  cellspacing=0 width=156 border=0>
			<tr><td align=center style='padding-bottom:5px;'><img src='$default_path/v3/images/".$admininfo[language]."/left_title_goodss.gif'></td></tr>
	</table>";
	$mstring .= "
...
global $admin_config, $admininfo;
	...
	$mstring = "
	<table cellpadding=0  cellspacing=0 width=156 border=0>
			<tr><td align=center style='padding-bottom:5px;'><img src='$default_path/v3/images/".$admininfo[language]."/left_title_share.gif'></td></tr>
	</table>";
	$mstring .= "
	<table cellpadding=0 class='line_color' cellspacing=0 width=156 border=0 style='border-collapse:collapse; border-spacing:1px;'>";//class='table_border'
...
function inventory_menu($default_path='/admin', $category_tree=""){
	...
	$mstring = "
	<table cellpadding=0  cellspacing=0 width=196 border=0>
			<tr><td align=center style='padding-bottom:5px;'><img src='$default_path/v3/images/".$admininfo[language]."/left_title_inventory.gif'></td></tr>
	</table>";
	$mstring .= "";//class='table_border'
	...
	$mstring .= "<table cellpadding=0  cellspacing=0 width=196 border=0>
							<tr height=23 bgcolor='#FFFFFF'>
								<td style='border-collapse:separate; border:1px solid #c0c0c0;padding:5px 5px 15px 5px;' align=left >
									<div style='float:left;border:1px solid gray;background-color:#efefef;width:44%;text-align:center;padding:4px;'><a href='#' id='btnExpandAll'>전체열기</a></div>
									<div style='float:left;border:1px solid gray;background-color:#efefef;width:44%;text-align:center;padding:4px;margin-left:2px;'><a href='#' id='btnCollapseAll'>전체닫기</a></div>
									<div id='leftmenu_tree'  style='overflow:auto;width:180px;height:750px;border: 0px solid silver;text-align:left;'></div>
								</td>
							</tr>";
	$mstring.= "</table>
...
$('#menu_tree').treeview({
		url: 'inventory_category.act.php?act=getLeftmenuData',
	});
...
<script src='../lib/dynatree/jquery.dynatree.js' type='text/javascript'></script>
<script language='javascript'>
MenuTreeView('leftmenu_tree', 'inventory');
</script>
		";
	//$menus = getMenuData("inventory");
	//$menus = getMenuTreeData("inventory");
	//print_r($menus);

	//$mstring .= getnerateLeftMenu($default_path, $menus);
	//$mstring .= generateLeftTreeMenu($default_path, $menus);
	/*
	for($i=0;$i < count($menus);$i++){
		$mstring.= "<tr height=20 bgcolor='#FFFFFF'><td align=left class='leftmenu'><IMG id=SM114641I src='".$default_path."/images/icon/dot_orange_triangle.gif' border=0>&nbsp;<a href='".$menus[$i][menu_link]."' class='menu_style1_a'>".$menus[$i][menu_name]."</a></td></tr>";
	}
...
function campaign_menu($default_path='/admin', $category_tree=""){
	...
	$mstring = "
	<table cellpadding=0  cellspacing=0 width=156 border=0>
			<tr><td align=center style='padding-bottom:5px;'><img src='$default_path/v3/images/".$admininfo[language]."/left_title_campaign.gif'></td></tr>
	</table>";
...
function global_menu($default_path='/admin'){
global $admin_config, $admininfo;

 
	$mstring = "
	<table cellpadding=0  cellspacing=0 width=156 border=0>
			<tr><td align=center style='padding-bottom:5px;'><img src='$default_path/v3/images/".$admininfo[language]."/left_title_global.gif'></td></tr>
	</table>";
	

	$menus = getMenuData("global");
...
function buyingservice_menu($default_path='/admin', $category_tree=""){
	...
	$mstring = "
	<table cellpadding=0  cellspacing=0 width=156 border=0>
			<tr><td align=center style='padding-bottom:5px;'><img src='$default_path/v3/images/".$admininfo[language]."/left_title_buyingservice.gif'></td></tr>
	</table>";
	$menus = getMenuData("buyingservice");
	$mstring .= getnerateLeftMenu($default_path, $menus);
...
function sellertool_menu($default_path='/admin', $category_tree=""){
	global $admininfo;
	$mstring = "
	<table cellpadding=0  cellspacing=0 width=156 border=0>
			<tr><td align=center style='padding-bottom:5px;'><img src='$default_path/v3/images/".$admininfo[language]."/left_title_sellertool.gif'></td></tr>
	</table>";
	$menus = getMenuData("sellertool");
	$mstring .= getnerateLeftMenu($default_path, $menus);
...
function econtract_menu($default_path='/admin', $category_tree=""){
	global $admininfo;
	$mstring = "
	<table cellpadding=0  cellspacing=0 width=156 border=0>
			<tr><td align=center style='padding-bottom:5px;'><img src='$default_path/v3/images/".$admininfo[language]."/left_title_econtract.gif'></td></tr>
	</table>";
	$menus = getMenuData("econtract");
	$mstring .= getnerateLeftMenu($default_path, $menus);
...
function buyingservice_menu2($default_path='/admin', $category_tree=""){
	global $admininfo;
	$mstring = "
	<table cellpadding=0  cellspacing=0 width=156 border=0>
			<tr><td align=center style='padding-bottom:5px;'><img src='$default_path/v3/images/".$admininfo[language]."/left_title_campaign.gif'></td></tr>
...
//	$mstring .= getnerateLeftMenu($default_path, $menus);
	...
	$mstring .= "<tr height=23 bgcolor='#FCE7C8'><td align=left class='leftmenu' ><IMG id=SM114641I src='".$default_path."/images/icon/dot_orange_triangle.gif' border=0>&nbsp;<a href='$default_path/buyingservice/buying_company.php' class='menu_style1_a'>사입처관리</a></td></tr>";
	//if(substr_count ($admininfo[permit], "14-02")){
		$mstring .= "<tr height=23 bgcolor='#FCE7C8'><td align=left class='leftmenu' ><IMG id=SM114641I src='".$default_path."/images/icon/dot_orange_triangle.gif' border=0>&nbsp;<a href='$default_path/buyingservice/buying_list.php' class='menu_style1_a'>사입신청목록</a></td></tr>";
	//}

	//if(substr_count ($admininfo[permit], "14-01")){
		$mstring .= "<tr height=23 bgcolor='#FCE7C8'><td align=left class='leftmenu' ><IMG id=SM114641I src='".$default_path."/images/icon/dot_orange_triangle.gif' border=0>&nbsp;<a href='$default_path/buyingservice/buying_list.php?buying_status=IR' class='menu_style1_a'>입금예정리스트</a></td></tr>";
	//}

	//if(substr_count ($admininfo[permit], "14-03")){
		$mstring .= "<tr height=23 bgcolor='#FCE7C8'><td align=left class='leftmenu' ><IMG id=SM114641I src='".$default_path."/images/icon/dot_orange_triangle.gif' border=0>&nbsp;<a href='$default_path/buyingservice/buying_list.php?buying_status=IC' class='menu_style1_a'>입금확인리스트</a></td></tr>";
	//}
	$mstring .= "<tr height=23 bgcolor='#FCE7C8'><td align=left class='leftmenu' ><IMG id=SM114641I src='".$default_path."/images/icon/dot_orange_triangle.gif' border=0>&nbsp;<a href='$default_path/buyingservice/buying_goods_list.php' class='menu_style1_a'>사입상품 리스트</a></td></tr>";

	$mstring .= "<tr height=23 bgcolor='#FCE7C8'><td align=left class='leftmenu' ><IMG id=SM114641I src='".$default_path."/images/icon/dot_orange_triangle.gif' border=0>&nbsp;<a href='$default_path/buyingservice/commercial_area.php' class='menu_style1_a'>상권관리</a></td></tr>";

	$mstring .= "<tr height=23 bgcolor='#FCE7C8'><td align=left class='leftmenu' ><IMG id=SM114641I src='".$default_path."/images/icon/dot_orange_triangle.gif' border=0>&nbsp;<a href='$default_path/buyingservice/shopping_center_list.php' class='menu_style1_a'>상가 정보 관리</a></td></tr>";
...
function reseller_menu($default_path='/admin', $category_tree=""){
	global $admininfo;
	$mstring = "
	<table cellpadding=0  cellspacing=0 width=156 border=0>
			<tr><td align=center style='padding-bottom:5px;'><img src='$default_path/v3/images/".$admininfo[language]."/left_title_reseller.gif'></td></tr>
	</table>";
	$menus = getMenuData("reseller");
	$mstring .= getnerateLeftMenu($default_path, $menus);
...
function work_menu2($default_path='/admin', $category_tree=""){
	...
	$mstring = "
	<table cellpadding=0  cellspacing=0 width=156 border=0>
			<tr><td align=center style='padding-bottom:5px;'><img src='$default_path/v3/images/".$admininfo[language]."/left_title_work.gif'></td></tr>
	</table>
	<table cellpadding=0 width=100% cellspacing=1 border=0 bgcolor='#c0c0c0' style='border-collapse:separate; border-spacing:1px;margin-bottom:5px;' >
		...
		<tr bgcolor='#FCE7C8'>
			...
			<a href='work_list.php?list_type=weekly&list_view_type=weekly'><b>업무</b></a>
			</td>
		</tr>
	</table>
	<table cellpadding=0 width=100% cellspacing=0 border=0 bgcolor='#c0c0c0' style='margin-bottom:5px;' class='table_border'>
		<tr height=24 bgcolor='#FCE7C8'>
			...
			<a href='work_add.php?list_view_type=".$list_view_type."&list_type=weekly'><b>스케줄/업무 등록</b></a>
			</td>
...
function work_menu(){
global $mdb, $admininfo, $list_view_type, $admininfo, $work_tab;

//$work_tab = array("pj"=>"프로젝트","ww"=>"주간업무","be"=>"미처리업무","my"=>"내업무","my"=>"내업무","to"=>"금일업무","se"=>"상세검색");

if($admininfo["work_confs"]["basic_tab"] == "pj"){
	$work_link = "work_project_list.php?mmode=&list_view_type=&list_type=project";
	$work_title = $work_tab[$admininfo["work_confs"]["basic_tab"]];
}else if($admininfo["work_confs"]["basic_tab"] == "ww"){
	$work_link = "work_list.php?list_type=weekly&list_view_type=weekly";
	$work_title = $work_tab[$admininfo["work_confs"]["basic_tab"]];
}else if($admininfo["work_confs"]["basic_tab"] == "be"){
	$work_link = "work_list.php?mmode=&list_view_type=&list_type=before";
	$work_title = $work_tab[$admininfo["work_confs"]["basic_tab"]];
}else if($admininfo["work_confs"]["basic_tab"] == "my"){
	$work_link = "work_list.php?mmode=&list_view_type=&list_type=myjob";
	$work_title = $work_tab[$admininfo["work_confs"]["basic_tab"]];
}else if($admininfo["work_confs"]["basic_tab"] == "mydepartment"){
	$work_link = "work_list.php?mmode=&list_view_type=&list_type=mydepartment";
	$work_title = $work_tab[$admininfo["work_confs"]["basic_tab"]];
}else if($admininfo["work_confs"]["basic_tab"] == "to"){
	$work_link = "work_list.php?mmode=&list_view_type=&list_type=today";
	$work_title = $work_tab[$admininfo["work_confs"]["basic_tab"]];
}else if($admininfo["work_confs"]["basic_tab"] == "se"){
	$work_link = "work_list.php?mmode=&list_view_type=&list_type=search";
	$work_title = $work_tab[$admininfo["work_confs"]["basic_tab"]];
}else{
	$work_link = "work_list.php?list_type=weekly&list_view_type=weekly";
	$work_title = "업무";
}
$mstring = "<div style='position:fixed;'>
<table cellpadding=0  cellspacing=0 width=196 border=0>
	<tr><td align=center style='padding-bottom:5px;'><a href='../work/'><img src='../v3/images/".$admininfo[language]."/left_title_work.gif'></a></td></tr>
</table>
...
<table cellpadding='0' width='100%' bgcolor='#c0c0c0' cellspacing=1 style='border-collapse:separate; border-spacing:1px;margin-bottom:5px;'  >
	<tr height=24 bgcolor='#efefef'>
		...
		<div style='height:1px;width:1px;position:absolute;z-index:999;".(($_COOKIE["company_goal_view"] == 1 || $_COOKIE["company_goal_view"] == '')? "display:inline;":"display:none;")."' id='company_goal' >
			<div style='z-index:100px;position:relative;left:110px;top:-100px;'>
...
<table width='100%' border='0' cellpadding='0' cellspacing='1' bgcolor='#c0c0c0' style='border-collapse:separate; border-spacing:1px;margin-bottom:5px;'>
	...
	<tr bgcolor='#efefef'>
		...
		<td align=center class='leftmenu' style='padding:0px''>
		<a href='".$work_link."'><b>".$work_title."</b></a>
		</td>
	</tr>
</table>
<table cellpadding='0' width='100%' bgcolor='#c0c0c0' cellspacing=1 style='border-collapse:separate; border-spacing:1px;margin-bottom:5px;' >
...
$mstring .= "
	<tr height=20 bgcolor='#efefef'>
		<td align=left class='leftmenu' onclick=\"ToggleTreeUser();\" style='height:25px;padding:0px 0px 0px 10px;cursor:hand;' title='클릭하시면 부서/직원 트리메뉴가 노출됩니다.'>
	...
	<tr height=0 bgcolor='#FFFFFF'>
		...
		<div id='tree_user'  style='".($_COOKIE["tree_user_view"] == '0' ? "display:none;":"")."overflow:auto;width:195px;height:150px;border: 0px solid silver;'></div>
		</td>
	</tr>";
}
...
$mstring .= "
	...
	<tr height=0 bgcolor='#FFFFFF'>
		<td align=left style='border-top:0px solid gray;vertical-align:top;".($_COOKIE["tree_work_group_view"] == '0' ? "height:0px;":"height:150px;")."'>
			<div id='tree_work_group' style='".($_COOKIE["tree_work_group_view"] == '0' ? "display:none;":"")."overflow:auto;height:150px;width:195px;border: 0px solid red;'></div>
		</td>
	</tr>";
}
...
<table cellpadding=0 width=100% bgcolor='#c0c0c0' cellspacing=1 style='border-collapse:separate; border-spacing:1px;margin-top:5px;' >";
	...
	if($admininfo[master] == "Y"){
		$mstring .= "<tr height=20 bgcolor='#efefef'><td align=left class='leftmenu' style='height:25px;padding:0px 0px 0px 10px;border-top:1px solid #c0c0c0'><IMG id=SM114641I src='../images/icon/dot_orange_triangle.gif' border=0>&nbsp;<a href='../work/user.php' class='menu_style1_a'>사용자 관리</a></td></tr>";
	}else{
		$mstring .= "<tr height=20 bgcolor='#efefef'><td align=left class='leftmenu' style='height:25px;padding:0px 0px 0px 10px;border-top:1px solid #c0c0c0'><IMG id=SM114641I src='../images/icon/dot_orange_triangle.gif' border=0>&nbsp;<a href='../work/user.php' class='menu_style1_a'>나의정보 관리</a></td></tr>";
	}
	//}
if($admininfo[master] == "Y"){
	//if(substr_count ($admininfo[permit], "15-01")){
		$mstring .= "<tr height=20 bgcolor='#efefef'><td align=left class='leftmenu' ><IMG id=SM114641I src='../images/icon/dot_orange_triangle.gif' border=0>&nbsp;<a href='../work/work_group.php' class='menu_style1_a'>업무 그룹관리</a></td></tr>";
	//}
}
	//if(substr_count ($admininfo[permit], "15-01")){
		$mstring .= "<tr height=20 bgcolor='#efefef'><td align=left class='leftmenu' style='height:25px;padding:0px 0px 0px 10px;'><IMG id=SM114641I src='../images/icon/dot_orange_triangle.gif' border=0>&nbsp;<a href='../work/work_report_list.php' class='menu_style1_a'>보고서 </a></td></tr>";
//if($admininfo[master] == "Y" && $admininfo["charger_id"] == "sigi1074"){
		$mstring .= "<tr height=20 bgcolor='#efefef'><td align=left class='leftmenu' style='height:25px;padding:0px 0px 0px 10px;'><IMG id=SM114641I src='../images/icon/dot_orange_triangle.gif' border=0>&nbsp;<a href='../work/work_comment_list.php' class='menu_style1_a'>컴멘트 목록 </a></td></tr>";

		$mstring .= "<tr height=20 bgcolor='#efefef'><td align=left class='leftmenu' ><IMG id=SM114641I src='../images/icon/dot_orange_triangle.gif' border=0>&nbsp;<a href='../work/work_issue_list.php' class='menu_style1_a'>이슈 목록 </a></td></tr>";
		
		$mstring .= "<tr height=20 bgcolor='#efefef'><td align=left class='leftmenu' ><IMG id=SM114641I src='../images/icon/dot_orange_triangle.gif' border=0>&nbsp;<a href='../work/work_memo_list.php' class='menu_style1_a'>메모 목록 </a></td></tr>";
//}
	//}
	$mstring .= "</table>";
if($admininfo[master] == "Y" && $admininfo["charger_id"] == "sigi1074" || $admininfo["charger_id"] == "forbiz"){
$mstring .= "
<table cellpadding=0 width=100% cellspacing=1 border=0 bgcolor='#c0c0c0' style='margin-top:5px;' style='border-collapse:separate; border-spacing:1px;margin-top:5px;'>
	<tr height=23 bgcolor='#efefef'>
		<td align=center class='leftmenu' style='padding:0px;height:25px;'>
		<a href=\"javascript:PopSWindow('work_report.php?mmode=pop',900,750,'work_report_make')\"><b>보고서 작성하기</b></a>
		</td>
	</tr>
</table>
	";
...
$work_notices = work_fetch_bbs("bbs_work_notice",5);

$mstring .= "<div style='float:left;background-color:#000000;color:#ffffff;padding:5px 0px 5px 0px;margin-top:5px;width:80px' class='small'><b>공지사항</b></div>
<table cellpadding=0 width=100% cellspacing=1 border=0 bgcolor='#c0c0c0' style='border-collapse:separate; border-spacing:1px;'>
<tr height=23 bgcolor='#efefef'>
		<td align=left class='leftmenu' style='background-color:#ffffff;padding:5px 5px 5px 6px;height:25px;'>";
for($i=0;$i < count($work_notices);$i++){
$mstring .= "<div style='padding:2px 0px;'><img src='../image/icon_list.gif' border=0 align=bottom >
	
		<a href=\"bbs.php?mode=read&board=work_notice&bbs_ix=".$work_notices[$i][bbs_ix]."\" title=\"".$work_notices[$i][bbs_subject]."\">".Cut_Str($work_notices[$i][bbs_subject],28)."</a></div>
		";
}
$mstring .= "
		</td>
...
$mstring = "
<table cellpadding=0  cellspacing=0 width=156 border=0>
	<tr><td align=center style='padding-bottom:5px;'><img src='../v3/images/".$admininfo[language]."/left_title_myservice.gif'></td></tr>
</table>";

$menus = getMenuData("myservice");
	$mstring .= getnerateLeftMenu($default_path, $menus);
...
<table cellpadding=0 width=100% bgcolor='#c0c0c0' cellspacing=1 style='border-collapse:separate; border-spacing:1px;'>";
	//if(substr_count ($admininfo[permit], "15-01")){
		$mstring .= "<tr bgcolor='#efefef'><td align=left class='leftmenu' style='height:16px;'><IMG id=SM114641I src='../images/icon/dot_orange_triangle.gif' border=0>&nbsp;<a href='../myservice/' class='menu_style1_a'>서비스현황</a></td></tr>";
	//}
	//if(substr_count ($admininfo[permit], "15-01")){
		$mstring .= "<tr bgcolor='#efefef'>
							<td align=left class='leftmenu' style='height:16px;'>
							<IMG id=SM114641I src='../images/icon/dot_orange_triangle.gif' border=0>&nbsp;<a href='../myservice/payment_history.php' class='menu_style1_a'>결제내역</a>
							</td>
						</tr>";
	//}
	$mstring .= "<tr bgcolor='#efefef'><td align=left class='leftmenu' style='height:16px;'><IMG id=SM114641I src='../images/icon/dot_orange_triangle.gif' border=0>&nbsp;<a href='../myservice/documentary_evidence.php' class='menu_style1_a'>증빙서류 관리</a></td></tr>";
	$mstring .= "
...
$mstring = "
<table cellpadding=0  cellspacing=0 width=196 border=0>
	<tr><td align=center style='padding-bottom:5px;'><img src='../v3/images/".$admininfo[language]."/left_title_kms.gif'></td></tr>
</table>

<table cellpadding='0' width='100%' cellspacing='1' border='0' bgcolor='#c0c0c0' style='margin-bottom:5px;' style='border-collapse:separate; border-spacing:1px;'>
	<tr height=24 bgcolor='#efefef'>
		<td align=center class='leftmenu' style='padding:0px;'>
...
<table cellpadding=0 bgcolor='#c0c0c0' cellspacing='1' width=100%   style='border-collapse:separate; border-spacing:1px;'>

	<tr height=20 bgcolor='#efefef'>
		<td align=left class='leftmenu' onclick=\"ToggleTreeUser();\" style='cursor:hand;height:25px;padding:0px 0px 0px 10px;' title='클릭하시면 부서/직원 트리메뉴가 노출됩니다.'>
	...
	<tr height=0 bgcolor='#FFFFFF'>
		...
		<div  id='kms_tree' style=height:180px;width:175px;overflow:auto;padding:10px 10px 40px 10px'>".Category()."</div>
		</td>
	</tr>
	...
	";
$mstring .= "
<table cellpadding=0 width=100% cellspacing=1 border=0 bgcolor='#c0c0c0' style='margin-top:5px;' style='border-collapse:separate; border-spacing:1px;'>
	<tr height=23 bgcolor='#efefef'>
		<td align=center class='leftmenu' style='padding:0px;height:25px;'>
...
function delivery_menu($default_path='/admin'){
	...
	$mstring .= "<table cellpadding=0 bgcolor='#c0c0c0' cellspacing=1 width=156 border=0>";
		$mstring .= "<tr height=20 bgcolor='#E1F0F6'><td align=left class='leftmenu'><!--IMG id=SM114641I src='".$default_path."/images/dot_org.gif' border=0>&nbsp;--><b >배송대행신청목록</b></td></tr>";
		...
		$mstring .= "<tr height=20 bgcolor='#E1F0F6'><td align=left class='leftmenu' style='padding-left:17px;'><IMG id=SM114641I src='".$default_path."/images/icon/dot_orange.gif' border=0>&nbsp;<a href='$default_path/delivery/application_list.php?status=AC&delivery_type=$delivery_type' class='menu_style1_a'>고객 신청완료목록</a></td></tr>";
		$mstring .= "<tr height=20 bgcolor='#E1F0F6'><td align=left class='leftmenu' style='padding-left:17px;'><IMG id=SM114641I src='".$default_path."/images/icon/dot_orange.gif' border=0>&nbsp;<a href='$default_path/delivery/application_list.php?status=TC&delivery_type=$delivery_type' class='menu_style1_a'>관리자 접수완료목록</a></td></tr>";
		$mstring .= "<tr height=20 bgcolor='#E1F0F6'><td align=left class='leftmenu' style='padding-left:17px;'><IMG id=SM114641I src='".$default_path."/images/icon/dot_orange.gif' border=0>&nbsp;<a href='$default_path/delivery/application_list.php?status=IC&delivery_type=$delivery_type' class='menu_style1_a'>출고목록</a></td></tr>";
		$mstring .= "<tr height=20 bgcolor='#E1F0F6'><td align=left class='leftmenu' style='padding-left:17px;'><IMG id=SM114641I src='".$default_path."/images/icon/dot_orange.gif' border=0>&nbsp;<a href='$default_path/delivery/group.php' class='menu_style1_a small'>신청분류관리</a></td></tr>";
		$mstring .= "<tr height=20 bgcolor='#E1F0F6'><td align=left class='leftmenu'><!--IMG id=SM114641I src='".$default_path."/images/dot_org.gif' border=0>&nbsp;--><a href='$default_path/delivery/application.php' class='menu_style1_a'><b >배송대행접수하기</b></a></td></tr>";
		$mstring .= "<tr height=20 bgcolor='#E1F0F6'><td align=left class='leftmenu'><!--IMG id=SM114641I src='".$default_path."/images/dot_org.gif' border=0>&nbsp;--><a href='$default_path/delivery/application.php' class='menu_style1_a'><b >배송대행송장등록</b></a></td></tr>";
		$mstring .= "</table><br>";
...
function getMenuData($menu_div, $data_type="array"){
	global $admininfo, $admin_config;
	$mdb = new Database;
	$default_path = "/admin";
	...
	}else if($admininfo[mall_type] == "O"){ // 오픈마켓형(스탠다드)
		$auth_where = 	" and am.use_openmarket = 'Y' ";
	}else if($admininfo[mall_type] == "E"){ // 오픈마켓형(프리미엄)
		$auth_where = 	" and am.use_enterprise = 'Y' ";
	}else if($admininfo[mall_type] == "H"){ // 홈빌더
		$auth_where = 	" and am.use_home = 'Y' ";
	}else{
		$auth_where = 	" ";
	}
	...
	if($menu_div == "work" || $menu_div == "campaign"){
	//if($menu_div == "work" ){
		$sql = "Select am.*, md5(TRIM(SUBSTRING_INDEX(menu_name, '>',1))) as parent_code, aatd.auth_read, aatd.auth_write_update, aatd.auth_delete
			...
			where am.menu_div = '".$menu_div."' and disp_auth = 'Y' $auth_where
			order by  am.view_order asc, am.regdate asc ";//order by view_order asc
	}else if($menu_div == "logstory/report"){
	/*
	$sql = "Select am.*, aatd.auth_read, aatd.auth_write_update, aatd.auth_delete
			...
			order by  am.view_order asc, am.regdate asc ";//order by view_order as
	*/
	$sql = "Select am.*, md5(TRIM(SUBSTRING_INDEX(menu_name, '>',1))) as parent_code, aatd.auth_read, aatd.auth_write_update, aatd.auth_delete
			from admin_menus am left join admin_auth_templet_detail aatd on am.menu_code = aatd.menu_code and auth_templet_ix = '".$admininfo[charger_roll]."'
			and am.menu_div in ('".$menu_div."','logstory/manage') and am.disp_auth = 'Y'
			...
			$auth_where $inventory_str
			order by  am.view_order asc, am.regdate asc ";//order by view_order asc
	}else if($_SESSION['admininfo']['admin_level'] == 8 && $menu_div == 'logstory/commerce'){// 관리자 레벨 8 이상일 경우 이커머스 특정 페이지만 노출

	$sql = "Select am.*, md5(TRIM(SUBSTRING_INDEX(menu_name, '>',1))) as parent_code, aatd.auth_read, aatd.auth_write_update, aatd.auth_delete
			from admin_menus am left join admin_auth_templet_detail aatd on am.menu_code = aatd.menu_code
			and am.menu_div = '".$menu_div."' and am.disp_auth = 'Y'
			where am.menu_div = '".$menu_div."' and am.menu_code in ('1fc3c0298ba7b384f4cc5ae2b1573fd3','48384a8c23d7c0c324d6f45807520ae4','471dcb24fe78752e2538a3ed542fb8dc')
			group by menu_code
			order by  am.view_order asc, am.regdate asc";//order by view_order asc
		...
		//echo "test";
			...
			for($i=0;$i < count($dics);$i++){
				for($j=0; $j < count($menus);$j++){
					$menus[$j][menu_name] = str_replace($dics[$i]["text_korea"], str_replace("\n","",trim($dics[$i]["text_trans"])), $menus[$j][menu_name]);
					//echo "<pre>";
					//echo $menus[$j][menu_name].":::".$dics[$i]["text_korea"].":::".$dics[$i]["text_trans"]."\n<br><br>";
				}
			}
		}
	}
	if($data_type == "array"){	 
		...
		for($i=0;$i < count($menus);$i++){
			if($admininfo[mall_type] == "H"){
				...
				$menu_name = trim(str_replace("이벤트/기획전","행사",$menu_name));
				$navis = explode(">",str_replace("HOME > ","",$menu_name));
			}else{
				$menu_name = trim($menus[$i][menu_name]);
				$navis = explode(">",str_replace("HOME > ","",$menus[$i][menu_name]));
			}
			//echo $menu_name."<br>";

			if(count($navis[1]) >= 1){
				if($depth1_name != trim($navis[0])){				
					//echo $menus[$i][parent_code]."<br><br><br><br>";
					$sub_menus = filter_by_value($menus, 'parent_code', $menus[$i][parent_code]); 
					//print_r($sub_menus);
					//for($j=0;$j < count($sub_menus);$j++){
					$j = 0;
					foreach($sub_menus as $key => $_sub_menu_info){
						//print_r($value);
						if($admininfo[mall_type] == "H"){
							$sub_menu_name = str_replace("쇼핑몰","사이트",$_sub_menu_info[menu_name]);
							$sub_menu_name = str_replace("이벤트/기획전","행사",$sub_menu_name);
							$sub_navis = explode(">",str_replace("HOME > ","",$sub_menu_name));
						}else{
							$sub_menu_name = $_sub_menu_info[menu_name];
							$sub_navis = explode(">",str_replace("HOME > ","",$_sub_menu_info[menu_name]));
						}
						//echo $_sub_menu_info[menu_name]."<br>";

						$__trees[$j] = array("title"=>"".(count($sub_navis[0]) >= 1 ? $sub_navis[1]:$sub_menu_name)."".($_SESSION[menu_check][md5($_sub_menu_info[menu_link])][type]=='icon' ? ($_SESSION[menu_check][md5($_sub_menu_info[menu_link])][value] ? "<IMG src='".$default_path."/v3/images/btns/new_icon.gif' style='width:8px;height:8px;' border=0>":'') : ($_SESSION[menu_check][md5($_sub_menu_info[menu_link])][type]=='num' ? "(<span class='red str' >".number_format($_SESSION[menu_check][md5($_sub_menu_info[menu_link])][value])."</span>)":''))."","href"=>str_replace("/admin/","/admin/",$_sub_menu_info[menu_link]), "tooltip"=>"".(count($sub_navis[0]) >= 1 ? $sub_navis[1]:$sub_menu_name)."","key"=>md5($_sub_menu_info[menu_link]));
						$j++;
					} 
					$trees[$x] = array("title"=>"".trim($navis[0])."","tooltip"=>"".$navis[0]."","href"=>$menus[$i][menu_link],"key"=>md5($menus[$i][menu_link]), "isFolder"=>'true', "children"=>$__trees);
					$x++;
					unset($__trees);
					unset($sub_menus);
				...
				$depth1_name = trim($navis[0]);
				
			}else{			
				$trees[$x] = array("title"=>"".$menu_name."","tooltip"=>"".$menu_name."","href"=>$menus[$i][menu_link],"key"=>md5($menus[$i][menu_link]),"isFolder"=> "false");
				$x++;
			}
		}
		//echo "<pre>";
			...
			if(count($navis[1]) >= 1){
				if($depth1_name != $navis[0]){				
					...
					foreach($sub_menus as $key => $_sub_menu_info){
						...
						$__trees[] = array("text"=>"<a href='".str_replace("/admin/","/admin/",$_sub_menu_info[menu_link])."' class='menu_style1_a'><b>".(count($sub_navis[0]) >= 1 ? $sub_navis[1]:$sub_menu_name)."</b></a>","id"=>md5($_sub_menu_info[menu_link]));
					}

					$trees[] = array("text"=>"<a href='".str_replace("/admin/","/admin/",$menus[$i][menu_link])."' class='menu_style1_a'><b>".$navis[0]."</b></a>","id"=>md5($menus[$i][menu_link]),"expanded"=> true, "children"=>$__trees);
					unset($__trees);
					unset($sub_menus);
				...
				$trees[] = array("text"=>"<a href='".str_replace("/admin/","/admin/",$menus[$i][menu_link])."' class='menu_style1_a'><b>".$menu_name."</b></a>","id"=>md5($menus[$i][menu_link]),"hasChildren"=> "false");
			}
		}
		//echo "language:".$_SESSION["admininfo"]["language"];
		if($_SESSION["admininfo"]["language"] != "korea"){
			$sql = "select text_korea, text_trans from admin_language where language_type = '".$admininfo["language"]."' and text_div LIKE '%_lnb' order by CHAR_LENGTH(text_korea) desc  ";
			//echo $sql;
			$mdb->query($sql);
			...
			//echo "test";
				...
				for($i=0;$i < count($dics);$i++){
					$trees = str_replace($dics[$i]["text_korea"], str_replace("\n","",trim($dics[$i]["text_trans"])), $trees);
				}
			}
		}
		return $trees;
...
function getnerateLeftMenu($default_path, $menus, $category_tree="", $width=180){
	global $admininfo;
	//echo "aaa : ".$_SERVER["PHP_SELF"];
	$mdb = new Database;

	$mstring .= "<div style='position:relative;overflow-y:auto;overflow-x:hidden;height:100%;top:0px;'>
	<table cellpadding=0  cellspacing=0 width='$width' border=0 style='height:100%;border-collapse:collapse;border:1px solid #c0c0c0; margin:0px auto; '>";//class='table_border'
	if($category_tree){
	$mstring .= "<tr height=0 bgcolor='#FFFFFF'><td align=left >$category_tree</td></tr>";
	}
	//print_r($menus);
	for($i=0;$i < count($menus);$i++){
		//$menus[$i] = array_change_key_case($menus[$i]);
		if($admininfo[mall_type] == "H"){
			$menu_name = str_replace("쇼핑몰","사이트",$menus[$i][menu_name]);
			$menu_name = str_replace("이벤트/기획전","행사",$menu_name);
			$navis = explode(">",str_replace("HOME > ","",$menu_name));
		...
		if(count($navis[1]) >= 1){
//<IMG src='".$default_path."/images/icon/new.gif' border=0>
			if($depth1_name != $navis[0]){
								...
								<td style='border-collapse:separate; border:1px solid #c0c0c0;' align=left class='".(substr_count($menu_name,$navis[0]) ? "leftmenu":"")."'>
									<table cellpadding=0 cellspacing=0 style='margin:0px;'>
										<tr>
											...
											<td><a href='".str_replace("/admin/","/admin/",$menus[$i][menu_link])."' class=''><b>".$navis[0]."</b></a></td><!--".$selectedClass."-->
										</tr>
									</table>
			...
			$mstring .= "<tr height=23 bgcolor='#FFFFFF' style='border-collapse:collapse;'>
							...
							<table cellpadding=0 cellspacing=0 style='margin:0px;'>
								<tr>
									...
									<td><a href='".str_replace("/admin/","/admin/",$menus[$i][menu_link])."' class='".$selectedClass."'>".(count($navis[0]) >= 1 ? $navis[1]:$menu_name)." ".($_SESSION[menu_check][$menus[$i][menu_code]][type]=='icon' ? ($_SESSION[menu_check][$menus[$i][menu_code]][value] ? "<IMG src='".$default_path."/v3/images/btns/new_icon.gif' border=0>":'') : ($_SESSION[menu_check][$menus[$i][menu_code]][type]=='num' ? "(<span class='red str' >".number_format($_SESSION[menu_check][$menus[$i][menu_code]][value])."</span>)":''))."</a></td>
								</tr>
							</table>
							</td>
						</tr>";
			...
			$mstring.= "<tr height=20 bgcolor='#FFFFFF'>
				<td align=left class='leftmenu' style='border-collapse:separate; border:1px solid #c0c0c0;padding:3px 0px 3px 10px;'>
					<table cellpadding=0 cellspacing=0>
						<tr>
							<!--td width=10><IMG id=SM114641I src='".$default_path."/images/icon/dot_orange_triangle.gif' border=0></td-->
							<td><a href='".str_replace("/admin/","/admin/",$menus[$i][menu_link])."' class='menu_style1_a'><b>".$menu_name."</b></a></td>
						</tr>
					</table>
				</td>
				</tr>";
		...
		if($menus[$i][menu_code]  == "1b587c1d3f70fde0f9ab898f6217f5b7" && false){
			$sql = 	"SELECT * FROM shop_promotion_div where disp='1' and leftmenu_view = '1'   ";
			//echo $sql;
			$mdb->query($sql);
			$sub_menus = $mdb->fetchall();
			//print_r($sub_menus);
			if(is_array($sub_menus) ){
				foreach($sub_menus as $key => $sub_menu){
					$mstring .= "<tr height=23 bgcolor='#FFFFFF' style='border-collapse:collapse;'>
							<td align=left class='".$selectedClass."' style='padding:0px 0px 0px 25px;border-collapse:collapse;border:0px;' >
							<table cellpadding=0 cellspacing=0>
								<tr>
									<td width=10><img src='/admin/images/icon/dot_orange.gif' align=absmiddle>  </td>
									<td><a href='/admin/display/promotion_goods.list.php?div_ix=".$sub_menu[div_ix]."' class='".$selectedClass."' ".($sub_menu[div_ix] == $_GET["div_ix"] ? "style='color:#000000;font-weight:bold;'":"style='color:#000000;font-weight:normal'")." >".$sub_menu[div_name]." </a></td>
								</tr>
							</table>
							</td>
						</tr>";
						/*
					$mstring.= "<tr height=20 bgcolor='#FFFFFF'>
						<td align=left  style='border-collapse:separate; border:1px solid #c0c0c0;padding:3px 0px 3px 10px;'>
							<table cellpadding=0 cellspacing=0>
								<tr>
									<td><a href='".str_replace("/admin/","/admin/",$sub_menu[div_ix])."' class='menu_style1_a'><b>".$sub_menu[div_name]."</b></a></td>
								</tr>
							</table>
						</td>
						</tr>";
...
function checkMenuAuth($menu_code, $check_type = "R", $debug= false){
	global $admininfo;
	$mdb = new Database;
	//print_r($admininfo);
//echo nl2br($sql);
//	exit;
	...
	if($mdb->total){
		...
		return false;
		//echo "<script language='javascript'>alert('해당메뉴에 대한 접근권한이 없습니다. ".$check_type." ".$_SERVER["PHP_SELF"]."');history.back();</script>";
		exit;
	}
...
function gnbGenerate($display_type='all'){
	...
	if($admininfo[mall_type] == "B"){// 입점형
		...
		$sql = 	"SELECT * FROM admin_menu_div where disp = 1 and gnb_use_home = 'Y'  order by vieworder asc ";
	}else{
			...
			for($j=0,$add_j=0;$j < count($menus);$j++){
				...
				if(count($navis[1]) >= 1){
					if($depth1_name != $navis[0]){
						...
						$mstring .= "subMenus_".str_replace("/","_",$gnbs[$i][div_name])."[".($j+$add_j)."][0] = \"document.location.href='".str_replace("/admin/","/admin/",$menus[$j][menu_link])."'\";\n";
					...
					$mstring .= "subMenus_".str_replace("/","_",$gnbs[$i][div_name])."[".($j+$add_j)."] = new Array();\n";
					$mstring .= "subMenus_".str_replace("/","_",$gnbs[$i][div_name])."[".($j+$add_j)."][0] = \"document.location.href='".str_replace("/admin/","/admin/",$menus[$j][menu_link])."'\";\n";
					$mstring .= "subMenus_".str_replace("/","_",$gnbs[$i][div_name])."[".($j+$add_j)."][1] = \"".(count($navis[0]) >= 1 ? trim($navis[1]):trim($menu_name)).($_SESSION[menu_check][$menus[$j][menu_code]][type]=='icon' ? ($_SESSION[menu_check][$menus[$j][menu_code]][value] ? " <IMG src='/admin/v3/images/btns/new_icon.gif' border=0>":'') : ($_SESSION[menu_check][$menus[$j][menu_code]][type]=='num' ? ' ('.number_format($_SESSION[menu_check][$menus[$j][menu_code]][value]).')':''))."\";\n";
...
function getTransDiscription($menu_code , $dic_code, $db="", $var_name="coupon_data", $trans_link_bool=true){
	...
	//echo str_replace($admin_root,"/admin",$_SERVER["PHP_SELF"]);
	//$menu_code = md5(str_replace($admin_root,"/admin",$_SERVER["PHP_SELF"]));

	//md5(str_replace($admin_root,"/admin",$_SERVER["PHP_SELF"]))
	$sql = "select * from admin_dic where menu_code = '".$menu_code."' and language_type = '".$admininfo["language"]."' and dic_type = 'DESC' and dic_code = '$dic_code' ";
	//echo $sql."<br><br>";
	$mdb = new Database;
	$mdb->query($sql);
	$mdb->fetch();
...
function tax_receipt($default_path='/admin'){
global $admin_config, $admininfo;
