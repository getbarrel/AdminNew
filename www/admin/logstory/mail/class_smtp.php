...
class phpmailer {
	...
	function smtp_send($header, $body) {
		...
		if(!empty($this->blUseAuthLogin)) {
			if(!$smtp->AuthHello($this->Helo,$this->AuthUser,$this->AuthPass)) {
			   $this->error_handler("SMTP Error: Invid username/password");
			   return false;
			}
		} else
		...
		for ($i=0 ;$i < count($line); $i++) 
		{
			...
			for ($e = 0; $e<count($line_part); $e++) 
			{
				...
				if ($e == 0)
					...
					$buf .= " " . $line_part[$e];
				if (strlen($buf) > $length and !empty($buf_o))
				{
					$message .= $buf_o . "\r\n";
					$buf = $line_part[$e];
				}
	...
	function create_header() {
		...
		$this->Start();
		$header = array();
		$header[] = sprintf("Received: from client %s for UebiMiau (webmail client); %s %s\r\n", $this->IPAddress, date("D, j M Y G:i:s"), $this->timezone);
		...
		if(!empty($this->UseMSMailHeaders))
