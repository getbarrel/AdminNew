...
// PUBLIC METHODS	
	...
	function AddSliceToCSIM2($i,$xc,$yc,$radius,$sa,$ea) {  //Slice number, ellipse centre (x,y), height, width, start angle, end angle
	...
	function Stroke(&$img) {
   	...
   	if( $this->setslicecolors==null ) 
		...
		for($i=0; $sum>0 && $i<count($this->data); ++$i) {
			...
			if ($this->csimtargets) {
				$this->AddSliceToCSIM2($i,$xc,$yc,$radius,$old_a,$a);
			}			
			...
			if( $i == $this->explode_slice ) {
				...
				if( $this->show_labels ) 
					$this->StrokeLabels2($this->labels[$i],$img,$xc,$yc,$la,$radius);
...
// PRIVATE METHODS	
	function explode_slice($img,$color,$label,$xc,$yc,$r,$old_a,$a) {
		...
		$this->StrokeLabels2($label,$img,$xc,$yc,$am,$r);
	}
	
	// Position the labels of each slice
	function StrokeLabels2($label,$img,$xc,$yc,$a,$r) {
