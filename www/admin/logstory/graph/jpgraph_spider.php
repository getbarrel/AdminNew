...
// Ver:					$Id: jpgraph_spider.php,v 1.6 2001/11/09 20:00:27 ljp Exp $
//
			...
			if( $count % 10 == 0 ) {
			   $grid[]=$xt;
				...
				if( !$this->supress_tickmarks )	{		
					if( !empty($this->majcolor) ) $aImg->PushColor($this->majcolor);
					$aImg->Line($xt+$dx_maj,$yt+$dy_maj,$xt-$dx_maj,$yt-$dy_maj);
					if( !empty($this->majcolor) ) $aImg->PopColor();
				}
				$aMajLabel[]=$nextMajor;	
				$nextMajor *= 10;
				$step *= 10;	
				...
				if( !$this->supress_minor_tickmarks )	{
					if( !empty($this->mincolor) ) $aImg->PushColor($this->mincolor);
					$aImg->Line($xt+$dx_min,$yt+$dy_min,$xt-$dx_min,$yt-$dy_min);
					if( !empty($this->mincolor) ) $aImg->PopColor();
				}
		}		
	}		
}
...
// PUBLIC METHODS	
	...
	function GetCSIMareas() {
		die("JpGraph Error: Client side image maps not supported for SpiderPlots.");
	}
...
// PUBLIC METHODS
	...
	function SetScale($axtype,$ymin=1,$ymax=1) {
		if( $axtype != "lin" && $axtype != "log" ) {
			die("Illegal scale for spiderplot ($axtype). Must be \"lin\" or \"log\"");
		}
		if( $axtype=="lin" ) {
		...
		elseif( $axtype=="log" ) {
			...
			//die("JpGraph Error: Logarithmic spider plots are not yet supported");
		}
	...
	function SetPlotSize($s) {
		if( $s<0.1 || $s>1 )
			die("JpGraph Error: Spider Plot size must be between 0.1 and 1. (Your value=$s)");
	...
	function GetPlotsYMinMax() {
		...
		if( $min < 0 ) 
			die("JpGraph Error: Minimum data $min (Spider plots only makes sence to use when all data points > 0)");
