...
DEFINE("TTF_DIR",$_SERVER["DOCUMENT_ROOT"]."/ttf/");
//DEFINE("TTF_DIR","/usr/share/groff/1.17.2/font");
...
// CONSTRUCTOR
	...
	function Graph($aWidth=300,$aHeight=200,$aCachedName="",$aTimeOut=0,$aInline=true) {
		...
		if( !empty($aCachedName) && READ_CACHE && $aInline )
...
// PRIVATE METHODS	
	...
	function GetPlotsYMinMax(&$aPlots) {
		...
		for($i=0; $i<count($aPlots); ++$i ) {
			...
			if (!is_string($ymax) || !empty($ymax)) $max=max($max,$ymax);
			if (!is_string($ymin) || !empty($ymin)) $min=min($min,$ymin);
		}
		if( empty($min) ) $min = 0;
		if( empty($max) ) $max = 0;
		if( $min == 0 && $max == 0 ) {
	...
	function Stroke(&$img,&$scale,$pos) {
		...
		if( $scale->type == "x" ) {
			...
			for( $i=0; $label<=$scale->GetMaxVal()+$this->label_offset; ++$i ) {
				...
				// Apply format
				if( !empty($this->label_formfunc) ) {
					$f=$this->label_formfunc;
					$l = $f($label);
				}	
				elseif( !empty($this->label_formatstr) ) 
				...
				if(!($this->xtick_offset > 0 && $i==$nbrmajticks-1) && !$this->supress_tickmarks) {
					if( !empty($this->majcolor) ) $img->PushColor($this->majcolor);
					$img->Line($xtick,$pos,$xtick,$yu);
					if( !empty($this->majcolor) ) $img->PopColor();
				}
			}
			...
			for( $i=0,$x=$scale->scale_abs[0]; $x<$limit; ++$i ) {
				...
				if( !$this->supress_tickmarks && !$this->supress_minor_tickmarks)	{						
					if( !empty($this->mincolor) ) $img->PushColor($this->mincolor);
					$img->Line($x,$pos,$x,$yu); 
					if( !empty($this->mincolor) ) $img->PopColor();
				}
			}
		}
		elseif( $scale->type == "y" ) {
...
class Image {
	...
	function SetFont($family,$style=FS_NORMAL,$size=10) {
		...
		$this->font_size=$size;
		if( ($this->font_family==FF_FONT1 || $this->font_family==FF_FONT2) && $this->font_style==FS_BOLD  && $this->font_style==FF_GULIM ){ // 요기수정했음.
				++$this->font_family;
		}
	...
	function StrokeText($x,$y,$txt,$dir=0) {
		...
		elseif($this->font_family >= FF_COURIER && $this->font_family <= FF_GULIM)  { // TTF font  //elseif($this->font_family >= FF_COURIER && $this->font_family <= FF_BOOK)  { // TTF font
...
class Legend {
	...
	var $font_family=FF_FONT1 ;
	var $font_style=FS_NORMAL,$font_size=12;
