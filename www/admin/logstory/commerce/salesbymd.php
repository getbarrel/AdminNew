...
function ReportTable($vdate,$SelectReport=1){
    ...
    if ($SelectReport == 1){
        if($depth == "" || $depth < 0){
            ...
            $sql = "Select IFNULL(cmd.code,'0') as code,  IFNULL(AES_DECRYPT(UNHEX(cmd.name),'".$fordb->ase_encrypt_key."'),'기타') as name, 
							...
							left join common_member_detail cmd on od.md_code = cmd.code
        ...
        if($depth == '' || $depth < 0 ){
            ...
            //AND od.status NOT IN ('".implode("','",$non_sale_status)."')
            //and substr(k.code,1,".(($depth)*3).") = substr(b.vreferer_id,1,3)
    ...
    if(isset($_GET["mode"]) && $_GET["mode"] == "excel"){
        ...
        $sheet->getActiveSheet(0)->setCellValue('A2', "MD 별 매출 분석");
        ...
        for($i=0;$i<$fordb->total;$i++){
            ...
            $sheet->getActiveSheet()->setCellValue('B' . ($i + $start + 4), $fordb->dt['name']);
        ...
        //$sheet->getActiveSheet()->setCellValue('B' . ($i + $start + 4), getIventoryCategoryPathByAdmin($goods_infos[$i]['code'], 4));
        ...
        //header('Content-Disposition: attachment;filename="'.iconv("utf-8","cp949","MD 별 매출 분석_매출상세분석.xls").'"');
        //header('Cache-Control: max-age=0');

		$filename = "MD별매출분석_매출상세분석";
		...
		$sheet->getActiveSheet()->setTitle('MD별매출분석');
		$sheet->setActiveSheetIndex(0);
    ...
    if(isset($_GET["mode"]) && ($_GET["mode"] == "pop" || $_GET["mode"] == "print")){
        $mstring .= "<tr><td >".TitleBar("MD 별 매출 분석 : ".($code ? "-".getCategoryPath($code,4):""),$dateString,false, $exce_down_str)."</td></tr>";
    }else{
        $mstring .= "<tr><td >".TitleBar("MD 별 매출 분석 : ".($code ? "-".getCategoryPath($code,4):""),$dateString,true, $exce_down_str)."</td></tr>";
    }
    $mstring .= "
						<tr height=50>
							<td >
    ...
    $mstring .= "
		<tr height=30>
			...
			<td class=m_td rowspan=3>MD명</td>
    ...
    for($i=0;$i<$fordb->total;$i++){
        ...
        if($fordb->dt["name"] == ""){
            $mstring .= "-";
        }else if(substr($fordb->dt["code"],0,6) == "000001"){
            $mstring .= "".$fordb->dt['name']."";
        }else{
            $mstring .= $fordb->dt['name'];
        }
        $mstring .= "
    ...
    */
    ...
    $mstring .= HelpBox("MD 별 매출 분석", $help_text);
    return $mstring;
}
...
//	echo "<Script>parent.document.getElementById('calendararea').innerHTML = document.calendarfrm.calendarvalue.value;parent.ChangeCalenderView($SelectReport);</Script>";
    ...
    $p->forbizLeftMenu = commerce_munu('salesbymd.php', "<div id=TREE_BAR style=\"margin:5px;\">".GetTreeNode('salesbymd.php',date("Ymd", time()),'product')."</div>");
    if($report_type == 2){
...
function ReportTable2($vdate,$SelectReport=1){
    ...
    if ($SelectReport == 1){
        if($depth == 0){
            $sql = "select data.cid, data.code, IF(data.code = 0 , '기타',AES_DECRYPT(UNHEX(cmd.name),'".$fordb->ase_encrypt_key."')) as name, sum(ncnt) as nview_cnt, sum(pcnt) as pcnt, sum(ptprice) as ptprice, sum(order_detail_cnt) as order_detail_cnt from 

						( 
						select od.cid, od.md_code as code, 0 ncnt , sum(od.pcnt) as pcnt, sum(od.pt_dcprice) as ptprice, count(*) as order_detail_cnt  
						from shop_order_detail od 
						where od.regdate  between '".$selected_date." 00:00:00' and '".$selected_date." 23:59:59'  
						and od.status NOT IN ('".implode("','",$non_sale_status)."')  ";
            ...
            $sql .= " 
						group by code
						) data left join common_member_detail cmd on cmd.code = data.code
						where (data.code != 0 and AES_DECRYPT(UNHEX(cmd.name),'".$fordb->ase_encrypt_key."') != '')  or data.code = 0
						  	";
            //where date_format(od.regdate,'%Y%m%d') = '".$vdate."'
            $sql .= "group by data.code";
            $sql .= $orderbyString;
            ...
            $sql .= " 
						and substr(od.cid,1,".(($depth)*3).") = '".substr($cid,0,(($depth)*3))."' 
						group by code
						) data left join common_member_detail cmd on cmd.code = data.codeand AES_DECRYPT(UNHEX(cmd.name),'".$fordb->ase_encrypt_key."') != ''
						where substr(data.cid,1,".(($depth)*3).") = '".substr($cid,0,(($depth)*3))."' 
						and (data.code != 0 and AES_DECRYPT(UNHEX(cmd.name),'".$fordb->ase_encrypt_key."') != '')  or data.code = 0 ";
            //where date_format(od.regdate,'%Y%m%d') = '".$vdate."'
            $sql .= "group by data.code";
            $sql .= $orderbyString;
        }
        ...
        if($depth == 0){
            ...
            $sql .= " 
						...
						where substr(data.cid,1,".(($depth)*3).") = '".substr($cid,0,(($depth)*3))."' 
						and (data.code != 0 and AES_DECRYPT(UNHEX(cmd.name),'".$fordb->ase_encrypt_key."') != '')  or data.code = 0
						and AES_DECRYPT(UNHEX(cmd.name),'".$fordb->ase_encrypt_key."') != '' ";
            //where date_format(od.regdate,'%Y%m') = '".substr($vdate,0,6)."'
            $sql .= "group by data.code";
            $sql .= $orderbyString;
        }
    ...
    if(isset($_GET["mode"]) && $_GET["mode"] == "excel"){
        ...
        $sheet->getActiveSheet(0)->setCellValue('A2', "MD 별 매출 분석 - 구매전환분석");
        ...
        $sheet->getActiveSheet(0)->setCellValue('A' . ($i+1), "순");
        $sheet->getActiveSheet(0)->setCellValue('B' . ($i+1), "MD명");
        ...
        for($i=0;$i<$fordb->total;$i++){
            ...
            $sheet->getActiveSheet()->setCellValue('G' . ($i + $start + 3), (!empty($fordb->dt['nview_cnt'])?$fordb->dt['order_detail_cnt']/$fordb->dt['nview_cnt']:0)); // 엑셀 포맷 정의시 자동으로 *100 이 처리됨
            ...
            $order_change_rate_sum += (!empty($fordb->dt['nview_cnt'])?$fordb->dt['order_detail_cnt']/$fordb->dt['nview_cnt']:0);
        ...
        //$sheet->getActiveSheet()->setCellValue('B' . ($i + $start + 3), getIventoryCategoryPathByAdmin($goods_infos[$i]['code'], 4));
        ...
        //header('Content-Type: application/vnd.ms-excel');
        //header('Content-Disposition: attachment;filename="'.iconv("utf-8","cp949","MD 별 매출 분석_구매전환분석.xls").'"');
        //header('Cache-Control: max-age=0');

		$filename = "MD별매출분석_구매전환분석";
    ...
    //$mstring = $mstring.TitleBar("MD 별 매출 분석 : ".($code ? getRefererCategoryPath($code,4):""),$dateString);
    $mstring = "<table width='100%' border=0>";
    if(isset($_GET["mode"]) && ($_GET["mode"] == "pop" || $_GET["mode"] == "print")){
        $mstring .= "<tr><td >".TitleBar("MD 별 매출 분석 : ",$dateString."".($code ? "-".getCategoryPath($code,4):""),false, $exce_down_str)."</td></tr>";
    }else{
        $mstring .= "<tr><td >".TitleBar("MD 별 매출 분석 : ",$dateString."".($code ? "-".getCategoryPath($code,4):""),true, $exce_down_str)."</td></tr>";
    }
    ...
    $mstring .= "<table cellpadding=3 cellspacing=0 width=100%  ID='MaxViewProductTable' class='list_table_box'>
						...
						<col width='10%'>
						<col width='8%'>";
    $mstring .= "<tr height=30 align=center><td class=s_td rowspan=2>순</td><td class=m_td rowspan=2>MD명</td><!--td class=m_td colspan=2>이벤트/기획전 조회정보</td--><td class=e_td colspan=6>상품판매정보</td></tr>\n";
    ...
    for($i=0;$i<$fordb->total;$i++){
        ...
        $mstring .= "<tr height=30 bgcolor=#ffffff  id='Report$i'>
		<td class='list_box_td list_bg_gray' onmouseover=\"mouseOnTD('".$i."',true)\" onmouseout=\"mouseOnTD('$i',false)\">".($i+1)."</td>
		<!--td class='list_box_td point' style='text-align:left;' onmouseover=\"mouseOnTD('$i',true)\" onmouseout=\"mouseOnTD('$i',false)\" title='".$fordb->dt['code']."'>".$fordb->dt['name']."</td>
...
function ReportTable3($vdate,$SelectReport=1){
    ...
    if ($SelectReport == 1){
        ...
        if($depth == '' || $depth < 0 ){
            ...
            $depth = $depth -1;
            $sql = "Select IFNULL(cmd.code,'0') as code,  AES_DECRYPT(UNHEX(cmd.name),'".$fordb->ase_encrypt_key."')  as name, 
							sum(case when od.status NOT IN ('".implode("','",$non_sale_status)."')  then 1 else 0 end) as order_cnt, 
							sum(case when (o.payment_agent_type = 'W' and od.status NOT IN ('".implode("','",$non_sale_status)."'))  then 1 else 0 end) as order_cnt_web, 
							sum(case when (o.payment_agent_type = 'M' and od.status NOT IN ('".implode("','",$non_sale_status)."'))  then 1 else 0 end) as order_cnt_mobile, 
							sum(case when od.status NOT IN ('".implode("','",$non_sale_status)."')  then od.pcnt else 0 end) as order_sale_cnt, 
    ...
    if(isset($_GET["mode"]) && $_GET["mode"] == "excel"){
        ...
        //header('Content-Type: application/vnd.ms-excel');
        //header('Content-Disposition: attachment;filename="'.iconv("utf-8","cp949","MD 별 매출 분석_매출요약분석.xls").'"');
        //header('Cache-Control: max-age=0');

		$filename = "MD별매출분석_매출요약분석";
    ...
    $mstring = "<table width='100%' border=0>";
    if(isset($_GET["mode"]) && ($_GET["mode"] == "pop" || $_GET["mode"] == "print")){
        $mstring .= "<tr><td >".TitleBar("MD 별 매출 분석 : ",$dateString."".($cid ? "-".getCategoryPath($cid,4):""),false, $exce_down_str)."</td></tr>";
    }else{
        $mstring .= "<tr><td >".TitleBar("MD 별 매출 분석 : ",$dateString."".($cid ? "-".getCategoryPath($cid,4):""),true, $exce_down_str)."</td></tr>";
    }
    $mstring .= " 
						<tr height=50>
							<td >
    ...
    $mstring .= "
		<tr height=30>
			<td class=s_td rowspan=3>순</td>
			<td class=m_td rowspan=3>".OrderByLink("MD명", "code", $ordertype)."</td>
			<td class=m_td colspan=2>주문매출</td>
