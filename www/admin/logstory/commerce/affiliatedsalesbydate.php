...
if ($mode == "iframe"){
    ...
    $ca->LinkPage = 'affiliatedsalesbydate.php';
    ...
    $p->forbizLeftMenu = commerce_munu('affiliatedsalesbydate.php');//.text_button("#", "test","190").colorCirCleBoxStart("#efefef",190)."test<br>test<br>test<br>test<br>test<br>".colorCirCleBoxEnd("#efefef");
    $p->forbizContents = ReportTable($vdate,$SelectReport);
    $p->Navigation = "이커머스분석 > 매출종합분석 > 제휴 매출분석(일자별)";
    $p->ContentsWidth = "97%";
    $p->title = "제휴 매출분석(일자별)";
    $p->PrintReportPage();
}
...
function ReportTable2($vdate,$SelectReport=1){
    ...
    $mstring = $mstring.TitleBar("제휴 매출분석(일자별)",$dateString);
    if($SelectReport == 1){
        $fordb->query($sql);
        $fordb2->query($sql2);
        $fordb3->query($sql3);
...
//	$mstring .= "</table>\n";
    ...
    $mstring .= HelpBox("제휴 매출분석(일자별)", $help_text);
    return $mstring;
}
