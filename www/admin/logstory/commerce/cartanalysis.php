...
//	echo "<Script>parent.document.getElementById('calendararea').innerHTML = document.calendarfrm.calendarvalue.value;parent.ChangeCalenderView($SelectReport);</Script>";
    ...
    $P->addScript = $Script;
    $P->Navigation = "이커머스분석 > 상품별 종합분석 > 실시간 장바구니분석";

    $P->title = "실시간 장바구니분석";
    //$P->strContents = ReportTable($vdate,$SelectReport);

    $P->NaviTitle = "실시간 장바구니분석 - 구매전환분석";
...
function ReportTable2($vdate,$SelectReport=1){
    ...
    if($SelectReport == "4"){
        $vdate = date("Ymd", time());
        $vweekenddate = date("Y-m-d", time());
        $vweekenddate2 = date("Ymd", time());
    }
    ...
    if($vdate == ""){
        ...
        $vweekenddate2 = date("Ymd",mktime(0,0,0,substr($vdate,4,2),substr($vdate,6,2),substr($vdate,0,4))+60*60*24*6);
        $vyesterday = date("Ymd",mktime(0,0,0,substr($vdate,4,2),substr($vdate,6,2),substr($vdate,0,4))-60*60*24);
        $voneweekago = date("Ymd",mktime(0,0,0,substr($vdate,4,2),substr($vdate,6,2),substr($vdate,0,4))-60*60*24*7);
    }

    if(isset($_GET["orderby"]) && $_GET["orderby"] != "" && isset($_GET["ordertype"]) && $_GET["ordertype"] != ""){
        $orderbyString = " order by ".$_GET["orderby"]." ".$_GET["ordertype"].", cart_total_cnt desc, realtime_cart_pcnt desc  , order_goods_cnt desc ";
    }else{
        $orderbyString = " order by cart_total_cnt desc, realtime_cart_pcnt desc , order_goods_cnt desc ";
    }
    ...
    if ($SelectReport == 1){
        if($depth == 0){
            $sql = "select data.cid, data.pid, p.pname as pname, sum(nview_cnt) as nview_cnt, sum(order_goods_cnt) as order_goods_cnt, sum(realtime_cart_pcnt) as realtime_cart_pcnt, sum(cart_total_cnt) as cart_total_cnt, sum(ptprice) as ptprice,
						case when sum(cart_total_cnt) = 0 then 0 else sum(order_goods_cnt)/sum(cart_total_cnt)*100 end as cart_change_rate
						from
						(Select LPAD(b.pid,10,'0') as pid, b.cid as cid, sum(b.nview_cnt) as nview_cnt , 0 as order_goods_cnt, 0 as realtime_cart_pcnt , 0 cart_total_cnt , 0 as ptprice
						from ".TBL_COMMERCE_VIEWINGVIEW." b
						where b.vdate = '".$vdate."'
						group by pid
						union
						select LPAD(od.pid,10,'0') as pid, od.cid as cid, 0 as nview_cnt , count(*) as order_goods_cnt,  0 as realtime_cart_pcnt,  0 as cart_total_cnt , sum(od.ptprice) as ptprice
            ...
            $sql .= " group by pid
						union
						select LPAD(c.id,10,'0') as pid, c.cid as cid, 0 nview_cnt ,0 as order_goods_cnt,  count(*) as realtime_cart_pcnt,  0 as cart_total_cnt   , 0 as ptprice
						from shop_cart c

						group by pid
						union
						Select LPAD(ss.pid,10,'0') as pid, ss.cid as cid, 0 as nview_cnt , 0 as order_goods_cnt, 0 as realtime_cart_pcnt , sum(step1) as cart_total_cnt , 0 as ptprice
        ...
        if($depth == 0){
            ...
            $sql = "select data.cid, data.pid, p.pname as pname, sum(nview_cnt) as nview_cnt, sum(order_goods_cnt) as order_goods_cnt, sum(realtime_cart_pcnt) as realtime_cart_pcnt, sum(cart_total_cnt) as cart_total_cnt, sum(ptprice) as ptprice,
						...
						and od.status NOT IN ('".implode("','",$non_sale_status)."') ";
            //between '".$vdate."' and '".$vweekenddate2."'
            ...
            $sql .= " group by pid
						...
						";
            //ss.vdate between '".$vdate."' and '".$vweekenddate2."' and step1 = '1'
            ...
            $sql = "select data.cid, data.pid, p.pname as pname, sum(nview_cnt) as nview_cnt, sum(order_goods_cnt) as order_goods_cnt, sum(realtime_cart_pcnt) as realtime_cart_pcnt, sum(cart_total_cnt) as cart_total_cnt, sum(ptprice) as ptprice,
						...
						where od.regdate between '".$selected_date." 00:00:00' and '".$vweekenddate." 23:59:59'
						and od.status NOT IN ('".implode("','",$non_sale_status)."') ";
            //b.vdate between '".$vdate."' and '".$vweekenddate2."'
            ...
            $sql .= " group by pid
						...
						where   substr(data.cid,1,".(($depth)*3).") = '".substr($cid,0,(($depth)*3))."'
						";
            //between '".$vdate."' and '".$vweekenddate2."' and step1 = '1'
            //where date_format(od.regdate,'%Y%m%d') between '".$vdate."' and '".$vweekenddate."'
    ...
    if(isset($_GET["mode"]) && $_GET["mode"] == "excel"){
        ...
        $sheet->getActiveSheet(0)->setCellValue('A2', "실시간 장바구니분석 - 구매전환분석");
        ...
        header('Content-Disposition: attachment;filename="'.iconv("utf-8","cp949","실시간 장바구니분석_구매전환분석.xls").'"');
    ...
    //$mstring = $mstring.TitleBar("실시간 장바구니분석 : ".($cid ? getCategoryPath($cid,4):""),$dateString);
    $mstring = "<table width='100%' border=0>";
    if(isset($_GET["mode"]) && ($_GET["mode"] == "pop" || $_GET["mode"] == "print")){
        //$mstring .= "<tr><td colspan=2>".TitleBar("실시간 장바구니분석 : ",$dateString."".($cid ? "-".getCategoryPath($cid,4):""),false, $exce_down_str)."</td></tr>";
    }else{
        //$mstring .= "<tr><td colspan=2>".TitleBar("실시간 장바구니분석 : ",$dateString."".($cid ? "-".getCategoryPath($cid,4):""),true, $exce_down_str)."</td></tr>";
    }
    $mstring .= "<tr>
    ...
    $mstring .= "<tr height=30 align=center>
						...
						<td class=m_td colspan=2>장바구니 정보</td>
						<td class=m_td rowspan=2>".OrderByLink("실시간<br>장바구니수", "realtime_cart_pcnt", $ordertype)."</td>
    ...
    for($i=0;$i<$fordb->total;$i++){
		...
		<td class='list_box_td  number' onmouseover=\"mouseOnTD('$i',true)\" onmouseout=\"mouseOnTD('$i',false)\">".number_format(returnZeroValue($fordb->dt['realtime_cart_pcnt']),0)."&nbsp;</td>
    ...
    */
    ...
    $mstring .= HelpBox("실시간 장바구니분석", $help_text);
    return $mstring;
}
?>
