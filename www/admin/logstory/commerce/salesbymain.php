...
function ReportTable($vdate,$SelectReport=1){
    ...
    global $mg_ix, $cid, $depth, $non_sale_status, $order_status, $cancel_status, $return_status, $all_sale_status;
    ...
    //$mg_ix = $referer_id; // 기여사이트 코드와 충돌 되서 변경함
    ...
    if ($SelectReport == 1){
        if($depth == "" || $depth < 0){
            ...
            $sql = "Select IFNULL(mpr.mg_ix,'0') as mg_ix,  IFNULL(mg.mg_title,'기타') as mg_title, 
							...
							left join shop_main_product_relation mpr on od.mpr_ix = mpr.mpr_ix
							left join shop_main_goods mg on mpr.mg_ix = mg.mg_ix
        ...
        if($depth == '' || $depth < 0 ){
            ...
            //AND od.status NOT IN ('".implode("','",$non_sale_status)."')
            //and substr(k.mg_ix,1,".(($depth)*3).") = substr(b.vreferer_id,1,3)
    ...
    $dateString .= " (".($mg_ix ? getCategoryPath($mg_ix,4):"전체").")";
    ...
    if(isset($_GET["mode"]) && $_GET["mode"] == "excel"){
        ...
        $sheet->getActiveSheet(0)->setCellValue('A2', "메인페이지 기여 매출 분석");
        ...
        for($i=0;$i<$fordb->total;$i++){
            ...
            $sheet->getActiveSheet()->setCellValue('B' . ($i + $start + 4), $fordb->dt['mg_title']);
        ...
        //$sheet->getActiveSheet()->setCellValue('B' . ($i + $start + 4), getIventoryCategoryPathByAdmin($goods_infos[$i]['mg_ix'], 4));
        ...
        header('Content-Disposition: attachment;filename="'.iconv("utf-8","cp949","메인페이지 기여 매출 분석_매출상세분석.xls").'"');
    ...
    if(isset($_GET["mode"]) && ($_GET["mode"] == "pop" || $_GET["mode"] == "print")){
        $mstring .= "<tr><td >".TitleBar("메인페이지 기여 매출 분석 : ".($mg_ix ? "-".getCategoryPath($mg_ix,4):""),$dateString,false, $exce_down_str)."</td></tr>";
    }else{
        $mstring .= "<tr><td >".TitleBar("메인페이지 기여 매출 분석 : ".($mg_ix ? "-".getCategoryPath($mg_ix,4):""),$dateString,true, $exce_down_str)."</td></tr>";
    }
    $mstring .= "
						<tr height=50>
							<td >
    ...
    $mstring .= "
		<tr height=30>
			...
			<td class=m_td rowspan=3>프로모션명</td>
    ...
    for($i=0;$i<$fordb->total;$i++){
        ...
        if($fordb->dt["mg_title"] == ""){
            $mstring .= "-";
        }else if(substr($fordb->dt["mg_ix"],0,6) == "000001"){
            $mstring .= "".$fordb->dt['mg_title']."";
        }else{
            $mstring .= $fordb->dt['mg_title'];
        }
        $mstring .= "
    ...
    */
    ...
    $mstring .= HelpBox("메인페이지 기여 매출 분석", $help_text);
    return $mstring;
}
...
//	echo "<Script>parent.document.getElementById('calendararea').innerHTML = document.calendarfrm.calendarvalue.value;parent.ChangeCalenderView($SelectReport);</Script>";
    ...
    $P->addScript = $script;
    $P->Navigation = "이커머스분석 > 상품별 종합분석 > 메인페이지 기여 매출 분석";

    $P->title = "메인페이지 기여 매출 분석";
    //$P->strContents = ReportTable($vdate,$SelectReport);
    if($report_type == 2){
        $P->NaviTitle = "메인페이지 기여 매출 분석 - 구매전환 분석";
        $P->strContents = ReportTable2($vdate,$SelectReport);
    }else if($report_type == 3){
        $P->NaviTitle = "메인페이지 기여 매출 분석 - 매출요약 분석";
        $P->strContents = ReportTable3($vdate,$SelectReport);
    }else{
        $P->NaviTitle = "메인페이지 기여 매출 분석 - 매출상세분석";
        $P->strContents = ReportTable($vdate,$SelectReport);
    }
    $P->OnloadFunction = "";
    ...
    $p->forbizLeftMenu = commerce_munu('salesbymain.php', "<div id=TREE_BAR style=\"margin:5px;\">".GetTreeNode('salesbymain.php',date("Ymd", time()),'product')."</div>");
    if($report_type == 2){
...
function ReportTable2($vdate,$SelectReport=1){
    ...
    if ($SelectReport == 1){
        if($depth == 0){
            $sql = "select data.cid, data.mpr_ix, IF(data.mpr_ix = 0 , '기타',mg.mg_title) as mg_title, sum(ncnt) as nview_cnt, sum(pcnt) as pcnt, sum(ptprice) as ptprice, sum(order_detail_cnt) as order_detail_cnt from 
						(Select '' as cid,b.mpr_ix as mpr_ix, sum(b.ncnt) as ncnt , 0 pcnt , 0 ptprice , 0 order_detail_cnt
						from logstory_maingoods_click b 						
						where b.vdate = '".$vdate."' 			
						group by mpr_ix
						union 
						select od.cid, od.mpr_ix as mpr_ix, 0 ncnt , sum(od.pcnt) as pcnt, sum(od.pt_dcprice) as ptprice, count(*) as order_detail_cnt  
						from shop_order_detail od 
						where od.regdate  between '".$selected_date." 00:00:00' and '".$selected_date." 23:59:59'  
						and od.status NOT IN ('".implode("','",$non_sale_status)."') ";
            ...
            $sql .= " group by mpr_ix
						...
						left join shop_main_product_relation mpr on data.mpr_ix = mpr.mpr_ix  
						left join shop_main_goods mg on mpr.mg_ix = mg.mg_ix
						where (data.mpr_ix != 0 and mg.mg_title != '')  or data.mpr_ix = 0
						  	";
            //where date_format(od.regdate,'%Y%m%d') = '".$vdate."'
            $sql .= "group by mpr.mg_ix";
            $sql .= $orderbyString;
            ...
            $sql .= " and substr(od.cid,1,".(($depth)*3).") = '".substr($cid,0,(($depth)*3))."' 
						...
						and (data.mpr_ix != 0 and mg.mg_title != '')  or data.mpr_ix = 0 ";
            //where date_format(od.regdate,'%Y%m%d') = '".$vdate."'
            $sql .= "group by mpr.mg_ix";
            $sql .= $orderbyString;
        }
    ...
    if(isset($_GET["mode"]) && $_GET["mode"] == "excel"){
        ...
        $sheet->getActiveSheet(0)->setCellValue('A2', "메인페이지 기여 매출 분석 - 구매전환분석");
        ...
        $sheet->getActiveSheet(0)->setCellValue('A' . ($i+1), "순");
        $sheet->getActiveSheet(0)->setCellValue('B' . ($i+1), "프로모션명");
        ...
        for($i=0;$i<$fordb->total;$i++){
            ...
            if($fordb->dt['nview_cnt'] > 0){
                $order_cnt_rate = $fordb->dt['order_detail_cnt']/$fordb->dt['nview_cnt'];
            }else{
                $order_cnt_rate = 0;
            }

            $sheet->getActiveSheet()->setCellValue('A' . ($i + $start + 3), ($i + 1));
            $sheet->getActiveSheet()->setCellValue('B' . ($i + $start + 3), $fordb->dt['mg_title']);
            ...
            $sheet->getActiveSheet()->setCellValue('G' . ($i + $start + 3), $order_cnt_rate); // 엑셀 포맷 정의시 자동으로 *100 이 처리됨
            ...
            $order_change_rate_sum += $order_cnt_rate*100;
        ...
        //$sheet->getActiveSheet()->setCellValue('B' . ($i + $start + 3), getIventoryCategoryPathByAdmin($goods_infos[$i]['mpr_ix'], 4));
        ...
        header('Content-Type: application/vnd.ms-excel');
        header('Content-Disposition: attachment;filename="'.iconv("utf-8","cp949","메인페이지 기여 매출 분석_구매전환분석.xls").'"');
        header('Cache-Control: max-age=0');
    ...
    $exce_down_str = "<a href='?mode=excel&".str_replace("&mode=iframe", "",$_SERVER["QUERY_STRING"])."'><img src='../../images/".$_SESSION["admininfo"]["language"]."/btn_excel_save.gif' border=0></a>";
    //$mstring = $mstring.TitleBar("메인페이지 기여 매출 분석 : ".($mpr_ix ? getRefererCategoryPath($mpr_ix,4):""),$dateString);
    $mstring = "<table width='100%' border=0>";

    if(isset($_GET["mode"]) && ($_GET["mode"] == "pop" || $_GET["mode"] == "print")){
        $mstring .= "<tr><td >".TitleBar("메인페이지 기여 매출 분석 : ",$dateString."".($mpr_ix ? "-".getCategoryPath($mpr_ix,4):""),false, $exce_down_str)."</td></tr>";
    }else{
        $mstring .= "<tr><td >".TitleBar("메인페이지 기여 매출 분석 : ",$dateString."".($mpr_ix ? "-".getCategoryPath($mpr_ix,4):""),true, $exce_down_str)."</td></tr>";
    }
    ...
    $mstring .= "<table cellpadding=3 cellspacing=0 width=100%  ID='MaxViewProductTable' class='list_table_box'>
						...
						<col width='10%'>
						<col width='8%'>";
    $mstring .= "<tr height=30 align=center><td class=s_td rowspan=2>순</td><td class=m_td rowspan=2>프로모션명</td><td class=m_td colspan=2>이벤트/기획전 조회정보</td><td class=e_td colspan=6>상품판매정보</td></tr>\n";
    ...
    for($i=0;$i<$fordb->total;$i++){
        ...
        $mstring .= "<tr height=30 bgcolor=#ffffff  id='Report$i'>
		<td class='list_box_td list_bg_gray' onmouseover=\"mouseOnTD('".$i."',true)\" onmouseout=\"mouseOnTD('$i',false)\">".($i+1)."</td>
		<td class='list_box_td point' style='text-align:left;' onmouseover=\"mouseOnTD('$i',true)\" onmouseout=\"mouseOnTD('$i',false)\" title='".$fordb->dt['mpr_ix']."'>".$fordb->dt['mg_title']."</td>
...
function ReportTable3($vdate,$SelectReport=1){
    ...
    if ($SelectReport == 1){
        if($depth == "" || $depth < 0){
            ...
            $sql = "Select IFNULL(mpr.mpr_ix,'0') as mpr_ix,  IFNULL(mg.mg_title,'기타') as mg_title, 
    ...
    if(isset($_GET["mode"]) && $_GET["mode"] == "excel"){
        ...
        header('Content-Type: application/vnd.ms-excel');
        header('Content-Disposition: attachment;filename="'.iconv("utf-8","cp949","메인페이지 기여 매출 분석_매출요약분석.xls").'"');
        header('Cache-Control: max-age=0');
    ...
    $mstring = "<table width='100%' border=0>";
    if(isset($_GET["mode"]) && ($_GET["mode"] == "pop" || $_GET["mode"] == "print")){
        $mstring .= "<tr><td >".TitleBar("메인페이지 기여 매출 분석 : ",$dateString."".($cid ? "-".getCategoryPath($cid,4):""),false, $exce_down_str)."</td></tr>";
    }else{
        $mstring .= "<tr><td >".TitleBar("메인페이지 기여 매출 분석 : ",$dateString."".($cid ? "-".getCategoryPath($cid,4):""),true, $exce_down_str)."</td></tr>";
    }
    $mstring .= " 
						<tr height=50>
							<td >
    ...
    $mstring .= "
		<tr height=30>
			<td class=s_td rowspan=3>순</td>
			<td class=m_td rowspan=3>".OrderByLink("프로모션명", "mpr_ix", $ordertype)."</td>
			<td class=m_td colspan=2>주문매출</td>
    ...
    for($i=0;$i<$fordb->total;$i++){
        ...
        $mstring .= "<tr height=30 bgcolor=#ffffff  id='Report$i'>
		<td class='list_box_td list_bg_gray' onmouseover=\"mouseOnTD('".$i."',true)\" onmouseout=\"mouseOnTD('$i',false)\">".($i+1)."</td>
		<td class='list_box_td point' style='text-align:left;' onmouseover=\"mouseOnTD('$i',true)\" onmouseout=\"mouseOnTD('$i',false)\">".$fordb->dt['mg_title'];
        $mstring .= "</td>
