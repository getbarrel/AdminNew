...
function ReportTable($vdate,$SelectReport=1){
    ...
    if($SelectReport == 1){
        ...
        $sql = "Select a.cid, c.pname ,  c.sellprice as vprice, IFNULL(cs.vquantity,0) as order_cnt , IFNULL(cs.vprice,0) as order_price, sum(a.nview_cnt) as nview_cnt
			from ".TBL_COMMERCE_VIEWINGVIEW." a, ".TBL_SHOP_PRODUCT." c 
				join (Select a.cid, sum(a.vquantity) as vquantity , a.vprice, a.pid
				from ".TBL_COMMERCE_SALESTACK." a
				where vdate = '$vdate' and step6 = 1 group by a.pid, a.cid, a.vprice
				order by vquantity desc) cs	on c.id = cs.pid
			where a.pid = c.id and vdate = '$vdate'
			group by a.pid, a.cid, c.pname, c.sellprice ,cs.vquantity , cs.vprice
        ...
        $sql = "Select a.cid, c.pname ,  c.sellprice as vprice, IFNULL(cs.vquantity,0) as order_cnt , IFNULL(cs.vprice,0) as order_price, sum(a.nview_cnt) as nview_cnt
				from ".TBL_COMMERCE_VIEWINGVIEW." a, ".TBL_SHOP_PRODUCT." c  join
					...
					from ".TBL_COMMERCE_SALESTACK." a
					where vdate between '$vdate' and '$vweekenddate' and step6 = 1
					group by a.cid , a.vprice, a.pid
					order by vquantity desc) cs	on c.id = cs.pid
				where a.pid = c.id and vdate between '$vdate' and '$vweekenddate'
				group by a.cid, c.pname , c.sellprice, cs.vquantity, cs.vprice
				order by nview_cnt desc limit 0,50";
        ...
        $sql = "Select a.cid, c.pname,  IFNULL(c.sellprice,0) as vprice, IFNULL(cs.vquantity,0) as order_cnt , IFNULL(cs.vprice,0) as order_price, sum(a.nview_cnt) as nview_cnt
				from ".TBL_COMMERCE_VIEWINGVIEW." a, ".TBL_SHOP_PRODUCT." c
				 join
					(
					Select a.cid, a.vprice, a.pid, sum(a.vquantity) as vquantity
					from ".TBL_COMMERCE_SALESTACK." a
					where vdate LIKE '".substr($vdate,0,6)."%'  and step6 = 1 group by a.cid, a.vprice, a.pid
    ...
    $mstring = TitleBar("최다조회상품",$dateString);
...
*/
    ...
    $mstring = $mstring."<tr height=30 align=center><td width=50 class=s_td width=30 colspan=2>합계</td><td class=m_td >&nbsp;".$nview_cnt."</td><td class=m_td >".number_format($vprice)."</td><td class=m_td >&nbsp;".number_format($order_cnt)."</td><td class=m_td >".number_format($order_price)."</td><td class=e_td >".number_format($vsale,0)."</td></tr>\n";
    ...
    $mstring .= HelpBox("최다조회상품", $help_text);
...
if ($mode == "iframe"){
    ...
    $p->Navigation = "이커머스분석 > 상품분석 > 최다조회상품";
    $p->title = "최다조회상품";
    $p->PrintReportPage();
}
