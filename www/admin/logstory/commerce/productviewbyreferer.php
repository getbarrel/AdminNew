...
function ReportTable($vdate,$SelectReport=1){
    ...
    global $depth,$referer_id, $cid, $depth, $non_sale_status, $order_status, $cancel_status, $return_status, $all_sale_status;
    ...
    //$cid = $referer_id; // 기여사이트 코드와 충돌 되서 변경함
    //echo $cid."::::".$depth;
    if($cid == "" || $depth == ""){
        $depth = -1;
    }
    ...
    if($depth == "" && $cid == "000000000000000"){
        $depth = 0;
    }else{
        $depth = $depth+1;
    }
    if($depth > 0 && $cid == "000000000000000"){
        $depth = -1;
    }
    ...
    if(isset($_GET["orderby"]) && $_GET["orderby"] != "" && isset($_GET["ordertype"]) && $_GET["ordertype"] != ""){
        $orderbyString = " order by ".$_GET["orderby"]." ".$_GET["ordertype"].", order_sale_cnt desc ";
    }else{
    ...
    $coprice_str = "case when od.commission > 0 then od.pt_dcprice*(100-od.commission)/100 else od.coprice*od.pcnt end ";
    if ($SelectReport == 1){
        if($depth == "" || $depth < 0){

            $sql = "Select IFNULL(c.cid,'9') as cid, c.depth, IFNULL(c.cname,'직접방문') as cname, 
							sum(case when od.status NOT IN ('".implode("','",$non_sale_status)."')  then od.pcnt else 0 end) as order_sale_cnt, 
							count(distinct od.oid) as buyer_cnt,
							sum(case when od.status NOT IN ('".implode("','",$non_sale_status)."')  then od.pt_dcprice else 0 end) as order_sale_sum, 
							sum(case when od.status NOT IN ('".implode("','",$non_sale_status)."')  then ".$coprice_str." else 0 end) as order_coprice_sum, 
							...
							sum(case when od.status not in ('".implode("','",$all_sale_status)."')  then ".$coprice_str." else 0 end) as coprice_all_sum, 
							...
							sum(case when od.status IN ('".implode("','",$cancel_status)."')  then ".$coprice_str." else 0 end) as cancel_coprice_sum, 
							...
							sum(case when od.status IN ('".implode("','",$return_status)."')  then ".$coprice_str." else 0 end) as return_coprice_sum, 

							sum(od.pcnt) as amount_cnt, sum(od.pt_dcprice) as sale_sum
							from  shop_order_detail od 
							left join ".TBL_LOGSTORY_REFERER_CATEGORYINFO." c on od.rfid = c.cid 
        ...
        if($depth == '' || $depth < 0 ){
            ...
            //AND od.status NOT IN ('".implode("','",$non_sale_status)."')
            //and substr(c.cid,1,".(($depth)*3).") = substr(b.vreferer_id,1,3)
    ...
    $dateString .= " (".($cid ? getCategoryPath($cid,4):"전체").")";
    ...
    if(isset($_GET['mode']) && $_GET["mode"] == "excel"){
        ...
        $sheet->getActiveSheet(0)->setCellValue('A2', "유입사이트별 분석");
        ...
        for($i=0;$i<$fordb->total;$i++){
            ...
            $sheet->getActiveSheet()->setCellValue('B' . ($i + $start + 4), ($fordb->dt['cid'] == "9" ? "직접방문" : strip_tags(getRefererCategoryPath($fordb->dt['cid'],(isset($_COOKIE['view_detail']) && $_COOKIE['view_detail'] == 1 ? 4:$depth)))));
        ...
        //header('Content-Disposition: attachment;filename="'.iconv("utf-8","cp949","유입사이트별 분석_매출상세분석.xls").'"');
        //header('Cache-Control: max-age=0');

		$filename = "유입사이트별분석_매출상세분석";
		...
		$sheet->getActiveSheet()->setTitle('유입사이트별분석');
		$sheet->setActiveSheetIndex(0);
    ...
    if(isset($_GET["mode"]) && ($_GET["mode"] == "pop" || $_GET["mode"] == "print")){
        $mstring .= "<tr><td >".TitleBar("상품군별 분석 : ".($cid ? "-".getCategoryPath($cid,4):""),$dateString,false, $exce_down_str)."</td></tr>";
    }else{
        $mstring .= "<tr><td >".TitleBar("상품군별 분석 : ".($cid ? "-".getCategoryPath($cid,4):""),$dateString,true, $exce_down_str)."</td></tr>";
    }
    $mstring .= "
						<tr height=50>
							<td >
								<div class='tab'>
											...
											<tr>
												<td class='tab'> 
													...
													<tr>
														<th class='box_01'></th>
														<td class='box_02' onclick=\"document.location.href='?report_type=3'\">매출요약 분석</td>
														<th class='box_03'></th>
													</tr>
													</table> 
												...
												<td class='btn' style='padding:5px 0px 0px 10px;'>
													<input type=checkbox name='view_detail' id='view_detail' value='1' style='position:relative;top:3px;'  onclick=\"reloadView()\"  ".(isset($_COOKIE['view_detail']) && $_COOKIE['view_detail'] == 1 ? "checked":"")." ><label for='view_detail'> 유입사이트 상세분류로 보기</label>
    ...
    $mstring .= "
		<tr height=30>
			...
			<td class=m_td rowspan=3>".OrderByLink("유입사이트", "cid", $ordertype)."</td>
    ...
    for($i=0;$i<$fordb->total;$i++){
        ...
        if(substr($fordb->dt["cid"],0,6) == "000001"){
            $mstring .= "
		<a href=\"javascript:PoPWindow3('../report/keywordbysearchengine.php?mode=pop&referer_id=".$fordb->dt['cid']."&rdepth=".($fordb->dt['depth']-1)."',960,900,'category_select')\"'>".($fordb->dt['cid'] == "9" ? "직접방문" : strip_tags(getRefererCategoryPath($fordb->dt['cid'],(isset($_COOKIE['view_detail']) && $_COOKIE['view_detail'] == 1 ? 4:1))))."</a> ";
        }else{
            $mstring .= "
		".($fordb->dt['cid'] == "9" ? "직접방문" : strip_tags(getRefererCategoryPath($fordb->dt['cid'],(isset($_COOKIE['view_detail']) && $_COOKIE['view_detail'] == 1 ? 4:1))))."";
        }
        $mstring .= "
		</td>
    ...
    */
    ...
    $mstring .= HelpBox("유입사이트별 분석", $help_text);
    return $mstring;
}
...
function reloadView(){
		...
		if($('#view_detail').attr('checked') == true || $('#view_detail').attr('checked') == 'checked'){		
			...
			$.cookie('referer_depth', $('#referer_depth').val(), {expires:1,domain:document.domain, path:'/', secure:0});
		}else{		
			$.cookie('view_detail', '0', {expires:1,domain:document.domain, path:'/', secure:0});
			$.cookie('referer_depth', '', {expires:1,domain:document.domain, path:'/', secure:0});
		}
...
//	echo "<Script>parent.document.getElementById('calendararea').innerHTML = document.calendarfrm.calendarvalue.value;parent.ChangeCalenderView($SelectReport);</Script>";
    ...
    $P->addScript = $script;
    $P->Navigation = "이커머스분석 > 상품별 종합분석 > 유입사이트별 분석";

    $P->title = "유입사이트별 분석";
    //$P->strContents = ReportTable($vdate,$SelectReport);
    if($report_type == 2){
        $P->NaviTitle = "유입사이트별 분석 - 구매전환 분석";
        $P->strContents = ReportTable2($vdate,$SelectReport);
    }else if($report_type == 3){
        $P->NaviTitle = "유입사이트별 분석 - 매출요약 분석";
        $P->strContents = ReportTable2($vdate,$SelectReport);
    }else{
        $P->NaviTitle = "유입사이트별 분석 - 매출상세분석";
        $P->strContents = ReportTable($vdate,$SelectReport);
    }
    $P->OnloadFunction = "";
    ...
    $p->forbizLeftMenu = commerce_munu('productviewbyreferer.php', "<div id=TREE_BAR style=\"margin:5px;\">".GetTreeNode('productviewbyreferer.php',date("Ymd", time()),'product')."</div>");
    if($report_type == 2){
...
function ReportTable2($vdate,$SelectReport=1){
    global $rdepth,$referer_id, $cid, $depth;
...
//echo $cid .":::".$depth;
    ...
    if ($SelectReport == 1){
        if($rdepth == 0){
            $sql = "select data.cid, sum(nview_cnt) as nview_cnt, sum(visit_cnt) as visit_cnt, sum(pcnt) as pcnt, sum(ptprice) as ptprice, sum(order_detail_cnt) as order_detail_cnt , sum(buyer_cnt) as buyer_cnt 
						from 
						(Select b.vreferer_id as cid, count(distinct visitid) as visit_cnt, sum(b.nview_cnt) as nview_cnt , 0 pcnt , 0 ptprice , 0 order_detail_cnt,  0 buyer_cnt
						from ".TBL_COMMERCE_VIEWINGVIEW." b  						
						where b.vdate = '".$vdate."' 			
						group by vreferer_id
						union 
						select od.rfid as cid, 0 nview_cnt , 0 as visit_cnt,  sum(od.pcnt) as pcnt, sum(od.pt_dcprice) as ptprice, count(*) as order_detail_cnt  , count(distinct od.oid) as buyer_cnt 
						from shop_order_detail od 
						where od.regdate between '".ChangeDate($vdate,"Y-m-d")." 00:00:00' and '".ChangeDate($vdate,"Y-m-d")." 23:59:59'
						and od.status NOT IN ('".implode("','",$non_sale_status)."')  ";
            ...
            $sql .=" group by rfid
						) data, ".TBL_LOGSTORY_REFERER_CATEGORYINFO." r
						where substr(r.cid,1,3) = substr(data.cid,1,3) and r.depth = 0  	";
            //date_format(od.regdate,'%Y%m%d') = '".$vdate."'
            if(isset($_COOKIE['view_detail']) && $_COOKIE['view_detail'] == 1){
                if(empty($_COOKIE['referer_depth'])){
                    $sql .= "group by data.cid ";
                }else{
                    $sql .= "group by substr(data.cid,1,".(empty($_COOKIE['referer_depth']) ? 6:$_COOKIE['referer_depth']*3).") ";
                }

            }else{
                $sql .= "group by substr(data.cid,1,6) ";
            }
            $sql .= $orderbyString;
            ...
            $sql = "select data.cid, sum(nview_cnt) as nview_cnt, sum(visit_cnt) as visit_cnt, sum(pcnt) as pcnt, sum(ptprice) as ptprice, sum(order_detail_cnt) as order_detail_cnt , sum(buyer_cnt) as buyer_cnt 
						from 
						(Select b.vreferer_id as cid, sum(b.nview_cnt) as nview_cnt , count(distinct visitid) as visit_cnt, 0 pcnt , 0 ptprice , 0 order_detail_cnt,  0 buyer_cnt
						from ".TBL_COMMERCE_VIEWINGVIEW." b  						
						where b.vdate = '".$vdate."' 			
						group by vreferer_id
						union 
            ...
            $sql .=" group by rfid
						...
						where substr(r.cid,1,".(($rdepth)*3).") = substr(data.cid,1,".(($rdepth)*3).") and r.depth = ".$rdepth."  			
						and substr(data.cid,1,".(($rdepth)*3).") = '".substr($referer_id,0,(($rdepth)*3))."'";
            if(isset($_COOKIE['view_detail']) && $_COOKIE['view_detail'] == 1){
                if(empty($_COOKIE['referer_depth'])){
                    $sql .= "group by data.cid ";
                }else{
        ...
        if($rdepth == 0){
            $sql = "select data.cid, sum(nview_cnt) as nview_cnt, sum(visit_cnt) as visit_cnt, sum(pcnt) as pcnt, sum(ptprice) as ptprice, sum(order_detail_cnt) as order_detail_cnt , sum(buyer_cnt) as buyer_cnt  
						...
						select od.rfid as cid, 0 nview_cnt , 0 as visit_cnt, sum(od.pcnt) as pcnt, sum(od.pt_dcprice) as ptprice, count(*) as order_detail_cnt  , count(distinct od.oid) as buyer_cnt 
						from shop_order_detail od 
						where od.regdate between '".ChangeDate($vdate,"Y-m-d")." 00:00:00' and '".ChangeDate($vweekenddate,"Y-m-d")." 23:59:59'
						and od.status NOT IN ('".implode("','",$non_sale_status)."')  ";
        ...
        if($rdepth == 0){
            $sql = "select data.cid, sum(nview_cnt) as nview_cnt, sum(visit_cnt) as visit_cnt, sum(pcnt) as pcnt, sum(ptprice) as ptprice, sum(order_detail_cnt) as order_detail_cnt ,  sum(buyer_cnt) as buyer_cnt 
						...
						select od.rfid as cid, 0 nview_cnt , 0 as visit_cnt, sum(od.pcnt) as pcnt, sum(od.pt_dcprice) as ptprice, count(*) as order_detail_cnt  , count(distinct od.oid) as buyer_cnt 
						from shop_order_detail od 
						where od.regdate between '".ChangeDate($vdate,"Y-m")."-01 00:00:00' and '".ChangeDate($vdate,"Y-m")."-31 23:59:59'
						and od.status NOT IN ('".implode("','",$non_sale_status)."')  ";
    ...
    if(isset($_GET['mode']) && $_GET["mode"] == "excel"){
        ...
        $sheet->getActiveSheet(0)->setCellValue('A2', "유입사이트별 분석 - 구매전환분석");
        ...
        $sheet->getActiveSheet(0)->mergeCells('E'.($i+1).':K'.($i+1));
        ...
        $sheet->getActiveSheet(0)->setCellValue('E' . ($i+2), "구매건수");
        $sheet->getActiveSheet(0)->setCellValue('F' . ($i+2), "상품품목수");
        $sheet->getActiveSheet(0)->setCellValue('G' . ($i+2), "구매점유율");
        $sheet->getActiveSheet(0)->setCellValue('H' . ($i+2), "구매전환율(b/a)");
        $sheet->getActiveSheet(0)->setCellValue('I' . ($i+2), "상품수량");
        $sheet->getActiveSheet(0)->setCellValue('J' . ($i+2), "매출액");
        $sheet->getActiveSheet(0)->setCellValue('K' . ($i+2), "매출점유율");
        ...
        for($i=0;$i<$fordb->total;$i++){
            ...
            $buyer_cnt_sum += $fordb->dt['buyer_cnt'];
        ...
        for($i=0;$i<$fordb->total;$i++){
            ...
            $sheet->getActiveSheet()->setCellValue('B' . ($i + $start + 3), strip_tags(getRefererCategoryPath($fordb->dt['cid'],(isset($_COOKIE['view_detail']) && $_COOKIE['view_detail'] == 1 ? 4:$depth))));
            ...
            $sheet->getActiveSheet()->setCellValue('E' . ($i + $start + 3), $fordb->dt['buyer_cnt']);

            $sheet->getActiveSheet()->setCellValue('F' . ($i + $start + 3), $fordb->dt['order_detail_cnt']);

            $sheet->getActiveSheet()->setCellValue('G' . ($i + $start + 3), $order_rate);
            $sheet->getActiveSheet()->getStyle('G' . ($i + $start + 3))->getNumberFormat()->setFormatCode( PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE_00);

            $sheet->getActiveSheet()->setCellValue('H' . ($i + $start + 3), $fordb->dt['order_detail_cnt']/$fordb->dt['nview_cnt']); // 엑셀 포맷 정의시 자동으로 *100 이 처리됨
            $sheet->getActiveSheet()->getStyle('H' . ($i + $start + 3))->getNumberFormat()->setFormatCode( PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE_00);

            $sheet->getActiveSheet()->setCellValue('I' . ($i + $start + 3), $fordb->dt['pcnt']);
            $sheet->getActiveSheet()->getStyle('I' . ($i + $start + 3))->getNumberFormat()->setFormatCode( PHPExcel_Style_NumberFormat::FORMAT_NUMBER);

            $sheet->getActiveSheet()->setCellValue('J' . ($i + $start + 3), $fordb->dt['ptprice']);
            $sheet->getActiveSheet()->getStyle('J' . ($i + $start + 3))->getNumberFormat()->setFormatCode( PHPExcel_Style_NumberFormat::FORMAT_NUMBER);

            $sheet->getActiveSheet()->setCellValue('K' . ($i + $start + 3), $sale_rate);
            $sheet->getActiveSheet()->getStyle('K' . ($i + $start + 3))->getNumberFormat()->setFormatCode( PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE_00);


            $nview_cnt_sum = $nview_cnt_sum + returnZeroValue($fordb->dt['nview_cnt']);
            $nview_cnt_sum_new = $nview_cnt_sum / 2;
        ...
        $sheet->getActiveSheet()->setCellValue('C' . ($i + $start + 3), $nview_cnt_sum_new);
        $sheet->getActiveSheet()->setCellValue('D' . ($i + $start + 3), "100%");
        $sheet->getActiveSheet()->setCellValue('E' . ($i + $start + 3), $buyer_cnt_sum);
        $sheet->getActiveSheet()->setCellValue('F' . ($i + $start + 3), $order_detail_cnt_sum);
        $sheet->getActiveSheet()->setCellValue('G' . ($i + $start + 3), "100%");
        $sheet->getActiveSheet()->setCellValue('H' . ($i + $start + 3), number_format($order_change_rate_sum,1));
        $sheet->getActiveSheet()->setCellValue('I' . ($i + $start + 3), $pcnt_sum);
        $sheet->getActiveSheet()->setCellValue('J' . ($i + $start + 3), $ptprice_sum);
        $sheet->getActiveSheet()->setCellValue('K' . ($i + $start + 3), "100%");
        ...
        //header('Content-Type: application/vnd.ms-excel');
        //header('Content-Disposition: attachment;filename="'.iconv("utf-8","cp949","유입사이트별 분석_구매전환분석.xls").'"');
        //header('Cache-Control: max-age=0');

		$filename = "유입사이트별분석_구매전환분석";
    ...
    //$mstring = $mstring.TitleBar("상품군별 분석 : ".($cid ? getRefererCategoryPath($cid,4):""),$dateString);
    $mstring = "<table width='100%' border=0>";
    if(isset($_GET["mode"]) && ($_GET["mode"] == "pop" || $_GET["mode"] == "print")){
								...
								<div class='tab'>
											...
											<tr>
												...
												<table padding=5>
													...
													<col width=*>
													<tr>
														<td style='vertical-align:top;padding:5px 0px 5px 5px;'>
														<select name='referer_depth' id='referer_depth' onchange=\"reloadView()\" style='border:1px solid silver; width:100%;margin-bottom:5px; position:relative; top:-5px;'>
															<option value='1' ".(isset($_COOKIE['referer_depth']) && $_COOKIE['referer_depth'] == "1" ? "selected":"").">1depth</option>
															<option value='2' ".(isset($_COOKIE['referer_depth']) && $_COOKIE['referer_depth'] == "2" ? "selected":"").">2depth</option>
															<option value='3' ".(isset($_COOKIE['referer_depth']) && $_COOKIE['referer_depth'] == "3" ? "selected":"").">3depth</option>
															<option value='4' ".(isset($_COOKIE['referer_depth']) && $_COOKIE['referer_depth'] == "4" ? "selected":"").">4depth</option>
														</select>
														</td>
														<td style='vertical-align:top;padding-top:2px;'>

														<input type=checkbox name='view_detail' id='view_detail' value='1' style='float:left;position:relative; height:16px;margin-left:10px;top:2px;' align='absmiddle' onclick=\"reloadView()\"  ".(isset($_COOKIE['view_detail']) && $_COOKIE['view_detail'] == 1 ? "checked":"")." ><label for='view_detail' style='float:left; margin-left:5px;'> 유입사이트 상세분류로 보기</label>
														<input type=checkbox name='view_self_sale' id='view_self_sale' value='1' onclick=\"reloadSaleView()\"  ".(isset($_COOKIE['view_self_sale']) && $_COOKIE['view_self_sale'] == 1 ? "checked":"")." ><label for='view_self_sale'> 자사매출만 보기</label>
														</td>
													</tr>
													</table>
    ...
    $mstring .= "<table cellpadding=3 cellspacing=0 width=100%  ID='MaxViewProductTable' class='list_table_box'>
						...
						<col width='10%'>
						<col width='8%'>";
    $mstring .= "<tr height=30 align=center><td class=s_td rowspan=2>순</td><td class=m_td rowspan=2>".OrderByLink("유입사이트", "cid", $ordertype)."</td><td class=m_td colspan=3>상품조회정보</td><td class=e_td colspan=7>상품판매정보</td></tr>\n";
    $mstring .= "<tr height=30 align=center>
							<td class=m_td >".OrderByLink("방문횟수", "visit_cnt", $ordertype)."</td>
							<td class=m_td >".OrderByLink("조회횟수(a)", "nview_cnt", $ordertype)."</td>
							<td class=m_td >점유율</td>							
							<td class=m_td >".OrderByLink("구매건수", "buyer_cnt", $ordertype)."</td>
							<td class=m_td >".OrderByLink("상품품목수", "order_detail_cnt", $ordertype)."</td>
    ...
    for($i=0;$i<$fordb->total;$i++){
		...
		<td class='list_box_td list_bg_gray' onmouseover=\"mouseOnTD('".$i."',true)\" onmouseout=\"mouseOnTD('$i',false)\">".($i+1)."</td>
		<td class='list_box_td point' style='text-align:left;' onmouseover=\"mouseOnTD('$i',true)\" onmouseout=\"mouseOnTD('$i',false)\"> 
		<a href=\"javascript:PoPWindow3('../report/keywordbysearchengine.php?mode=pop&referer_id=".$fordb->dt['cid']."&depth=".($fordb->dt['depth']-1)."',960,900,'category_select')\"'>
		".str_replace("전체 >","",strip_tags(getRefererCategoryPath($fordb->dt['cid'],(isset($_COOKIE['view_detail']) && $_COOKIE['view_detail'] == 1 ? (empty($_COOKIE['referer_depth']) ? 4:$_COOKIE['referer_depth']-1):$depth+1))))."
		</a> <!--".$fordb->dt['cid']."--></td>
		<td class='list_box_td  number' onmouseover=\"mouseOnTD('$i',true)\" onmouseout=\"mouseOnTD('$i',false)\">".number_format(returnZeroValue($fordb->dt['visit_cnt']),0)."</td>
		...
		<td class='list_box_td point number' onmouseover=\"mouseOnTD('$i',true)\" onmouseout=\"mouseOnTD('$i',false)\">".number_format(returnZeroValue($fordb->dt['buyer_cnt']),0)."</td>
		...
		<td class='list_box_td blue_point number' onmouseover=\"mouseOnTD('$i',true)\" onmouseout=\"mouseOnTD('$i',false)\">".number_format(returnZeroValue($order_change_rate),1)."%</td>
        ...
        $visit_cnt_sum_new = $visit_cnt_sum / 2;
    ...
    */
    ...
    $mstring .= "<tr height=25 align=right>
	<td class=s_td align=center colspan=2>합계</td>
	<td class=e_td style='padding-right:20px;'>".number_format($visit_cnt_sum_new,0)."</td>
	<td class=e_td style='padding-right:20px;'>".number_format($nview_cnt_sum_new,0)."</td>
	<td class=e_td style='padding-right:20px;'>100%</td>
	<td class=e_td style='padding-right:20px;'>".number_format($buyer_cnt_sum,0)."</td>
...
function ReportTable3($vdate,$SelectReport=1){
    ...
    if ($SelectReport == 1){
        if($depth == "" || $depth < 0){
            ...
            $sql = "Select IFNULL(c.cid,'9') as cid, IFNULL(c.cname,'직접방문') as cname, 
							...
							(
								...
								sum(case when od.status IN ('".implode("','",$return_status)."')  then ".$coprice_str." else 0 end)
							) as real_sale_sum , 
							...
							(
								...
								sum(case when (o.mem_reg_date = o.static_date and od.status IN ('".implode("','",$return_status)."'))  then ".$coprice_str." else 0 end)
							...
							(
								...
								sum(case when (o.payment_agent_type = 'W' and od.status IN ('".implode("','",$return_status)."'))  then ".$coprice_str." else 0 end)
							...
							(
								...
								sum(case when (o.payment_agent_type = 'M' and od.status IN ('".implode("','",$return_status)."'))  then ".$coprice_str." else 0 end)
    ...
    if(isset($_GET["mode"]) && $_GET["mode"] == "excel"){
        ...
        for($i=0;$i<$fordb->total;$i++){
            ...
            $sheet->getActiveSheet()->setCellValue('A' . ($i + $start + 4), ($i + 1));
            $sheet->getActiveSheet()->setCellValue('B' . ($i + $start + 4), ($fordb->dt['cid'] == "9" ? "직접방문" : strip_tags(getRefererCategoryPath($fordb->dt['cid'],(isset($_COOKIE['view_detail']) && $_COOKIE['view_detail'] == 1 ? 4:2)))));
            $sheet->getActiveSheet()->setCellValue('C' . ($i + $start + 4), $fordb->dt['order_sale_cnt']);
            $sheet->getActiveSheet()->getStyle('C' . ($i + $start + 4))->getNumberFormat()->setFormatCode( PHPExcel_Style_NumberFormat::FORMAT_NUMBER);
        ...
        //header('Content-Type: application/vnd.ms-excel');
        //header('Content-Disposition: attachment;filename="'.iconv("utf-8","cp949","유입사이트별 분석_매출요약분석.xls").'"');
        //header('Cache-Control: max-age=0');

		$filename = "유입사이트별분석_매출요약분석";
								...
								<div class='tab'>
									...
									<tr>
										<td class='tab'> 
											...
											<tr>
												<th class='box_01'></th>
												<td class='box_02' onclick=\"document.location.href='?report_type=3&".str_replace(array("mode=iframe&","report_type=$report_type"), "",str_replace("mode=iframe&", "",$_SERVER["QUERY_STRING"]))."'\">매출요약 분석</td>
												<th class='box_03'></th>
											</tr>
											</table> 
											<table id='tab_02'  ".(($report_type == '1' || $report_type == '') ? "class=on":"").">
											<tr>
												<th class='box_01'></th>
												<td class='box_02' onclick=\"document.location.href='?report_type=1&".str_replace("report_type=$report_type&", "",str_replace(array("mode=iframe&","report_type=$report_type"), "",$_SERVER["QUERY_STRING"]))."'\">매출상세 분석</td>
												<th class='box_03'></th>
											</tr>
											</table>
											 <!--
											...
											<tr>
												<th class='box_01'></th>
												<td class='box_02' onclick=\"document.location.href='?report_type=2&".str_replace("report_type=$report_type&", "",str_replace(array("mode=iframe&","report_type=$report_type"), "",$_SERVER["QUERY_STRING"]))."'\">구매전환 분석</td>
												<th class='box_03'></th>
											</tr>
											</table>
											 -->
    ...
    for($i=0;$i<$fordb->total;$i++){
        ...
        if(substr($fordb->dt["cid"],0,6) == "000001"){
            $mstring .= "
		<a href=\"javascript:PoPWindow3('../report/keywordbysearchengine.php?mode=pop&referer_id=".$fordb->dt['cid']."&rdepth=".($fordb->dt['depth']-1)."',960,900,'category_select')\"'>".($fordb->dt['cid'] == "9" ? "직접방문" : str_replace("전체 >","",strip_tags(getRefererCategoryPath($fordb->dt['cid'],(isset($_COOKIE['view_detail']) && $_COOKIE['view_detail'] == 1 ? 4:2)))))."</a>  <!--".$fordb->dt['cid']."-->";
        }else{
            $mstring .= "
		".($fordb->dt['cid'] == "9" ? "직접방문" : str_replace("전체 >","",strip_tags(getRefererCategoryPath($fordb->dt['cid'],(isset($_COOKIE['view_detail']) && $_COOKIE['view_detail'] == 1 ? 4:2)))))."";
        }
        $mstring .= "</td>
