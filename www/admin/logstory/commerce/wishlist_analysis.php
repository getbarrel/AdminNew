...
//	echo "<Script>parent.document.getElementById('contents_frame').innerHTML = document.tablefrm.reportvalue.value</Script>";
    ...
    $ca = new Calendar();
    $ca->LinkPage = 'wishlist_analysis.php';
...
//	echo "<Script>parent.document.getElementById('calendararea').innerHTML = document.calendarfrm.calendarvalue.value;parent.ChangeCalenderView($SelectReport);</Script>";
    ...
    $P->addScript = $Script;
    $P->Navigation = "이커머스분석 > 상품별 종합분석 > 실시간 찜상품분석";

    $P->title = "실시간 찜상품분석";
    //$P->strContents = ReportTable($vdate,$SelectReport);

    $P->NaviTitle = "실시간 찜상품분석 - 구매전환분석";
    ...
    $p->forbizLeftMenu = commerce_munu('wishlist_analysis.php', "<div id=TREE_BAR style=\"margin:5px;\">".GetTreeNode('wishlist_analysis.php',date("Ymd", time()),'product')."</div>");
...
function ReportTable2($vdate,$SelectReport=1){
    ...
    if(isset($_GET["orderby"]) && $_GET["orderby"] != "" && isset($_GET["ordertype"]) && $_GET["ordertype"] != ""){
        $orderbyString = " order by ".$_GET["orderby"]." ".$_GET["ordertype"].", realtime_wishlist_pcnt desc , wish_total desc,  order_goods_cnt desc ";
    }else{
        $orderbyString = " order by realtime_wishlist_pcnt desc , wish_total desc, order_goods_cnt desc ";
    }
    ...
    if ($SelectReport == 1){
        if($depth == 0){
            $sql = "select data.cid, data.pid, p.pname as pname, sum(nview_cnt) as nview_cnt, sum(order_goods_cnt) as order_goods_cnt, sum(realtime_wishlist_pcnt) as realtime_wishlist_pcnt, sum(wish_total) as wish_total, sum(cart_total_cnt) as cart_total_cnt, sum(ptprice) as ptprice,
						case when sum(cart_total_cnt) = 0 then 0 else sum(order_goods_cnt)/sum(cart_total_cnt)*100 end as cart_change_rate
						from 
						(Select LPAD(b.pid,10,'0') as pid, b.cid as cid, sum(b.nview_cnt) as nview_cnt , 0 as order_goods_cnt, 0 as realtime_wishlist_pcnt , sum(0) as wish_total,  0 cart_total_cnt , 0 as ptprice
						from ".TBL_COMMERCE_VIEWINGVIEW." b  	
						where b.vdate = '".$vdate."' 			
						group by pid
						union
						select LPAD(od.pid,10,'0') as pid, od.cid as cid, 0 as nview_cnt , count(*) as order_goods_cnt,  0 as realtime_wishlist_pcnt, sum(0) as wish_total,   0 as cart_total_cnt , sum(od.pt_dcprice) as ptprice
            ...
            $sql .= " group by pid
						union
						select LPAD(w.pid,10,'0') as pid, w.cid as cid, 0 nview_cnt ,0 as order_goods_cnt,  count(*) as realtime_wishlist_pcnt,  sum(0) as wish_total, 0 as cart_total_cnt   , 0 as ptprice
						from shop_wishlist w
						
						group by pid
						union
						Select LPAD(ss.pid,10,'0') as pid, ss.cid as cid, 0 as nview_cnt , 0 as order_goods_cnt, 0 as realtime_wishlist_pcnt , sum(step_wish) as wish_total, sum(step1) as cart_total_cnt , 0 as ptprice
						from ".TBL_COMMERCE_SALESTACK." ss  	
						where ss.vdate = '".$vdate."' and step1 = '1'	
						group by pid
						) data left join ".TBL_SHOP_PRODUCT." p on data.pid = p.id
						";
            //where date_format(w.regdate,'%Y%m%d') = '".$vdate."'
    ...
    if(isset($_GET["mode"]) && $_GET["mode"] == "excel"){
        ...
        $sheet->getActiveSheet(0)->setCellValue('A2', "실시간 찜상품분석 - 구매전환분석");
        ...
        header('Content-Disposition: attachment;filename="'.iconv("utf-8","cp949","실시간 찜상품분석_구매전환분석.xls").'"');
    ...
    //$mstring = $mstring.TitleBar("실시간 찜상품분석 : ".($cid ? getCategoryPath($cid,4):""),$dateString);
    $mstring = "<table width='100%' border=0>";
    if(isset($_GET["mode"]) && ($_GET["mode"] == "pop" || $_GET["mode"] == "print")){
        $mstring .= "<tr><td colspan=2>".TitleBar("실시간 찜상품분석 : ",$dateString."".($cid ? "-".getCategoryPath($cid,4):""),false, $exce_down_str)."</td></tr>";
    }else{
        $mstring .= "<tr><td colspan=2>".TitleBar("실시간 찜상품분석 : ",$dateString."".($cid ? "-".getCategoryPath($cid,4):""),true, $exce_down_str)."</td></tr>";
    }
    $mstring .= "<tr>
    ...
    $mstring .= "<tr height=30 align=center>
						...
						<td class=m_td colspan=2>찜상품 정보</td>
						<td class=m_td rowspan=2>".OrderByLink("실시간<br>찜상품수", "realtime_wishlist_pcnt", $ordertype)."</td>
						<td class=m_td rowspan=2>매출액</td>
						<td class=m_td rowspan=2>매출액<br>점유율</td>
						<td class=m_td rowspan=2>프로모션</td>
						<!--td class=m_td rowspan=2>쿠폰발급/사용/사용율</td-->
						<!--td class=e_td rowspan=2>SMS 발송/확인</td-->
					   </tr>\n";
    $mstring .= "<tr height=30 align=center>
							...
							<td class=m_td >".OrderByLink("전체담은건수", "cart_total_cnt", $ordertype)."</td>
							<td class=m_td >".OrderByLink("찜상품<br>구매전환율", "cart_change_rate", $ordertype)." </td>
						</tr>\n";
    $order_goods_cnt_sum = 0;
    ...
    for($i=0;$i<$fordb->total;$i++){
		...
		<td class='list_box_td point number' onmouseover=\"mouseOnTD('$i',true)\" onmouseout=\"mouseOnTD('$i',false)\">".number_format($fordb->dt['wish_total'])."</td>
		<td class='list_box_td list_bg_gray number' onmouseover=\"mouseOnTD('$i',true)\" onmouseout=\"mouseOnTD('$i',false)\">".number_format(returnZeroValue($fordb->dt['cart_change_rate']),1)." %</td>
		<td class='list_box_td  number' onmouseover=\"mouseOnTD('$i',true)\" onmouseout=\"mouseOnTD('$i',false)\">".number_format(returnZeroValue($fordb->dt['realtime_wishlist_pcnt']),0)."&nbsp;</td>
		...
		<td class='list_box_td number' onmouseover=\"mouseOnTD('$i',true)\" onmouseout=\"mouseOnTD('$i',false)\" style='text-align:center;padding:5px;'>
    ...
    */
    ...
    $mstring .= HelpBox("실시간 찜상품분석", $help_text);
    return $mstring;
}
?>