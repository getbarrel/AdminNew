...
function ReportTable($vdate,$SelectReport=1){
    ...
    if($vdate == ""){
        ...
        $search_sdate = $_GET['search_sdate'];
        $search_edate = $_GET['search_edate'];
    }
                      ...
                      and 
                        op.oid in (
							...
							and od.order_from = 'self' 
							   group by od.oid
                        ) group by date_format(op.regdate,'%Y%m%d')
            ";

    }else if($SelectReport == 2 || $SelectReport == 4){
        if($SelectReport == "4"){
            $vdate = date('Y-m-d', strtotime($search_sdate));
            $vweekenddate = date("Y-m-d", mktime(0,0,0,substr($search_edate,4,2),substr($search_edate,6,2),substr($search_edate,0,4)));
            $where = "od.regdate  between '".$vdate." 00:00:00' and '".$vweekenddate." 23:59:59'";
            $daySearchWhere = "od.regdate  between '".$vdate." 00:00:00' and '".$vweekenddate." 23:59:59'";
        }else {
    ...
    for($i=0;$i<$fordb->total;$i++){
        ...
        $org_group_name = $fordb->dt['vdate'];

        if($SelectReport == '1'){
            $distinct_column = "od.oid";
            $order_search_date = "od.regdate  between '".$selected_date." 00:00:00' and '".$selected_date." 23:59:59' and DATE_FORMAT(od.regdate, '%Y%m%d') = '".$org_group_name."' ";
        }else{
            $distinct_column = "od.oid";
            if($SelectReport == "4"){
                $order_search_date = $daySearchWhere;
                $order_search_date .= " and DATE_FORMAT(od.regdate, '%Y%m%d') like '".$org_group_name."%' ";
            }else{
                $order_search_date = "DATE_FORMAT(od.regdate, '%Y%m%d') like '".$org_group_name."%' ";
            }
		...
		<td class='list_box_td number' onmouseover=\"mouseOnTD('$i',true)\" onmouseout=\"mouseOnTD('$i',false)\">".number_format($fordb->dt['order_sale_cnt'],0)."&nbsp;(".number_format($order_cnt,0).")</td>
		...
		<td class='list_box_td number' onmouseover=\"mouseOnTD('$i',true)\" onmouseout=\"mouseOnTD('$i',false)\">".number_format($fordb->dt['sale_all_cnt'],0)."&nbsp;(".number_format($sale_order_cnt,0).")</td>
		...
		<td class='list_box_td number' onmouseover=\"mouseOnTD('$i',true)\" onmouseout=\"mouseOnTD('$i',false)\">".number_format($fordb->dt['cancel_sale_cnt'],0)."&nbsp;(".number_format($cancel_order_cnt,0).")</td>
		...
		<td class='list_box_td number' onmouseover=\"mouseOnTD('$i',true)\" onmouseout=\"mouseOnTD('$i',false)\">".number_format($fordb->dt['return_sale_cnt'],0)."&nbsp;(".number_format($return_order_cnt,0).")</td>
        ...
        $mileage_orders_sum = $mileage_orders_sum + returnZeroValue($sale_mileage_order);
	...
	<td class='e_td number' style='padding-right:10px;'>".number_format($order_sale_cnt,0)." (".number_format($order_cnt_sum,0).")</td>
	<td class='e_td number' style='padding-right:10px;'>".number_format($order_sale_sum,0)."</td>
	<td class='e_td number' style='padding-right:10px;'>".number_format($sale_all_cnt,0)." (".number_format($sale_order_cnt_sum,0).")</td>
	<td class='e_td number' style='padding-right:10px;'>".number_format($sale_all_sum,0)."</td>
	<td class='e_td number' style='padding-right:10px;'>".number_format($cancel_sale_cnt,0)." (".number_format($cancel_order_cnt_sum,0).")</td>
	<td class='e_td number' style='padding-right:10px;'>".number_format($cancel_sale_sum,0)."</td>
	<td class='e_td number' style='padding-right:10px;'>".number_format($return_sale_cnt,0)." (".number_format($return_order_cnt_sum,0).")</td>
...
if ($mode == "iframe"){
    ...
    $p->Navigation = "이커머스분석 > 매출종합분석 > 매출액(일자별)";
    $p->title = "매출액(일자별)";
    $p->PrintReportPage();
}
...
function ReportTable2($vdate,$SelectReport=1){
    ...
    $mstring = $mstring.TitleBar("매출액(일자별)",$dateString);
    if($SelectReport == 1){
        $fordb->query($sql);
        $fordb2->query($sql2);
        $fordb3->query($sql3);
...
//	$mstring .= "</table>\n";
    ...
    $mstring .= HelpBox("매출액(일자별)", $help_text);
    return $mstring;
}
