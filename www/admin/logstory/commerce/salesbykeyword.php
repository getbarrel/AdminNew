...
function ReportTable($vdate,$SelectReport=1){
    ...
    global $kid, $cid, $depth, $non_sale_status, $order_status, $cancel_status, $return_status, $all_sale_status;
    ...
    if ($SelectReport == 1){
        if($depth == "" || $depth < 0){
            ...
            $sql = "Select IFNULL(k.kid,'9') as kid,  IFNULL(k.keyword,'기타') as keyword, 
							...
							left join ".TBL_LOGSTORY_KEYWORDINFO." k on od.kwid = k.kid 
        ...
        if($depth == '' || $depth < 0 ){
            ...
            //AND od.status NOT IN ('".implode("','",$non_sale_status)."')
            //and substr(k.kid,1,".(($depth)*3).") = substr(b.vreferer_id,1,3)
    ...
    $dateString .= " (".($kid ? getCategoryPath($kid,4):"전체").")";
    ...
    if(isset($_GET["mode"]) && $_GET["mode"] == "excel"){
        ...
        $sheet->getActiveSheet(0)->setCellValue('A2', "키워드별 분석");
        ...
        for($i=0;$i<$fordb->total;$i++){
            ...
            $sheet->getActiveSheet()->setCellValue('B' . ($i + $start + 4), $fordb->dt['keyword']);
        ...
        //header('Content-Disposition: attachment;filename="'.iconv("utf-8","cp949","키워드별 분석_매출상세분석.xls").'"');
        //header('Cache-Control: max-age=0');

		$filename = "키워드별분석_매출상세분석";
		...
		$sheet->getActiveSheet()->setTitle('키워드별분석');
		$sheet->setActiveSheetIndex(0);
    ...
    $mstring .= "
		<tr height=30>
			...
			<td class=m_td rowspan=3>키워드명</td>
    ...
    for($i=0;$i<$fordb->total;$i++){
        ...
        if($fordb->dt["keyword"] == ""){
            $mstring .= "-";
        }else if(substr($fordb->dt["kid"],0,6) == "000001"){
            $mstring .= "
		<a href=\"javascript:PoPWindow3('../report/keywordbysearchengine.php?mode=pop&referer_id=".$fordb->dt['kid']."&rdepth=".($fordb->dt['depth']-1)."',960,900,'category_select')\"'>".$fordb->dt['keyword']."</a>  <!--".$fordb->dt['kid']."-->";
        }else{
            $mstring .= $fordb->dt['keyword'];
        }
        $mstring .= "
		</td>
    ...
    */
    ...
    $mstring .= HelpBox("키워드별 분석", $help_text);
    return $mstring;
}
...
//	echo "<Script>parent.document.getElementById('contents_frame').innerHTML = document.tablefrm.reportvalue.value</Script>";
    ...
    $ca = new Calendar();
    $ca->LinkPage = 'salesbykeyword.php';
...
//	echo "<Script>parent.document.getElementById('calendararea').innerHTML = document.calendarfrm.calendarvalue.value;parent.ChangeCalenderView($SelectReport);</Script>";
    ...
    $p->forbizLeftMenu = commerce_munu('salesbykeyword.php', "<div id=TREE_BAR style=\"margin:5px;\">".GetTreeNode('salesbykeyword.php',date("Ymd", time()),'product')."</div>");
    if($report_type == 2){
...
function ReportTable2($vdate,$SelectReport=1){
    ...
    if ($SelectReport == 1){
        if($depth == 0){
            $sql = "select data.cid, data.kid, IF(data.kid = 0 , '기타',k.keyword) as keyword, sum(nview_cnt) as nview_cnt, sum(pcnt) as pcnt, sum(ptprice) as ptprice, sum(order_detail_cnt) as order_detail_cnt from 
						...
						group by kid
						union 
						select od.cid, od.kwid as kid, 0 nview_cnt , sum(od.pcnt) as pcnt, sum(od.pt_dcprice) as ptprice, count(*) as order_detail_cnt  
						from shop_order_detail od 
						where od.regdate  between '".$selected_date." 00:00:00' and '".$selected_date." 23:59:59' 
						and od.status NOT IN ('".implode("','",$non_sale_status)."') ";
            ...
            $sql .= " group by kid
						) data left join ".TBL_LOGSTORY_KEYWORDINFO." k on k.kid = data.kid 
						where (data.kid != 0 and k.keyword != '')  or data.kid = 0
						  	";
            //where date_format(od.regdate,'%Y%m%d') = '".$vdate."'
            ...
            $sql .= " and substr(od.cid,1,".(($depth)*3).") = '".substr($cid,0,(($depth)*3))."' 
						group by kid
						) data left join ".TBL_LOGSTORY_KEYWORDINFO." k on k.kid = data.kid and k.keyword != ''
						where substr(data.cid,1,".(($depth)*3).") = '".substr($cid,0,(($depth)*3))."' 
						and (data.kid != 0 and k.keyword != '')  or data.kid = 0 ";
            //where date_format(od.regdate,'%Y%m%d') = '".$vdate."'
            $sql .= "group by data.kid";
            $sql .= $orderbyString;
        }
    ...
    if(isset($_GET["mode"]) && $_GET["mode"] == "excel"){
        ...
        $sheet->getActiveSheet(0)->setCellValue('A2', "키워드별 분석 - 구매전환분석");
        ...
        $sheet->getActiveSheet(0)->setCellValue('A' . ($i+1), "순");
        $sheet->getActiveSheet(0)->setCellValue('B' . ($i+1), "키워드명");
        ...
        for($i=0;$i<$fordb->total;$i++){
            ...
            if(!empty($fordb->dt['nview_cnt'])) {
                ...
                $sheet->getActiveSheet()->setCellValue('G' . ($i + $start + 3), 0);
            }
        ...
        //header('Content-Type: application/vnd.ms-excel');
        //header('Content-Disposition: attachment;filename="'.iconv("utf-8","cp949","키워드별 분석_구매전환분석.xls").'"');
        //header('Cache-Control: max-age=0');

		$filename = "키워드별분석_구매전환분석";
    ...
    $mstring .= "<table cellpadding=3 cellspacing=0 width=100%  ID='MaxViewProductTable' class='list_table_box'>
						...
						<col width='10%'>
						<col width='8%'>";
    $mstring .= "<tr height=30 align=center><td class=s_td rowspan=2>순</td><td class=m_td rowspan=2>키워드명</td><td class=m_td colspan=2>상품조회정보</td><td class=e_td colspan=6>상품판매정보</td></tr>\n";
    ...
    for($i=0;$i<$fordb->total;$i++){
        ...
        $mstring .= "<tr height=30 bgcolor=#ffffff  id='Report$i'>
		<td class='list_box_td list_bg_gray' onmouseover=\"mouseOnTD('".$i."',true)\" onmouseout=\"mouseOnTD('$i',false)\">".($i+1)."</td>
		<td class='list_box_td point' style='text-align:left;' onmouseover=\"mouseOnTD('$i',true)\" onmouseout=\"mouseOnTD('$i',false)\" title='".$fordb->dt['kid']."'><a href=\"javascript:PoPWindow3('../report/keywordbysearchengine.php?mode=pop&referer_id=".$fordb->dt['kid']."&depth=".($fordb->dt['depth']-1)."',960,900,'category_select')\"' >".$fordb->dt['keyword']."</a> </td>
...
function ReportTable3($vdate,$SelectReport=1){
    ...
    if(isset($_GET["mode"]) && $_GET["mode"] == "excel"){
        ...
        //header('Content-Type: application/vnd.ms-excel');
        //header('Content-Disposition: attachment;filename="'.iconv("utf-8","cp949","키워드별 분석_매출요약분석.xls").'"');
        //header('Cache-Control: max-age=0');

		$filename = "키워드별분석_매출요약분석";
    ...
    $mstring .= "
		<tr height=30>
			<td class=s_td rowspan=3>순</td>
			<td class=m_td rowspan=3>".OrderByLink("키워드명", "kid", $ordertype)."</td>
			<td class=m_td colspan=2>주문매출</td>
    ...
    for($i=0;$i<$fordb->total;$i++){
        ...
        $mstring .= "<tr height=30 bgcolor=#ffffff  id='Report$i'>
		<td class='list_box_td list_bg_gray' onmouseover=\"mouseOnTD('".$i."',true)\" onmouseout=\"mouseOnTD('$i',false)\">".($i+1)."</td>
		<td class='list_box_td point' style='text-align:left;' onmouseover=\"mouseOnTD('$i',true)\" onmouseout=\"mouseOnTD('$i',false)\">".$fordb->dt['keyword'];
        $mstring .= "</td>
