...
//	echo "<Script>parent.document.getElementById('calendararea').innerHTML = document.calendarfrm.calendarvalue.value;parent.ChangeCalenderView($SelectReport);</Script>";
    ...
    $P->addScript = $Script;
    $P->Navigation = "이커머스분석 > 고객 종합분석 > 최다로그인고객 분석";

    $P->title = "최다로그인고객 분석";
    //$P->strContents = ReportTable($vdate,$SelectReport);

    $P->NaviTitle = "최다로그인고객 분석";
    $P->strContents = ReportTable($vdate,$SelectReport);
    ...
    $p->forbizLeftMenu = commerce_munu('loginanalysis.php', "");
...
function ReportTable($vdate,$SelectReport=1){
    ...
    if ($SelectReport == 1){
        $sql = "select  data.ucode,  AES_DECRYPT(UNHEX(cmd.name),'".$fordb->ase_encrypt_key."') as name, cu.id, sum(nview_cnt) as nview_cnt, sum(order_goods_cnt) as order_goods_cnt, 
						...
						from 
						(Select b.ucode as ucode, sum(b.nview_cnt) as nview_cnt , 0 as order_goods_cnt, 0 as pcnt , 0 login_total_cnt , 0 as ptprice
						from ".TBL_COMMERCE_VIEWINGVIEW." b  	
						where b.vdate = '".$vdate."' 			
						group by ucode
						union
						select o.user_code as ucode, 0 as nview_cnt , count(*) as order_goods_cnt,  sum(od.pcnt) as pcnt,  0 as login_total_cnt , sum(od.pt_dcprice) as ptprice
						from shop_order o, shop_order_detail od 
						where o.oid = od.oid 
						and od.regdate  between '".$selected_date." 00:00:00' and '".$selected_date." 23:59:59'  
						and od.status NOT IN ('".implode("','",$non_sale_status)."') 
						and od.order_from = 'self' 
						group by ucode 
						union
						Select mem_ix as ucode , 0 as nview_cnt , 0 as order_goods_cnt, 0 as pcnt , sum(ncnt) as login_total_cnt , 0 as ptprice
        ...
        $sql = "select  data.ucode,  AES_DECRYPT(UNHEX(cmd.name),'".$fordb->ase_encrypt_key."') as name, cu.id, sum(nview_cnt) as nview_cnt, sum(order_goods_cnt) as order_goods_cnt, 
						...
						from shop_order o, shop_order_detail od 
						where o.oid = od.oid 
						and od.regdate between '".$selected_date." 00:00:00' and '".$vweekenddate." 23:59:59' 
						and od.status NOT IN ('".implode("','",$non_sale_status)."') 
						and od.order_from = 'self' 
						group by ucode 
						union
        ...
        $sql = "select  data.ucode,  AES_DECRYPT(UNHEX(cmd.name),'".$fordb->ase_encrypt_key."') as name, cu.id, sum(nview_cnt) as nview_cnt, sum(order_goods_cnt) as order_goods_cnt, 
						...
						from shop_order o, shop_order_detail od 
						where o.oid = od.oid 
						and od.regdate between '".substr($selected_date,0,7)."-01 00:00:00' and '".substr($selected_date,0,7)."-31 23:59:59'   
						and od.status NOT IN ('".implode("','",$non_sale_status)."') 
						and od.order_from = 'self' 
						group by ucode 
						union
...
//	$str_page_bar = page_bar($total, $page, $max, "&max=$max&info_type=$info_type","");
    ...
    if(isset($_GET["mode"]) && ($_GET["mode"] == "pop" || $_GET["mode"] == "print")){
        $mstring .= "<tr><td colspan=2>".TitleBar("최다로그인고객 분석 : ",$dateString."".($cid ? "-".getCategoryPath($cid,4):""),false, $exce_down_str)."</td></tr>";
    }else{
        $mstring .= "<tr><td colspan=2>".TitleBar("최다로그인고객 분석 : ",$dateString."".($cid ? "-".getCategoryPath($cid,4):""),true, $exce_down_str)."</td></tr>";
    }
    $mstring .= "<tr>
						<td height=30> </td>
						<td align=right>
    ...
    $mstring .= HelpBox("최다로그인고객 분석", $help_text);
    $mstring .= "</form>";
    return $mstring;
}
?>