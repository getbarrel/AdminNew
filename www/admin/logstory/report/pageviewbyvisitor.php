...
function ReportTable($vdate,$SelectReport=1){
    ...
    if($SelectReport == 1){
        $sql = "Select case when IFNULL(v.nh00,0) = 0 then 0 else p.nh00/v.nh00 end,
			...
			from ".TBL_LOGSTORY_VISITOR." v, ".TBL_LOGSTORY_PAGEVIEWTIME." p where v.vdate = '$vdate' and v.vdate = p.vdate";
        if($fordb1->dbms_type == "oracle"){
            $sql1 = "Select case when IFNULL(sum(v.nh00),0) = 0 then 0 else TO_NUMBER(avg(p.nh00)/avg(v.nh00)) end,
			...
			from ".TBL_LOGSTORY_VISITOR." v, ".TBL_LOGSTORY_PAGEVIEWTIME." p where substr(v.vdate,1,6) = '".substr($vdate,0,6)."' and v.vdate = p.vdate ";
        }else{
			...
			from ".TBL_LOGSTORY_VISITOR." v, ".TBL_LOGSTORY_PAGEVIEWTIME." p where substring(v.vdate,1,6) = '".substr($vdate,0,6)."' and v.vdate = p.vdate ";
        }
        $sql2 = "Select p.nh00/v.nh00, p.nh01/v.nh01, p.nh02/v.nh02, p.nh03/v.nh03, p.nh04/v.nh04, p.nh05/v.nh05, p.nh06/v.nh06,
			...
			p.nh23/v.nh23 from ".TBL_LOGSTORY_VISITOR." v, ".TBL_LOGSTORY_PAGEVIEWTIME." p where v.vdate = '$vyesterday' and v.vdate = p.vdate";
        $sql3 = "Select p.nh00/v.nh00, p.nh01/v.nh01, p.nh02/v.nh02, p.nh03/v.nh03, p.nh04/v.nh04, p.nh05/v.nh05, p.nh06/v.nh06,
			p.nh07/v.nh07, p.nh08/v.nh08, p.nh09/v.nh09, p.nh10/v.nh10, p.nh11/v.nh11, p.nh12/v.nh12, p.nh13/v.nh13, p.nh14/v.nh14,
			p.nh15/v.nh15, p.nh16/v.nh16, p.nh17/v.nh17, p.nh18/v.nh18, p.nh19/v.nh19, p.nh20/v.nh20, p.nh21/v.nh21, p.nh22/v.nh22,
			p.nh23/v.nh23 from ".TBL_LOGSTORY_VISITOR." v, ".TBL_LOGSTORY_PAGEVIEWTIME." p where v.vdate = '$voneweekago' and v.vdate = p.vdate";
        ...
        $sql .= " from ".TBL_LOGSTORY_VISITOR." v, ".TBL_LOGSTORY_PAGEVIEWTIME." p where v.vdate between '$vdate' and '".date("Ymd",mktime(0,0,0,substr($vdate,4,2),substr($vdate,6,2),substr($vdate,0,4))+60*60*24*6)."'
        ...
        $sql2 .= " from ".TBL_LOGSTORY_VISITOR." v, ".TBL_LOGSTORY_PAGEVIEWTIME." p
        ...
        $sql3 .= " from ".TBL_LOGSTORY_VISITOR." v, ".TBL_LOGSTORY_PAGEVIEWTIME." p
...
//		$sql1 = "Select FORMAT(avg(nh00),1), FORMAT(avg(nh01),1), FORMAT(avg(nh02),1), FORMAT(avg(nh03),1), FORMAT(avg(nh04),1), FORMAT(avg(nh05),1), FORMAT(avg(nh06),1), FORMAT(avg(nh07),1), FORMAT(avg(nh08),1), FORMAT(avg(nh09),1), FORMAT(avg(nh10),1), FORMAT(avg(nh11),1), FORMAT(avg(nh12),1), FORMAT(avg(nh13),1), FORMAT(avg(nh14),1), FORMAT(avg(nh15),1), FORMAT(avg(nh16),1), FORMAT(avg(nh17),1), FORMAT(avg(nh18),1), FORMAT(avg(nh19),1), FORMAT(avg(nh20),1), FORMAT(avg(nh21),1), FORMAT(avg(nh22),1), FORMAT(avg(nh23),1) from ".TBL_LOGSTORY_VISITOR." where substring(vdate,1,6) = '".substr($vdate,0,6)."'";
        ...
        $sql2 .= "from ".TBL_LOGSTORY_VISITOR." v, ".TBL_LOGSTORY_PAGEVIEWTIME." p where v.vdate LIKE '".substr($vonemonthago,0,6)."%' and p.vdate LIKE '".substr($vonemonthago,0,6)."%' ";
        ...
        $sql3 .= "from ".TBL_LOGSTORY_VISITOR." v, ".TBL_LOGSTORY_PAGEVIEWTIME." p where v.vdate LIKE '".substr($vtwomonthago,0,6)."%' and p.vdate LIKE '".substr($vtwomonthago,0,6)."%'  ";
...
//	echo $sql3 ."<br>";
    ...
    if($SelectReport == 1){
        ...
        $mstring = $mstring.TitleBar("순수방문자 당 페이지뷰 ","일간 : ". getNameOfWeekday(0,$vdate,"dayname"));
        $mstring .= "<table cellpadding=0 cellspacing=0 width=100% border=0>\n";
        //$mstring .= "<tr  align=center><td colspan=3 style='padding-bottom:10px;'><img src='pageviewbyvisitor.chart.php?vdate=".$vdate."&SelectReport=".$SelectReport."'></td></tr>\n";
        ...
        $mstring .= "<tr height=30 align=center><td class=s_td width=33%>항목</td><td class=m_td width=33%>시간대</td><td class=e_td width=34%>평균 페이지뷰</td></tr>\n";
		...
		<td align=right onmouseover=\"mouseOnTD('$i',true)\" onmouseout=\"mouseOnTD('$i',false)\" style='padding-right:20px;'>".number_format(returnZeroValue($fordb->dt[$i]),2)."</td>
		<td bgcolor=#efefef align=right onmouseover=\"mouseOnTD('$i',true)\" onmouseout=\"mouseOnTD('$i',false)\" style='padding-right:20px;'>".number_format(returnZeroValue($fordb1->dt[$i]),2)."</td>
		<td align=right onmouseover=\"mouseOnTD('$i',true)\" onmouseout=\"mouseOnTD('$i',false)\" style='padding-right:20px;'>".number_format(returnZeroValue($fordb2->dt[$i]),2)."</td>
		<td bgcolor=#efefef  align=right onmouseover=\"mouseOnTD('$i',true)\" onmouseout=\"mouseOnTD('$i',false)\" style='padding-right:20px;'>".number_format(returnZeroValue($fordb3->dt[$i]),2)."</td></tr>\n";
	...
	<td class=e_td style='padding-right:20px;'>".number_format($pageview04,2)."</td>
        ...
        $mstring = $mstring.TitleBar("순수방문자 당 페이지뷰","주간 : ". getNameOfWeekday(0,$vdate)."~".getNameOfWeekday(6,$vdate));
        $mstring .= "<table cellpadding=0 cellspacing=0 width=100% border=0>\n";
        //$mstring .= "<tr  align=center><td colspan=3 style='padding-bottom:10px;'><img src='pageviewbyvisitor.chart.php?vdate=".$vdate."&SelectReport=".$SelectReport."'></td></tr>\n";
        $mstring .= "<tr  align=center><td colspan=3 style='padding-bottom:10px;'><div id='line-chart' style='height: 300px; position: relative;'></div></td></tr>\n";
        $mstring .= "<tr  align=center><td colspan=3 ></td></tr>\n";
        ...
        $mstring .= "<table cellpadding=3 cellspacing=0 width=100%  class='list_table_box'>\n";
        $mstring .= "<tr height=30 align=center><td class=s_td width=33%>항목</td><td class=m_td width=33%>요일</td><td class=e_td width=33%>평균 페이지뷰</td></tr>\n";
        ...
        $mstring .= "<table cellpadding=3 cellspacing=0 width=100%  ID='MaxViewProductTable' class='list_table_box'>\n";
        $mstring .= "<tr height=30 align=center><td class=s_td width=150>요일</td><td class=m_td width=200>해당주</td><td class=m_td width=200>1주전</td><td class=e_td width=200>4주전</td></tr>\n";
		...
		<td class='point' align=right id='Report$i' onmouseover=\"mouseOnTD('$i',true)\" onmouseout=\"mouseOnTD('$i',false)\" style='padding-right:20px;'>".number_format(returnZeroValue($fordb->dt[$i]),2)."</td>

		<td bgcolor=#efefef align=right id='Report$i' onmouseover=\"mouseOnTD('$i',true)\" onmouseout=\"mouseOnTD('$i',false)\" style='padding-right:20px;'>".number_format(returnZeroValue($fordb2->dt[$i]),2)."</td>
		<td  align=right id='Report$i' onmouseover=\"mouseOnTD('$i',true)\" onmouseout=\"mouseOnTD('$i',false)\" style='padding-right:20px;'>".number_format(returnZeroValue($fordb3->dt[$i]),2)."</td></tr>\n";
...
//		$pageview02 = $pageview02 + returnZeroValue($fordb1->dt[$i]);
	...
	<td class=e_td style='padding-right:20px;'>".number_format($pageview04,2)."</td>
        ...
        $mstring = $mstring.TitleBar("순수방문자 당 페이지뷰","월간 : ". getNameOfWeekday(0,$vdate,"monthname"));
        $mstring .= "<table cellpadding=0 cellspacing=0 width=100% border=0>\n";
        //$mstring .= "<tr  align=center><td colspan=3 style='padding-bottom:10px;'><img src='pageviewbyvisitor.chart.php?vdate=".$vdate."&SelectReport=".$SelectReport."'></td></tr>\n";
        $mstring .= "<tr  align=center><td colspan=3 style='padding-bottom:10px;'><div id='line-chart' style='height: 300px; position: relative;'></div></td></tr>\n";
        $mstring .= "<tr  align=center><td colspan=3 ></td></tr>\n";
        ...
        $mstring .= "<table cellpadding=3 cellspacing=0 width=100%  class='list_table_box'>\n";
        $mstring .= "<tr height=30 align=center><td class=s_td width=33%>항목</td><td class=m_td width=33%>날짜</td><td class=e_td width=33%>평균 페이지뷰</td></tr>\n";
        $mstring .= "<tr height=30 bgcolor=#ffffff>
		<td bgcolor=#efefef align=center id='Report10000' onmouseover=\"mouseOnTD('10000',true)\" onmouseout=\"mouseOnTD('10000',false)\">최다 요청</td>
		<td class='point' align=center id='Report10000' onmouseover=\"mouseOnTD('10000',true)\" onmouseout=\"mouseOnTD('10000',false)\">{{MAXTIMESTRING}}</td>
		<td bgcolor=#efefef  align=right id='Report10000' style='padding-right:20px;' onmouseover=\"mouseOnTD('10000',true)\" onmouseout=\"mouseOnTD('10000',false)\">{{MAXVALUE}}</td>
		</tr>\n";
        $mstring .= "<tr height=30 bgcolor=#ffffff>
		<td bgcolor=#efefef align=center id='Report10001' onmouseover=\"mouseOnTD('10001',true)\" onmouseout=\"mouseOnTD('10001',false)\">최소 요청</td>
    ...
    /*
        ...
        <table>
            <tr>
                ...
                - 순수방문자당 페이지뷰란 ? 순수방문자 1인당 사이트에서 본 페이지뷰수를  나타내는 리포트로 순수방문자의 사이트의 관심도, 충성도를 보여주는 리포트 입니다<br>
                - 순수방문자당 페이지뷰를 시간대별로 보여주고 있으며, 주간리포트는 요일별, 월간리포트는 일자별로 제공됩니다<br>
    ...
    $mstring .= HelpBox("순수방문자당 페이지뷰", $help_text);
    return $mstring;
}
...
//	echo "<Script>parent.document.getElementById('contents_frame').innerHTML = document.tablefrm.reportvalue.value</Script>";
    ...
    $ca = new Calendar();
    $ca->LinkPage = 'pageviewbyvisitor.php';
    echo "<html>";
    ...
    $p->Navigation = "로그분석 > 방문자 분석 > 순수방문자당  페이지뷰";
    $p->title = "순수방문자당  페이지뷰";
    $p->forbizLeftMenu = Stat_munu('pageviewbyvisitor.php');
