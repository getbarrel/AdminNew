...
if($groupbytype=="day"){
	$P->Navigation = "영업매출통계 > 거래처별매출액(월별)";
	$P->title = "거래처별매출액(월별)";
}elseif($groupbytype=="term"){
	$P->Navigation = "영업매출통계 > 거래처별매출액(기간별)";
	$P->title = "거래처별매출액(기간별)";
}
...
function salesByDateReportTable($vdate,$groupbytype="day",$SelectReport=1){
	...
	$where =" and o.oid=od.oid and od.order_from in ('offline') and o.user_com_id !='' ";

	if($groupbytype=="day"){
		$select=" o.user_com_id ,";
		...
		$group_by = "group by o.user_com_id";
	}elseif($groupbytype=="term"){
			$select=" o.user_com_id ,";
	...
	$sql = "select ccd.com_name, sum(data.order_sale_cnt) as order_sale_cnt, sum(order_sale_sum) as order_sale_sum, sum(order_coprice_sum) as order_coprice_sum, 
			...
			$sql .= "
				...
				where data.user_com_id = ccd.company_id
				group by user_com_id";
	...
	$mstring .= "
		<tr height=30>";
			...
			<td class=m_td rowspan=3>매출처명</td>";
	...
	if($fordb->total){
		for($i=0;$i<$fordb->total;$i++){
			...
			//if($groupbytype=="day"){
				...
				$mstring .= "<td class='list_box_td point' style='text-align:left;' onmouseover=\"mouseOnTD('$i',true)\" onmouseout=\"mouseOnTD('$i',false)\" nowrap>".$fordb->dt[com_name]." </td>";
