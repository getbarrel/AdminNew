...
function product_search_click(result){
	json_result = JSON.parse(result);
	select_different_product(NOW_OBJ,json_result);
}
...
function select_same_product(obj){
	...
	if(DeliveryPackageCheck(tmp['delivery_package']) && !($("#ea_product_table_"+NOW_SELECT_OD_IX+" input[name='ec_product_select_type[225][]'][value=S]").length > 0) ){
		ClaimeAddGoods(tmp);
	}
}
