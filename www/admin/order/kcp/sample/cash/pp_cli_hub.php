...
<?
    ...
    /* ------------------------------------------------------------------------------ */
       ...
       // echo $res_cd."::".iconv("cp949","UTF-8",$res_msg);
    /* ============================================================================== */
    ...
    /* = -------------------------------------------------------------------------- = */
        if ( $req_tx == "pay" )
        {
            if ( $res_cd == "0000" )
            {
				...
				$db = new Database;
				$sql = "insert into receipt_result (oid,m_resultMsg, m_rcash_noappl,m_pgAuthDate,m_tid,m_rcr_price, m_rsup_price, m_rtax,m_rsrvc_price,m_ruseopt,regdate) values ('".$ordr_idxx."','".$res_cd."','".$receipt_no."','".$trad_time."','".$cash_no."','".$amt_tot."','".$amt_sup."','".$amt_tax."','".$amt_svc."','".$tr_code."',NOW())";
				$db->query($sql);
				$sql = "update receipt set receipt_yn = 'Y' where order_no = '".$ordr_idxx."' ";
				$db->query($sql);
    ...
    /* = -------------------------------------------------------------------------- = */
        else if ( $req_tx == "mod" )
        {
            if ( $res_cd == "0000" )
            {
				...
				session_start();
				$db = new Database;
				//$sql = "update receipt_result set m_rcash_noappl = '$receipt_no' , m_pgAuthDate = '$app_time', m_tid = '$cash_no',m_rcr_price= '$mod_mny' where oid = '".$ordr_idxx."' ";

				//$db->query($sql);
				if($mod_type_str == "2" || $mod_type_str == "3"){
					$sql = "insert into receipt_modify_history (mh_ix, tid, modify_type, company_id, admin_id, rprice, price, regdate) values ('','".$cash_no."','".$mod_type_str."','".$admininfo[company_id]."','".$admininfo[admin_id]."','".$rem_mny."','".$mod_mny."',NOW())";
	...
	//echo iconv("CP949","UTF-8",$res_msg);
	//exit;
    /* ============================================================================== */
    /* =   07. 폼 구성 및 결과페이지 호출                                           = */
    /* ============================================================================== */
