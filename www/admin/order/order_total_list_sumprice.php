...
$Contents .= "
											...
											<tr height=30>
												...
												<td class='search_box_item' style='text-align:left;' colspan='0'>
...
<table width='100%' border='0' cellpadding='3' cellspacing='0' align='center' class='this_board' >
	<tr>";
...
if(count($o_array)>0){
    ...
    $sql = "
   	SELECT od.oid
            ...
            , od.pname
            , option_text
            , (case when os.pcount IS null then od.pcnt ELSE os.pcount  END) AS pcnt
            , SUM(dcprice) AS product_price
            , SUM(pt_dcprice) AS total_price
            ,p.id
    FROM shop_order_detail od
    LEFT JOIN shop_order_set os ON (os.oid=od.oid AND os.shop_order_set_id=od.set_group)
    inner join shop_product p on od.pid = p.id
    INNER JOIN shop_order o ON od.oid = o.oid
    ".$where."
        AND od.gift_type = 'N'
        ".$where_op."
    GROUP BY od.oid, od.pid, od.set_group
    ORDER BY od.order_from ASC, od.oid ASC, od.pid ASC, od.set_group ASC
   ";


    $wh = "od.ic_date >= DATE_FORMAT('".$startDate."','%Y-%m-%d') 
		AND  od.ic_date <= DATE_FORMAT('".$endDate." 23:59:59','%Y-%m-%d %H:%i:%s')
		AND  od.status = 'IC'  
	".$where_op."";

    $Contents .= "<tr>
								<td class='' style='#fffheight:30px;font-weight:bold;' class=blue >
								";
    $view_content = "
									<table width='100%' border='1' cellpadding='0' cellspacing='0' align='center' class='data_table' style='display: border-collapse: separate; border-spacing: 1px; background: #c5c5c5; border: 1px;' >
									    <thead>
									    <tr style='border-color: silver;'>
									        <th width='150px' style='background: #efefef;'>제휴사</th>
									        <th width='*' style='background: #efefef;'>상품명</th>
									        <th width='160px' style='background: #efefef;'>상품금액(옵션추가금액)</th>
									        <th width='100px' style='background: #efefef;'>판매수량</th>
									        <th width='100px' style='background: #efefef;'>실결제금액</th>
									    </tr>	
									    </thead>					
										";
    ...
    for ($i = 0; $i < count($order_list); $i++){
        if($order_list[$i][oid] != $b_oid){
            ...
            $img_str = PrintImage($admin_config[mall_data_root]."/images/product", $order_list[$i][id], "s", $order_list[$i]);
            $img_str = "<img src='".$img_str."' width='30' height='30'>";
            if($_GET['excel'] == "act") { $img_str =""; }
            $view_content .= "              <tr style='border-color: silver;'>
											<td align='center' style='background: #ffffff'>".sellertool_site_find_code($order_list[$i][order_from], &$db)."</td>
											<td  align='left'  style='background: #ffffff'><div style='margin-left: 5px; width: 100%; height: 55px; display:inline;'>
											    <div style='float:left;'>".$img_str."</div> <div style='float:left; margin-top: 5px; margin-left: 15px; width:80%;'>".$order_list[$i][pname]."**</div></div>
										    </td>
											<td  align='center'  style='background: #ffffff'>".number_format($order_list[$i][product_price])."</td>
											<td  align='center'  style='background: #ffffff'>".number_format($order_list[$i][pcnt])."</td>
											<td  align='center'  style='background: #ffffff'>".number_format($order_list[$i][total_price])."</td>
										</tr>
							";
            $sum_product_price = $sum_product_price + $order_list[$i][product_price];
            $sum_pcnt = $sum_pcnt + $order_list[$i][pcnt];
            $sum_total_price = $sum_total_price + $order_list[$i][total_price];

            //옵션 검색
            $data = make_option_tr($order_list[$i][oid], $order_list[$i][set_group], &$db);
            $view_content .= $data;
            //print_r($data);
        }
    }
    ...
    $view_content .= "
                                        <tr style='border-color: silver;'>
											<td align='center' style='background: #efefef;' colspan='2'>합계</td>
											<td  align='center'  style='background: #fff'>".number_format($sum_product_price)."</td>
											<td  align='center'  style='background: #fff'>".number_format($sum_pcnt)."</td>
											<td  align='center'  style='background: #fff'>".number_format($sum_total_price)."</td>
										</tr>
    ";
    ...
    $view_content = "
									<table width='100%' border='1' cellpadding='0' cellspacing='0' align='center' class='data_table'  style='display: border-collapse: separate; border-spacing: 1px; background: #c5c5c5; border: 1px;'>
									    <thead>
									     <tr style='border-color: silver;'>
									        <th width='150px' style='background: #efefef;'>제휴사</th>
									        <th width='*' style='background: #efefef;'>상품명</th><th width='160px' style='background: #efefef;'>상품금액(옵션추가금액)</th>
									        <th width='100px' style='background: #efefef;'>판매수량</th>
									        <th width='100px' style='background: #efefef;'>실결제금액</th>
									    </tr>	
									    </thead>					
										";
    if($mode=="search"){
        $view_content .= "<tr height=50 style='border-color: silver;'><td colspan='5' align=center style='background: #fff'>조회된 결과가 없습니다.</td></tr>";
    }else{
        $view_content .= "<tr height=50 style='border-color: silver;'><td colspan='5' align=center style='background: #fff'>조회하시고자 하는 검색조건을 선택후 검색해주세요.</td></tr>";
    }
}
...
if($_GET['excel'] == "act") {
    ...
    header('Content-Disposition: attachment;filename="' . iconv("UTF-8", "CP949", "종합통계현황") . '_' . date("Ymd") . '.xls"');
...
<form id='form_excels' action='./order_total_list_sumprice.php' method='get'>
 ...
 * @param $db
 */
function make_option_tr($oid, $set_group, $db){
    $data = "";

    if($oid) {
        $sql = "  select * from shop_order_detail where oid = '".$oid."' ";
        if($set_group) {
            $sql .= " AND set_group = '".$set_group."'";
        }
        $db->query($sql);
        $rows = $db->fetchAll("object");

//        if($oid == '201902281652-0000017'){
//            echo $sql;
//        }
        ...
        if($rows) {
            ...
            foreach ($rows as $key => $val) {
                if(!$val[option_text])$val[option_text] = $val[pname];
                $data .= "<tr style='border-color: silver;'>
                        <td style='background: #efefef'></td>
                        <td align='left' style='background: #efefef'><span style='margin-left: 5px;     word-break: break-all;'>" . $val[option_text] . "</span></td>
                        <td align='center' style='background: #efefef'>" . number_format($val[psprice]) . "</td>
                        <td align='center' style='background: #efefef'>" . number_format($val[pcnt]) . "</td>
                        <td align='center' style='background: #efefef'>" . number_format($val[ptprice]) . "</td>
                     </tr>";
            }
