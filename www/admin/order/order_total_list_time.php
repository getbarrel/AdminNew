...
SELECT 
	...
	, od.ic_date
   , date_format(od.ic_date, '%H') AS times
...
WHERE 
	...
	AND  od.status = 'IC' 
GROUP BY date_format(od.ic_date, '%H') 
...
<table width='100%' cellpadding='0' cellspacing='0' border=0>
	<tr>
		<td align='left' colspan=6 > ".GetTitleNavigation($title_str, "매출액(시간대) > $title_str ")."</td>
	</tr>
</table>
<table width='100%' cellpadding='0' cellspacing='0' border=0>
	<tr>
...
$view_data = "
        <table width='100%' border='1' cellpadding='3' cellspacing='0' align='center'  class='data_table' style='display: border-collapse: separate; border-spacing: 1px; background: #c5c5c5; border: 1px;'>
            <thead>
                <tr style='border-color: silver;'><th rowspan='2' style='background: #efefef;'>시간</th><th colspan='3' style='background: #efefef;'>매출(입금예정 제외)</th></tr>
...
if($table_list) {
    foreach ($table_list as $key => $val) {
        $tr .= "<tr style='border-color: silver;'>
                <td style='background: #fff;'>" . $val['times'] . "</td>
...
if($_GET['excel'] == "act") {
    ...
    header('Content-Disposition: attachment;filename="' . iconv("UTF-8", "CP949", "매출액(시간별)") . '_' . date("Ymd") . '.xls"');
...
<form id='form_excels' action='./order_total_list_time.php' method='get'>
...
if($mmode == "personalization"){
    ...
    //$P->addScript = "<script language='javascript' src='../order/orders.js'></script>\n<script language='JavaScript' src='../ckeditor/ckeditor.js'></script>\n".$Script; //필요없음
    $P->addScript = $js;
    $P->Navigation = "주문관리 > 매출종합 분석 > 매출액(시간대)";
    $P->title = "매출액(시간대)";
    $P->strContents = $Contents;
    echo $P->PrintLayOut();
}
