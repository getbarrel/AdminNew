...
function idsearch_in(formname,a,b,c,d,e,f,g,h,i,j,k,l,m,n,id,gp_ix,page_type){
	var obj = formname;
	var cArr = c.split('-');
	var fArr = f.split('-');
	var gArr = g.split('-');
	...
	if(page_type == 'estimates'){
		obj.buser_id.value = id;
		obj.bmem_group.value = gp_ix;
	}
								...
								<td class='box_05' align=center>
									...
									<table border='0' width='100%' cellspacing='1' cellpadding='0'>
										...
										<tr height='30' valign='middle'>
											...
											<td align='center' >
												<input type=text name='search_text' class='textbox' value='".$search_text."' style='width:90%;font-size:12px;padding:1px;' >
											</td>
											<td>
...
if($act == "search"){
	if ($admininfo[mall_type] == "O"){
		if($db->dbms_type == "oracle"){
			$where = " where cu.code = cmd.code and cmd.gp_ix = mg.gp_ix and gp_level != 0 and cu.mem_type in ('M','C','A') ";
		}else{
			$where = " where cu.code != '' and cu.code = cmd.code and cmd.gp_ix = mg.gp_ix and gp_level != 0 and cu.mem_type in ('M','C','A') ";
		}
	}else{
		if($db->dbms_type == "oracle"){
			$where = " where cu.code = cmd.code and cmd.gp_ix = mg.gp_ix and gp_level != 0 and cu.mem_type in ('M','C','A') ";
	...
	if($db->dbms_type == "oracle"){
		$sql = "SELECT mg.gp_name, cmd.code, AES_DECRYPT(cmd.name,'".$db->ase_encrypt_key."') as name, cu.id,cmd.gp_ix,
					AES_DECRYPT(cmd.tel,'".$db->ase_encrypt_key."') tel, AES_DECRYPT(cmd.pcs,'".$db->ase_encrypt_key."') pcs,
					AES_DECRYPT(cmd.zip,'".$db->ase_encrypt_key."') as zip,
					AES_DECRYPT(cmd.addr1,'".$db->ase_encrypt_key."') as addr1, AES_DECRYPT(cmd.addr2,'".$db->ase_encrypt_key."') as addr2, AES_DECRYPT(cmd.mail,'".$db->ase_encrypt_key."') as mail
					FROM ".TBL_COMMON_USER." cu, ".TBL_COMMON_MEMBER_DETAIL." cmd, ".TBL_SHOP_GROUPINFO." mg
					".$where."
					ORDER BY name";
	}else{
		$sql = "SELECT mg.gp_name, cmd.code, AES_DECRYPT(UNHEX(cmd.name),'".$db->ase_encrypt_key."') as name, cu.id,cmd.gp_ix,
	...
	if($db->total){
		for($i=0;$i < $db->total ; $i++){
			...
			$Contents .= "
					<tr height=25 bgcolor='#F8F9FA' style='cursor:pointer;' onMouseOver=\"this.style.backgroundColor='#E8ECF1'; this.style.cursor='hand';\" onMouseOut=\"this.style.backgroundColor='';\" onClick=\"javscript:idsearch_in(opener.document.order_form, '$code', '$name','$zip','$addr1','$addr2','$tel','$pcs','$mail','$sc_nm','$com_number','$zip_code','$cs_addr1','$cs_addr2','$com_ceo','$id','$gp_name','$page_type');\">
