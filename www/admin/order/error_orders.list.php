...
if ($startDate == ""){
	//100일에서 14일로 수정 16 02 16
	$before10day = mktime(0, 0, 0, date("m")  , date("d")-14, date("Y"));
...
<table width='100%' border='0' cellpadding='3' cellspacing='0' align='center' >
 <tr>";
	...
	*/
	...
	//and od.regdate between '".$startDate."' and '".$endDate."'   ->   o.order_date between '".$startDate." 00:00:00' and '".$endDate." 23:59:59' 16 02 16 for 속도향상
	//date_format(regdate,'%Y-%m-%d') or date_format(od.regdate,'%Y-%m-%d')
	$sql = "SELECT oid, count(*) as cnt, regdate
			...
			SELECT 
				distinct o.oid, 1 as cnt,
				o.order_date as regdate
			FROM
				".TBL_SHOP_ORDER." o,
				".TBL_SHOP_ORDER_DETAIL." od
				left join shop_order_payment as op on (od.oid = op.oid and op.pay_type = 'G')
			WHERE 
				o.oid = od.oid and od.status not in ('SR') 
				and o.order_date between '".$startDate." 00:00:00' and '".$endDate." 23:59:59'
				and od.order_from in ('self','offline','pos')
				and od.co_oid!=''
				and (op.tid = '' or authcode = '')
			...
			FROM
				".TBL_SHOP_ORDER." o,
				".TBL_SHOP_ORDER_DETAIL." od
				inner join shop_order_payment as op on (od.oid = op.oid and op.pay_type = 'G')
			WHERE 
				o.oid = od.oid and od.status not in ('SR','IR','IB') 
				and o.order_date between '".$startDate." 00:00:00' and '".$endDate." 23:59:59'
				and od.order_from in ('self','offline','pos')
				and op.tid != '' 
				and op.authcode = ''
				and op.method in ('1')
				and op.pay_status != 'IR'
				ORDER BY regdate DESC";

	$db->query($sql);
	$oid_array = $db->fetchall("object");
	...
	$sql = "SELECT distinct o.oid
			...
			$where 
			group by od.oid
			having pt_dcprice != product_price ";
	$db->query($sql);
	$total = $db->total;
...
$Contents .= "<td colspan=3 align=left></td><td colspan=10 align=right>";
...
if($db->total){
	...
	$sql = "SELECT 
		...
		od.od_ix, od.product_type, od.pid, od.pname, 
	...
	where
		...
		$addWhere
		ORDER BY o.oid desc, od.delivery_type, delivery_package, od.ori_company_id asc";
	...
	for ($i = 0; $i < count($order_list); $i++){
		...
		if($_SESSION["admininfo"]["charger_id"]=="forbiz"){
			...
			}elseif ($order_list[$i][status] != ORDER_STATUS_CANCEL_COMPLETE && $order_list[$i][method] == "1"){
								...
								<td class='' style='background-color:#fff7da;height:30px;font-weight:bold;' class=blue colspan='".($admininfo[admin_level]==9 ? "10" : "9")."' >
									<table width='100%' border='0' cellpadding='0' cellspacing='0' align='center' >
										...
										<tr>
											...
											if($admininfo[admin_level]==9){
												$Contents .= "
												<td  align='center' style='background-color:#fff7da;'><font color='#000000' class=small><b>".getOrderFromName($order_list[$i][order_from])."</b></font></td>";
											}
											...
											<td  align='center'  style='background-color:#fff7da;'>
												...
												<span class='helpcloud' help_width='55' help_height='15' help_html='주문서'>
													<img src='../images/icon/paper.gif' style='cursor:pointer' align='absmiddle' onclick=\"PopSWindow('../order/orders.read.php?oid=".$order_list[$i][oid]."&pid=".$order_list[$i][pid]."&mmode=pop',960,600,'order_read')\"/>
												</span>
											</td>
											<td  align='center'  style='background-color:#fff7da;'>
											...
											if($admininfo[admin_level]==9){
												$Contents .= "<td  align='center'  style='background-color:#fff7da;'nowrap><font color='red' class=small><b>".$currency_display[$admin_config["currency_unit"]]["front"]." ".number_format($order_list[$i][total_price])." ".$currency_display[$admin_config["currency_unit"]]["back"]."</b></font></td>";
											}
										...
										if(checkMenuAuth(md5($_SERVER["PHP_SELF"]),"U")){
											...
											if($mmode == "personalization"){
												$Contents .= "<a href=\"javascript:PopSWindow('../order/orders.edit.php?oid=".$order_list[$i][oid]."&pid=".$order_list[$i][pid]."&mmode=".$mmode."&mem_ix=".$mem_ix."',960,600,'order_edit');\" ><img src='../images/".$admininfo["language"]."/btn_invoice.gif' border=0 align=absmiddle style='margin:2px;'></a> ";
											}else{
		...
		$Contents .= "
						<tr class='order_list_tr_".$order_list[$i][oid]."' style='display:none;'>
							<td >
								<TABLE>
									<TR>
										<TD align='center'>
				...
				if($order_list[$i][product_type]=='99'||$order_list[$i][product_type]=='21'||$order_list[$i][product_type]=='31'){
					$Contents .= "<b class='".($order_list[$i][product_type]=='99' ? "red" : "blue")."' >".$order_list[$i][pname]."</b><br/><strong>".$order_list[$i][set_name]."<br/></strong>".$order_list[$i][sub_pname];
				}else{
					$Contents .= $order_list[$i][pname];
				}
				...
				if($order_list[$i][delivery_package] == 'N' && $order_list[$i][delivery_pay_method] == '1'){	//묶음이거나 선불일겨우 (착불은 개별로 처리)

					if($b_oid != $order_list[$i][oid] || $bori_company_id != $order_list[$i][ori_company_id]){
						$sql = "SELECT 
							...
							where 
								...
								and od.ori_company_id='".$order_list[$i][ori_company_id]."'
								and od.delivery_type = '".$order_list[$i][delivery_type]."'
								and od.delivery_package = 'N'
								and od.delivery_method = '".$order_list[$i][delivery_method]."'
								and od.delivery_pay_method = '".$order_list[$i][delivery_pay_method]."'
								and od.delivery_addr_use = '".$order_list[$i][delivery_addr_use]."'
								and od.factory_info_addr_ix = '".$order_list[$i][factory_info_addr_ix]."'
								$addWhere 
								";
						...
						$Contents .="<td class='' align=center style='line-height:140%;' rowspan='".$com_cnt."'>
							".number_format($order_list[$i][delivery_totalprice])."원<br>
							".($order_list[$i][delivery_type] == '1' ? "통합배송":"입점업체배송")."<br>
							".$delivery_method."<br>
							".($order_list[$i][delivery_addr_use] == '1' ? "출고지별 배송<br>".getFactoryAddrName($order_list[$i][factory_info_addr_ix])."<br>":"")."
							".$delivery_pay_type."<br>
							".($order_list[$i][delivery_package] == 'N' ? "묶음배송":"")."</td>";
					}

					$bori_company_id = $order_list[$i][ori_company_id];
					...
					if(ch_set_product_bool($order_list[$i][option_kind])){		//세트상품일경우

						if($b_oid != $order_list[$i][oid] || $bproduct_id != $order_list[$i][pid]){

							$Contents .="<td class='' align=center style='line-height:140%;' ".($b_oid != $order_list[$i][oid] || $bproduct_id != $order_list[$i][pid] ? "rowspan='".$com_cnt."'":"").">
								".number_format($order_list[$i][delivery_totalprice])."원<br>
								".($order_list[$i][delivery_type] == '1' ? "통합배송":"입점업체배송")."<br>
								".$delivery_method."<br>
								".($order_list[$i][delivery_addr_use] == '1' ? "출고지별 배송<br>".getFactoryAddrName($order_list[$i][factory_info_addr_ix])."<br>":"")."
						...
						if($b_oid != $order_list[$i][oid] || $bproduct_id != $order_list[$i][pid] || $bset_group != $order_list[$i][set_group]){	//rowspan='".$com_cnt."'
							$Contents .="<td class='' align=center style='line-height:140%;' >
								".number_format($order_list[$i][delivery_totalprice])."원<br>
								".($order_list[$i][delivery_type] == '1' ? "통합배송":"입점업체배송")."<br>
								".$delivery_method."<br>
								".($order_list[$i][delivery_addr_use] == '1' ? "출고지별 배송<br>".getFactoryAddrName($order_list[$i][factory_info_addr_ix])."<br>":"")."
	...
	$Contents .= "<tr height=50><td colspan='".($_SESSION["admininfo"]["admin_level"]==9 ? "10" : "9")."' align=center>조회된 결과가 없습니다.</td></tr>";
}
	...
	$help_text .= "
			...
			<td valign=top style='padding:0 0 0 10px'>
				<table cellpadding=0 cellspacing=0 >
					...
					<tr height=22><td align=left class=small nowrap>{name} : </td><td nowrap> 고객명( 수취인or주문자) </td></tr>
...
if($mmode == "personalization"){
	...
	$P->Navigation = "주문관리 > 비정상주문리스트";
	$P->title = "비정상주문리스트";
    $P->NaviTitle = "비정상주문리스트"; 
