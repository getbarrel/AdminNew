...
include("../include/campaign.lib.php");
	...
	if($_GET["memreg_sdate"] && $_GET["memreg_edate"]){
		$memreg_sdate = $_GET["memreg_sdate"];
		$memreg_edate = $_GET["memreg_edate"];

		$where .= "and date_format(cu.date,'%Y%m%d') between $memreg_sdate and $memreg_edate ";
	}
	...
	if($_GET["sdate"] && $_GET["edate"]){
		...
		$order_where = "and ".$date_str." between $startDate and $endDate ";
		$where .= $order_where;
	}else{
		$sdate = date("Ymd");
		$edate = date("Ymd");

		$order_where = "and ".$date_str." between $sdate and $edate ";
		$where .= $order_where;
	}
...
function ReportTable($vdate,$SelectReport=1){
	...
	global $all_sale_status, $cancel_status, $return_status;
	...
	if($SelectReport == 1 || true){
		if($fordb->dbms_type == "oracle"){//uid 안됨
			...
			*/
			$sql = "Select AES_DECRYPT(UNHEX(cmd.name),'".$db->ase_encrypt_key."') as name, date_format(od.regdate,'%Y%m%d') as vdate, date_format(od.regdate,'%H') as vtime,  
					cu.id as userid, o.uid, od.pcnt as vquantity , od.pname, od.pid,  od.psprice as sellprice
					...
					and o.uid != '' and o.uid != '6955'
					".$where."
					order by date_format(od.regdate,'%Y%m%d') ";
		}else{
			/*
			$sql = "Select AES_DECRYPT(UNHEX(cmd.name),'".$db->ase_encrypt_key."') as name, cu.id as userid,c.pname, a.*
				...
				where  a.pid = c.id and cu.code = cmd.code and vdate = '$vdate' and step6 = 1 order by a.vdate, vtime";
			*/
				$sql = "Select AES_DECRYPT(UNHEX(cmd.name),'".$db->ase_encrypt_key."') as name, date_format(od.regdate,'%Y%m%d') as vdate, date_format(od.regdate,'%H') as vtime,  cu.date as mem_regdate, mg.gp_name , cu.visit, cmd.order_total_price, 
							cu.id as userid, o.uid, count(*) as order_cnt,  sum(od.pcnt) as vquantity ,  sum(od.ptprice-od.member_sale_price-od.use_coupon) as sellprice ";
				//echo DateDiff();
				$startDate_unixtimestamp = mktime(0,0,0,substr($startDate,4,2),substr($startDate,6,2),substr($startDate,0,4));
				$endDate_unixtimestamp = mktime(0,0,0,substr($endDate,4,2),substr($endDate,6,2),substr($endDate,0,4));
...
//echo "endDate_unixtimestamp:".$endDate_unixtimestamp."<br>";
				...
				for($i = 0; $i <= $nLoop;$i++){
					$LoopDate = date("Ymd",mktime(0,0,0,substr($startDate,4,2),substr($startDate,6,2),substr($startDate,0,4))+60*60*24*$i);
		$sql .= ",sum(case when date_format(od.regdate,'%Y%m%d') = '".$LoopDate."' then od.ptprice-od.member_sale_price-od.use_coupon else 0 end) as sellprice_".$LoopDate."
					";
				}
				$sql .= "from ".TBL_SHOP_ORDER." o 
							right join (
								select oid, regdate, sum(od.pcnt) as pcnt, sum(psprice) as psprice , sum(ptprice) as ptprice  , sum(member_sale_price) as member_sale_price  , sum(use_coupon) as use_coupon 
								";
				...
				$sql .= "	from ".TBL_SHOP_ORDER_DETAIL." od 
							...
							right join ".TBL_COMMON_USER." cu on o.uid = cu.code
							right join  ".TBL_COMMON_MEMBER_DETAIL." cmd on  cu.code = cmd.code
							left join ".TBL_SHOP_GROUPINFO." mg on cmd.gp_ix = mg.gp_ix
							where o.uid != '' and o.uid != '6955' 
							".$where."
							group by o.uid
		...
		if($fordb->dbms_type == "oracle"){
			...
			*/
			$sql = "Select AES_DECRYPT(UNHEX(cmd.name),'".$db->ase_encrypt_key."') as name, date_format(od.regdate,'%Y%m%d') as vdate, date_format(od.regdate,'%H') as vtime,  
				...
				left join ".TBL_COMMON_USER." cu on o.uid = cu.code
				left join  ".TBL_COMMON_MEMBER_DETAIL." cmd on  cu.code = cmd.code
				where date_format(od.regdate,'%Y%m') LIKE '".substr($vdate,0,6)."%'  
				and o.uid != '' and o.uid != '6955'
				".$where."
				order by date_format(od.regdate,'%Y%m%d') ";
...
if($admininfo[admin_id] == "forbiz"){
	...
	$mstring .= "<tr height=40 align=center style='font-weight:bold'>
			...
			for($i = 0; $i <= $nLoop;$i++){
					$w = date("w",mktime(0,0,0,substr($startDate,4,2),substr($startDate,6,2),substr($startDate,0,4))+60*60*24*$i);
					$LoopDate = date("y.m.d D",mktime(0,0,0,substr($startDate,4,2),substr($startDate,6,2),substr($startDate,0,4))+60*60*24*$i);
	...
	if($fordb->total == 0){
		$mstring = $mstring."<tr height=150 bgcolor=#ffffff align=center><td colspan=14>결과값이 없습니다.</td></tr>\n";
	}else{
		$sale_sum =0;
		...
		for($i=0;$i<$fordb->total;$i++){
			...
			<td class='list_box_td'><input type=checkbox name=code[] id='code' value='".$fordb->dt[uid]."'></td>
			<td class='list_box_td'>".($i+1)."</td>
			<td class='list_box_td '  align=center onmouseover=\"mouseOnTD('".$i."',true)\" onmouseout=\"mouseOnTD('$i',false)\">".$fordb->dt[gp_name]."</td>
			<td class='list_box_td point'  onmouseover=\"mouseOnTD('".$i."',true)\" onmouseout=\"mouseOnTD('$i',false)\" >							
			<a href=\"javascript:PoPWindow('../../member/member_view.php?code=".$fordb->dt[uid]."',950,700,'member_view')\">".$fordb->dt[name]."</a></td>
	...
	*/
	...
	//			echo "sellprice_".$LoopDate."<br>";
				...
				$mstring .= "<td class='e_td number'>".number_format($$sum_name)."</td>";
