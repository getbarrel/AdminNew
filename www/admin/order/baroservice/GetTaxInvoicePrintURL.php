	...
	try {
		...
		if ($client->fault) {
			...
			if ($err) {
				...
				$GetTaxInvoicePrintURLResult = $result['GetTaxInvoicePrintURLResult'];	
				if( $GetTaxInvoicePrintURLResult >-1) {
					echo "URL : ". $GetTaxInvoicePrintURLResult;
				} else {
					$errCode = errCodeCheck($GetTaxInvoicePrintURLResult);
						echo $errCode;
				}
