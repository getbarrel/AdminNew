<?php
	...
	try {
		...
		*/
		// Soap 문장 전송
		$result = $client->call('CheckCERTIsValid', '', '', '', false, true);
		...
		if ($client->fault) {
			...
			if ($err) {
				...
				$CheckCERTIsValidResult = $result['CheckCERTIsValidResult'];	
				if( $CheckCERTIsValidResult >-1) {
					echo $CheckCERTIsValidResult;
				} else {
					$errCode = errCodeCheck($CheckCERTIsValidResult);
					echo $errCode;
				}
