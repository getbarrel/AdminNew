	...
	try {
		...
		if ($client->fault) {
			...
			if ($err) {
				...
				$GetTaxInvoicePopUpURLResult = $result['GetTaxInvoicePopUpURLResult'];	
				if( $GetTaxInvoicePopUpURLResult >-1) {
					echo "URL : ". $GetTaxInvoicePopUpURLResult;
				} else {
					$errCode = errCodeCheck($GetTaxInvoicePopUpURLResult);
						echo $errCode;
				}
