<?php
	...
	try {
		...
		// Soap 문장 전송
		$result = $client->call('UpdateUserPWD', $params, '', '', false, true);
		...
		if ($client->fault) {
			...
			if ($err) {
				...
				$UpdateUserPWDResult = $result['UpdateUserPWDResult'];
				if( $UpdateUserPWDResult == 1) {
					echo "수정성공";
				} else {
					$errCode = errCodeCheck($UpdateUserPWDResult);
						echo $errCode;
				}
			}
		}
