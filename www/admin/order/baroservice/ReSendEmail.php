<?php
	...
	try {
		...
		$params = array(
			...
			ToEmailAddress	=> "tester2@pallet.pe.kr"
		);

		// Soap 문장 전송
		$result = $client->call('ReSendEmail', $params, '', '', false, true);
		...
		if ($client->fault) {
			...
			if ($err) {
				...
				$ReSendEmailResult = $result['ReSendEmailResult'];
				if( $ReSendEmailResult == 1) {
					echo "재전송 요청 성공";
				} else {
					$errCode = errCodeCheck($ReSendEmailResult);
						echo $errCode;
				}
			}
		}
