<?php
	...
	try {
		...
		$params = array(
			...
			CorpNum			=> "4168138772",
			DocType			=> 2,
			MgtKey			=> "INTEROP20100830144325"
			
		);

		// Soap 문장 전송
		$result = $client->call('GetLinkedDocs', $params, '', '', false, true);
		...
		if ($client->fault) {
			...
			if ($err) {
				...
				$GetLinkedDocsResult = $result['GetLinkedDocsResult'];				
				if( $GetJicInRegistURLResult  > -1) {
					
				} else {
					
						$errCode = errCodeCheck($GetLinkedDocsResult);
						echo $errCode;
				}
			}
		}
