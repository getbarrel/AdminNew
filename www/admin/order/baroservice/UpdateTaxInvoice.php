<?php
  ...
  $NowTime = date("YmdHis", time()); 	
	try {		
		...
		// Soap 문장 전송		
		$result = $client->call('UpdateTaxInvoice', $params, '', '', false, true);
		...
		if ($client->fault) {
			...
			if ($err) {
				...
				if ( $result['UpdateTaxInvoiceResult'] < 0 ) {
					$errCode = errCodeCheck($result['UpdateTaxInvoiceResult']);
						echo $errCode;
				} else {
					echo $result['UpdateTaxInvoiceResult'];
				}
