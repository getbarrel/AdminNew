<?php
	...
	try {
		...
		// Soap 문장 전송
		$result = $client->call('GetLoginURL', $params, '', '', false, true);
		...
		if ($client->fault) {
			...
			if ($err) {
				...
				$GetLoginURLResult = $result['GetLoginURLResult'];
				
				if( $GetLoginURLResult) {
					echo "URL주소 반환 : ". $GetLoginURLResult;
				} else {
					$errCode = errCodeCheck($GetLoginURLResult);
						echo $errCode;
				}
			}
		}
