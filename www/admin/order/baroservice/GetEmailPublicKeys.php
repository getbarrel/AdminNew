<?php
	...
	try {
		...
		// Soap 문장 전송
		$result = $client->call('GetEmailPublicKeys', $params, '', '', false, true);
		...
		if ($client->fault) {
			...
			if ($err) {
				...
				$GetEmailPublicKeysResult = $result['GetEmailPublicKeysResult'];
				if( $GetEmailPublicKeysResult  > -1) {
					
				} else {
					
						$errCode = errCodeCheck($GetEmailPublicKeysResult);
						echo $errCode;
				}
			}
		}
