<?php
	...
	try {
		...
		// Soap 문장 전송
		$result = $client->call('UpdateCorpInfo', $params, '', '', false, true);
		...
		if ($client->fault) {
			...
			if ($err) {
				...
				$UpdateCorpInfoResult = $result['UpdateCorpInfoResult'];
				if( $UpdateCorpInfoResult == 1) {
					echo "수정성공";
				} else {
					$errCode = errCodeCheck($UpdateCorpInfoResult);
						echo $errCode;
				}
			}
		}
