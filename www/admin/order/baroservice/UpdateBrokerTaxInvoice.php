<?php
  ...
  $NowTime = date("YmdHis", time()); 	
	try {		
		...
		// Soap 문장 전송		
		$result = $client->call('UpdateBrokerTaxInvoice', $params, '', '', false, true);
		...
		if ($client->fault) {
			...
			if ($err) {
				...
				if ( $result['UpdateBrokerTaxInvoiceResult'] < 0 ) {
					$errCode = errCodeCheck($result['UpdateBrokerTaxInvoiceResult']);
						echo $errCode;
				} else {
					echo "수정완료.";
				}
