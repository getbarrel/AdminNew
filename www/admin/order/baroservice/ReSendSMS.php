<?php
	...
	try {
		...
		$params = array(
			...
			ToCorpNum		=> "1231212312",
			ToName			=> "tester",
			ToNumber		=> "01000000000",
			Contents		=> "test"
		);

		// Soap 문장 전송
		$result = $client->call('ReSendSMS', $params, '', '', false, true);
		...
		if ($client->fault) {
			...
			if ($err) {
				...
				$ReSendSMSResult = $result['ReSendSMSResult'];
				if( $ReSendSMSResult == 1) {
					echo "재전송 요청 성공";
				} else {
					$errCode = errCodeCheck($ReSendSMSResult);
						echo $errCode;
				}
			}
		}
