<?php
	...
	try {
		...
		// Soap 문장 전송
		$result = $client->call('GetNTSSendOption', $params, '', '', false, true);
		...
		if ($client->fault) {
			...
			if ($err) {
				...
				$GetNTSSendOptionResult = $result['GetNTSSendOptionResult'];				
				if( $GetNTSSendOptionResult) {
					
				} else {
					$errCode = errCodeCheck($GetNTSSendOptionResult);
						echo $errCode;
				}
			}
		}
