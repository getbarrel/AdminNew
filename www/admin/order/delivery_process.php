...
$Contents .= "
						...
						if($admininfo[admin_level] == 9){
							...
							if($view_type == 'inventory'){
								...
								$Contents .= "
										<tr height=30>
											<th class='search_box_title'>조건검색 
	...
	*/
	...
	if(checkMenuAuth(md5($_SERVER["PHP_SELF"]),"E")){
		...
		*/
		...
		onclick=\"if(jQuery('#oet_ix').val().length > 0){ ig_excel_dn_chk('../order/orders_excel2003.php?oet_ix='+jQuery('#oet_ix').val()+'&excel_type=delivery&view_type=$view_type&pre_type=$pre_type&invoice_no_bool=$invoice_no_bool&stock_use_yn=$stock_use_yn&is_check_delivery=$is_check_delivery&".$type_param."&".$delivery_type_param."&".$QUERY_STRING."'); }else{alert('엑셀양식을선택해주세요.');}\" >
		</span>";
	...
	for ($i = 0; $i < $db1->total; $i++)
	{
		...
		for($j=0;$j < $ddb->total;$j++){
			...
			if($ddb->dt[oid] != $b_oid){
				...
				$b_mem_info = "<b style='cursor:pointer' class='helpcloud' help_width='230' help_height='100' help_html='주문자 <br/>ID/성명 : ".$ddb->dt[buserid]."/".wel_masking_seLen($ddb->dt[bname], 1, 1)."<br/>핸드폰 : ".$ddb->dt[bmobile]." <br/>회원그룹 : ".$ddb->dt[mem_group]." <br/>최근로그인 : ".$u_etc_info["user_last"]." <br/>최근주문(30일) : ".$u_etc_info["user_order_cnt"]."건' />".Black_list_check($ddb->dt[user_id],wel_masking_seLen($ddb->dt[bname], 1, 1).( $ddb->dt[buserid] ? "(<span class='small'>".$ddb->dt[buserid]."</span>)" : "(<span class='small'>비회원</span>)"))."</b> <br/> ".($_SESSION["admininfo"]["admin_level"] > 8 && $ddb->dt[user_id] ? "<img src='../v3/images/".$_SESSION["admininfo"]["language"]."/btn_search.gif' align=absmiddle onclick=\"PopSWindow2('/admin/member/member_cti.php?mmode=pop&code=".$ddb->dt[user_id]."&mmode=pop',1280,800,'member_view')\"  style='cursor:pointer;'>" : "");
