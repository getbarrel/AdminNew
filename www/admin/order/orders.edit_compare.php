...
$sql = "SELECT p.* FROM shop_order_payment p WHERE p.oid = '".$oid."' and p.pay_type = 'G' and pay_status in ('IC','IR') and taxsheet_yn ='Y' limit 0,1";
$odb->query($sql);
$basicPayment = $odb->fetch();


$sql = "SELECT p.*,rr.m_tid FROM shop_order_payment p left join receipt_result rr on (p.oid=rr.oid and p.receipt_yn='Y') 
WHERE p.oid = '".$oid."' and p.pay_type = 'G' and pay_status='IC' and method in ('".ORDER_METHOD_BANK."','".ORDER_METHOD_CARD."','".ORDER_METHOD_PHONE."','".ORDER_METHOD_VBANK."','".ORDER_METHOD_ICHE."','".ORDER_METHOD_MOBILE."','".ORDER_METHOD_NOPAY."') ";
$odb->query($sql);
...
od.order_from, od.rfid,  od.ic_date
FROM ".TBL_SHOP_ORDER." o inner join ".TBL_SHOP_ORDER_DETAIL." as od on (o.oid = od.oid)
...
if($admininfo["admin_level"] == '9'){
    $Contents .= "
                                ".($odb->dt[user_code] ? " <img src='../images/".$admininfo["language"]."/btn_crm.gif' border=0 align=absmiddle onClick=\"PopSWindow('../member/member_cti.php?code=".$odb->dt[user_code]."&mem_ix=".$odb->dt[mem_ix]."&con_view=member',1280,800,'member_view')\" style='cursor:pointer;' alt='고객상담' title='고객상담'/>" :"")."";
}
$Contents .= "
                        </td>
                    ...
                    <tr bgcolor='#ffffff' >
                        ...
                        <td class='input_box_item'>
                            ...
                            ".(trim(str_replace('-','',$odb->dt[bmobile])) !=""? "&nbsp;<img src='../images/".$admininfo["language"]."/btn_sms.gif' align=absmiddle onclick=\"PoPWindow('/admin/member/member_sns_pop.php?pcs='+$('input[name=bmobile]').val()+'&name=".$odb->dt[bname]."&user_id=".$odb->dt[buserid]."&oid=".$odb->dt[oid]."',710,570,'sendsms')\" style='cursor:pointer;' title='SMS보내기'>" : "")."
                        </td>
                    </tr>";

$sql="
                        select
                            odd.odd_ix,odd.order_type,odd.rname,odd.rtel,odd.rmobile,odd.rmail,odd.zip,odd.addr1,odd.addr2,odd.msg
                        from
...
foreach($deliveryinfo as $key => $val){
    ...
    $Contents .= "
                                    ...
                                    <tr>
                                        ...
                                        <td class='input_box_item' style='padding:3px' rowrap>
                                            <span class='helpcloud' help_width='45' help_height='15' help_html='이름'><input type='text' name='deliveryinfo[".$val[odd_ix]."][rname]' class='textbox' value='".$val[rname]."' style='height:20px;width:45px;text-align:center;margin-top:3px;'></span>
                                            <span class='helpcloud' help_width='55' help_height='15' help_html='이메일'><input type='text' name='deliveryinfo[".$val[odd_ix]."][rmail]' class='textbox' value='".$val[rmail]."' style='height:20px;width:135px;margin-top:3px;'></span><br/>
                                            ...
                                            &nbsp;<img src='../images/".$admininfo["language"]."/btn_sms.gif' align=absmiddle onclick=\"PoPWindow('/admin/member/member_sns_pop.php?pcs='+$(this).prev().find('input').val()+'&name=".$odb->dt[bname]."&user_id=".$odb->dt[buserid]."&oid=".$odb->dt[oid]."',710,570,'sendsms')\" style='cursor:pointer;' title='SMS보내기'>
                                        ...
                                        <td class='m_td'>
                                            ...
                                            <img src='../images/".$admininfo["language"]."/btn_search_address.gif' align=absmiddle style='cursor:pointer;' onClick=\"zipcode_front('1')\">
                                        </td>
                                        <td class='input_box_item' style='padding:3px;' id='zip_td_".$key."'>
                                            <input type='text' name='deliveryinfo[".$val[odd_ix]."][zipcode1]' id='zipcode1' size='3' maxlength='3' class='textbox' readonly value='".$zipcode[0]."'> - <input type='text' name='deliveryinfo[".$val[odd_ix]."][zipcode2]' id='zipcode2' size='3' maxlength='3' class='textbox' readonly value='".$zipcode[1]."'>
    ...
    $Contents .= "
                                    <tr id='di_mp_tr_".$key."' style='display:none;' >
                                        <td class='m_td' >상품정보</td>
                                        <td class='input_box_item' style='padding:3px' colspan='3'>";

    $sql="select od_ix,pname,set_name,sub_pname,option_text,msgbyproduct,due_date from shop_order_detail where oid='".$odb->dt[oid]."' and odd_ix = '".$val[odd_ix]."' ".($_SESSION["admininfo"]["admin_level"]!=9 ? " and company_id='".$_SESSION["admininfo"]["company_id"]."' " : "")." ";
    $db->query($sql);
        ...
        $Contents .= "
                                                <table border='0' cellspacing='0' cellpadding='0' style='float:left;min-width:350px;'>
                                                    <tr height='30'>
                                                        ...
                                                        <td>
                                                            &nbsp; ".$dpinfo[pname]." ".$dpinfo[set_name]." ".$dpinfo[sub_pname]." ".strip_tags($dpinfo[option_text])."
                                                        </td>
                                                    </tr>
                                                    <tr height='30'>
                                                        <td colspan='2'>
...
if($basicPayment['taxsheet_yn'] == 'Y'){
    $Contents .= "
                    <tr>
                        <td class='input_box_title'>세금계산서발행용</p>(사업자정보)</td>
                            <td class='input_box_item' colspan='3' style='padding:5px;'>
                                <input type='text' size=25 name='tax_com_number' class='textbox' value='".$basicPayment['tax_com_number']."' validation='false' title='사업자등록번호'>
                                <input type='text' size=25 name='tax_com_ceo' class='textbox' value='".$basicPayment['tax_com_ceo']."' validation='false' title='대표자명'>
                                <input type='text' size=25 name='tax_com_name' class='textbox' value='".$basicPayment['tax_com_name']."' validation='false' title='업체명'>
                            </td>
                        </td>
                    </tr>";
}
...
if($mmode != "pop"){
    $Contents .= "<a href='javascript:history.back();'><img src='../images/".$admininfo["language"]."/btn_back.gif' border='0' align=absmiddle></a> ";
}

if(checkMenuAuth(md5($_SERVER["PHP_SELF"]),"U" && $admininfo[admin_level] == 9)){
...
$Contents .= "
                                    ...
                                    <tr bgcolor='#ffffff' >
                                        ...
                                        <td class='input_box_item'>
                                            &nbsp;".$odb->dt[bmobile]."
                                            ".(trim(str_replace('-','',$odb->dt[bmobile])) !=""? "&nbsp;<img src='../images/".$admininfo["language"]."/btn_sms.gif' align=absmiddle onclick=\"PoPWindow('/admin/member/member_sns_pop.php?pcs=".$odb->dt[bmobile]."&name=".$odb->dt[bname]."&user_id=".$odb->dt[buserid]."&oid=".$odb->dt[oid]."',710,570,'sendsms')\" style='cursor:pointer;' title='SMS보내기'>" : "")."
                                        </td>
                                    </tr>
                                    <tr bgcolor='#ffffff'>
                                        <td class='input_box_title'>수취인정보</td>
...
foreach($deliveryinfo as $key => $val){
    ...
    $Contents .= "
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td class='input_box_item' style='padding:3px;'>
                                                            ".$val[rmobile]." ".(trim(str_replace('-','',$val[rmobile])) !=""? "&nbsp;<img src='../images/".$admininfo["language"]."/btn_sms.gif' align=absmiddle onclick=\"PoPWindow('/admin/member/member_sns_pop.php?pcs=".$val[rmobile]."&name=".$val[rname]."&oid=".$odb->dt[oid]."',710,570,'sendsms')\" style='cursor:pointer;' title='SMS보내기'>" : "")."
                                                        </td>
                                                    </tr>
...
if($admininfo[admin_level] == 9){
    ...
    $Contents .= "
                                        <tr>
                                            ...
                                            <td class='input_box_item'>&nbsp;".substr($odb->dt[ic_date], 0, 10)."<br>(주문번호 : ".$odb->dt[oid].")</td>
                                        </tr>
                                        ...
                                        <tr>
                                            ...
                                            <td class='input_box_item'>&nbsp; ".$currency_display[$admin_config["currency_unit"]]["front"]." ".number_format($total_real_pay_price)." ".$currency_display[$admin_config["currency_unit"]]["back"]."(적립금,장바구니 쿠폰 제외)</td>
                                        </tr>";
    ...
    if($db->total){
        ...
        foreach($dc_info as $dc){
            ...
            if($dc[dc_type]=="DCP" || $dc[dc_type]=="DE"){
                $delivery_discount .=" > ".$_DISCOUNT_TYPE[$dc[dc_type]]." : ".$currency_display[$admin_config["currency_unit"]]["front"]."".number_format($dc[dc_price])."".$currency_display[$admin_config["currency_unit"]]["back"]."";
            }else{
                $product_discount .=" > ".$_DISCOUNT_TYPE[$dc[dc_type]]." : ".$currency_display[$admin_config["currency_unit"]]["front"]."".number_format($dc[dc_price])."".$currency_display[$admin_config["currency_unit"]]["back"]."";
            }
        }
    }
    ...
    $Contents .= "
                                        ...
                                        <col width='10%' />
                                            <tr>
                                                ...
                                                <td class='input_box_title' colspan='6'><b>결제상세내역</b></td>
                                            </tr>
                                            <tr>
                                                ...
                                                <td class='input_box_title'><b>장바구니쿠폰</b></td>
                                                <td class='input_box_title'><b>위포인트</b></td>
                                            </tr>";
    ...
    $sql="select 
                                                ...
                                                (pg_payment_price + saveprice_payment_price + reserve_payment_price + WP_payment_price) as total_payment_price,
                                                pg_payment_price,
                                        ...
                                        from (
                                            select
                                                ...
                                                SUM(case when pay_status='IC' and method not in ('".ORDER_METHOD_SAVEPRICE."','".ORDER_METHOD_RESERVE."','".ORDER_METHOD_CART_COUPON."', '".ORDER_METHOD_WP."') then payment_price else 0 end) as pg_payment_price,
                                                ...
                                                SUM(case when pay_status='IC' and method in ('".ORDER_METHOD_WP."') then payment_price else 0 end) as WP_payment_price
                                            from 
    ...
    if($db->total){
        for($i=0;$i<$db->total;$i++){
            ...
            $Contents .= "<tr>
                                                ...
                                                <td class='input_box_item' style='text-align:right;'>".$currency_display[$admin_config["currency_unit"]]["front"]." ".number_format($db->dt[expect_total_price]*$p_status)." ".$currency_display[$admin_config["currency_unit"]]["back"]."&nbsp;</td>
                                                <td class='input_box_item' style='text-align:right;'>".$currency_display[$admin_config["currency_unit"]]["front"]." ".number_format($db->dt[expect_product_price]*$p_status)." ".$currency_display[$admin_config["currency_unit"]]["back"]."&nbsp;</td>
                                                <td class='input_box_item' style='text-align:right;'>".$currency_display[$admin_config["currency_unit"]]["front"]." ".number_format($db->dt[expect_delivery_price]*$p_status)." ".$currency_display[$admin_config["currency_unit"]]["back"]."&nbsp;</td>
                                                <td class='input_box_item' style='text-align:right;'>".$currency_display[$admin_config["currency_unit"]]["front"]." ".number_format($db->dt[total_payment_price]*$p_status)." ".$currency_display[$admin_config["currency_unit"]]["back"]."&nbsp;</td>
                                                <td class='input_box_item' style='text-align:right;'>".$currency_display[$admin_config["currency_unit"]]["front"]." ".number_format($db->dt[pg_payment_price]*$p_status)." ".$currency_display[$admin_config["currency_unit"]]["back"]."&nbsp;</td>
                                                <td class='input_box_item' style='text-align:right;'>".$currency_display[$admin_config["currency_unit"]]["front"]." ".number_format($db->dt[saveprice_payment_price]*$p_status)." ".$currency_display[$admin_config["currency_unit"]]["back"]."&nbsp;</td>
                                                <td class='input_box_item' style='text-align:right;'>".$currency_display[$admin_config["currency_unit"]]["front"]." ".number_format($db->dt[reserve_payment_price]*$p_status)." ".$currency_display[$admin_config["currency_unit"]]["back"]."&nbsp;</td>
                                                <td class='input_box_item' style='text-align:right;'>".$currency_display[$admin_config["currency_unit"]]["front"]." ".number_format($db->dt[cartcoupon_payment_price]*$p_status)." ".$currency_display[$admin_config["currency_unit"]]["back"]."&nbsp;</td>
                                                <td class='input_box_item' style='text-align:right;'>".$currency_display[$admin_config["currency_unit"]]["front"]." ".number_format($db->dt[WP_payment_price]*$p_status)." ".$currency_display[$admin_config["currency_unit"]]["back"]."&nbsp;</td>
                                            </tr>";
        }
    }else{
        $Contents .= "<tr>
                                                <td class='input_box_item' colspan='10' style='text-align:center;'>사용 내역이 없습니다.</td>
                                            </tr>";
    }
    $Contents .= "
                                            </table>
...
if($admininfo[admin_level] == 9){
    $db->query("select os.regdate, os.status, os.status_message, os.pid, c.com_name,os.invoice_no,os.quick,admin_message from ".TBL_SHOP_ORDER_STATUS." os left join ".TBL_COMMON_COMPANY_DETAIL." c on os.company_id = c.company_id where os.oid ='$oid'     order by regdate asc");
...
function OrderGoodsList($odb, $list_type = "general"){
    ...
    $sql = "SELECT
        od.*, (CAST(od.ptprice AS SIGNED) - CAST(od.pt_dcprice AS SIGNED)) as total_sale_price , odd.delivery_dcprice, odd.ode_ix
    ...
    $Contents .= "
<form name='order_edit_".$list_type."' method='post' onSubmit=\"return orderStatusUpdate(this,".$admininfo[admin_level].")\"  action='orders.goods_list.act.php' target='act'>
...
<table width='100%' border='0' cellpadding='0' cellspacing='1' bgcolor=silver>";
    if($list_type == "bs_general" || $list_type == "general"){
        ...
        $Contents = $Contents."
            ...
            <td>
                ".deliveryCompanyList2("quick","id='quick' style='display:none' onchange='changeDeliberyCode($(this).val());'",$_SESSION[admininfo][company_id])."
            </td>
            <td><div class='deliverycode' style='display:none'>송장번호 : <input type='text' name='deliverycode' onkeyup='check_number(this);' onkeydown='check_number(this);' class=textbox size=15></div></td>
    ...
    for($j = 0; $j < $oddb->total; $j++)
    {
        ...
        $discount_info = $currency_display[$admin_config["currency_unit"]]["front"].number_format($oddb->dt[total_sale_price]).$currency_display[$admin_config["currency_unit"]]["back"];
        ...
        if($list_type == "bs_general" || $list_type == "general"){
            ...
            $Contents .= "
                                            <td bgColor='#ffffff' ><input type=checkbox name='od_ix[]' id='od_ix' class='od_ix' od_status='".$oddb->dt[status]."' od_refund_status='".$oddb->dt[refund_status]."' set_group='".$oddb->dt[set_group]."' company_id='".$oddb->dt[company_id]."' ea_check='".$oddb->dt[ori_company_id]."|".$oddb->dt[delivery_type]."|".$oddb->dt[delivery_package]."|".$oddb->dt[delivery_method]."|".$oddb->dt[delivery_pay_method]."|".$oddb->dt[delivery_addr_use]."|".$oddb->dt[factory_info_addr_ix].( $oddb->dt[delivery_package]=="Y" ? "|".$oddb->dt[pid] : "")."' value='".$oddb->dt[od_ix]."' ></td>";
        }
        $Contents .= "
        ...
        $Contents .= "
                                                            <span style='color:#007DB7;font-weight:bold;' class='small'>".$oddb->dt[od_ix]."</span> <b class='small' style='color:red;font-weight:bold;'>배송(".$oddb->dt[odd_ix].")</b><br/>";
        ...
        if(in_array($oddb->dt[product_type],$sns_product_type)){
            $Contents .= "<a href=\"/sns/shop/goods_view.php?id=".$oddb->dt[pid]."\" target=_blank>".$oddb->dt[pname]."</a>";
        }else{
            ...
            if($oddb->dt[product_type]=='99'||$oddb->dt[product_type]=='21'||$oddb->dt[product_type]=='31'){
                $Contents .= "<b class='".($oddb->dt[product_type]=='99' ? "red" : "blue")."' >".$oddb->dt[pname]."</b><br/><strong>".$oddb->dt[set_name]."<br /></strong>".$oddb->dt[sub_pname];
            }else{
                $Contents .= $oddb->dt[pname];
            }
            $Contents .= "</a>";
        }

        $Contents .= "
                                                                <br/><input type='button' value='상담내역리스트 추가' onclick=\"$('textarea[name=memo]').html($('textarea[name=memo]').html() + '".$oddb->dt[pname].($oddb->dt[pcode] ? "(".$oddb->dt[pcode].")" : "").(strip_tags($oddb->dt[option_text]) ? " ".strip_tags($oddb->dt[option_text])."" : "")." ') \" />
                                                        </td>
                                                    </tr>
                                                </table>
                                            </td>
                                                ...
                                                <table width='100%' height='87' border=0 cellpadding=0 cellspacing=0>
                                                    <tr>
                                                        <td align='center' valign='middle' style='border-bottom:1px solid #c5c5c5'>
                                                            ".strip_tags($oddb->dt[option_text])."".($oddb->dt[option_price] != '' ? " + ".$currency_display[$admin_config["currency_unit"]]["front"]." ".number_format($oddb->dt[option_price])."".$currency_display[$admin_config["currency_unit"]]["back"]."":"")."
                                                        </td>
                                                    </tr>
                                                    <tr>
                                                        <td align='center' valign='middle'>
                                                            ".$currency_display[$admin_config["currency_unit"]]["front"]."".number_format($oddb->dt[psprice]+$oddb->dt[option_price])." ".$currency_display[$admin_config["currency_unit"]]["back"]."
                                                        </td>
                                                    </tr>
                                                </table>
                                            </td>
                                            <td bgColor='#ffffff'>".$oddb->dt[pcnt]." 개</td>
                                            <td bgColor='#ffffff' align=left>
                                                <table width='100%' height='87' border=0 cellpadding=0 cellspacing=0>
                                                    <tr>
                                                        <td align='center' valign='middle' style='border-bottom:1px solid #c5c5c5'>
                                                            ".number_format($oddb->dt[pt_dcprice])."".$currency_display[$admin_config["currency_unit"]]["back"]."
                                                        </td>
                                                    </tr>
                                                    <tr>
                                                        <td align='center' valign='middle' style='border-bottom:1px solid #c5c5c5'>
                                                    ...
                                                    <tr>
                                                        <td align='center' valign='middle'>
                                                            ".number_format($oddb->dt[reserve])." P
                                                        </td>
                                                    </tr>
                                                </table>
                                            </td>
                                            <td bgColor='#ffffff' align=left>
                                                <table width='100%' height='87' border=0 cellpadding=0 cellspacing=0>
                                                    <tr>
                                                        ...
                                                        <td align='center' valign='middle' style='border-bottom:1px solid #c5c5c5;'>
                                                             ".$currency_display[$admin_config["currency_unit"]]["front"]." ".number_format($oddb->dt[pt_dcprice])." ".$currency_display[$admin_config["currency_unit"]]["back"]."
                                                        </td>
                                                    </tr>
                                                    <tr>
                                                        <td class='m_td' style='border-bottom:1px solid #c5c5c5;background-color:#fff;' width='40%'>공급가</td>
                                                        <td align='center' valign='middle' style='border-bottom:1px solid #c5c5c5;'>
                                                            ".$currency_display[$admin_config["currency_unit"]]["front"]." ".number_format(round($oddb->dt[pt_dcprice])/1.1)." ".$currency_display[$admin_config["currency_unit"]]["back"]."
                                                        </td>
                                                    </tr>
                                                    <tr>
                                                        <td class='m_td' style='background-color:#fff;'>세액</td>
                                                        <td align='center' valign='middle' style=''>
                                                            ".$currency_display[$admin_config["currency_unit"]]["front"]." ".number_format($oddb->dt[pt_dcprice]-round($oddb->dt[pt_dcprice])/1.1)." ".$currency_display[$admin_config["currency_unit"]]["back"]."
                                                        </td>
                                                    </tr>
                                                </table>
                                            </td>";
        ...
        if($oddb->dt[delivery_package] == 'N' && $oddb->dt[delivery_pay_method] == '1'){    //묶음이거나 선불일겨우 (착불은 개별로 처리)

            //$bori_company_id != $oddb->dt[ori_company_id] || $b_product_type != $oddb->dt[product_type] || $b_factory_info_addr_ix != $oddb->dt[factory_info_addr_ix] || $b_delivery_type  != $oddb->dt[delivery_type]
            ...
            if($b_ode_ix != $oddb->dt[ode_ix] ){
                ...
                $Contents .="<td bgColor='#ffffff' class='' align=center style='line-height:140%;' rowspan='".$com_cnt."'>
                                                        ".number_format($oddb->dt[delivery_dcprice])."원<br>
                                                        ".($oddb->dt[delivery_type] == '1' ? "통합배송":"입점업체배송")."<br>
                                                        ".$delivery_method."<br>
                                                        ".($oddb->dt[delivery_addr_use] == '1' ? "출고지별 배송<br>".getFactoryAddrName($oddb->dt[factory_info_addr_ix])."<br>":"")."
                                                        ".$delivery_pay_type."<br>
                                                        ".($oddb->dt[delivery_package] == 'N' ? "묶음배송":"")."</td>";
            }

            $bori_company_id = $oddb->dt[ori_company_id];
            ...
            if(ch_set_product_bool($oddb->dt[option_kind])){        //세트상품일경우

                if($b_ode_ix != $oddb->dt[ode_ix] ){

                    $Contents .="<td bgColor='#ffffff' class='' align=center style='line-height:140%;' ".($b_oid != $oddb->dt[oid] || $bproduct_id != $oddb->dt[pid] ? "rowspan='".$com_cnt."'":"").">
                                                            ".number_format($oddb->dt[delivery_dcprice])."원<br>
                                                            ".($oddb->dt[delivery_type] == '1' ? "통합배송":"입점업체배송")."<br>
                                                            ".$delivery_method."<br>
                                                            ".($oddb->dt[delivery_addr_use] == '1' ? "출고지별 배송<br>".getFactoryAddrName($oddb->dt[factory_info_addr_ix])."<br>":"")."
        ...
        if(($oddb->dt[invoice_no] || $oddb->dt[quick])){
            ...
            if(substr_count($oddb->dt[invoice_no],",") > 0){
                ...
                $Contents .= "
                                                    ...
                                                    <table border=1 id='invoice_no_table_".$oddb->dt[od_ix]."' class='invoice_no_table' style='position:absolute;width:100px;display:none;' cellpadding=1  cellspacing=0>
                ...
                foreach($explode_invoice_no as $invoice_no){
                    $Contents .= "
                                                                    <div style='cursor:pointer;' ".($invoice_no == "직배송" ? "" : "onclick=\"searchGoodsFlow('".$oddb->dt[quick]."', '".str_replace("-","",$invoice_no)."');\"").">".$invoice_no."</div>";
                }
                $Contents .= "
                                                            </td>
                                                            <th class='box_06'></th>
                ...
                $Contents .= "
                                                    ...
                                                    <table border=1 id='invoice_no_table_".$explode_invoice_no[0]."' class='invoice_no_table' style='position:absolute;width:100px;display:none;' cellpadding=1  cellspacing=0>
                                                        <col width=3><col width=*><col width=13><col width=*><col width=3>
                                                        <tr>
                                                            <th class='box_01'></th>
                                                            <td class='box_02' colspan=3></td>
                ...
                if($oddb->dt[invoice_no] == "직배송"){
                    $Contents .= "<br>".codeName($oddb->dt[quick])."<br/>(".$oddb->dt[invoice_no].")";
                }else{
                                    ...
                                    <td bgColor='#ffffff'>
                                        <table>
                                            <tr>
                                                ...
                                                <td align='center'>
                                                    <span ".($oddb->dt[pcnt] > 1 && $oddb->dt[refund_status]=="" && $oddb->dt[order_from] != "auction" && $oddb->dt[order_from] != "gmarket" && $oddb->dt[order_from] != "storefarm" && $oddb->dt[order_from] != "11st" ? "style='display:inline;'":"style='display:none;'")."><img src='../images/".$admininfo["language"]."/btn_order_split.gif' align=absmiddle onclick=\"PoPWindow3('./order_detail_separation.php?mmode=pop&od_ix=".$oddb->dt[od_ix]."',1000,600,'order_detail_separation')\" style='cursor:pointer;' /></span>";
        if($list_type == "claim"|| $list_type == "incom_after_cancel"){
            ...
            if($db->total){
                foreach($status_history as $history){
                    ...
                    if(in_array($oddb->dt[status],$order_status_exchange)){
                        ...
                        $sql="SELECT * FROM shop_order_detail_deliveryinfo WHERE oid='".$odb->dt[oid]."' and od_ix='".$oddb->dt[od_ix]."' and order_type='3' ";
                        $db->query($sql);
...
function PrintOrderMemo($oid){
    ...
    if ($total == 0){
        ...
        for($i=0;$i < $mdb->total;$i++){
            ...
            2016-11-28 관리자만 수정 할 수 있게 변경*/
            if($admininfo[admin_level] == 9 && ($admininfo[department] == 6 || $admininfo[department] == 12)){
                //print_r ($admininfo);
                $mString .= "
