...
if(!$title_str){
    $title_str  = "거래처별 판매수량 ".$startDate."(월)";
}


// 거래처별 월별 상품 판매 수량
$qryOrderInfo = "
SELECT t2.gid, t2.pname, t2.order_from, SUM(t2.pcnt) AS pcnt, SUM(t2.ptprice) AS amount
  FROM shop_order t1
 ...
 GROUP BY t2.gid, t2.order_from
 ORDER BY t2.gid, t2.order_from ASC";
...
if (count($orderInfo) > 0){
    ...
    foreach($orderInfo as $p=>$v){
        ...
        if(!in_array($v['order_from'], $sellerList)){
            array_push($sellerList, $v['order_from']);
        }

        $orderList[$v['gid']]['order_from'] = $v['order_from'];
        $orderList[$v['gid']]['pname'] = $v['pname'];
        $orderList[$v['gid']]['pcnt'] = $v['pcnt'];
        $orderList[$v['gid']]['amount'] = $v['amount'];
    }
}
...
$view_data = "<table width='100%' border='1' cellpadding='3' cellspacing='0' align='center'  class='data_table' style='display: border-collapse: separate; border-spacing: 1px; background: #c5c5c5; border: 1px;'>
            <thead>
                ...
                <tr style='border-color: silver;'>
                    <th style='background: #efefef;width: 100px'>품목코드</th>
                    <th style='background: #efefef;width: 20%''>상품명</th>
                    <th style='background: #efefef;width: 10%'>결제금액 합계</th>
                    <th style='background: #efefef;width: 10%'>합계</th>";

for($i=0;$i<count($sellerList);$i++){
    $view_data .= "<th style='background: #efefef;width:".round(55/count($sellerList))."%'>".$sellerList[$i]."</th>";
}
...
if($orderCnt > 0) {
    ...
    foreach($orderList as $p=>$v) {
        ...
        $tr .= "<tr style='border-color: silver;'>
                <th style='background: #efefef;text-align: center;width: 10px'>".$p."</th>
                <th style='background: #efefef;text-align: left;'>".$v['pname']."</th>
                <td style='background: #fff;text-align: right'>" . number_format($v['amount']) . "</td>
                <td style='background: #fff;text-align: right'>" . number_format($v['pcnt']) . "</td>";

        for($i=0;$i<count($sellerList);$i++){

            if($v['order_from'] == $sellerList[$i]){
                $tr .= "<td style='background: #fff;text-align: right'>".$v['pcnt']."</td>";
                $sellerTotal[$sellerList[$i]] += $v['pcnt'];
            }else{
                $tr .= "<td style='background: #fff;text-align: right'>0</td>";
            }
        }
    ...
    $tr .= "<tr style='border-color: silver;'>
                <th style='background: #efefef;text-align: center;' colspan='2'>합 계</th>
                <td style='background: #efefef;text-align: right'>" . number_format($totalAmount) . "</td>
                <td style='background: #efefef;text-align: right'>" . number_format($totalPcnt) . "</td>";


    foreach($sellerTotal as $p1 => $v1){
            $tr .= "<td style='background: #efefef;text-align: right'>".number_format($v1)."</td>";
    }

    $tr .= "</tr>";

}else{
    $tr .= "<tr style='border-color: silver;'><td colspan='".(4 + count($sellerList))."' style='background: #fff;'>검색 조건에 따른 주문 내역이 없습니다. </td></tr>";
}
// end loop
...
if($_GET['excel'] == "act") {
    ...
    header('Content-Disposition: attachment;filename="' . iconv("UTF-8", "CP949", "거래처별 월 매출") . '_' . date("Ymd") . '.xls"');
...
<form id='form_excels' action='./order_statistics4.php' method='get'>
...
if($mmode == "personalization"){
    ...
    $P->addScript = $js;
    $P->Navigation = "주문관리 > 매출종합 분석 > 거래처별 판매수량 ".$startDate."월";
    $P->title = "거래처별 판매수량 ".$startDate."월";
    $P->strContents = $Contents;
    echo $P->PrintLayOut();
}
?>