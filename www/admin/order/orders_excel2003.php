...
$sql = "SELECT 
		...
		if(left(o.buserid,'3') = 'ka@', '*********', o.bname) as bname, 
		...
		//2021-09-06 카카오계정으로 가입한 회원은 모두 이름대신 ***** 처리 o.bname as bname, 
		//2021-09-06 다중쿼리 수정 (select code from common_user where o.buserid = id) as code, 
        ...
        odd.city,
        odd.state
	FROM ".TBL_SHOP_ORDER." o left join ".TBL_SHOP_ORDER_DETAIL." od on o.oid = od.oid 
	left join common_user cu on o.buserid = cu.id
...
if($db->total){
	...
	if($admininfo['userAuth'] == 22 && $oet_ix == 33){
		for ($j=0,$col='A'; $j < $orders_total; $j++,$rownum++,$col='A'){
			...
			for ($i=0; $i < count($columsinfo); $i++,$col++){
				...
				switch($columsinfo[$col]["code"])
				{
					...
					case "btel":
					case "rtel":
						$ordersXL->getActiveSheet()->setCellValueExplicit($col . ($rownum), wel_masking("P", $value_str), PHPExcel_Cell_DataType::TYPE_STRING);
						break;


					case "bmail":
					case "rmail":
						$ordersXL->getActiveSheet()->setCellValueExplicit($col . ($rownum), wel_masking("E", $value_str), PHPExcel_Cell_DataType::TYPE_STRING);
						break;

					case "bname":
					case "rname":
						$ordersXL->getActiveSheet()->setCellValueExplicit($col . ($rownum), wel_masking_seLen($value_str, 1, 1), PHPExcel_Cell_DataType::TYPE_STRING);
						break;
...
*/
	...
	//	ig 엑셀 파일 zip 파일로 생성 후 패스워드 걸기	방법2
		if($excel_type == "delivery"){
			$download_filename = '배송정보_'.date("YmdHis").'.zip'; 
			$igExcel_file = '../excelDn/배송정보_'.date("YmdHis").'.xls'; 
		}else{
			$download_filename = '주문목록_'.date("YmdHis").'.zip'; 
			$igExcel_file = '../excelDn/주문목록_'.date("YmdHis").'.xls'; 
		}
