    ...
    $Contents .= "
											...
											<tr height=30>
												...
												<td class='search_box_item' nowrap>
													...
													<input type='checkbox' name='method[]' id='method_".ORDER_METHOD_CART_COUPON."' value='".ORDER_METHOD_CART_COUPON."' ".CompareReturnValue(ORDER_METHOD_CART_COUPON,$method,' checked')." ><label for='method_".ORDER_METHOD_CART_COUPON."' class='helpcloud' help_width='55' help_height='15' help_html='".getMethodStatus(ORDER_METHOD_CART_COUPON)."'>".getMethodStatus(ORDER_METHOD_CART_COUPON)."<!-- img src='../images/".$admininfo[language]."/s_order_method_".ORDER_METHOD_CART_COUPON.".gif' align='absmiddle' --></label>
												...
												<td class='search_box_item'>
													...
													<input type='checkbox' name='payment_agent_type[]' id='payment_agent_type_p' value='P' ".CompareReturnValue("P",$payment_agent_type,' checked')." ><label for='payment_agent_type_p' class='helpcloud' help_width='90' help_height='15' help_html='POS'><img src='../images/".$admininfo[language]."/s_payment_agent_type_p.gif' align='absmiddle'></label>
												</td>
											</tr>
											";
}
	...
	<tr>
		<td colspan=2 align=center style='padding:10px 0px;position:relative;'>
			...
			<div style='position:absolute;right:0;top:-4px;'>
				<img src='../images/".$admininfo[language]."/btn_field_open.gif' style='cursor:pointer;' />
				<img src='../images/".$admininfo[language]."/btn_field_close.gif' style='cursor:pointer;' />
			</div>
		</td>
	</tr>
</table>
...
if(count($o_array)>0){
	...
	where
		...
		$addWhere
		ORDER BY o.oid desc, od.delivery_type, delivery_package, od.ori_company_id asc,od.ode_ix ASC";
    ...
    for ($i = 0; $i < count($order_list); $i++){
        ...
        if($order_list[$i][delivery_package] == 'N' && $order_list[$i][delivery_pay_method] == '1'){	//묶음이거나 선불일겨우 (착불은 개별로 처리)
            ...
            if($b_oid != $order_list[$i][oid] || $bori_company_id != $order_list[$i][ori_company_id] || $bori_ode_ix != $order_list[$i][ode_ix]){
                $sql = "SELECT 
            ...
            if(ch_set_product_bool($order_list[$i][option_kind])){		//세트상품일경우
                ...
                if($b_oid != $order_list[$i][oid] || $bori_ode_ix != $order_list[$i][ode_ix]){

                    $Contents .="<td class='' align=center style='line-height:140%;' ".($b_oid != $order_list[$i][oid] || $bori_ode_ix != $order_list[$i][ode_ix] ? "rowspan='".$com_cnt."'":"").">
								".number_format($order_list[$i][delivery_totalprice])."원<br>
								".($order_list[$i][delivery_type] == '1' ? "통합배송":"입점업체배송")."<br>
								".$delivery_method."<br>
								".($order_list[$i][delivery_addr_use] == '1' ? "출고지별 배송<br>".getFactoryAddrName($order_list[$i][factory_info_addr_ix])."<br>":"")."
