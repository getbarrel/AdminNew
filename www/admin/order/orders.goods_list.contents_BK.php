		...
		for ($i = 0; $i < count($order_datas); $i++)
		{
			...
			for($j=0;$j < count($order_detail_datas);$j++){
				...
				if($order_detail_datas[$j][oid] != $b_oid){
					...
					$b_mem_info = "<b style='cursor:pointer' class='helpcloud' help_width='230' help_height='100' help_html='주문자 <br/>ID/성명 : ".$order_detail_datas[$j][buserid]."/".$order_detail_datas[$j][bname]."<br/>핸드폰 : ".$order_detail_datas[$j][bmobile]." <br/>회원그룹 : ".$order_detail_datas[$j][mem_group]." <br/>최근로그인 : ".$u_etc_info["user_last"]." <br/>최근주문(30일) : ".$u_etc_info["user_order_cnt"]."건' />".Black_list_check($order_detail_datas[$j][user_id],($order_detail_datas[$j][gp_ix]=='2' ? "<b class='red'>VIP</b>" : "").$order_detail_datas[$j][bname].( $order_detail_datas[$j][buserid] ? "(<span class='small'>".$order_detail_datas[$j][buserid]."</span>)" : "(<span class='small'>비회원</span>)"))."</b> <br/> ".($_SESSION["admininfo"]["admin_level"] > 8 && $order_detail_datas[$j][user_id] ? "<img src='../v3/images/".$_SESSION["admininfo"]["language"]."/btn_search.gif' align=absmiddle onclick=\"PopSWindow2('/admin/member/member_cti.php?mmode=pop&code=".$order_detail_datas[$j][user_id]."&mmode=pop',1280,800,'member_view')\"  style='cursor:pointer;'>" : "");
				...
				$Contents .= "<td class='list_box_td' style='padding-left:10px'>
								<TABLE style='text-align:left;'>
									<TR>
										<TD align='center'>
											<a  href='/shop/goods_view.php?id=".$order_detail_datas[$j][pid]."' target=_blank><img src='".PrintImage($admin_config[mall_data_root]."/images/product", $order_detail_datas[$j][pid], "m", $order_detail_datas[$j])."'  width=50 style='margin:5px;'></a><br/>";
		...
		<col width='6%'/>
					...
					if($admininfo[admin_level]==9){
						if($pre_type=='refund' || $pre_type==ORDER_STATUS_REFUND_APPLY || $pre_type==ORDER_STATUS_REFUND_COMPLETE){
							...
							if(substr_count($method_ ,ORDER_METHOD_BANK) > 0 || substr_count($method_ ,ORDER_METHOD_VBANK) > 0){
