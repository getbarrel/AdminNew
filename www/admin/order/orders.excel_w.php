...
header( "Content-Disposition: attachment; filename=order_list_report.xls" );
...
$sql = "SELECT o.oid, uid, o.bname,o.bmobile,p.surtax_yorn,p.pack_method, o.rname, tid, o.status, method, total_price, payment_price, date, o.method, o.receipt_y, 
				...
				od.company_id, od.company_name, o.mem_group, od.use_baymoney, od.use_coupon, od.use_reserve, o.delivery_price, od.commission, 
...
if($db1->total){
	...
	for ($i = 0, $j=0; $i < $db1->total; $i++, $j++)
	{
		...
		$mstring .= "<tr bgcolor=#ffffff><td>".($j+1)."</td>";
				...
				//상품매출
				$mstring .= "<td>".($db1->dt[surtax_yorn] == "Y" ? $db1->dt[ptprice]:round($db1->dt[ptprice]/1.1))."</td>";
				$mstring .= "<td>".($db1->dt[surtax_yorn] == "Y" ? "0":$db1->dt[ptprice]-round($db1->dt[ptprice]/1.1))."</td>";
				$mstring .= "<td bgcolor=#efefef>".($db1->dt[ptprice])."</td>";
				//배송비 정산금액
				$mstring .= "<td>0</td>";
				$mstring .= "<td>0</td>";
				$mstring .= "<td bgcolor=#efefef>0</td>";
				// 합계				
				$mstring .= "<td>".(($db1->dt[surtax_yorn] == "Y" ? $db1->dt[ptprice]:round($db1->dt[ptprice]/1.1)))."</td>";
				$mstring .= "<td>".(($db1->dt[surtax_yorn] == "Y" ? "0":$db1->dt[ptprice]-round($db1->dt[ptprice]/1.1)))."</td>";
				$mstring .= "<td bgcolor=#efefef>".($db1->dt[surtax_yorn] == "Y" ? $db1->dt[ptprice]:intval($db1->dt[ptprice]))."</td>";
				// 매출원가
				$mstring .= "<td>".(($db1->dt[surtax_yorn] == "Y" ? $account_price:round($account_price/1.1)))."</td>";
				$mstring .= "<td>".(($db1->dt[surtax_yorn] == "Y" ? "0":$account_price-round($account_price/1.1)))."</td>";
				
				$sale_base_sum = ($db1->dt[surtax_yorn] == "Y" ? $db1->dt[ptprice]:intval($db1->dt[ptprice]));
				$mstring .= "<td bgcolor=#efefef>".($db1->dt[surtax_yorn] == "Y" ? intval($account_price):intval($account_price))."</td>";
				...
				if(($next_db[oid] != $db1->dt[oid] or ($db1->total-1) == $i) && $db1->dt[delivery_price] > 0){
					$j++;
					$mstring .= "<tr bgcolor=#ffffff><td>".($j+1)."</td>";
					$mstring .= "<td>".$db1->dt[company_name]."</td>";
					$mstring .= "<td>".$db1->dt[date]."</td>";
					$mstring .= "<td>".$db1->dt[oid]."</td>";
					$mstring .= "<td>배송료(".$db1->dt[delivery_price].")</td>";
					$mstring .= "<td></td>";
					$mstring .= "<td></td>";
					$mstring .= "<td></td>";
					$mstring .= "<td></td>";
					...
					$delivery_base = $db1->dt[delivery_price];
					if(($db1->dt[use_baymoney_price] - $use_baymoney_price_cal) >= $delivery_base ){ //베이머니 사용금액 남은 금액이 합계금액 보다 클때는 
						$this_use_baymoney_price = $delivery_base;
					}else{
						if(($db1->dt[use_baymoney_price] - $use_baymoney_price_cal) <= 0){
							$this_use_baymoney_price = 0;
						}else{
						...
						if(($db1->dt[use_reserve_price] - $use_reserve_price_cal) >= ($delivery_base -$this_use_baymoney_price)){
							$this_reserve_price = ($delivery_base-$this_use_baymoney_price);
						}else{
							if(($db1->dt[use_reserve_price] - $use_reserve_price_cal) <= 0){
								$this_reserve_price = 0;
							}else{
						...
						if(($db1->dt[use_reserve_price] - $use_reserve_price_cal) >=  ($delivery_base-$this_use_baymoney_price)){
							$use_reserve_price_cal +=  ($delivery_base-$this_use_baymoney_price);
						}else{
							$use_reserve_price_cal += $db1->dt[use_reserve_price];
						}
					...
					$mstring .= "<td>";
					/*
					if($db1->dt[delivery_price] <= ($db1->dt[use_baymoney_price] - $use_baymoney_price_cal)){
						$mstring .= $db1->dt[delivery_price];
						$use_baymoney_price_cal += $db1->dt[delivery_price];
						$remain_delivery_price =0;
					}else{
						$mstring .= ($db1->dt[use_baymoney_price] - $use_baymoney_price_cal);
						$use_baymoney_price_cal += ($db1->dt[use_baymoney_price] - $use_baymoney_price_cal);
						$remain_delivery_price = ($db1->dt[delivery_price]-($db1->dt[use_baymoney_price] - $use_baymoney_price_cal));
					}
					*/
					$mstring .= "".($this_use_baymoney_price);
					$mstring .= "</td>";
					if(($db1->dt[use_baymoney_price] - $use_baymoney_price_cal) >= $delivery_base){
						$use_baymoney_price_cal += $delivery_base;
					}else{
						$use_baymoney_price_cal += $db1->dt[use_baymoney_price];
					}
					$mstring .= "<td>".($this_reserve_price)."</td>";
						
					$mstring .= "<td>".($delivery_base-$this_use_baymoney_price-$this_reserve_price)."</td>";
					//$mstring .= "<td>".intval($remain_delivery_price)."</td>";	
					$mstring .= "<td></td>";			
					$mstring .= "<td>".($status)."</td>";
					$mstring .= "<td>".$receipt_str."</td>";
					$mstring .= "<td>".$method."</td>";
