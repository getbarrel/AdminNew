		...
		<td align='left' colspan=2  width='100%' valign=top style='padding-top:0px;'>
			...
			<tr>
				<td class='box_05' valign=top style='padding:0px;'>
					...
					<tr height='27'>
						<td class='search_box_title' bgcolor='#efefef' align=center>
							<label for='search_check'><b>모금기간</b></label>
						</td>
						<td class='search_box_item'>
							".search_date('term_sdate','term_edate',$term_sdate,$term_edate)."
						</td>
					</tr>
					</table>
				</td>
...
$where = "where div_depth = 'thank_you' ";
if($_GET['mode'] == 'search'){
    if($_GET['info_type']){
        $where .= " and div_depth_sub = '".$_GET['info_type']."' ";
    }
    if($_GET['title']){
    ...
    if($_GET['term_sdate'] && $_GET['term_sdate']){
        $where .= " and term_sdate <=  '".$_GET['term_sdate']." 00:00:00' and  term_edate >= '".$_GET['term_edate']." 23:59:59' ";
    }
}
...
$Contents01 .= "
	...
	<tr bgcolor=#efefef style='font-weight:bold' align=center height=28>		
		...
		<td class='m_td'>모금기간</td>	
		<td class='m_td'>후원자수</td>
		<td class='m_td'>총후원금액</td>	
		<td class='e_td'>관리</td>
	</tr>";
if(is_array($contents_array) && $total > 0){
	...
	<tr height=28 align=center>        
        ...
        <td class='list_box_td list_bg_gray'>".$val['title']."</td>
        <td class='list_box_td' >".$disp_text."</td>		
        <td class='list_box_td list_bg_gray'>".$val['regdate']."</td>
        <td class='list_box_td' >".number_format($val['view_cnt'])."</td>	
        <td class='list_box_td list_bg_gray'>".SearchLikeCnt($val['co_ix'])."</td>
        <td class='list_box_td' >".$val['term_sdate']." ~ ".$val['term_edate']."</td>	
        <td class='list_box_td list_bg_gray'>작업필</td>
        <td class='list_box_td' >작업필</td>
        <td class='list_box_td list_bg_gray'>             
            <input type='button' value='수정' onclick=\"BienViewInput('thank_you', '$val[co_ix]');\"/>
            <input type='button' value='삭제' onclick=\"BienViewDelete('thank_you', '$val[co_ix]');\"/>
        </td>
    </tr>";
        $i++;
    }
...
$Contents01 .= "
    <tr height=28 align=center>        
        <td class='list_box_td' colspan='10'>
            <span>등록된 Thank You  목록이 없습니다.</span>            
        </td>
    </tr> ";
}
$Contents01 .= "   
    ...
    <table width='100%' border='0' cellpadding='3' cellspacing='0' align='center' >
        <tr height=40>
         <td colspan='9' align='right'><input type='button' onclick=\"BienViewInput('thank_you','');\" value='신규등록'></td>
        </tr>
    </table>
	";
$Contents01 .= "
...
$P->Navigation = "BIENVIEW관리 > 콘텐츠 관리 > Thank You 목록 ";
$P->title = "Thank You 목록";
