...
function CategorySave(frm,vMode)
{
	...
	if (frm.this_category.value.length < 1){
		alert(language_data['sns_category.js']['E'][language]);//'수정/삭제 하시고자 하는 상품카테고리를 선택해 주세요'
		return false;	
	}
...
function SubCategorySave(frm,vMode)
{
	...
	if (frm.cid.value.length != 15){
		alert(language_data['sns_category.js']['A'][language]);//'추가 하시고자 하는 상품카테고리를 선택해 주세요'
		return false;	
	}
	
	if (frm.sub_category.value.length < 1){
		alert(language_data['sns_category.js']['A'][language]);//'추가 하시고자 하는 상품카테고리를 입력해 주세요'
		return false;	
	}
	
	if (frm.sub_depth.value >= 4){
		alert(language_data['sns_category.js']['B'][language]);//'카테고리구성은 4단계까지만 가능합니다.'
		return false;	
	}
...
function setCategory(cname,cid,depth, category_display_type)
{
	...
	document.getElementById("category_link").innerHTML = "<img src='/admin/image/url_new.gif' style='cursor:hand;' align=absmiddle onclick=\"UrlCopy('/shop/goods_list.php?cid="+cid+"&depth="+depth+"');\"> &nbsp;/shop/goods_list.php?cid="+cid+"&depth="+depth+""; 
...
function UrlCopy(url) {	 
	 ...
	 alert('['+url+']\n'+language_data['sns_category.js']['C'][language]);//선택한 주소가 클립보드에 복사되었습니다.!
}

function order_up(frm){
	frm.mode.value = "up";
	if (frm.this_depth.value.length < 1){
		alert(language_data['sns_category.js']['D'][language]);//'상품카테고리를 선택해주세요'
		return false;	
	}
