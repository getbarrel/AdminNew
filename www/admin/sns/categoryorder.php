...
/*
	...
	if ($depth == 0){
		$sql = "select * from ".TBL_SNS_CATEGORY_INFO." where depth = $depth and cid LIKE '".substr($cid,0,$sPos)."%'";
	}else{
		$sql = "select * from ".TBL_SNS_CATEGORY_INFO." where depth = $depth and cid LIKE '".substr($cid,0,$sPos)."%'";
	}
...
//			echo "tarcagetcid:".getTargetCid($depth,$this_cid,$select_level,+1)."<br>";
		...
		echo "<Script Language='JavaScript'>alert(language_data['sns_categoryorder.php']['A'][language])</Script>";
		//'더이상 진행할 방향이 없습니다.'
	}
...
function getlevel($cid,$depth){
	...
	$levelnum = $depth+1;
	$sql = "select vlevel$levelnum from ".TBL_SNS_CATEGORY_INFO." where depth = $depth and cid = '$cid'";
//	echo($sql)."<br>";
	$db->query($sql);
	$db->fetch(0);
...
function getTargetCid($depth,$cid,$select_level,$num){
	...
	$sql = "select cid from ".TBL_SNS_CATEGORY_INFO." where depth = $depth and vlevel$levelnum = '".($select_level+$num)."' and cid LIKE '".substr($cid,0,$sPos)."%'";
//	echo($sql)."<br>";
	$db->query($sql);
//	echo $db->total;
	if ($db->total){
...
function Pluslevel($cid,$depth,$selectlevel,$target_cid)
{
	...
	if ($depth == 0){
		$sql = "UPDATE ".TBL_SNS_CATEGORY_INFO." SET vlevel$levelnum = '".($selectlevel)."' where vlevel$levelnum = '".($selectlevel-1)."' and cid LIKE '".substr($target_cid,0,$sPos)."%'";
		$db->query($sql);
	}else{
		//$sql = "UPDATE ".TBL_SNS_CATEGORY_INFO." SET vlevel$levelnum = '".($selectlevel)."' where vlevel$levelnum = '".($selectlevel-1)."' and depth = $depth and cid LIKE '".substr($target_cid,0,$sPos)."%'";
		$sql = "UPDATE ".TBL_SNS_CATEGORY_INFO." SET vlevel$levelnum = '".($selectlevel)."' where vlevel$levelnum = '".($selectlevel-1)."' and cid LIKE '".substr($target_cid,0,$sPos)."%'";
		$db->query($sql);
	}
//	echo($sql)."<br>";

	$sql = "UPDATE ".TBL_SNS_CATEGORY_INFO." SET vlevel$levelnum = '".($selectlevel - 1)."' where vlevel$levelnum = '$selectlevel'  and cid LIKE '".substr($cid,0,$sPos)."%'";
	$db->query($sql);
//	echo($sql)."<br>";
}
