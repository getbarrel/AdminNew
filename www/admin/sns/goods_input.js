...
if(obj_table.id == "options_input"){
	...
	obj_table_text = obj_table_text.replace(/options_item_option_etc_0/g,"options_item_option_etc_"+(parseInt(select_idx)+1));
	...
	//obj_table_text = obj_table_text.replace("<!-- 옵션 삭제 -->","<img src='../images/i_close.gif' align=absmiddle style='cursor:hand;' ondblclick=\"if(document.all."+obj+".length > 1){this.parentNode.parentNode.parentNode.removeNode(true);}else{alert(language_data['sns_goods_input.js']['A'][language]);}\" alt='더블클릭시 해당 라인이 삭제 됩니다.'>");
	var child_txt="$('."+obj+"[idx="+(parseInt(select_idx)+1)+"]')[0]";
	var parent_txt=child_txt+'.parentNode';
	//alert(obj_table_text);
	obj_table_text = obj_table_text.replace("<!-- 옵션 삭제 -->","<img src='../images/i_close.gif' align=absmiddle style='cursor:pointer;' ondblclick=\"if($('."+obj+"').length > 1){"+parent_txt+".removeChild("+child_txt+");/*this.parentNode.parentNode.parentNode.removeNode(true);*/}else{alert(language_data['sns_goods_input.js']['A'][language]);}\" title='더블클릭시 해당 테이블이 삭제 됩니다.'>");// 호환성 2011-04-08 kbk '마지막 한개는 삭제 하실 수 없습니다.'
	//alert(obj_table_text);
	...
	obj_table_text+="<td>";
	obj_table_text+="<b >이미지복사</b><input type=checkbox name='addimages[0][add_copy_allimg]' onclick=\"copyAddImageCheckAll('0');\" id='add_copy_allimg_0'  inputid='add_copy_allimg_0' value=1 checked>";
	obj_table_text+="</td>";
	obj_table_text+="<td></td>";
	obj_table_text+="<td>";
	obj_table_text+="복사함<input type=checkbox name='addimages[0][add_chk_mimg]' id='add_copy_img_0' value=1 inputid='add_copy_img_0' checked>";
	obj_table_text+="</td>";
	obj_table_text+="<td></td>";
	obj_table_text+="<td>";
	obj_table_text+="복사함<input type=checkbox name='addimages[0][add_chk_cimg]' id='add_copy_img_0' value=1 inputid='add_copy_img_0' checked>";
	obj_table_text+="</td>";
	obj_table_text+="</tr>";
	obj_table_text+="<tr align='left'>";
	obj_table_text+="<td></td>";
	...
	obj_table_text+="<b class=small> deepzoom 이미지 생성</b><input type=checkbox name='addimages[0][add_copy_deepzoomimg]' value=1 >";
	obj_table_text+="</td>";
	...
	var parent_txt=child_txt+'.parentNode';
	//alert(obj_table_text);
	obj_table_text = obj_table_text.replace("<!-- 옵션 삭제 -->","<img src='../images/i_close.gif' align=absmiddle style='cursor:pointer;' ondblclick=\"if($('."+obj+"').length > 1){"+parent_txt+".removeChild("+child_txt+");/*this.parentNode.parentNode.parentNode.removeNode(true);*/}else{alert(language_data['sns_goods_input.js']['A'][language]);}\" title='더블클릭시 해당 테이블이 삭제 됩니다.'>");// 호환성 2011-04-08 kbk
}else{// 옵션 항목 추가 부분
	...
	//obj_table_text = obj_table.outerHTML.replace(/options_item_option_etc_0/g,"options_item_option_etc_"+(parseInt(select_idx)));
	...
	var child_txt="$('."+obj+"[idx="+(parseInt(select_idx)+1)+"]')[0]";
	var parent_txt=child_txt+'.parentNode';
	obj_table_text = obj_table_text.replace("<!-- 옵션 삭제 -->","<img src='../images/i_close.gif' align=absmiddle style='cursor:pointer;' ondblclick=\"if($('."+obj+"').length > 1){"+parent_txt+".removeChild("+child_txt+");/*this.parentNode.parentNode.parentNode.removeNode(true);*/}else{alert(language_data['sns_goods_input.js']['A'][language]);}\" title='더블클릭시 해당 라인이 삭제 됩니다.'>");
...
function calcurate(frm){	
	...
	{
		alert(language_data['sns_goods_input.js']['B'][language]);	
		//'판매가격이 입력되지 않았습니다.'
		return false;
	}
	...
	if(frm.rate1.value.length < 1)	{
		alert(language_data['sns_goods_input.js']['C'][language]);	
...
//alert(sellprice);
	...
	if(frm.card_pay.value.length < 1)	{
		alert(language_data['sns_goods_input.js']['F'][language]);	//'카드수수료가 입력되지 않았습니다.'
		return false;
	}else{
...
function ProductInput(frm,act)
{
	...
	if(categorys.length < 1){
		alert(language_data['sns_goods_input.js']['G'][language]);//'카테고리를 선택해주세요'
		return false;
	}
	...
	/*if (frm.pname.value.length < 1){
		alert(language_data['sns_goods_input.js']['H'][language]);//'제품명이 입력되지 않았습니다.'
		frm.pname.focus();
		return false;	
	}
	...
	if(!surtax_yorn_bool){
		alert(language_data['sns_goods_input.js']['I'][language]);//'면세여부를 선택해주세요'
		frm.surtax_yorn[0].focus();
		return false;	
	}*/
	
	/*if (frm.delivery_method.value.length < 1){
		alert(language_data['sns_goods_input.js']['J'][language]);//'배송방법이 선택되지않았습니다. 배송방법을 선택해주세요.'
		return false;	
	}*/
	...
	if(!pack_method_bool){
		alert(language_data['sns_goods_input.js']['K'][language]);//'포장 방법을 선택해주세요'
	...
	if (frm.sellprice.value != frm.bsellprice.value){
		alert(language_data['sns_goods_input.js']['L'][language]);//"가격에 대한 정보가 변경되었습니다."
	...
	/*if(frm.coprice.value.length < 1)
	{
		alert(language_data['sns_goods_input.js']['M'][language]);//"공급가격이 입력되지 않았습니다."
		frm.coprice.focus();
		return false;
	}*/
	/*
	if (frm.shotinfo.value.length < 1){
		alert(language_data['sns_goods_input.js']['N'][language]);//'제품소개가 입력되지 않았습니다.'
		return false;	
	}
	*/
	...
	for(j=0;j < options_price_stock_option_divs.length;j++){
			if(options_price_stock_option_divs[options_price_stock_option_divs.length-j-1].value && substr_count(options_price_stock_option_div_str, "|"+options_price_stock_option_divs[options_price_stock_option_divs.length-j-1].value+"|") > 1){
				alert(language_data['sns_goods_input.js']['O'][language]);//'중복된 가격+재고 옵션구분명이 있습니다. 수정후 다시 시도해주세요'
	...
	for(i=0;i < option_names.length;i++){
		...
		if(option_names[option_names.length-i-1].value){
			...
			if(option_names[option_names.length-i-1].value && substr_count(option_names_str, "|"+option_names[option_names.length-i-1].value+"|") > 1){
				alert(language_data['sns_goods_input.js']['P'][language]);//'중복된 옵션명이 있습니다. 수정후 다시 시도해주세요'
				...
				for(j=0;j < option_details_divs.length;j++){		
					//alert(option_details_div_str+":::"+substr_count(option_details_div_str, "|"+option_details_divs[option_details_divs.length-j-1].value+"|"));
						if(option_details_divs[option_details_divs.length-j-1].value && substr_count(option_details_div_str, "|"+option_details_divs[option_details_divs.length-j-1].value+"|") > 1){							
							alert(language_data['sns_goods_input.js']['AK'][language]);
	...
	for(i=0;i < display_option_titles.length;i++){		
		...
		if(display_option_titles[display_option_titles.length-i-1].value){
			if(display_option_titles[display_option_titles.length-i-1].value && substr_count(display_option_titles_str, "|"+display_option_titles[display_option_titles.length-i-1].value+"|") > 1){
				alert(language_data['sns_goods_input.js']['Q'][language]);
...
function dpCheckOptionData(frm){
	if(frm.dp_title.value.length < 1){
		alert(language_data['sns_goods_input.js']['R'][language]);	//'옵션구분값을 입력해주세요'
		frm.option_div.focus();
		return false;
	}
	
	if(frm.dp_desc.value.length < 1){
		alert(language_data['sns_goods_input.js']['S'][language]);	//'옵션별 비회원가를 입력해주세요'
...
function CheckOptionData(frm){
	...
	if(option_kind == 'b' || option_kind == 'p'){
		if(frm.opn_ix.value == ""){
			alert(language_data['sns_goods_input.js']['T'][language]);	//'옵션이름을 선택해주세요'
			frm.option_div.focus();
			return false;
		}
		...
		if(frm.option_m_price.value.length < 1){
			alert(language_data['sns_goods_input.js']['W'][language]);	//'옵션별 회원가를 입력해주세요'
			frm.option_m_price.focus();
			return false;
		}
		
		if(frm.option_d_price.value.length < 1){
			alert(language_data['sns_goods_input.js']['X'][language]);	//'옵션별 딜러가를 입력해주세요'
			frm.option_d_price.focus();
			return false;
		}
		
		if(frm.option_a_price.value.length < 1){
			alert(language_data['sns_goods_input.js']['Y'][language]);	//'옵션별 대리점가를 입력해주세요'
		...
		*/
		...
		if(frm.option_div.value.length < 1){
			alert(language_data['sns_goods_input.js']['U'][language]);	//'옵션구분값을 입력해주세요'
			frm.option_div.focus();
			return false;
		}
	}else{
...
function FormatNumber2_old(num){
        ...
        // ie5.5이상에서 사용할것
        fl=""
        if(isNaN(num)) { alert(language_data['sns_goods_input.js']['AC'][language]);return 0}//"문자는 사용할 수 없습니다."
        if(num==0) return num
...
function num_check() {
        ...
        if ((keyCode < 48 || keyCode > 57) && keyCode != 8){
                alert(language_data['sns_goods_input.js']['AC'][language]+"["+keyCode+"]")
				//"문자는 사용할 수 없습니다."
...
//Base = i 이면 소숫점위의 자릿수에서, f 이면 소숫점아래의 자릿수에서 반올림
	...
	return Math.round(Num); 
	                ...
	                alert(language_data['sns_goods_input.js']['AD'][language]);//"자릿수는 정수로만 구분합니다."
	                return false;
	 }
...
function commaSplit(srcNumber) {
	...
	if (isNaN(txtNumber) || txtNumber == "") {
		//alert(language_data['sns_goods_input.js']['AE'][language]);//"숫자만 입력 하세요"
		return 0;
	}
	else {
...
function copyPrice(frm, copy_price, step){
	...
	if(step == 1){
		if(copy_price == ""){
			alert(language_data['sns_goods_input.js']['AF'][language]);	//'구매단가(공급가)를 입력후 복사 버튼을 클릭해주세요'
			return false;
		}
		...
		if(copy_price == ""){
			alert(language_data['sns_goods_input.js']['AG'][language]);	//'정가를 입력후 복사 버튼을 클릭해주세요'
			return false;
		}
		frm.sellprice.value = copy_price;
...
function copyPrice(frm, copy_price, step,commi){
	...
	if(step == 2){
		if(copy_price == ""){
			alert(language_data['sns_goods_input.js']['AH'][language]);	//'판매가를 입력후 복사 버튼을 클릭해주세요'
			return false;
		}
		var per = 100;
		//alert(commi);
...
function commissionChange(frm){	
	...
	if(eval("frm.one_commission[0].checked")){
		frm.coprice.value = parseInt(listprice) - parseInt(Math.round(listprice*(frm.commission.getAttribute("company_commission")/100)));// getAttribute 사용 kbk
		frm.commission.value = frm.commission.getAttribute("company_commission");// getAttribute 사용 kbk
	}else{
		frm.coprice.value = parseInt(listprice) - parseInt(Math.round(listprice*(frm.commission.getAttribute("goods_commission")/100)));// getAttribute 사용 kbk	
...
function ShowGoodsTypeInfo(vid){
	var area = new Array('GoodsInfo','CouponInfo','TravelInfo');
	for(var i=0; i<area.length; ++i){
		if(area[i]==vid){
			$('#'+vid).show();
		}else{			
...
function stockCheck(vtype){
	if(vtype == "1"){
		...
		document.getElementById('safestock').disabled = false;
	}else{
		document.getElementById('stock').disabled = true;
		document.getElementById('safestock').disabled = true;
	}
}
