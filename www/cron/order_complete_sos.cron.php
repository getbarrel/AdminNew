...
$sql = "select
		...
		and
			config_name = 'check_sos_order_day'
		and 
			config_value is not null";
$db->query($sql);

$db->fetch();
$check_sos_order_day = $db->dt[config_value];
if(empty($check_sos_order_day)){
    $check_sos_order_day = 7;
}
$order_details=array();

$sql="select od.*,o.user_code from ".TBL_SHOP_ORDER." o ,".TBL_SHOP_ORDER_DETAIL." od where od.product_type = '55' and o.oid=od.oid and DATE_ADD(od.ic_date,INTERVAL ".$check_sos_order_day." DAY)  <= '".$today."' and od.status = '".ORDER_STATUS_INCOM_COMPLETE."' LIMIT 2000 ";
$db->query($sql);
...
for($i=0;$i < count($order_details);$i++){
    ...
    set_order_status($order_details[$i][oid],ORDER_STATUS_BUY_FINALIZED,$msg,"".$check_sos_order_day."일 경과 시스템 자동구매확정(SOS)",$_SESSION["admininfo"]["company_id"],$order_details[$i][od_ix],$order_details[$i][pid]);
