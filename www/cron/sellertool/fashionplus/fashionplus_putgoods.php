...
$OAL = new OpenAPI('fashionplus');

$array_pid2 = array();

$sql = "select pid, if( update_date is null, regist_date, update_date ) sr_date from sellertool_regist_relation where site_code = 'fashionplus' and result_code in ('200','500') and pid in (select id from shop_product where is_delete = '1') group by pid ";
$db->query($sql);
...
where gp.site_code = 'fashionplus' and gp.state = '1' and (nc.state !='1' or nc.state is null) and p.state not in ('6','7','8')
";
$db2->query($sql);
...
if($db2->total){
	...
	foreach($pinfo as $pi){
		$sql = "select if( rr.update_date is null, rr.regist_date, rr.update_date ) sr_date from sellertool_regist_relation rr where rr.pid = '".$pi[id]."' and site_code = 'fashionplus' ";
		$db2->query($sql);
