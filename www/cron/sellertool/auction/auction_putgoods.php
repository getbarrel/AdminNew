...
$sql = "select pid, if( update_date is null, regist_date, update_date ) sr_date from sellertool_regist_relation where site_code = 'auction' and result_code in ('200','500') and pid in (select id from shop_product where is_delete = '1') group by pid ";
$db->query($sql);
...
left join sellertool_not_company on ( p.admin = nc.company_id and gp.site_code=nc.site_code )

where gp.site_code = 'auction' and gp.state = '1' and (nc.state !='1' or nc.state is null) and p.state not in ('6','7','8')
";
$db2->query($sql);
...
if($db2->total){
	...
	foreach($pinfo as $pi){
		$sql = "select if( rr.update_date is null, rr.regist_date, rr.update_date ) sr_date from sellertool_regist_relation rr where rr.pid = '".$pi[id]."' and site_code = 'auction' ";
		$db2->query($sql);
...
if(count($pinfo) > 0){
	...
	foreach($pinfo as $pid){
		...
		//echo "|||||".$pid."|".$resulte->resultCode."|".$resulte->message."<br/>";
