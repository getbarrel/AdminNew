...
$sql = "select mall_ix,mall_data_root, mall_type, mall_domain from shop_shopinfo  where mall_div = 'B'  ";
$db->query($sql);
$db->fetch();
...
$sql = "select pid, if( update_date is null, regist_date, update_date ) sr_date from sellertool_regist_relation where site_code = 'interpark_api' and result_code in ('200','500') and pid in (select id from shop_product where is_delete = '1') group by pid ";
$db->query($sql);
...
if(count($pinfo) > 0){
	...
	$fp = fopen($_SERVER["DOCUMENT_ROOT"].'/data/entersix_data/_logs/sellertool/interpark_cron_' . date('Ymd') . '.log', 'a');
	...
	foreach($pinfo as $pid){
		...
		$fh = '---------- START - ('.count($pinfo).') '. $pid .' : '.$dandom_num.' [' . date('Y-m-d H:i:s') . '] ----------'. chr(13);
		fwrite($fp, $fh);
